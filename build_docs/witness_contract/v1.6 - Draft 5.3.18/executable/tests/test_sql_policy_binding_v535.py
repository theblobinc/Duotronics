...
class PolicySqlTests(unittest.TestCase):
    def setUp(self):
        ...
        self.key = MLDSA87PrivateKey.generate(); public = self.key.public_key()
    ...
    def records(self):
        ...
        canonical = authority.canonical_text(decision); digest = authority.shake256_512_bytes(canonical.encode()); full = dict(decision, canonical_record_shake256_512=digest)
    ...
    def insert_registry(self, registry):
        ...
        self.connection.execute("INSERT INTO wc_proof_policy_registries_v1 VALUES (?,?,?,?,?,?,?)", ("registry:535","proof_policy_registry/v1","gov:key:535",payload,authority.shake256_512_bytes(payload.encode()),registry["signature"],"2026-07-31T00:00:00Z"))
    ...
    def test_policy_registry_membership_hash_and_immutability(self):
        ...
        bad = list(values); bad[0] = "policy:other"; bad[4] = "0" * 128
