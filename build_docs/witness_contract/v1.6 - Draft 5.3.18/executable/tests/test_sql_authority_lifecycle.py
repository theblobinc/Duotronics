...
class AuthorityDatabase:
    def __init__(self):
        ...
        self.private = MLDSA87PrivateKey.generate()
    ...
    def register_key(
        ...
        *,
        ...
        self.connection.execute(
            ...
            (verifier_id, "verifier_principal/v2", key_id, "ML-DSA-87", fingerprint, "active", valid_from, valid_until, valid_from),
        )
        self.connection.execute(
            "INSERT INTO wc_verifier_keys_v3 VALUES (?,?,?,?,?,?,?,?,?)",
            (key_id, "verifier_key/v3", verifier_id, "ML-DSA-87", authority.public_key_raw_b64url(public), fingerprint, valid_from, valid_until, valid_from),
        )
        self.connection.execute(
    ...
    def insert_chain_before_gate(self, *, hardening: bool = True, insert_bindings: bool = True) -> None:
        ...
        compiler_unsigned = {
            ...
            "claim_content_shake256_512": self.claim_hash,
            "theorem_statement_shake256_512": self.theorem_hash,
            "proof_artifact_shake256_512": self.artifact_hash,
            "proof_artifact_relative_path": "Proof.lean",
            "proof_module": "Proof",
            "source_tree_shake256_512": "d" * 128,
            "lakefile_shake256_512": "e" * 128,
            "generated_witness_module_shake256_512": "9" * 128,
            "generated_witness_module_path": ".witness_authority/Check_valid.lean",
            "exact_build_target": ".witness_authority/Check_valid.lean",
            "compiler_executable_shake256_512": "8" * 128,
            "build_output_shake256_512": "f" * 128,
        ...
        c.execute(
            """INSERT INTO wc_lean_compiler_witnesses_v2 (
              ...
              proof_artifact_relative_path,proof_module,generated_witness_module_shake256_512,generated_witness_module_path,
              exact_build_target,compiler_executable_shake256_512,axiom_dependencies_json,axiom_inspection_complete,
              statement_binding_confirmed,warnings_as_errors
            ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            (
                compiler["lean_compiler_witness_id"], compiler["schema_version"], compiler["claim_id"], compiler["claim_content_shake256_512"], compiler["theorem_statement_shake256_512"],
                compiler["proof_artifact_shake256_512"], compiler["source_tree_shake256_512"], compiler["lakefile_shake256_512"], compiler["build_output_shake256_512"], compiler["toolchain"], json.dumps(compiler["command"], separators=(",", ":")),
                ...
                compiler["verifier_principal_id"], compiler["key_id"], compiler["signature_algorithm"], compiler["signed_payload_shake256_512"], compiler["signature"], compiler["created_at"],
                compiler["proof_artifact_relative_path"], compiler["proof_module"], compiler["generated_witness_module_shake256_512"], compiler["generated_witness_module_path"],
                compiler["exact_build_target"], compiler["compiler_executable_shake256_512"], json.dumps(compiler["axiom_dependencies"], separators=(",", ":")), int(compiler["axiom_inspection_complete"]),
        ...
        if insert_bindings:
            c.execute(
                ...
                ("sigbind:lean", "authority_signature_binding/v1", "lean_compiler_witness", "lean:valid", key_id, authority.signed_payload_canonical_json(compiler), compiler["signed_payload_shake256_512"], self.binding_time),
            )
        ...
        c.execute(
            """INSERT INTO wc_proof_witnesses_v2 (
              ...
              verifier_principal_id,signed_payload_shake256_512,signature,created_at,key_id
            ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            (
                proof["proof_witness_id"], proof["schema_version"], proof["claim_id"], proof["claim_content_shake256_512"], proof["theorem_statement_shake256_512"],
                proof["proof_artifact_shake256_512"], proof["lean_compiler_witness_id"], proof["theorem_name"], proof["theorem_status"], proof["policy_decision_id"],
                proof["verifier_principal_id"], proof["signed_payload_shake256_512"], proof["signature"], proof["created_at"], proof["key_id"],
            ),
        )
        if insert_bindings:
            c.execute(
                "INSERT INTO wc_authority_signature_bindings_v1 VALUES (?,?,?,?,?,?,?,?)",
                ("sigbind:proof", "authority_signature_binding/v1", "proof_witness", "proof:valid", key_id, authority.signed_payload_canonical_json(proof), proof["signed_payload_shake256_512"], self.binding_time),
            )
        c.execute(
    ...
    def insert_gate(self, gate_id: str = "gate:valid") -> None:
        self.connection.execute(
            """INSERT INTO wc_theorem_promotion_gates_v2 (
              theorem_promotion_gate_id,schema_version,claim_id,claim_content_shake256_512,theorem_statement_shake256_512,
...
class SqlAuthorityLifecycleTests(unittest.TestCase):
    ...
    def test_superseded_key_cannot_create_gate(self):
        db = self.database()
        db.insert_chain_before_gate()
        replacement = MLDSA87PrivateKey.generate()
    ...
    def test_wrong_claim_hash_cannot_create_gate(self):
        ...
        expect_integrity_error(
            ...
            lambda: db.connection.execute(
                ...
                ("gate:wrong-hash", "theorem_promotion_gate/v2", "claim:proof", "0" * 128, db.theorem_hash, "status:valid", "proof:valid", "lean:valid", "nc:valid", "policy:allow", "verifier:1", 1, None, db.gate_time, "key:1"),
            ),
        )
    ...
    def test_tampered_canonical_payload_is_rejected(self):
        ...
        expect_integrity_error(
            ...
            lambda: db.connection.execute(
                "INSERT INTO wc_authority_signature_bindings_v1 VALUES (?,?,?,?,?,?,?,?)",
                ("sigbind:bad", "authority_signature_binding/v1", "lean_compiler_witness", "not-stored", key_id, authority.signed_payload_canonical_json(record) + " ", record["signed_payload_shake256_512"], db.binding_time),
            ),
        )
