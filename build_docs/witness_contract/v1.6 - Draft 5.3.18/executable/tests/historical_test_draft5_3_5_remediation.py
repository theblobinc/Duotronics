...
class OciSandboxTests(unittest.TestCase):
    ...
    def invocation(self, entrypoint="/opt/witness-authority/bin/compile-lean"):
        ...
        mounts = ({"source":"snapshot:x","destination":"/input/source","mode":"ro","domain":"untrusted_compilation","purpose":"sealed_source","lifecycle":"request"},)
        return runner, runner._invocation(domain="untrusted_compilation", profile=self.fixture.profile, entrypoint=entrypoint, arguments=("--generated","/input/generated"), mounts=mounts, timeout=600, uid=65534, gid=65534)
    ...
    def test_oci_argv_matches_effective_invocation_for_both_domains(self):
        for entrypoint in ("/opt/witness-authority/bin/compile-lean", "/opt/witness-authority/bin/verify-lean"):
            runner, invocation = self.invocation(entrypoint); command = runner._command(invocation, ({"host":"/tmp/source","container":"/input/source","mode":"ro"},))
            ...
            self.assertEqual(invocation.requested_controls, invocation.applied_controls)
    ...
    def test_every_effective_control_has_runtime_mapping(self):
        runner, invocation = self.invocation(); command = " ".join(runner._command(invocation, ({"host":"/tmp/source","container":"/input/source","mode":"ro"},)))
        expected_fragments = ("--entrypoint=", "--workdir=/work", "--network=none", "--read-only", "--cap-drop=ALL", "no-new-privileges", "seccomp=", "apparmor=", "label=type:", "--userns=private", "--pids-limit=", "--memory=", "--cpus=", "--user=65534:65534", "--ulimit=fsize=", "--ulimit=nofile=", "--volume=")
    ...
    def test_invocation_digest_changes_for_every_declared_control(self):
        ...
        mutations = {
            ...
            "apparmor_profile":"different", "selinux_label":"different_t", "user_namespace_mode":"host",
            "pid_limit":127, "memory_limit":"1g", "cpu_limit":"1", "file_size_limit":1048575,
    ...
    def test_runtime_inspection_matches_declared_security_properties(self):
        ...
        self.assertIn('"verified_controls": inspect_runtime_controls(invocation)', source)
...
class ApiConformanceTests(unittest.TestCase):
    class Resolver:
        def resolve(self, policy_decision_id, **_):
            return authority.ResolvedPolicyDecision(policy_decision_id, {}, "3" * 128)

    class Authority:
        def __init__(self, witness): self.witness = witness; self.calls = 0
        def verify(self, **arguments):
            self.calls += 1; result = json.loads(json.dumps(self.witness)); result["policy_decision_id"] = arguments["policy_decision_id"]; result["policy_decision_shake256_512"] = arguments["policy_decision_shake256_512"]; return result
    ...
    def test_openapi_response_matches_running_wsgi_adapter_and_idempotency(self):
        ...
        result_schema = json.loads((ROOT / "schemas/proof_check_result_v3.schema.json").read_text(encoding="utf-8"))
        self.assertTrue(set(result_schema["required"]).issubset(result))
        witness_schema = json.loads((ROOT / "schemas/lean_compiler_witness_v4.schema.json").read_text(encoding="utf-8"))
    ...
    def test_openapi_has_only_implemented_proof_route(self):
        import yaml
        document = yaml.safe_load((ROOT / "executable/openapi/draft5_3_5_evidence_language_openapi.yaml").read_text(encoding="utf-8"))
