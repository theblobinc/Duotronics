...
class HistoricalCacheBindingAndChronologyTests(unittest.TestCase):
    ...
    @staticmethod
    def key_record(key_id: str, key: MLDSA87PrivateKey, *, valid_from: str) -> dict:
        return {
    ...
    @staticmethod
    def registry(governance: MLDSA87PrivateKey, registry_id: str, created_at: str, record: dict) -> dict:
    ...
    def setUp(self):
        ...
        self.old_key = MLDSA87PrivateKey.generate()
        self.new_key = MLDSA87PrivateKey.generate()
        ...
        self.old_digest = authority.shake256_512_bytes(authority.canonical_bytes(self.old_registry))
        self.current_digest = authority.shake256_512_bytes(authority.canonical_bytes(self.current_registry))
        self.lineage = authority.sign_record({
            "schema_version": "cache_registry_lineage/v1", "lineage_id": "cache:lineage:1",
            "governance_key_id": "governance:key", "current_registry_shake256_512": self.current_digest,
            "historical_registries": [{
                "registry_shake256_512": self.old_digest, "registry_file": "cache-registry-old.json",
                "successor_registry_shake256_512": self.current_digest,
        ...
        old_application = service.ProofCheckApplication(
            ...
            cache_registry_shake256_512=self.old_digest, cache_clock=lambda: self.OLD_TIME,
        )
    ...
    def snapshots(self) -> dict[str, service.GovernedCacheRegistrySnapshot]:
        lineage_digest = self.lineage["signed_payload_shake256_512"]
        ...
        return {
            self.old_digest: service.build_governed_cache_registry_snapshot(
                ...
                lineage_signed_payload_shake256_512=lineage_digest, lineage_created_at=lineage_created_at,
            ),
    ...
    def current_application(self) -> service.ProofCheckApplication:
        return service.ProofCheckApplication(
            ...
            cache_registry_shake256_512=self.current_digest,
            cache_registry_snapshots_by_shake256_512=self.snapshots(),
    ...
    def rebind_database_slot(self, request: dict, *, principal_id: str = "principal:1") -> None:
        ...
        with closing(sqlite3.connect(self.root / ".proof-check-idempotency.sqlite")) as connection:
            connection.execute(
                "UPDATE proof_check_idempotency SET principal_id=?, idempotency_key=?, request_shake256_512=?",
                (principal_id, request["idempotency_key"], request_shake256_512),
            )
            connection.commit()
    ...
    def test_bound_historical_row_emits_self_contained_exact_envelope_evidence(self):
        ...
        self.assertEqual(evidence["cache_envelope_signed_payload_shake256_512"], envelope["signed_payload_shake256_512"])
        self.assertEqual(evidence["cache_envelope_canonical_shake256_512"], authority.shake256_512_bytes(authority.canonical_bytes(envelope)))
    ...
    def test_lineage_must_postdate_every_referenced_registry(self):
        ...
        with self.assertRaisesRegex(ValueError, "lineage predates"):
            service.validate_cache_registry_snapshot_lineage(
                snapshots, current_registry_shake256_512=self.current_digest, production_mode=True,
            )
...
class SignedCacheAuditSinkTests(unittest.TestCase):
    def setUp(self):
        ...
        self.audit_registry_shake256_512 = "d" * 128
        self.anchor_registry_shake256_512 = "e" * 128
        self.anchor_namespace = "cache:audit:anchor:test"
        self.audit_registry_records = {
            "cache:audit:key": {
                "key_id": "cache:audit:key", "principal_id": "cache:audit:principal",
                "public_key_base64url": authority.public_key_raw_b64url(self.key.public_key()),
        ...
        self.anchor = service.InMemoryMonotonicAuditAnchor(
            self.anchor_key, key_id="cache:audit:anchor:key",
        ...
        self.key_evidence = {
            ...
            "registry_shake256_512": self.audit_registry_shake256_512,
            "key_id": "cache:audit:key", "principal_id": "cache:audit:principal",
            "authorization_scope": "cache_audit_record_signing", "status": "active",
            "valid_from": "2026-01-01T00:00:00Z", "valid_until": None,
    ...
    def evidence(marker: int = 1) -> dict:
        digest = f"{marker % 16:x}" * 128
        decision = {
            "schema_version": "cache_key_validity_evidence/v1",
            "registry_shake256_512": "a" * 128,
        ...
        return {
            ...
            "cache_registry_shake256_512": "a" * 128,
    ...
    def genesis(self, suffix: str = "0001") -> dict:
        return authority.sign_record({
            ...
            "audit_signing_registry_shake256_512": self.audit_registry_shake256_512,
            "anchor_registry_shake256_512": self.anchor_registry_shake256_512,
    ...
    def sink(
        ...
        **kwargs,
        ...
        return service.SignedAppendOnlyAuditSink(
            ...
            audit_signing_registry_shake256_512=self.audit_registry_shake256_512,
            audit_registry_records=self.audit_registry_records,
    ...
    def test_startup_revalidates_embedded_event_schema(self):
        ...
        event.pop("cache_registry_shake256_512")
        event_bytes = authority.canonical_bytes(event)
        unsigned = {k: v for k, v in record.items() if k not in {"signed_payload_shake256_512", "signature"}}
        unsigned["event_canonical_json"] = event_bytes.decode("utf-8")
        unsigned["event_shake256_512"] = authority.shake256_512_bytes(event_bytes)
    ...
    def test_signed_append_only_chain_is_durable_and_externally_anchored(self):
        ...
        self.assertEqual(second["previous_audit_record_shake256_512"], authority.shake256_512_bytes(lines[0].encode("utf-8")))
    ...
    def test_competing_genesis_requires_external_anchor_compare_and_swap(self):
        ...
        with self.assertRaises(Exception):
            service.SignedAppendOnlyAuditSink(
                ...
                signer_key_id="cache:audit:key", audit_signing_registry_shake256_512=self.audit_registry_shake256_512,
                audit_registry_records=self.audit_registry_records, schema_validator=self.validator,
                ...
                anchor_namespace=self.anchor_namespace, anchor_registry_shake256_512=self.anchor_registry_shake256_512,
    ...
    def test_successor_requires_real_sealed_predecessor_and_governed_transition(self):
        ...
        transition = authority.sign_record({
            ...
            "predecessor_sealed_checkpoint_shake256_512": authority.shake256_512_bytes(authority.canonical_bytes(sealed_checkpoint)),
            "predecessor_terminal_record_shake256_512": authority.shake256_512_bytes(authority.canonical_bytes(terminal_record)),
        ...
        self.assertEqual(self.anchor.read()["previous_sealed_segment_tail_shake256_512"], sealed_checkpoint["tail_record_shake256_512"])

    def test_arbitrary_predecessor_tail_is_not_an_api_surface(self):
        self.sink(provision=True).seal_segment()
        with self.assertRaises(TypeError):
            self.sink(provision=True, suffix="0002", previous_sealed_segment_tail_shake256_512="f" * 128)
    ...
    def test_checkpoint_failure_removes_temporary_and_governed_recovery_reconciles(self):
        ...
        authorization = authority.sign_record({
            "schema_version": "cache_audit_recovery_authorization/v2",
            ...
            "log_sequence_after": 1, "log_tail_after": authority.shake256_512_bytes(log_line.encode()),
            "target_file_identities": [],
            "operator_principal_id": "cache:audit:recovery:operator",
            "reason": "reconcile one fully signed dangling record after injected checkpoint fsync failure",
            "decision": "authorize_governed_recovery", "created_at": "2026-08-01T11:59:00Z",
