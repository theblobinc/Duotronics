#!/usr/bin/env python3
"""Descriptor-reconciled fail-closed validator for Draft 5.3.7."""
...
DESCRIPTOR_PATH = ROOT / "CANONICAL_CORPUS_v1_6_draft_5_3_7.json"
REPORT_PATH = ROOT / "DRAFT5_3_7_VALIDATION_REPORT.json"
REGRESSION_COUNTS_PATH = ROOT / "DRAFT5_3_7_REGRESSION_COUNTS.json"
VALIDATION_SUMMARY_PATH = ROOT / "DRAFT5_3_7_VALIDATION_SUMMARY.txt"
HISTORY_ARCHIVES = {
    ...
    "v1.6 - Draft 5.3.5.zip": ("6a417c9363f29867b5dadafd8ac484b9e43371d451049cb7391a31669f96390c", 1427),
    "v1.6 - Draft 5.3.6.zip": ("1dded199d4642e51dc493e2d2abd682bc111c1e76aa3956219a0b6c8f1a94c17", 1491),
}
...
def _run_discovered_suite(*, development_mode: bool) -> dict[str, Any]:
    ...
    command.extend([
        "-m", "unittest", "discover", "-s", "executable/tests",
        "-p", "test*.py", "-v",
    ])
    ...
    assert_true(match is not None, "unittest discovery did not report a discovered count")
    discovered = int(match.group(1))
    passed_names = re.findall(r"^(test\S+ \([^\n]+\)) \.\.\. ok$", combined, re.MULTILINE)
    skipped_lines = re.findall(r"^test\S+ \([^\n]+\) \.\.\. skipped", combined, re.MULTILINE)
    duplicate_names = sorted(name for name, count in Counter(passed_names).items() if count > 1)
    warning_lines = [
    ...
    assert_true(not skipped_lines, f"unexpected skipped tests: {skipped_lines}")
    assert_true(not duplicate_names, f"duplicate test discovery: {duplicate_names}")
    assert_true(passed == discovered, f"passed/discovered mismatch: {passed}/{discovered}")
    if development_mode:
        assert_true(not warning_lines, f"warning output is forbidden: {warning_lines}")
    return {
        "command": command,
        "tests_discovered": discovered,
        "tests_passed": passed,
        "tests_skipped": len(skipped_lines),
        "duplicate_test_ids": duplicate_names,
...
def phase_descriptor() -> dict[str, Any]:
    ...
    assert_true(descriptor["active_version"] == "v1.6-draft-5.3.7", "active version mismatch")
    ...
    for token in (DESCRIPTOR_PATH.name, "PACKAGE_INVENTORY_v1_6_draft_5_3_7.json", "CHECKSUMS_v1_6_draft_5_3_7.shake256_512"):
        assert_true(token in resolver, f"resolver does not boot canonical token {token}")
    accounting = _run_python_tests("-W", ["error", "executable/tests/test_validator_phase_reconciliation_v537.py"])
...
def phase_dependencies() -> dict[str, Any]:
    ...
    try:
        ...
        connection.close()
    scope = load_json("refs/validation_scope_v1_6_draft_5_3_7.json")
...
def phase_hash_closure() -> dict[str, Any]:
    inventory = load_json("PACKAGE_INVENTORY_v1_6_draft_5_3_7.json")
    ...
    checksum_lines = (ROOT / "refs/manifest/CHECKSUMS_v1_6_draft_5_3_7.shake256_512").read_text(encoding="utf-8").splitlines()
...
def _schema_phase(name: str) -> dict[str, Any]:
    command = ["node", "executable/validators/validate_draft5_3_7_schemas.mjs", "--phase", name]
...
def phase_domain_specific_rlimit_fsize() -> dict[str, Any]:
    tests = [
        "Draft537SandboxTests.test_compiler_domain_uses_compiler_artifact_file_size_limit",
        "Draft537SandboxTests.test_inspection_domain_uses_inspection_output_bytes_limit",
        "Draft537SandboxTests.test_rlimit_fsize_is_bound_into_invocation_digest",
        "Draft537RuntimeInspectionTests.test_matching_rlimit_fsize_is_measured",
        "Draft537RuntimeInspectionTests.test_excessive_rlimit_fsize_fails",
        "Draft537RuntimeInspectionTests.test_missing_domain_limit_fails_closed",
        "Draft537RuntimeInspectionTests.test_wrong_domain_limit_fails_closed",
        "Draft537RuntimeInspectionTests.test_obsolete_limit_fails_real_inspection_path",
    ]
    return _run_python_tests("executable/tests/test_draft5_3_7_remediation.py", tests)


def phase_control_state_semantics() -> dict[str, Any]:
    tests = [
        "Draft537SandboxTests.test_preexecution_invocation_has_no_accepted_controls",
        "Draft537SandboxTests.test_preexecution_invocation_has_no_applied_controls",
        "Draft537SandboxTests.test_requested_controls_are_not_treated_as_evidence",
        "Draft537SandboxTests.test_emitted_controls_are_derived_from_exact_argv",
    ]
    return _run_python_tests("executable/tests/test_draft5_3_7_remediation.py", tests)
...
def phase_sql_migration() -> dict[str, Any]:
    ...
    assert_true("v1.6-draft-5.3.7" in generations, "5.3.7 schema generation missing")
    ...
    connection.close()
    assert_true(objects == len(required_objects), "5.3.7 policy/witness objects missing")
...
def phase_sql_v537_authority_closure() -> dict[str, Any]:
    ...
    for token in (
        ...
        assert_true(token in sql, f"Draft 5.3.7 SQL closure omits {token}")
...
def phase_openapi_conformance() -> dict[str, Any]:
    ...
    spec = yaml.safe_load((ROOT / "executable/openapi/draft5_3_7_evidence_language_openapi.yaml").read_text(encoding="utf-8"))
    assert_true(spec["info"]["version"] == "v1.6-draft-5.3.7", "OpenAPI version mismatch")
    ...
    request_properties = load_json("schemas/proof_check_request_v1.schema.json")["properties"]
    forbidden = {
...
def phase_effective_sandbox_invocation() -> dict[str, Any]:
    profile = load_json("authority/hermetic_lean_sandbox_profile_v4.json")
    ...
    assert_true(profile["effective_invocation_schema"] == "schemas/effective_sandbox_invocation_v4.schema.json", "sandbox profile does not select v4 effective invocation")
...
def phase_schema_registry_authority() -> dict[str, Any]:
    registry = load_json("refs/schema_registry_v1_6_draft_5_3_7.json")
...
def phase_version_consistency() -> dict[str, Any]:
    expected = "v1.6-draft-5.3.7"
    descriptor = load_json(DESCRIPTOR_PATH.name)
    metadata = load_json("PACKAGE_METADATA_v1_6_draft_5_3_7.json")
    registry = load_json("refs/schema_registry_v1_6_draft_5_3_7.json")
    package = load_json("package.json")
    lock = load_json("package-lock.json")
    tla = load_json("refs/formal_toolchain/tla_toolchain_manifest_v1_0.json")
    gate_status = load_json("DRAFT5_3_7_RELEASE_GATE_STATUS.json")
    assert_true(descriptor["active_version"] == metadata["version"] == expected, "descriptor/package metadata version mismatch")
    assert_true(package["version"] == "1.6.5-3-7" and lock["version"] == package["version"], "npm package/lock version mismatch")
    assert_true(package["name"].endswith("draft-5-3-7-validation") and lock["name"] == package["name"], "npm package/lock identity mismatch")
    assert_true(registry.get("registry_version") == expected, "schema registry version mismatch")
    assert_true("Draft 5.3.7" in tla["package"] and any(item["module"] == "ProofAuthorityV8" for item in tla["tla_modules"]), "formal manifest version/active model mismatch")
    assert_true(gate_status["gates"]["I_package_provenance"] == "unpublished_workspace", "release provenance status is not fail-closed")
    assert_true('v!"5.3.7"' in (ROOT / "lakefile.lean").read_text(encoding="utf-8"), "Lake package version mismatch")
    active_documents = (
        "README.md", "START_HERE.md", "CANONICAL_CORPUS_v1_6_draft_5_3_7.json",
        "CORPUS_INDEX_v1_6_draft_5_3_7.md", "duotronic_witness_contract_v1_6_draft_5_3_7.md",
        "RELEASE_NOTES_v1_6_draft_5_3_7.md", "PACKAGE_METADATA_v1_6_draft_5_3_7.json",
    ...
    for relative in active_documents:
        ...
        assert_true("5.3.7" in text_value, f"active entrypoint lacks 5.3.7 identity: {relative}")
    ...
    for report_path in ROOT.glob("DRAFT5_3_7*.json"):
...
def phase_package_provenance() -> dict[str, Any]:
    provenance = load_json("PACKAGE_PROVENANCE_v1_6_draft_5_3_7.json")
    required = {
...
def _build_attestation(subject_type: str) -> dict[str, Any]:
    status = load_json("DRAFT5_3_7_BUILD_ATTESTATION_STATUS.json")
...
def phase_external_signature() -> dict[str, Any]:
    status = load_json("refs/trust/RELEASE_SIGNATURE_STATUS_v1_6_draft_5_3_7.json")
...
def main() -> int:
    ...
    inventory = load_json("PACKAGE_INVENTORY_v1_6_draft_5_3_7.json") if (ROOT / "PACKAGE_INVENTORY_v1_6_draft_5_3_7.json").is_file() else {"files": []}
    regular_directories = [
    ...
    report = {
        "schema_version": "draft5_3_7_validation_report/v1",
