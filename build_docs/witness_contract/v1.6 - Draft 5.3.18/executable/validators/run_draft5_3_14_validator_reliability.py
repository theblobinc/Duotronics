#!/usr/bin/env python3
"""Repeat the complete Draft 5.3.14 validator with bounded outer orchestration."""
...
VALIDATOR_PATH = ROOT / "executable/validators/validate_draft5_3_14_corpus.py"
REPORT_PATH = ROOT / "DRAFT5_3_14_VALIDATION_REPORT.json"

SPECIFICATION = importlib.util.spec_from_file_location("draft5314_reliability_validator", VALIDATOR_PATH)
...
def shake256_512(path: pathlib.Path) -> str:
...
def main() -> int:
    ...
    if output.is_file():
        ...
        if (
            prior.get("package_version") != "v1.6-draft-5.3.14"
    ...
    def write_checkpoint(status: str) -> None:
        evidence = {
            ...
            "complete_validator": "executable/validators/validate_draft5_3_14_corpus.py",
            "required_phase_count": 94,
            "optional_activation_phase_count": 8,
            "status": status,
            "runs": completed_runs,
        }
    ...
    for sequence in range(len(completed_runs) + 1, final_sequence + 1):
        ...
        process = VALIDATOR._run_bounded_text_command(
            [arguments.python, str(VALIDATOR_PATH)], cwd=ROOT,
        ...
        if (
            ...
            or statuses != {"passed": 94, "failed": 0, "skipped": 8}
        ):
        ...
        completed_runs.append({
            ...
            "report_shake256_512": shake256_512(REPORT_PATH),
            "required_phases_passed": 94,
            "optional_phases_skipped": 8,
