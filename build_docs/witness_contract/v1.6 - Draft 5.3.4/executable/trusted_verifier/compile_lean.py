...
def main() -> int:
    ...
    for _ in range(max(1, len(pending))):
        ...
        for source in list(pending):
            ...
            target.parent.mkdir(parents=True, exist_ok=True)
            process = subprocess.run([str(LEAN), "-o", str(target), str(source)], cwd=WORK, env=environment, text=True, capture_output=True, timeout=300)
            diagnostic = (process.stdout + process.stderr)[:MAX_DIAGNOSTIC_BYTES]
    ...
    process = subprocess.run([str(LEAN), "-o", str(binding_target), str(binding)], cwd=WORK, env=environment, text=True, capture_output=True, timeout=300)
    if process.returncode != 0:
        return 1
