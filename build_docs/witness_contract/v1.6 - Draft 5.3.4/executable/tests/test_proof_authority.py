...
class FakeTwoDomainRunner:
    ...
    def run(self, *, profile, snapshot_root, generated_module, request, timeout_seconds):
        ...
        forbidden: list[str] = []
        result = {
            "schema_version": "wc_lean_verifier_result/v2",
            "status": "passed",
        ...
        if self.mode == "statement_mismatch":
            ...
            return authority.SandboxExecution(0, None, "fake canonical JSON", "", False, ("compile",), ("verify",), self.INVOCATION_HASH, self._security())
        ...
        return authority.SandboxExecution(
            ...
            self.INVOCATION_HASH, security,
        )

    @staticmethod
    def _security():
        return ("result_channel_isolated", "no_network", "read_only_rootfs", "capabilities_dropped", "resource_limits", "trusted_result_only")
...
class ProofAuthorityTests(unittest.TestCase):
    ...
    def test_generated_module_is_deterministic_minimal_and_read_only(self):
        ...
        self.assertIn("example : (True) := t", text); self.assertNotIn("by\n", text); self.assertNotIn("#eval", text)
    ...
    def test_real_oci_command_separates_logical_hashes_from_private_host_paths(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            runner = authority.OciSandboxRunner(
                ...
                verifier_result_signing_key=self.result_private,
            )
