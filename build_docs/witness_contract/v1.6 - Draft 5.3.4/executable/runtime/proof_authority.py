#!/usr/bin/env python3
"""Fail-closed Lean proof authority for Witness Contract Draft 5.3.4.
...
def _source_relatives(source_root: Path) -> list[PurePosixPath]:
    ...
    return sorted(relatives, key=lambda item: item.as_posix())


def _stable_read_source(source_root: Path, relative: PurePosixPath) -> tuple[bytes, tuple[int, ...]]:
    path = source_root.joinpath(*relative.parts)
    fd = os.open(path, os.O_RDONLY | O_NOFOLLOW)
    try:
        before = os.fstat(fd)
        if not stat.S_ISREG(before.st_mode) or before.st_nlink != 1 or before.st_size > MAX_SOURCE_FILE_BYTES:
        ...
        while True:
            ...
            chunks.append(chunk)
        after = os.fstat(fd)
        identity = (before.st_dev, before.st_ino, before.st_size, before.st_mtime_ns, before.st_ctime_ns, before.st_nlink)
...
def create_immutable_snapshot(
    ...
    *,
    ...
    relatives = _source_relatives(source_root)
    if not relatives:
    ...
    for relative in relatives:
        data, identity = _stable_read_source(source_root, relative)
        target = destination.joinpath(*relative.parts)
        target.parent.mkdir(mode=0o700, parents=True, exist_ok=True)
        fd = os.open(target, os.O_WRONLY | os.O_CREAT | os.O_EXCL | O_NOFOLLOW, 0o400)
        try:
            ...
            os.close(fd)
        source_identities[relative.as_posix()] = identity
        source_hashes[relative.as_posix()] = sha256_bytes(data)
        if after_file_copy:
            after_file_copy(relative)

    # Detect changes to any already-copied file before snapshot completion.
    if _source_relatives(source_root) != relatives:
        raise AuthorityFailure("snapshot_mutation_detected", "source path set changed during snapshot creation")
    for relative in relatives:
        data, identity = _stable_read_source(source_root, relative)
        if identity != source_identities[relative.as_posix()] or sha256_bytes(data) != source_hashes[relative.as_posix()]:
...
class OciSandboxRunner:
    ...
    def __init__(self, runtime_executable: Path, expected_runtime_sha256: str, expected_runtime_version: str, *, verifier_result_signing_key: Ed25519PrivateKey, authority_uid: int | None = None, authority_gid: int | None = None):
        ...
        self.authority_uid = os.getuid() if authority_uid is None else authority_uid
        self.authority_gid = os.getgid() if authority_gid is None else authority_gid
    ...
    def _invocation(self, *, domain: str, profile: CompilerProfile, entrypoint: str, arguments: tuple[str, ...], mounts: tuple[dict[str, str], ...], timeout: int, uid: int, gid: int) -> EffectiveSandboxInvocation:
        return EffectiveSandboxInvocation(
            ...
            security_options=("no-new-privileges",), seccomp_profile_sha256="0" * 64,
            apparmor_profile="witness-authority-default", selinux_label="container_t",
            user_namespace_mode="private", pid_limit=128, memory_limit="2g", cpu_limit="2",
    ...
    def _command(self, invocation: EffectiveSandboxInvocation, runtime_mounts: tuple[dict[str, str], ...]) -> tuple[str, ...]:
        command = [
            str(self.runtime), "run", "--rm", "--pull=never", "--network=none", "--read-only",
            "--cap-drop=ALL", "--security-opt=no-new-privileges", "--pids-limit=128",
            "--memory=2g", "--cpus=2", f"--user={invocation.container_uid}:{invocation.container_gid}",
            "--ulimit=fsize=1048576:1048576", "--ulimit=nofile=256:256",
            "--tmpfs=/tmp:rw,noexec,nosuid,nodev,size=512m", "--tmpfs=/work:rw,nosuid,nodev,size=1g",
        ]
        for mount in runtime_mounts:
            command.append(f"--volume={mount['host']}:{mount['container']}:{mount['mode']}")
        for value in invocation.environment_allowlist:
            command.append(f"--env={value}")
        command.extend([f"{invocation.image_reference}@{invocation.image_digest}", invocation.entrypoint, *invocation.arguments])
    ...
    def run(self, *, profile: CompilerProfile, snapshot_root: Path, generated_module: Path, request: dict[str, Any], timeout_seconds: int) -> SandboxExecution:
        ...
        with tempfile.TemporaryDirectory(prefix="wc-handoff-") as handoff_name, tempfile.TemporaryDirectory(prefix="wc-control-") as control_name, tempfile.TemporaryDirectory(prefix="wc-inspection-parent-") as inspection_parent_name, tempfile.TemporaryDirectory(prefix="wc-result-parent-") as result_parent_name:
            ...
            compile_manifest = (
                {"host": f"snapshot:{request['immutable_snapshot_tree_sha256']}", "container": "/input/source", "mode": "ro"},
                {"host": f"generated:{request['generated_binding_module_sha256']}", "container": "/input/generated", "mode": "ro"},
                {"host": "ephemeral:compile-handoff", "container": "/handoff", "mode": "rw"},
            )
            ...
            try:
                compiled = subprocess.run(untrusted_command, text=True, capture_output=True, timeout=timeout_seconds, env=minimal_env)
            except subprocess.TimeoutExpired as error:
                return SandboxExecution(None, None, error.stdout or "", error.stderr or "", True, untrusted_command=untrusted_command, effective_sandbox_invocation_sha256=compile_invocation.sha256)
            if compiled.returncode != 0:
                return SandboxExecution(compiled.returncode, None, compiled.stdout, compiled.stderr, False, untrusted_command=untrusted_command, effective_sandbox_invocation_sha256=compile_invocation.sha256)
            ...
            verifier_manifest = (
                {"host": f"snapshot:{request['immutable_snapshot_tree_sha256']}", "container": "/input/source", "mode": "ro"},
                {"host": f"generated:{request['generated_binding_module_sha256']}", "container": "/input/generated", "mode": "ro"},
                {"host": "ephemeral:compile-handoff", "container": "/handoff", "mode": "ro"},
                {"host": f"control:{request['request_id']}", "container": "/input/control", "mode": "ro"},
                {"host": "ephemeral:trusted-inspection", "container": "/trusted-inspection", "mode": "rw"},
            )
            ...
            try:
                verified = subprocess.run(trusted_command, text=True, capture_output=True, timeout=timeout_seconds, env=minimal_env)
            except subprocess.TimeoutExpired as error:
                return SandboxExecution(None, None, (compiled.stdout or "") + (error.stdout or ""), (compiled.stderr or "") + (error.stderr or ""), True, untrusted_command, trusted_command, verifier_invocation.sha256)
            ...
            return SandboxExecution(
                ...
                ("result_channel_isolated", "no_network", "read_only_rootfs", "capabilities_dropped", "resource_limits", "trusted_result_only"),
            )
...
class ProofAuthorityService:
    ...
    def verify(
        ...
        with tempfile.TemporaryDirectory(prefix="wc-snapshot-parent-") as snapshot_parent_name, tempfile.TemporaryDirectory(prefix="wc-generated-parent-") as generated_parent_name:
            ...
            sandbox_template = {
                "profile": compiler_profile_id, "network": "none", "read_only_rootfs": True,
                "untrusted_result_mount": False, "trusted_result_mount": True, "runtime": profile.oci_runtime_sha256,
            }
            request = {
                "schema_version": "wc_lean_verifier_request/v2", "request_id": f"request:{descriptor_seed}",
        ...
        required_security = {"result_channel_isolated", "no_network", "read_only_rootfs", "capabilities_dropped", "resource_limits", "trusted_result_only"}
        passed = bool(
            ...
            and required_security.issubset(execution.security_properties)
        )
        if passed:
        ...
        unsigned = {
            "schema_version": "lean_compiler_witness/v3", "lean_compiler_witness_id": "lean:" + sha256_bytes(canonical_bytes(semantic))[:32],
            ...
            "build_output_sha256": diagnostic_hash, "result": result, "failure_code": failure_code,
            ...
            "result_channel_isolated": required_security.issubset(execution.security_properties),
    ...
    def _structured_result_valid(result: dict[str, Any] | None, request: dict[str, Any], request_hash: str, profile: CompilerProfile, execution: SandboxExecution) -> tuple[bool, str | None]:
        if not isinstance(result, dict) or result.get("schema_version") != "wc_lean_verifier_result/v2":
...
def load_production_authority_service(config_root: Path = Path("/etc/witness-authority")) -> ProofAuthorityService:
    ...
    allowed = {
        ...
        "authority_ledger_high_water_sequence", "artifact_store_root",
    }
...
__all__ = [
    "AtomicResultPublisher", "AuthorityFailure", "CompilerProfile", "EffectiveSandboxInvocation",
    "ImmutableSnapshot", "OciSandboxRunner", "ProofAuthorityService", "SandboxExecution",
    ...
    "sign_record", "signed_payload_canonical_json", "verify_governed_registry", "verify_record",
