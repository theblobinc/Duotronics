...
## Draft 2 Addendum: MCP Policy Snapshot Binding

The policy engine must ingest observed MCP policy explanations as `MCPPolicyExplanationWitness` records.
...
allowed_for_principal: boolean
principal_scopes: []
observed_at: string
```

Tools with `risk = external_action` require action-candidate and outcome records. Tools with `risk = db_write` require affected-object records. Tools with `risk in {repo_write, service_mutation}` require backup/git sync evidence or an explicit waiver.
