...
class FakePolicyResolver:
    ...
    def resolve(self, policy_decision_id, **arguments):
        ...
        return types.SimpleNamespace(policy_decision_id=policy_decision_id, canonical_record_sha256="a" * 64)
...
class ProofCheckServiceTests(unittest.TestCase):
    ...
    def test_exact_request_resolves_bundle_and_preserves_policy(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = ProofCheckApplication(authority, store, policy).handle(self.request())
    ...
    def test_request_cannot_supply_path_hash_environment_or_timestamp(self):
        with tempfile.TemporaryDirectory() as directory:
            application = ProofCheckApplication(FakeAuthority(), pathlib.Path(directory), FakePolicyResolver())
    ...
    def test_bundle_path_traversal_is_rejected(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertRaisesRegex(ValueError, "bundle-relative"):
                ProofCheckApplication(FakeAuthority(), pathlib.Path(directory), FakePolicyResolver()).handle(request)
    ...
    def test_concurrent_identical_idempotency_key_shares_one_execution(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            (bundle / "Proof.lean").write_text("theorem t : True := by trivial\n", encoding="utf-8")
            authority = SlowAuthority(); application = ProofCheckApplication(authority, store, FakePolicyResolver())
            results = []
