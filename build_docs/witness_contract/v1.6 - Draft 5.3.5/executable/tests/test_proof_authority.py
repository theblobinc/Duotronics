...
class FakeTwoDomainRunner:
    ...
    def run(self, *, profile, snapshot_root, generated_module, request, timeout_seconds):
        ...
        forbidden: list[str] = []
        result = {
            "schema_version": "wc_lean_verifier_result/v3",
            "status": "passed",
            ...
            "requested_controls": list(self.CONTROLS), "applied_controls": list(self.CONTROLS),
            "verified_controls": list(self.CONTROLS),
        ...
        if self.mode == "statement_mismatch":
            ...
            result["axiom_set"] = ["sorryAx"]; result["axiom_set_sha256"] = authority.sha256_bytes(authority.canonical_bytes(["sorryAx"])); result["sorry_ax_present"] = True
        elif self.mode == "forbidden_axiom":
            result["axiom_set"] = ["Unsafe.magic"]; result["axiom_set_sha256"] = authority.sha256_bytes(authority.canonical_bytes(["Unsafe.magic"])); result["forbidden_axiom_set"] = ["Unsafe.magic"]
        elif self.mode == "unsafe":
            result["unsafe_dependency_present"] = True
            ...
            return authority.SandboxExecution(0, None, "fake canonical JSON", "", False, ("compile",), ("verify",), self.INVOCATION_HASH, self._security(), self.CONTROLS, self.CONTROLS, self.CONTROLS)
...
class ProofAuthorityTests(unittest.TestCase):
    ...
    def test_real_oci_command_separates_logical_hashes_from_private_host_paths(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            logical = ({"host": "snapshot:" + "a" * 64, "container": "/input/source", "mode": "ro"},)
            invocation = runner._invocation(
                ...
                mounts=logical, timeout=600, uid=65534, gid=65534,
            )
            ...
            first = runner._command(invocation, ({"host": "/tmp/random-a", "container": "/input/source", "mode": "ro"},))
            second = runner._command(invocation, ({"host": "/tmp/random-b", "container": "/input/source", "mode": "ro"},))
        ...
        self.assertIn("--ulimit=fsize=1048576:1048576", first)
