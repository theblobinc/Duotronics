#!/usr/bin/env python3
"""Fail-closed Lean proof authority for Witness Contract Draft 5.3.5.
...
class ProofPolicyResolver:
    ...
    def __init__(self, registry: dict[str, Any], governance_key: Ed25519PublicKey, *, clock: Callable[[], datetime] | None = None):
    ...
    def resolve(
        ...
        return ResolvedPolicyDecision(policy_decision_id, dict(decision), observed_hash)
...
class OciSandboxRunner:
    ...
    REQUIRED_CONTROLS = frozenset({
        ...
        "capabilities_dropped", "no_new_privileges", "seccomp", "apparmor", "selinux",
        "private_user_namespace", "non_root_user", "pid_limit", "memory_limit", "cpu_limit",
        "file_size_limit", "open_file_limit", "environment_allowlist", "mount_manifest",
    })
    ...
    def __init__(
        ...
        apparmor_profile: str = "witness-authority-default", selinux_label: str = "container_t",
        ...
        available = supported_controls or self.REQUIRED_CONTROLS
    ...
    def _invocation(self, *, domain: str, profile: CompilerProfile, entrypoint: str, arguments: tuple[str, ...], mounts: tuple[dict[str, str], ...], timeout: int, uid: int, gid: int) -> EffectiveSandboxInvocation:
        if uid == 0 or gid == 0:
            raise AuthorityFailure("root_execution_forbidden", "sandbox invocation cannot use UID or GID zero")
        applied = tuple(sorted(self.REQUIRED_CONTROLS))
        return EffectiveSandboxInvocation(
            domain=domain, oci_runtime_sha256=self.runtime_sha256, oci_runtime_version=self.runtime_version,
            ...
            file_size_limit=MAX_RESULT_BYTES, open_file_limit=256, timeout=timeout,
            environment_allowlist=("HOME=/nonexistent", "LANG=C.UTF-8", "LC_ALL=C.UTF-8", "TZ=UTC", "SOURCE_DATE_EPOCH=0", "LEAN_ABORT_ON_PANIC=1"),
            mount_manifest=mounts, working_directory="/work",
            requested_controls=applied, applied_controls=applied,
            immutable_policy_files=({"path": str(self.seccomp_profile_path), "sha256": self.seccomp_profile_sha256},),
        )

    def _command(self, invocation: EffectiveSandboxInvocation, runtime_mounts: tuple[dict[str, str], ...]) -> tuple[str, ...]:
        command = [
            str(self.runtime), "run", "--rm", "--pull=never", f"--entrypoint={invocation.entrypoint}",
            f"--workdir={invocation.working_directory}", f"--network={invocation.network_mode}", "--read-only",
            "--cap-drop=ALL", "--security-opt=no-new-privileges",
            f"--security-opt=seccomp={invocation.seccomp_profile_path}",
            f"--security-opt=apparmor={invocation.apparmor_profile}",
            f"--security-opt=label=type:{invocation.selinux_label}",
            f"--userns={invocation.user_namespace_mode}", f"--pids-limit={invocation.pid_limit}",
            f"--memory={invocation.memory_limit}", f"--cpus={invocation.cpu_limit}",
            f"--user={invocation.container_uid}:{invocation.container_gid}",
            f"--ulimit=fsize={invocation.file_size_limit}:{invocation.file_size_limit}",
            f"--ulimit=nofile={invocation.open_file_limit}:{invocation.open_file_limit}",
            "--tmpfs=/tmp:rw,noexec,nosuid,nodev,size=512m", "--tmpfs=/work:rw,nosuid,nodev,size=1g",
        ]
        for mount in runtime_mounts:
            host = mount.get("host", mount.get("source")); container = mount.get("container", mount.get("destination"))
            command.append(f"--volume={host}:{container}:{mount['mode']}")
        for value in invocation.environment_allowlist:
            command.append(f"--env={value}")
        command.extend([f"{invocation.image_reference}@{invocation.image_digest}", *invocation.arguments])
    ...
    def run(self, *, profile: CompilerProfile, snapshot_root: Path, generated_module: Path, request: dict[str, Any], timeout_seconds: int) -> SandboxExecution:
        ...
        with tempfile.TemporaryDirectory(prefix="wc-handoff-") as handoff_name, tempfile.TemporaryDirectory(prefix="wc-control-") as control_name, tempfile.TemporaryDirectory(prefix="wc-inspection-parent-") as inspection_parent_name, tempfile.TemporaryDirectory(prefix="wc-result-parent-") as result_parent_name:
            ...
            compile_runtime_mounts = (
                {"host": str(snapshot_root), "container": "/input/source", "mode": "ro"},
                {"host": str(generated_root), "container": "/input/generated", "mode": "ro"},
                {"host": str(handoff), "container": "/handoff", "mode": "rw"},
            )
            compile_manifest = (
                {"source": f"snapshot:{request['immutable_snapshot_tree_sha256']}", "destination": "/input/source", "mode": "ro", "domain": "untrusted_compilation", "purpose": "sealed_source", "lifecycle": "request"},
                {"source": f"generated:{request['generated_binding_module_sha256']}", "destination": "/input/generated", "mode": "ro", "domain": "untrusted_compilation", "purpose": "generated_type_binding", "lifecycle": "request"},
                {"source": "ephemeral:compile-handoff", "destination": "/handoff", "mode": "rw", "domain": "untrusted_compilation", "purpose": "compiled_artifact_handoff", "lifecycle": "request"},
            )
            compile_invocation = self._invocation(domain="untrusted_compilation", profile=profile, entrypoint="/opt/witness-authority/bin/compile-lean", arguments=("--generated", "/input/generated/WitnessAuthorityGenerated"), mounts=compile_manifest, timeout=timeout_seconds, uid=65534, gid=65534)
            untrusted_command = self._command(compile_invocation, compile_runtime_mounts)
            minimal_env = {"PATH": "/usr/bin:/bin", "LANG": "C", "LC_ALL": "C", "TZ": "UTC"}
            compiled = run_bounded_process(untrusted_command, timeout_seconds=timeout_seconds, env=minimal_env)
            ...
            verifier_runtime_mounts = (
                {"host": str(snapshot_root), "container": "/input/source", "mode": "ro"},
                {"host": str(generated_root), "container": "/input/generated", "mode": "ro"},
                {"host": str(handoff), "container": "/handoff", "mode": "ro"},
                {"host": str(control), "container": "/input/control", "mode": "ro"},
                {"host": str(inspection_directory), "container": "/trusted-inspection", "mode": "rw"},
            )
            verifier_manifest = (
                {"source": f"snapshot:{request['immutable_snapshot_tree_sha256']}", "destination": "/input/source", "mode": "ro", "domain": "trusted_verifier", "purpose": "sealed_source", "lifecycle": "request"},
                {"source": f"generated:{request['generated_binding_module_sha256']}", "destination": "/input/generated", "mode": "ro", "domain": "trusted_verifier", "purpose": "generated_type_binding", "lifecycle": "request"},
                {"source": "ephemeral:compile-handoff", "destination": "/handoff", "mode": "ro", "domain": "trusted_verifier", "purpose": "compiled_artifact_input", "lifecycle": "request"},
                {"source": f"control:{request['request_id']}", "destination": "/input/control", "mode": "ro", "domain": "trusted_verifier", "purpose": "trusted_verifier_request", "lifecycle": "request"},
                {"source": "ephemeral:trusted-inspection", "destination": "/trusted-inspection", "mode": "rw", "domain": "trusted_verifier", "purpose": "private_inspection_output", "lifecycle": "request"},
            )
            verifier_invocation = self._invocation(domain="trusted_verifier", profile=profile, entrypoint="/opt/witness-authority/bin/verify-lean", arguments=("--request", "/input/control/request.json", "--result-dir", "/trusted-inspection"), mounts=verifier_manifest, timeout=timeout_seconds, uid=self.authority_uid, gid=self.authority_gid)
            ...
            control.chmod(0o555)
            trusted_command = self._command(verifier_invocation, verifier_runtime_mounts)
            verified = run_bounded_process(trusted_command, timeout_seconds=timeout_seconds, env=minimal_env)
            ...
            inspection = read_private_result(inspection_path, expected_uid=self.authority_uid) if inspection_path.exists() else None
            structured = None
            if inspection is not None:
                final_directory = result_parent / "private-result"
                publisher = AtomicResultPublisher(final_directory, expected_uid=self.authority_uid)
                result_path = publisher.publish(inspection, self.verifier_result_signing_key)
                structured = read_private_result(result_path, expected_uid=self.authority_uid)
            verified_controls = tuple(sorted(structured.get("verified_controls", []))) if structured else ()
            security_properties = tuple(sorted(set(verified_controls) | {"result_channel_isolated", "trusted_inspection_only"}))
            return SandboxExecution(
                verified.returncode, structured, compiled.stdout + verified.stdout, compiled.stderr + verified.stderr,
                False, untrusted_command, trusted_command, verifier_invocation.sha256,
                security_properties, verifier_invocation.requested_controls,
                verifier_invocation.applied_controls, verified_controls, False,
                sha256_bytes((compiled.stdout + verified.stdout).encode("utf-8")),
                sha256_bytes((compiled.stderr + verified.stderr).encode("utf-8")),
            )
...
class ProofAuthorityService:
    ...
    def verify(
        ...
        with tempfile.TemporaryDirectory(prefix="wc-snapshot-parent-") as snapshot_parent_name, tempfile.TemporaryDirectory(prefix="wc-generated-parent-") as generated_parent_name:
            snapshot = create_immutable_snapshot(source_root, Path(snapshot_parent_name) / "snapshot")
            ...
            request = {
                "schema_version": "wc_lean_verifier_request/v3", "request_id": f"request:{descriptor_seed}",
            ...
            execution = self.runner.run(profile=profile, snapshot_root=snapshot.root, generated_module=generated_path, request=request, timeout_seconds=timeout_seconds)
            verify_sealed_snapshot(snapshot.root, snapshot.tree_sha256)
        ...
        required_verified_controls = {
            ...
            "no_new_privileges", "seccomp", "apparmor", "selinux", "private_user_namespace",
            "non_root_user", "pid_limit", "memory_limit", "cpu_limit", "file_size_limit",
            "open_file_limit", "mount_manifest",
        }
        exact_controls = (
            set(execution.requested_controls) == set(execution.applied_controls)
            and set(execution.applied_controls).issubset(set(execution.verified_controls))
            and required_verified_controls.issubset(set(execution.verified_controls))
        )
        passed = bool(
            ...
            and structured.get("declaration_type_matches") is True and structured.get("expected_type_expression_hash") == structured.get("actual_type_expression_hash")
            and structured.get("build_from_source") is True and structured.get("prebuilt_artifacts_used") is False
            and structured.get("warnings_as_errors") is True and not contains_sorry and not unsafe_dependency and not unauthorized
        ...
        semantic_result = {
            ...
            for key in (
                ...
                "normalization_policy", "direct_dependencies", "transitive_dependencies_root", "axiom_set",
                ...
                "requested_controls", "applied_controls", "verified_controls", "oci_runtime_version_output_sha256",
            )
        }
        semantic = {
        ...
        unsigned = {
            "schema_version": "lean_compiler_witness/v4", "lean_compiler_witness_id": "lean:" + sha256_bytes(canonical_bytes(semantic))[:32],
            ...
            "verified_controls": sorted(execution.verified_controls),
    ...
    @staticmethod
    def _structured_result_valid(result: dict[str, Any] | None, request: dict[str, Any], request_hash: str, profile: CompilerProfile, execution: SandboxExecution) -> tuple[bool, str | None]:
        if not isinstance(result, dict) or result.get("schema_version") != "wc_lean_verifier_result/v3":
        ...
        if any(result.get(key) != value for key, value in exact.items()):
            return False, "verifier_result_invalid"
        for field in ("expected_type_expression_hash", "actual_type_expression_hash", "transitive_dependencies_root", "axiom_set_sha256"):
            if not SHA256_HEX.fullmatch(str(result.get(field, ""))):
                return False, "verifier_result_invalid"
        for field in ("direct_dependencies", "axiom_set", "forbidden_axiom_set", "requested_controls", "applied_controls", "verified_controls"):
        ...
        if result["axiom_set_sha256"] != sha256_bytes(canonical_bytes(result["axiom_set"])):
            return False, "verifier_result_invalid"
        if result["requested_controls"] != sorted(execution.requested_controls) or result["applied_controls"] != sorted(execution.applied_controls):
            return False, "verifier_result_invalid"
        if result["verified_controls"] != sorted(execution.verified_controls):
            return False, "verifier_result_invalid"
        return True, None
...
__all__ = [
    "AtomicResultPublisher", "AuthorityFailure", "BoundedProcessResult", "CompilerProfile", "EffectiveSandboxInvocation",
    ...
    "validate_trusted_root_ancestry", "verify_sealed_snapshot",
]
