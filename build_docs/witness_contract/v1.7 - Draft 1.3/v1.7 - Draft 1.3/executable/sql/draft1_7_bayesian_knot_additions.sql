-- Duotronic Witness Contract v1.7 Draft 1.3 Bayesian + Knot Theory First-Class Additions
...
CREATE TABLE IF NOT EXISTS srnn_bayesian_models (
  ...
  update_method TEXT NOT NULL CHECK (update_method IN ('exact_discrete_bayes','log_space_discrete_bayes','conjugate_closed_form','graphical_belief_propagation','hierarchical_update','approximate_monte_carlo','external_verified')),
  model_family TEXT NOT NULL CHECK (model_family IN ('discrete','conjugate','graphical_bayesian_network','hierarchical','particle_monte_carlo','external_verified')),
  model_family_notes TEXT,
...
CREATE TABLE IF NOT EXISTS srnn_bayesian_update_witnesses (
  ...
  update_method TEXT NOT NULL CHECK (update_method IN ('exact_discrete_bayes','log_space_discrete_bayes','conjugate_closed_form','graphical_belief_propagation','hierarchical_update','approximate_monte_carlo','external_verified')),
  reference_algorithm_id TEXT NOT NULL,
  ...
  approximation_error_bound REAL,
  approximation_method TEXT,
  negative_or_missing_evidence_refs_json TEXT,
  non_collapse_transition_id TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS srnn_bayesian_update_replay_witnesses (
  ...
  reference_algorithm_id TEXT NOT NULL,
  replay_method TEXT NOT NULL CHECK (replay_method IN ('linear_space_discrete_bayes','log_sum_exp_discrete_bayes','closed_form_conjugate','bounded_monte_carlo','external_verified')),
  ...
  log_normalization_constant REAL,
...
CREATE TABLE IF NOT EXISTS srnn_bayesian_decision_witnesses (
  ...
  loss_matrix_id TEXT,
  abstention_threshold REAL CHECK (abstention_threshold IS NULL OR (abstention_threshold >= 0 AND abstention_threshold <= 1)),
  human_review_threshold REAL CHECK (human_review_threshold IS NULL OR (human_review_threshold >= 0 AND human_review_threshold <= 1)),
...
CREATE TABLE IF NOT EXISTS srnn_bayesian_calibration_reports (
  ...
  scoring_rule TEXT NOT NULL CHECK (scoring_rule IN ('brier','log_score','reliability_bins','expected_calibration_error','custom')),
  scoring_rule_definition_id TEXT NOT NULL,
  score REAL,
...
CREATE TABLE IF NOT EXISTS srnn_knot_braid_word_witnesses (
  ...
  generator_bounds_policy TEXT NOT NULL CHECK (generator_bounds_policy IN ('index_must_be_1_to_strand_count_minus_1')),
  zero_exponent_policy TEXT NOT NULL CHECK (zero_exponent_policy IN ('reject_zero_exponents','normalize_away_zero_exponents','allow_explicit_noop')),
...
CREATE TABLE IF NOT EXISTS srnn_knot_reidemeister_move_witnesses (
  ...
  move_type TEXT NOT NULL CHECK (move_type IN ('R1','R1_inverse','R2','R2_inverse','R3','isotopy_relabeled')),
...
CREATE TABLE IF NOT EXISTS srnn_knot_invariant_witnesses (
  ...
  source_object_ref TEXT NOT NULL,
  invariant_kind TEXT NOT NULL CHECK (invariant_kind IN ('crossing_count','component_count','determinant','alexander_polynomial','jones_polynomial','signature','linking_number','fundamental_group_presentation','wirtinger_presentation','quandle_coloring','custom')),
  invariant_semantics TEXT NOT NULL CHECK (invariant_semantics IN ('invariant_computation','invariant_comparison','complete_invariant_for_bounded_domain','proof_backed_equivalence')),
...
-- v1.7 Draft 1.3 Bayesian correctness and knot transition hardening additions.

CREATE TABLE IF NOT EXISTS srnn_bayesian_posterior_predictive_witnesses (
  posterior_predictive_witness_id TEXT PRIMARY KEY,
  schema_version TEXT NOT NULL CHECK (schema_version = 'bayesian_posterior_predictive_witness/v1'),
  model_id TEXT NOT NULL REFERENCES srnn_bayesian_models(model_id),
  posterior_id TEXT NOT NULL REFERENCES srnn_bayesian_posterior_states(posterior_id),
  predictive_distribution_json TEXT NOT NULL,
  reference_algorithm_id TEXT NOT NULL,
  limitations_json TEXT,
  non_collapse_transition_id TEXT NOT NULL,
  metadata_json TEXT,
...
CREATE TABLE IF NOT EXISTS srnn_bayesian_marginalization_witnesses (
  marginalization_witness_id TEXT PRIMARY KEY,
  schema_version TEXT NOT NULL CHECK (schema_version = 'bayesian_marginalization_witness/v1'),
  model_id TEXT NOT NULL REFERENCES srnn_bayesian_models(model_id),
  source_distribution_ref TEXT NOT NULL,
  marginalized_variables_json TEXT NOT NULL,
  result_distribution_json TEXT NOT NULL,
  reference_algorithm_id TEXT NOT NULL,
  non_collapse_transition_id TEXT NOT NULL,
  metadata_json TEXT,
  created_at TEXT NOT NULL
...
CREATE TABLE IF NOT EXISTS srnn_bayesian_conditioning_witnesses (
  conditioning_witness_id TEXT PRIMARY KEY,
  schema_version TEXT NOT NULL CHECK (schema_version = 'bayesian_conditioning_witness/v1'),
  model_id TEXT NOT NULL REFERENCES srnn_bayesian_models(model_id),
  source_distribution_ref TEXT NOT NULL,
  condition_evidence_ref TEXT NOT NULL,
  condition_probability REAL NOT NULL CHECK (condition_probability > 0 AND condition_probability <= 1),
  conditioned_distribution_json TEXT NOT NULL,
  reference_algorithm_id TEXT NOT NULL,
  non_collapse_transition_id TEXT NOT NULL,
  metadata_json TEXT,
  created_at TEXT NOT NULL
...
CREATE TABLE IF NOT EXISTS srnn_bayesian_negative_evidence_witnesses (
  negative_evidence_witness_id TEXT PRIMARY KEY,
  schema_version TEXT NOT NULL CHECK (schema_version = 'bayesian_negative_evidence_witness/v1'),
  model_id TEXT NOT NULL REFERENCES srnn_bayesian_models(model_id),
  evidence_ref TEXT NOT NULL,
  evidence_kind TEXT NOT NULL CHECK (evidence_kind IN ('negative_evidence','missing_data','censored_observation','not_observed','unknown_rejected')),
  handling_policy TEXT NOT NULL CHECK (handling_policy IN ('explicit_absence_model','missing_at_random_assumption','missing_not_at_random_assumption','likelihood_mask','reject_update')),
  assumption_refs_json TEXT,
  non_collapse_transition_id TEXT NOT NULL,
  metadata_json TEXT,
  created_at TEXT NOT NULL
);

CREATE TABLE IF NOT EXISTS srnn_bayesian_loss_matrix_witnesses (
  loss_matrix_id TEXT PRIMARY KEY,
  schema_version TEXT NOT NULL CHECK (schema_version = 'bayesian_loss_matrix_witness/v1'),
  model_id TEXT NOT NULL REFERENCES srnn_bayesian_models(model_id),
  actions_json TEXT NOT NULL,
  hypotheses_json TEXT NOT NULL,
  loss_entries_json TEXT NOT NULL,
  abstention_threshold REAL NOT NULL CHECK (abstention_threshold >= 0 AND abstention_threshold <= 1),
...
CREATE TABLE IF NOT EXISTS srnn_knot_braid_relation_witnesses (
  braid_relation_witness_id TEXT PRIMARY KEY,
  schema_version TEXT NOT NULL CHECK (schema_version = 'knot_braid_relation_witness/v1'),
  source_braid_id TEXT NOT NULL,
  target_braid_id TEXT NOT NULL,
  relation_type TEXT NOT NULL CHECK (relation_type IN ('artin_commutation','artin_braid_relation','free_reduction','cyclic_conjugation')),
  checked INTEGER NOT NULL CHECK (checked IN (0,1)),
  checker_ref TEXT,
  non_collapse_transition_id TEXT NOT NULL,
  metadata_json TEXT,
...
CREATE TABLE IF NOT EXISTS srnn_knot_markov_move_witnesses (
  markov_move_witness_id TEXT PRIMARY KEY,
  schema_version TEXT NOT NULL CHECK (schema_version = 'knot_markov_move_witness/v1'),
  source_braid_id TEXT NOT NULL,
  target_braid_id TEXT NOT NULL,
  markov_move_type TEXT NOT NULL CHECK (markov_move_type IN ('conjugation','stabilization','destabilization')),
  checked INTEGER NOT NULL CHECK (checked IN (0,1)),
  checker_ref TEXT,
  non_collapse_transition_id TEXT NOT NULL,
  metadata_json TEXT,
...
CREATE TABLE IF NOT EXISTS srnn_knot_presentation_transition_witnesses (
  presentation_transition_witness_id TEXT PRIMARY KEY,
  schema_version TEXT NOT NULL CHECK (schema_version = 'knot_presentation_transition_witness/v1'),
  source_object_ref TEXT NOT NULL,
  target_object_ref TEXT NOT NULL,
  transition_kind TEXT NOT NULL CHECK (transition_kind IN ('reidemeister_move','reidemeister_trace','braid_relation','markov_move','canonicalization','proof_backed_equivalence')),
  transition_witness_ref TEXT NOT NULL,
  transition_status TEXT NOT NULL CHECK (transition_status IN ('candidate','checked','replayed','proof_verified','rejected')),
  non_collapse_transition_id TEXT NOT NULL,
  metadata_json TEXT,
  created_at TEXT NOT NULL
);
