...
def phase_descriptor() -> dict[str, Any]:
    ...
    for token in (DESCRIPTOR_PATH.name, "PACKAGE_INVENTORY_v1_6_draft_5_3_2.json", "CHECKSUMS_v1_6_draft_5_3_2.sha256"):
...
def phase_hash_closure() -> dict[str, Any]:
    ...
    checksum_lines = (ROOT / "refs/manifest/CHECKSUMS_v1_6_draft_5_3_2.sha256").read_text(encoding="utf-8").splitlines()
...
def phase_sql_migration() -> dict[str, Any]:
    ...
    for name in ("generated_witness_module_sha256", "exact_build_target", "compiler_executable_sha256", "axiom_inspection_complete", "statement_binding_confirmed", "warnings_as_errors"):
