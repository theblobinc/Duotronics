...
VALIDATOR = ROOT / "executable/validators/validate_draft5_3_10_corpus.py"
SPEC = importlib.util.spec_from_file_location("draft5310_validator_counts", VALIDATOR)
...
def main() -> int:
    ...
    record = {
        "schema_version": "draft5_3_10_regression_counts/v1",
        "package_version": "v1.6-draft-5.3.10",
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "draft5_3_9_baseline_tests_discovered": 211,
        "target_python_versions": ["3.12", "3.13"],
        "validated_python_versions": [normal["python_version"]],
        "unavailable_python_versions": ["3.13"],
    ...
    (ROOT / "DRAFT5_3_10_REGRESSION_COUNTS.json").write_text(
        json.dumps(record, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    matrix = {
        "schema_version": "python_matrix_validation/v3",
        "package_version": "v1.6-draft-5.3.10",
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "count_source": "unittest.TestResult",
        ...
        "runs": [{
            ...
            "normal_stdout_sha256": normal["stdout_sha256"],
            "normal_stderr_sha256": normal["stderr_sha256"],
            "development_stdout_sha256": warning_free["stdout_sha256"],
            "development_stderr_sha256": warning_free["stderr_sha256"],
        }],
        "unavailable_runs": [{
            "python_version": "3.13", "status": "interpreter_unavailable",
            "authority_effect": "no_current_draft_5_3_10_python_3_13_execution_claim",
        }],
        "overall_status": "passed_with_declared_environment_qualification",
    }
    (ROOT / "DRAFT5_3_10_PYTHON_MATRIX_VALIDATION.json").write_text(
        json.dumps(matrix, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    summary = "\n".join([
        "Duotronic Witness Contract v1.6 Draft 5.3.10 — Generated Validation Summary",
        "Status: non-authoritative portable conformance corpus; permanently not frozen",
        "Target Python versions: 3.12 and 3.13",
        f"Validated Python versions: {', '.join(record['validated_python_versions'])}",
        "Unavailable Python versions: 3.13",
        "Python 3.13 Draft 5.3.10 rerun: interpreter unavailable; no current-run claim",
        "Count source: unittest.TestResult",
        "Draft 5.3.9 baseline: 211 tests discovered",
    ...
    (ROOT / "DRAFT5_3_10_VALIDATION_SUMMARY.txt").write_text(summary, encoding="utf-8")
