...
The canonical Draft 5.2 primitive category enum contains 21 forever-distinct categories. No inference rule, policy, replay, migration, proof adapter, or training process may equate any two categories without recording a non-collapse transition witness.
...
Representative forbidden collapse pairs:
...
| `self_trained` | `authoritative` (NLA) | Self-training does not grant authority without gate passage. |
| `audit_only` | `active` | Audit observation is not operational authority. |
| `observation` | `proof` | Observation can support proof but cannot replace proof. |
| `explanation` | `fact` | Explanation is an account; fact status requires evidence. |
| `policy_approval` | `human_attestation` | Policy approval is not identical to human testimony. |
| `synthetic_witness` | `activation_witness` | Synthetic witness generation is not the same as activation authority. |
