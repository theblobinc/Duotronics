...
Draft 5.3.13 incorporates the complete current corpus and supersedes Draft 5.3.12 on the validator-orchestration, historical cache verification, production-loader integration, schema, and validation surfaces listed by the canonical descriptor. Earlier source-package archives are not embedded.
...
Only a fully bound authentic governed predecessor produces `cache_stale_row_evidence/v3`. That evidence binds the request, slot, principal, policy, source, compiler-witness signed-payload digest, cache signing time, historical registry creation time, lineage creation time, and the successful binding-verification result. The service durably publishes this evidence through a dedicated-key append-only signed hash chain before returning `cache_key_rotation_requires_new_idempotency_key`. Audit-publication failure returns `cache_audit_publication_failed`. The service does not silently delete the row or re-execute under the old key.
