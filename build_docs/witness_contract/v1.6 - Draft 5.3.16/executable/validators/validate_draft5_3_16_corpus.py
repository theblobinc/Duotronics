#!/usr/bin/env python3
"""Descriptor-reconciled fail-closed validator for Draft 5.3.16."""
...
DESCRIPTOR_PATH = ROOT / "CANONICAL_CORPUS_v1_6_draft_5_3_16.json"
REPORT_PATH = ROOT / "DRAFT5_3_16_VALIDATION_REPORT.json"
REGRESSION_COUNTS_PATH = ROOT / "DRAFT5_3_16_REGRESSION_COUNTS.json"
VALIDATION_SUMMARY_PATH = ROOT / "DRAFT5_3_16_VALIDATION_SUMMARY.txt"
LINEAGE_PATH = ROOT / "history/SOURCE_PACKAGE_LINEAGE_v1_6_draft_5_3_16.json"
...
VALIDATOR_RUN_TOKEN = uuid.uuid4().hex
...
def _subprocess_environment() -> dict[str, str]:
    return {
        **os.environ,
        ...
        "WITNESS_VALIDATOR_RUN_TOKEN": VALIDATOR_RUN_TOKEN,
    }
...
_ACTIVE_DESCENDANTS: set[tuple[int, int]] | None = None
_ACTIVE_STATE_LOCK = threading.RLock()


def _set_active_worker(process: subprocess.Popen[bytes] | None, process_group: int | None = None, descendants: set[tuple[int, int]] | None = None) -> None:
    global _ACTIVE_PROCESS, _ACTIVE_PROCESS_GROUP, _ACTIVE_DESCENDANTS
    with _ACTIVE_STATE_LOCK:
        _ACTIVE_PROCESS = process
        _ACTIVE_PROCESS_GROUP = process_group
        _ACTIVE_DESCENDANTS = descendants


def _external_termination_handler(selected_signal: int, _frame) -> None:
    with _ACTIVE_STATE_LOCK:
        process = _ACTIVE_PROCESS
        process_group = _ACTIVE_PROCESS_GROUP
        descendants = _ACTIVE_DESCENDANTS
    if process is not None and process_group is not None:
        known = set() if descendants is None else set(descendants)
        known.update(_descendant_identities(process.pid))
        known.update(_tagged_process_identities())
        try:
            _terminate_and_reap(process, process_group, known)
        except BaseException:
            _signal_process_group(process_group, signal.SIGKILL)
            _signal_identities(known, signal.SIGKILL)
    os._exit(128 + selected_signal)


def _install_external_termination_handlers() -> None:
    signal.signal(signal.SIGTERM, _external_termination_handler)
    signal.signal(signal.SIGINT, _external_termination_handler)
...
def _tagged_process_identities() -> set[tuple[int, int]]:
    """Find run descendants even after they escape and are reparented.

    Every validator child inherits a cryptographically random per-run token.
    Scanning /proc by that token closes the race where a nested child exits its
    parent process group immediately before external cancellation.
    """
    marker = f"WITNESS_VALIDATOR_RUN_TOKEN={VALIDATOR_RUN_TOKEN}".encode("ascii")
...
def _terminate_and_reap(
    ...
    known_descendants.update(_tagged_process_identities())
...
def _run_bounded_text_command(
    ...
    with tempfile.TemporaryDirectory(prefix="witness-validator-capture-") as directory:
        ...
        try:
            ...
            _set_active_worker(process, process_group, known_descendants)
...
def _run_discovered_suite(*, development_mode: bool) -> dict[str, Any]:
    ...
    assert_true(major_minor in {(3, 12), (3, 13)}, "Python version is outside the Draft 5.3.16 target matrix")
    return {
        "command": command,
        **structured,
...
def phase_descriptor() -> dict[str, Any]:
    ...
    assert_true(descriptor["active_version"] == "v1.6-draft-5.3.16", "active version mismatch")
    ...
    for token in (DESCRIPTOR_PATH.name, "PACKAGE_INVENTORY_v1_6_draft_5_3_16.json", "CHECKSUMS_v1_6_draft_5_3_16.sha256"):
        assert_true(token in resolver, f"resolver does not boot canonical token {token}")
    accounting = _run_python_tests("-W", ["error", "executable/tests/test_validator_phase_reconciliation_v5316.py"])
...
def phase_dependencies() -> dict[str, Any]:
    ...
    try:
        ...
        connection.close()
    scope = load_json("refs/validation_scope_v1_6_draft_5_3_16.json")
...
def phase_hash_closure() -> dict[str, Any]:
    inventory = load_json("PACKAGE_INVENTORY_v1_6_draft_5_3_16.json")
    ...
    checksum_lines = (ROOT / "refs/manifest/CHECKSUMS_v1_6_draft_5_3_16.sha256").read_text(encoding="utf-8").splitlines()
...
def _schema_phase(name: str) -> dict[str, Any]:
    command = ["node", "executable/validators/validate_draft5_3_16_schemas.mjs", "--phase", name]
...
def phase_python_version_portability() -> dict[str, Any]:
    generated = load_json(REGRESSION_COUNTS_PATH.name)
    matrix = load_json("DRAFT5_3_16_PYTHON_MATRIX_VALIDATION.json")
    metadata = load_json("PACKAGE_METADATA_v1_6_draft_5_3_16.json")
    ...
    for item in [*runs, *unavailable_runs]:
        ...
        assert_true(isinstance(relative, str) and relative.startswith("validation/python_matrix/draft5_3_16/"), "Python run evidence path is invalid")
        path = ROOT / relative
        assert_true(path.is_file() and sha256(path) == item.get("evidence_sha256"), "Python run evidence digest mismatch")
        record = load_json(relative)
        embedded = {key: value for key, value in item.items() if key not in {"evidence_file", "evidence_sha256"}}
        ...
        evidence_set.append({"path": relative, "sha256": item["evidence_sha256"], "target_python_version": target})
    evidence_set.sort(key=lambda item: item["target_python_version"])
    evidence_digest = hashlib.sha256(json.dumps(evidence_set, sort_keys=True, separators=(",", ":")).encode("utf-8")).hexdigest()
    assert_true(evidence_digest == matrix.get("evidence_set_sha256"), "Python evidence-set digest mismatch")
    assert_true(generated.get("python_matrix_sha256") == sha256(ROOT / "DRAFT5_3_16_PYTHON_MATRIX_VALIDATION.json"), "regression record does not bind the Python matrix")
    for item in runs:
...
def phase_openapi_conformance() -> dict[str, Any]:
    ...
    spec = yaml.safe_load((ROOT / "executable/openapi/draft5_3_16_evidence_language_openapi.yaml").read_text(encoding="utf-8"))
    assert_true(spec["info"]["version"] == "v1.6-draft-5.3.16", "OpenAPI version mismatch")
...
def phase_schema_registry_authority() -> dict[str, Any]:
    registry = load_json("refs/schema_registry_v1_6_draft_5_3_16.json")
...
def phase_version_consistency() -> dict[str, Any]:
    ...
    gate_status = load_json("DRAFT5_3_16_RELEASE_GATE_STATUS.json")
    assert_true(descriptor["active_version"] == metadata["version"] == expected, "descriptor/package metadata version mismatch")
    assert_true(package["version"] == "1.6.5-3-16" and lock["version"] == package["version"], "npm package/lock version mismatch")
    assert_true(package["name"].endswith("draft-5-3-16-validation") and lock["name"] == package["name"], "npm package/lock identity mismatch")
    assert_true(registry.get("registry_version") == expected, "schema registry version mismatch")
    assert_true("Draft 5.3.16" in tla["package"] and any(item["module"] == "ProofAuthorityV8" for item in tla["tla_modules"]), "formal manifest version/active model mismatch")
    assert_true(gate_status["gates"]["I_package_provenance"] == "unpublished_workspace", "release provenance status is not fail-closed")
    assert_true('v!"5.3.16"' in (ROOT / "lakefile.lean").read_text(encoding="utf-8"), "Lake package version mismatch")
    active_documents = (
        "README.md", "START_HERE.md", "CANONICAL_CORPUS_v1_6_draft_5_3_16.json",
        "CORPUS_INDEX_v1_6_draft_5_3_16.md", "duotronic_witness_contract_v1_6_draft_5_3_16.md",
        "RELEASE_NOTES_v1_6_draft_5_3_16.md", "PACKAGE_METADATA_v1_6_draft_5_3_16.json",
    ...
    for relative in active_documents:
        ...
        assert_true("5.3.16" in text_value, f"active entrypoint lacks 5.3.16 identity: {relative}")
    ...
    for report_path in ROOT.glob("DRAFT5_3_16*.json"):
...
def phase_package_provenance() -> dict[str, Any]:
    provenance = load_json("PACKAGE_PROVENANCE_v1_6_draft_5_3_16.json")
    required = {
...
def phase_standalone_merged_corpus() -> dict[str, Any]:
    ...
    assert_true(
        releases[-1].get("version") == "v1.6-draft-5.3.15"
        and releases[-1].get("sha256") == "f33d2c682afe312668c9d6256da59784a43cd91b29b73881109690b2709ce800",
...
def _build_attestation(subject_type: str) -> dict[str, Any]:
    status = load_json("DRAFT5_3_16_BUILD_ATTESTATION_STATUS.json")
...
def phase_external_signature() -> dict[str, Any]:
    status = load_json("refs/trust/RELEASE_SIGNATURE_STATUS_v1_6_draft_5_3_16.json")
...
def _run_v5314_audit_tests(tests: list[str]) -> dict[str, Any]:
    return _run_python_tests("executable/tests/test_draft5_3_14_remediation.py", tests)


def _run_v5315_tests(tests: list[str]) -> dict[str, Any]:
    return _run_python_tests("executable/tests/test_draft5_3_15_remediation.py", tests)


def _run_v5316_tests(tests: list[str]) -> dict[str, Any]:
    return _run_python_tests("executable/tests/test_draft5_3_16_remediation.py", tests)
...
def phase_bounded_validator_reaping() -> dict[str, Any]:
    ...
    evidence_records = [
        load_json("validation/validator_reliability/draft5_3_16/python_3_12.json"),
        load_json("validation/validator_reliability/draft5_3_16/python_3_13.json"),
    ]
    assert_true(
    ...
    for record in evidence_records:
        ...
        if status == "passed":
            ...
            assert_true(all(
                run.get("required_phases_passed") == len(load_json("CANONICAL_CORPUS_v1_6_draft_5_3_16.json")["required_validation_phases"])
        ...
        elif status == "interpreter_unavailable":
            assert_true(record.get("runs") == [], "unavailable reliability evidence contains runs")
        elif status == "pending_current_revision_execution":
            assert_true(
                os.environ.get("WITNESS_VALIDATOR_BOOTSTRAP_RELIABILITY") == "1"
                and target == "3.13"
                and record.get("runs") == [],
                "pending reliability evidence is accepted only by the explicit non-publishable bootstrap run",
            )
        else:
            raise AssertionError("unexecuted reliability evidence is not explicitly fail-closed")
    assert_true(
        statuses.get("3.13") == "passed"
        or (
            statuses.get("3.13") == "pending_current_revision_execution"
            and os.environ.get("WITNESS_VALIDATOR_BOOTSTRAP_RELIABILITY") == "1"
        ),
        "available Python 3.13 reliability campaign is not complete",
    )
    return {
...
def phase_complete_stale_evidence_bindings() -> dict[str, Any]:
    schema = load_json("schemas/cache_stale_row_evidence_v4.schema.json")
    required = set(schema.get("required", []))
    expected = {
        "authenticated_principal_id", "idempotency_key", "request_sha256", "request_id",
        "claim_id", "claim_content_sha256", "compiler_profile_id", "policy_decision_id",
        "policy_decision_sha256", "source_bundle_id", "proof_artifact_relative_path",
        "compiler_witness_signed_payload_sha256", "cache_signed_at",
        "historical_registry_created_at", "registry_lineage_created_at",
        "cache_envelope_signed_payload_sha256", "cache_envelope_canonical_sha256",
        "cache_envelope_signature", "binding_verification_result",
    }
    assert_true(expected.issubset(required), "stale-row evidence schema omits required request or chronology bindings")
    tests = _run_v5314_audit_tests([
        "HistoricalCacheBindingAndChronologyTests.test_bound_historical_row_emits_self_contained_exact_envelope_evidence",
    ])
    return {**tests, "required_binding_fields": sorted(expected), "schema_version": "cache_stale_row_evidence/v4"}


def phase_durable_signed_cache_audit_sink() -> dict[str, Any]:
    return _run_v5314_audit_tests([
        "SignedCacheAuditSinkTests.test_signed_append_only_chain_is_durable_and_externally_anchored",
        "SignedCacheAuditSinkTests.test_event_idempotency_reconciles_ambiguous_completion_without_duplicates",
        "SignedCacheAuditSinkTests.test_evidence_publication_failure_is_stable_fail_closed_error",
    ])


def phase_audit_rollback_resistance() -> dict[str, Any]:
    return _run_v5314_audit_tests([
        "SignedCacheAuditSinkTests.test_coordinated_local_log_and_checkpoint_rollback_is_rejected_by_external_anchor",
        "SignedCacheAuditSinkTests.test_deleted_audit_log_is_not_silently_recreated",
        "SignedCacheAuditSinkTests.test_competing_genesis_requires_external_anchor_compare_and_swap",
    ])


def phase_deadline_bounded_audit_publication() -> dict[str, Any]:
    first = _run_v5314_audit_tests([
        "SignedCacheAuditSinkTests.test_supervised_publication_is_hard_bounded_around_slow_fsync",
    ])
    second = _run_v5315_tests([
        "ExternalAnchorAndPublisherTests.test_unix_publisher_client_hard_times_out_without_waiting_for_server",
    ])
    return {"tests": first["tests"] + second["tests"], "local_supervisor": first, "production_socket_client": second}


def phase_governed_audit_segment_transition() -> dict[str, Any]:
    return _run_v5314_audit_tests([
        "SignedCacheAuditSinkTests.test_successor_requires_real_sealed_predecessor_and_governed_transition",
        "SignedCacheAuditSinkTests.test_arbitrary_predecessor_tail_is_not_an_api_surface",
    ])


def phase_terminal_seal_capacity_reservation() -> dict[str, Any]:
    return _run_v5314_audit_tests([
        "SignedCacheAuditSinkTests.test_normal_capacity_always_reserves_terminal_seal_record",
    ])


def phase_bounded_streaming_audit_verification() -> dict[str, Any]:
    return _run_v5314_audit_tests([
        "SignedCacheAuditSinkTests.test_oversized_log_is_rejected_by_fstat_before_any_log_read",
    ])


def phase_governed_audit_recovery() -> dict[str, Any]:
    return _run_v5314_audit_tests([
        "SignedCacheAuditSinkTests.test_checkpoint_failure_removes_temporary_and_governed_recovery_reconciles",
    ])


def phase_audit_signing_key_lifecycle() -> dict[str, Any]:
    return _run_v5316_tests([
        "RegistryLifecycleTests.test_rotation_cycle_is_rejected",
        "RegistryLifecycleTests.test_expired_or_revoked_artifact_key_is_rejected_at_artifact_timestamp",
    ])


def phase_audit_storage_ancestry() -> dict[str, Any]:
    first = _run_v5314_audit_tests([
        "SignedCacheAuditSinkTests.test_private_storage_directories_are_mode_0700",
    ])
    second = _run_v5316_tests([
        "ServiceAuthenticationTests.test_world_writable_socket_configuration_is_rejected",
    ])
    return {"tests": first["tests"] + second["tests"], "publisher_storage": first, "socket_policy": second}


def phase_validator_external_cancellation() -> dict[str, Any]:
    return _run_python_tests(
        "executable/tests/test_draft5_3_15_remediation.py",
        ["ValidatorExternalCancellationTests.test_sigterm_reaps_worker_and_escaped_descendant"],
        timeout=30,
    )


def phase_from_absent_artifact_regeneration() -> dict[str, Any]:
    return _run_python_tests(
        "executable/tests/test_draft5_3_16_regeneration.py",
        ["Draft5316RegenerationTests.test_schema_registry_regenerates_from_absence_byte_for_byte"],
    )


def phase_exact_cache_envelope_audit_binding() -> dict[str, Any]:
    schema = load_json("schemas/cache_stale_row_evidence_v4.schema.json")
    required = set(schema.get("required", []))
    exact = {
    ...
    assert_true(exact.issubset(required), "stale evidence does not bind the exact signed cache envelope")
    tests = _run_v5314_audit_tests([
        "HistoricalCacheBindingAndChronologyTests.test_bound_historical_row_emits_self_contained_exact_envelope_evidence",
    ])
    return {**tests, "exact_envelope_fields": sorted(exact)}


def phase_audit_event_schema_validation() -> dict[str, Any]:
    return _run_v5314_audit_tests([
        "SignedCacheAuditSinkTests.test_unknown_or_structurally_invalid_event_is_rejected_before_append",
        "SignedCacheAuditSinkTests.test_startup_revalidates_embedded_event_schema",
    ])


def phase_audit_peer_authentication() -> dict[str, Any]:
    tests = _run_v5316_tests([
        "ServiceAuthenticationTests.test_unapproved_peer_is_rejected_before_dispatch",
        "ServiceAuthenticationTests.test_world_writable_socket_configuration_is_rejected",
    ])
    evidence = load_json("validation/production_loader/draft5_3_16_nonroot_loader_evidence.json")
    assert_true(evidence.get("unrelated_uid_denied") is True, "cross-UID integration did not deny the unrelated peer")
    assert_true(evidence.get("publisher_socket_mode") == evidence.get("anchor_socket_mode") == "0660", "production audit sockets are not private")
    return {**tests, "so_peercred_integration": True, "unrelated_uid": evidence.get("unrelated_uid")}


def phase_strict_anchor_transitions() -> dict[str, Any]:
    return _run_v5316_tests([
        "AnchorTransitionTests.test_same_sequence_rewrite_and_registry_change_are_rejected",
        "AnchorTransitionTests.test_file_backed_anchor_is_explicitly_non_authoritative",
    ])


def phase_anchored_terminal_record_verification() -> dict[str, Any]:
    return _run_v5316_tests([
        "SinkAndPublisherTests.test_successor_rejects_unrelated_terminal_record",
    ])


def phase_global_audit_event_idempotency() -> dict[str, Any]:
    tests = _run_v5316_tests([
        "SinkAndPublisherTests.test_global_event_idempotency_survives_segment_rotation",
    ])
    evidence = load_json("validation/production_loader/draft5_3_16_nonroot_loader_evidence.json")
    assert_true(evidence.get("global_event_idempotency_survived_restart") is True, "production publisher restart did not reconcile the event globally")
    return {**tests, "production_restart_reconciliation": True}


def phase_recovery_authorization_lifecycle() -> dict[str, Any]:
    return _run_v5316_tests([
        "SinkAndPublisherTests.test_expired_and_replayed_recovery_authorizations_fail",
    ])


def phase_production_audit_service_launchers() -> dict[str, Any]:
    launchers = [
        ROOT / "executable/runtime/cache_audit_publisher_server.py",
        ROOT / "executable/runtime/cache_audit_anchor_server.py",
    ]
    assert_true(all(path.is_file() and os.access(path, os.X_OK) for path in launchers), "production audit service launcher is absent or not executable")
    for path in launchers:
        compile(path.read_text(encoding="utf-8"), str(path), "exec")
    return {"launchers": [path.relative_to(ROOT).as_posix() for path in launchers], "private_key_separation": True}


def phase_production_audit_service_roundtrip() -> dict[str, Any]:
    evidence = load_json("validation/production_loader/draft5_3_16_nonroot_loader_evidence.json")
    required_true = [
        "publisher_server_started", "anchor_server_started", "publication_round_trip_executed",
        "receipt_signature_verified", "anchor_advanced", "unrelated_uid_denied",
        "global_event_idempotency_survived_restart", "root_execution_rejection_retained",
    ]
    assert_true(evidence.get("status") == "passed", "production audit round trip did not pass")
    assert_true(all(evidence.get(field) is True for field in required_true), "production audit round trip omits a required proof")
    assert_true(
        [evidence.get("proof_service_uid"), evidence.get("publisher_uid"), evidence.get("anchor_uid")] == [65534, 65533, 65532],
        "production audit identities do not match the governed three-service model",
    )
    assert_true(evidence.get("file_anchor_authoritative") is False, "development file anchor was incorrectly represented as authoritative")
    return {"status": "passed", "identities": [65534, 65533, 65532], "restart_replay": True, "denied_peer": True}
...
def phase_production_nonroot_loader_contract() -> dict[str, Any]:
    ...
    assert_true(status == "passed", "current production-loader evidence must execute successfully")
    assert_true(
    ...
    return {**tests, "integration_status": status, "activation_satisfied": True}
...
REQUIRED_PHASE_FUNCTIONS: dict[str, Callable[[], dict[str, Any]]] = {
    ...
    "audit_rollback_resistance": phase_audit_rollback_resistance,
    "deadline_bounded_audit_publication": phase_deadline_bounded_audit_publication,
    "governed_audit_segment_transition": phase_governed_audit_segment_transition,
    "terminal_seal_capacity_reservation": phase_terminal_seal_capacity_reservation,
    "bounded_streaming_audit_verification": phase_bounded_streaming_audit_verification,
    "governed_audit_recovery": phase_governed_audit_recovery,
    "audit_signing_key_lifecycle": phase_audit_signing_key_lifecycle,
    "audit_storage_ancestry": phase_audit_storage_ancestry,
    "validator_external_cancellation": phase_validator_external_cancellation,
    "from_absent_artifact_regeneration": phase_from_absent_artifact_regeneration,
    "exact_cache_envelope_audit_binding": phase_exact_cache_envelope_audit_binding,
    "audit_event_schema_validation": phase_audit_event_schema_validation,
    "audit_peer_authentication": phase_audit_peer_authentication,
    "strict_anchor_transitions": phase_strict_anchor_transitions,
    "anchored_terminal_record_verification": phase_anchored_terminal_record_verification,
    "global_audit_event_idempotency": phase_global_audit_event_idempotency,
    "recovery_authorization_lifecycle": phase_recovery_authorization_lifecycle,
    "production_audit_service_launchers": phase_production_audit_service_launchers,
    "production_audit_service_roundtrip": phase_production_audit_service_roundtrip,
...
def main() -> int:
    _install_external_termination_handlers()
    bootstrap_reliability = "--bootstrap-reliability" in sys.argv
    if bootstrap_reliability:
        sys.argv.remove("--bootstrap-reliability")
        os.environ["WITNESS_VALIDATOR_BOOTSTRAP_RELIABILITY"] = "1"
    if len(sys.argv) == 2 and sys.argv[1] == "--external-cancellation-probe":
        command = [
            sys.executable, "-c",
            "import subprocess,sys,time; subprocess.Popen([sys.executable,'-c','import time; time.sleep(60)'],start_new_session=True); time.sleep(60)",
        ]
        _run_bounded_text_command(command, cwd=ROOT, timeout_seconds=60, stage="external-cancellation-probe", emit_progress=True)
        return 0
    ...
    inventory = load_json("PACKAGE_INVENTORY_v1_6_draft_5_3_16.json") if (ROOT / "PACKAGE_INVENTORY_v1_6_draft_5_3_16.json").is_file() else {"files": []}
    regular_directories = [
    ...
    report = {
        "schema_version": "draft5_3_16_validation_report/v1",
        ...
        "overall_status": (
            "bootstrap_passed_not_publishable"
            if reconciliation["all_required_passed"] and bootstrap_reliability
            else "passed" if reconciliation["all_required_passed"]
            else "failed"
        ),
        "bootstrap_reliability_mode": bootstrap_reliability,
    ...
    return 0 if report["overall_status"] in {"passed", "bootstrap_passed_not_publishable"} else 1
