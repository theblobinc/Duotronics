#!/usr/bin/env python3
"""Generate per-interpreter evidence and deterministically merge Draft 5.3.15."""
...
EVIDENCE_DIR = ROOT / "validation/python_matrix/draft5_3_15"
MATRIX_PATH = ROOT / "DRAFT5_3_15_PYTHON_MATRIX_VALIDATION.json"
COUNTS_PATH = ROOT / "DRAFT5_3_15_REGRESSION_COUNTS.json"
SUMMARY_PATH = ROOT / "DRAFT5_3_15_VALIDATION_SUMMARY.txt"
METADATA_PATH = ROOT / "PACKAGE_METADATA_v1_6_draft_5_3_15.json"
TARGETS = ("3.12", "3.13")
VALIDATOR = ROOT / "executable/validators/validate_draft5_3_15_corpus.py"

SPEC = importlib.util.spec_from_file_location("draft5315_validator_counts", VALIDATOR)
...
def record_current_interpreter() -> Path:
    ...
    if target not in TARGETS:
        raise SystemExit(f"Python {target} is outside the Draft 5.3.15 target matrix")
    record = {
...
def merge() -> dict[str, Any]:
    ...
    counts = {
        "schema_version": "draft5_3_15_regression_counts/v1",
        "package_version": "v1.6-draft-5.3.15",
        "generated_at": matrix["generated_at"],
        "draft5_3_14_baseline_tests_discovered": 290,
        "target_python_versions": list(TARGETS),
    ...
    summary = "\n".join([
        "Duotronic Witness Contract v1.6 Draft 5.3.15 — Generated Validation Summary",
        ...
        "Draft 5.3.14 baseline: 290 tests discovered",
