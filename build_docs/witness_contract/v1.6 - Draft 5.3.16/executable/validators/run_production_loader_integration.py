...
def _prepare_fixture(chroot_root: pathlib.Path) -> dict:
    ...
    audit_record_key = Ed25519PrivateKey.generate()
    audit_receipt_key = Ed25519PrivateKey.generate()
    audit_anchor_key = Ed25519PrivateKey.generate()
    proof_request_key = Ed25519PrivateKey.generate()
    publisher_request_key = Ed25519PrivateKey.generate()
    ...
    def signing_record(key_id: str, principal_id: str, key: Ed25519PrivateKey, scope: str) -> dict:
        return {
...
def run_integration() -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="witness-loader-chroot-") as directory:
        ...
        record_registry_sha = authority.sha256_bytes(authority.canonical_bytes(material["record_registry"]))
        receipt_registry_sha = authority.sha256_bytes(authority.canonical_bytes(material["receipt_registry"]))
        anchor_registry_sha = authority.sha256_bytes(authority.canonical_bytes(material["anchor_registry"]))
        ...
        genesis = authority.sign_record({
            ...
            "audit_signing_registry_sha256": record_registry_sha,
            "anchor_registry_sha256": anchor_registry_sha,
        ...
        if anchor_pid == 0:
            ...
            try:
                ...
                store = FileBackedMonotonicAnchorStore(
                    ...
                    anchor_registry_sha256=anchor_registry_sha, anchor_registry_records=anchor_records,
            ...
            if child == 0:
                ...
                try:
                    ...
                    anchor_client = UnixSocketAuditAnchorClient(
                        ...
                        verification_keys_by_id=anchor_keys, anchor_registry_sha256=anchor_registry_sha,
                    ...
                    publisher = AuditPublisherServer(
                        ...
                        receipt_signing_registry_sha256=receipt_registry_sha,
