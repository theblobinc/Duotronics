...
class RegistryLifecycleTests(unittest.TestCase):
    ...
    def test_rotation_cycle_is_rejected(self):
        first = Ed25519PrivateKey.generate(); second = Ed25519PrivateKey.generate()
    ...
    def test_expired_or_revoked_artifact_key_is_rejected_at_artifact_timestamp(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "expired, revoked, or unauthorized"):
            cache_audit.verify_artifact_key_lifecycle(
                registry_sha256="a" * 64, registry_records={"key:old": record},
        ...
        with self.assertRaisesRegex(RuntimeError, "revoked"):
            cache_audit.verify_artifact_key_lifecycle(
                registry_sha256="a" * 64, registry_records={"key:old": revoked},
                key_id="key:old", principal_id="principal:audit",
                required_scope="cache_audit_record_signing",
...
class AnchorTransitionTests(unittest.TestCase):
    def setUp(self):
        ...
        self.anchor = cache_audit.InMemoryMonotonicAuditAnchor(
            ...
            anchor_registry_sha256="b" * 64, anchor_registry_records=self.records,
    ...
    def genesis(self):
        return {
            ...
            "transition_authorization_sha256": "c" * 64,
            "audit_signing_registry_sha256": "d" * 64,
            "anchor_registry_sha256": "b" * 64,
            "previous_anchor_state_sha256": None, "updated_at": NOW.isoformat(),
        }
    ...
    def test_same_sequence_rewrite_and_registry_change_are_rejected(self):
        ...
        rewritten["tail_record_sha256"] = "e" * 64
        rewritten["previous_anchor_state_sha256"] = cache_audit.InMemoryMonotonicAuditAnchor.state_sha256(state)
        with self.assertRaisesRegex(RuntimeError, "exactly one"):
            self.anchor.compare_and_swap(cache_audit.InMemoryMonotonicAuditAnchor.state_sha256(state), rewritten)
        advanced = copy.deepcopy(rewritten)
        advanced["sequence"] = 1
        advanced["audit_signing_registry_sha256"] = "f" * 64
        with self.assertRaisesRegex(RuntimeError, "immutable"):
            self.anchor.compare_and_swap(cache_audit.InMemoryMonotonicAuditAnchor.state_sha256(state), advanced)
...
class ServiceAuthenticationTests(unittest.TestCase):
    ...
    def test_unapproved_peer_is_rejected_before_dispatch(self):
        ...
        try:
            payload = {"operation": "publish", "event": {}, "event_sha256": "a" * 64, "event_idempotency_key": "event:" + "a" * 64}
...
class SinkAndPublisherTests(unittest.TestCase):
    def setUp(self):
        ...
        self.validator = authority.CanonicalSchemaValidator(ROOT / "schemas")
        self.governance = Ed25519PrivateKey.generate()
        self.audit_key = Ed25519PrivateKey.generate()
        ...
        self.audit_registry_sha = "1" * 64
        self.anchor_registry_sha = "2" * 64
        self.receipt_registry_sha = "3" * 64
        self.anchor = cache_audit.InMemoryMonotonicAuditAnchor(
            self.anchor_key, key_id="anchor:key", signer_principal_id="anchor:principal",
            anchor_registry_sha256=self.anchor_registry_sha, anchor_registry_records=self.anchor_records,
            schema_validator=self.validator, clock=lambda: NOW,
        )
    ...
    def event(self, marker=1):
        ...
        decision = {
            "schema_version": "cache_key_validity_evidence/v1", "registry_sha256": "a" * 64,
        ...
        return {
            ...
            "cache_registry_sha256": "a" * 64, "signing_decision": decision,
    ...
    def genesis(self, suffix):
        return authority.sign_record({
            ...
            "anchor_namespace": "anchor:namespace", "audit_signing_registry_sha256": self.audit_registry_sha,
            "anchor_registry_sha256": self.anchor_registry_sha, "decision": "authorize_audit_genesis",
    ...
    def make_sink(self, suffix, *, provision=False, transition=None, predecessor_checkpoint=None, predecessor_terminal=None, clock=lambda: NOW):
        return cache_audit.SignedAppendOnlyAuditSink(
            ...
            audit_signing_registry_sha256=self.audit_registry_sha, audit_registry_records=self.audit_records,
            ...
            anchor_registry_sha256=self.anchor_registry_sha, clock=clock,
    ...
    def test_successor_rejects_unrelated_terminal_record(self):
        ...
        unrelated["event_sha256"] = authority.sha256_bytes(authority.canonical_bytes(unrelated_event))
        unrelated = authority.sign_record({k:v for k,v in unrelated.items() if k not in {"signed_payload_sha256","signature"}}, self.audit_key)
        ...
        transition = authority.sign_record({
            ...
            "predecessor_sealed_checkpoint_sha256": authority.sha256_bytes(authority.canonical_bytes(checkpoint)),
            "predecessor_terminal_record_sha256": authority.sha256_bytes(authority.canonical_bytes(unrelated)),
            ...
            "audit_signing_registry_sha256": self.audit_registry_sha,
            "anchor_registry_sha256": self.anchor_registry_sha,
    ...
    def test_global_event_idempotency_survives_segment_rotation(self):
        ...
        server = cache_audit_services.AuditPublisherServer(
            ...
            receipt_signing_registry_sha256=self.receipt_registry_sha,
            ...
            proof_service_principal_id="proof:service", request_verification_keys={"proof:req": Ed25519PrivateKey.generate().public_key()},
            socket_uid=os.getuid(), socket_gid=os.getgid(), clock=lambda: NOW,
        )
        try:
            payload = {"operation":"publish", "event":self.event(), "event_sha256":authority.sha256_bytes(authority.canonical_bytes(self.event())), "event_idempotency_key":"event:"+authority.sha256_bytes(authority.canonical_bytes(self.event()))}
            ...
            transition = authority.sign_record({
                "schema_version": "cache_audit_segment_transition/v1", "transition_id": "transition:1:2",
                "predecessor_segment_id": "segment:1", "predecessor_sealed_checkpoint_sha256": authority.sha256_bytes(authority.canonical_bytes(checkpoint)),
                "predecessor_terminal_record_sha256": authority.sha256_bytes(authority.canonical_bytes(terminal)),
                "predecessor_tail_record_sha256": anchor["tail_record_sha256"], "predecessor_anchor_state_sha256": authority.sha256_bytes(authority.canonical_bytes(anchor)),
                "successor_segment_id": "segment:2", "successor_anchor_epoch": 1,
                "audit_signing_registry_sha256": self.audit_registry_sha, "anchor_registry_sha256": self.anchor_registry_sha,
                "decision": "authorize_successor_segment", "created_at": NOW.isoformat(),
            }, self.governance)
    ...
    def test_expired_and_replayed_recovery_authorizations_fail(self):
        ...
        valid = authority.sign_record({
            **{k:v for k,v in expired.items() if k not in {"signed_payload_sha256","signature"}},
