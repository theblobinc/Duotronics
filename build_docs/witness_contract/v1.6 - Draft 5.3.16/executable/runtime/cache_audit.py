...
def socket_identity_sha256(path: Path) -> str:
...
def build_authenticated_service_request(
    *, operation: str, payload: dict[str, Any], peer_principal_id: str,
    request_signer_key_id: str, request_signing_key: Ed25519PrivateKey,
    ...
    unsigned = {
        ...
        "socket_identity_sha256": socket_identity_sha256(socket_path),
        "payload": payload,
...
def validate_governed_signing_registry(
    ...
    for record in registry["keys"]:
        ...
        try:
            key = Ed25519PublicKey.from_public_bytes(base64.urlsafe_b64decode(padded.encode("ascii")))
    ...
    for record in records.values():
        evidence = key_validity_evidence(registry_sha256=digest, record=record, evaluated_at=now)
...
def verify_artifact_key_lifecycle(
    *, registry_sha256: str, registry_records: Mapping[str, dict[str, Any]],
    ...
    evidence = key_validity_evidence(
        registry_sha256=registry_sha256, record=historical_record, evaluated_at=evaluated_at,
...
class InMemoryMonotonicAuditAnchor(AuditAnchorClient):
    ...
    def __init__(
        ...
        signer_principal_id: str, anchor_registry_sha256: str,
    ...
    def _unsigned_view(state: dict[str, Any]) -> dict[str, Any]:
        return {key: value for key, value in state.items() if key not in {"anchor_key_id", "signed_payload_sha256", "signature"}}
    ...
    def _validate_transition(current: dict[str, Any] | None, new: dict[str, Any]) -> None:
        if current is None:
            if (
                ...
                or new.get("previous_anchor_state_sha256") is not None
            ):
                raise RuntimeError("audit anchor genesis is not canonical")
            return
        current_digest = InMemoryMonotonicAuditAnchor.state_sha256(current)
        if new.get("previous_anchor_state_sha256") != current_digest:
        ...
        if new["segment_id"] == current["segment_id"]:
            immutable = (
                ...
                "previous_sealed_segment_tail_sha256", "transition_authorization_sha256",
                "audit_signing_registry_sha256", "anchor_registry_sha256",
            )
            ...
            if new["tail_record_sha256"] == current["tail_record_sha256"]:
            ...
            if new["tail_record_sha256"] is not None or new["segment_status"] != "open":
    ...
    def compare_and_swap(self, expected_state_sha256: str | None, unsigned_state: dict[str, Any]) -> dict[str, Any]:
        with self._lock:
            ...
            validity = verify_artifact_key_lifecycle(
                registry_sha256=self.anchor_registry_sha256,
...
class UnixSocketAuditAnchorClient(AuditAnchorClient):
    ...
    def __init__(
        ...
        anchor_registry_sha256: str, anchor_registry_records: Mapping[str, dict[str, Any]],
        schema_validator: CanonicalSchemaValidator,
        request_signing_key: Ed25519PrivateKey, request_signer_key_id: str,
...
class SignedAppendOnlyAuditSink:
    ...
    def __init__(
        ...
        audit_signing_registry_sha256: str, audit_registry_records: Mapping[str, dict[str, Any]],
        schema_validator: CanonicalSchemaValidator,
        ...
        self.audit_key_validity_evidence = verify_artifact_key_lifecycle(
            registry_sha256=self.audit_signing_registry_sha256,
    ...
    def _provision_segment(
        ...
        if current_anchor is None:
            ...
            if predecessor_terminal_sha != predecessor_checkpoint.get("tail_record_sha256") or predecessor_terminal_sha != current_anchor.get("tail_record_sha256"):
            ...
            if (
                ...
                or terminal_event.get("sealed_predecessor_tail_sha256") != predecessor_terminal_record.get("previous_audit_record_sha256")
            ):
    ...
    def _anchor_unsigned(
        ...
        return {
            ...
            "audit_signing_registry_sha256": self.audit_signing_registry_sha256,
            "anchor_registry_sha256": self.anchor_registry_sha256,
            "previous_anchor_state_sha256": self._state_digest(getattr(self, "_anchor_state", None)),
...
class UnixSocketAuditPublisherClient:
    ...
    def __init__(
        ...
        receipt_signing_registry_sha256: str, receipt_registry_records: Mapping[str, dict[str, Any]],
        schema_validator: CanonicalSchemaValidator,
        request_signing_key: Ed25519PrivateKey, request_signer_key_id: str,
        peer_principal_id: str,
        ...
        self.socket_path = Path(socket_path)
        self.verification_keys_by_id = dict(verification_keys_by_id)
        self.receipt_signing_registry_sha256 = receipt_signing_registry_sha256
    ...
    def __call__(self, evidence: dict[str, Any], *, deadline_monotonic: float) -> dict[str, Any]:
        ...
        verify_artifact_key_lifecycle(
            registry_sha256=self.receipt_signing_registry_sha256,
        ...
        if (
            ...
            or receipt.get("receipt_signing_registry_sha256") != self.receipt_signing_registry_sha256
        ):
