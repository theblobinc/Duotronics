...
from cache_audit import (
    ...
    socket_identity_sha256, verify_artifact_key_lifecycle,
)
...
class FileBackedMonotonicAnchorStore:
    ...
    def _verify_lines(self, data: bytes) -> dict[str, Any] | None:
        ...
        for line in data.splitlines():
            ...
            if state.get("previous_anchor_state_sha256") != self.state_sha256(previous):
    ...
    def compare_and_swap(self, expected_state_sha256: str | None, unsigned_state: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            if unsigned_state.get("previous_anchor_state_sha256") != self.state_sha256(current):
...
class DurableEventIdIndex:
    ...
    def __init__(self, path: Path, *, expected_uid: int | None = None):
        ...
        try:
            ...
            connection.execute("CREATE TABLE IF NOT EXISTS committed_events (event_id TEXT PRIMARY KEY, event_sha256 TEXT NOT NULL, result_json TEXT NOT NULL, committed_at TEXT NOT NULL)")
            connection.commit()
        finally:
            connection.close()
    ...
    def commit(self, event_id: str, event_sha256: str, result: dict[str, Any], committed_at: str) -> dict[str, Any]:
        ...
        try:
            ...
            row = connection.execute("SELECT event_sha256,result_json FROM committed_events WHERE event_id=?", (event_id,)).fetchone()
            if row is not None:
                if row[0] != event_sha256:
                    raise RuntimeError("global audit event idempotency collision")
                connection.rollback()
                return canonical_json_loads(row[1])
            connection.execute(
                "INSERT INTO committed_events(event_id,event_sha256,result_json,committed_at) VALUES(?,?,?,?)",
                (event_id, event_sha256, encoded, committed_at),
            )
            connection.commit()
            return result
        finally:
...
class GovernedUnixServer(socketserver.ThreadingMixIn, socketserver.UnixStreamServer):
    ...
    def __init__(
        ...
        request_verification_keys_by_principal: Mapping[str, Mapping[str, Ed25519PublicKey]],
        ...
        self.socket_identity_sha256 = socket_identity_sha256(self.socket_path)
    ...
    def authenticate(self, request: dict[str, Any], peer: dict[str, Any]) -> dict[str, Any]:
        ...
        if request.get("socket_identity_sha256") != self.socket_identity_sha256:
        ...
        if not isinstance(payload, dict) or request.get("payload_sha256") != sha256_bytes(canonical_bytes(payload)):
...
class AuditAnchorServer:
    def __init__(
        ...
        request_verification_keys: Mapping[str, Ed25519PublicKey],
        socket_uid: int | None = None, socket_gid: int | None = None,
    ):
        self.store = store
    ...
    def dispatch(self, request: dict[str, Any], peer: dict[str, Any]) -> dict[str, Any]:
        ...
        if operation == "compare_and_swap":
            ...
            return {
                "status": "ok",
                "state": self.store.compare_and_swap(request.get("expected_state_sha256"), state),
                "peer": peer,
            }
...
class AuditPublisherServer:
    ...
    def __init__(
        ...
        receipt_signer_principal_id: str, receipt_signing_registry_sha256: str,
    ...
    def _receipt(self, result: dict[str, Any], event_id: str, event_sha: str, decision: str) -> dict[str, Any]:
        ...
        unsigned = {
            ...
            "receipt_signing_registry_sha256": self.receipt_signing_registry_sha256,
