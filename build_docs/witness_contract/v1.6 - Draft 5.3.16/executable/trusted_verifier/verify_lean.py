...
def validate_handoff(schema_validator: CanonicalSchemaValidator, invocation: dict, profile: dict) -> tuple[dict, str]:
    ...
    if binding_module["olean_path"] != binding_path or binding_module["olean_sha256"] != manifest["binding_olean_sha256"] or binding_module["source_sha256"] != manifest["binding_module_sha256"]:
    ...
    if not LEAN.is_file() or sha256_file(LEAN) != manifest["lean_executable_sha256"] or manifest["lean_executable_sha256"] != profile["lean_executable_sha256"]:
        raise ValueError("handoff Lean executable digest differs from the trusted pinned executable")
    expected_environment_sha256 = sha256_bytes(canonical_bytes(_expected_compilation_environment()))
    if manifest["compilation_environment_sha256"] != expected_environment_sha256:
    ...
    for module in manifest["compiled_modules"]:
        reconstructed_commands.append({
            ...
            "argv_sha256": sha256_bytes(canonical_bytes(_reconstructed_compile_command(module))),
        })
...
def main() -> int:
    ...
    if request.get("schema_version") != "wc_lean_verifier_request/v5":
        return 64
    try:
        handoff_manifest, handoff_manifest_sha256 = validate_handoff(schema_validator, invocation, profile)
    except Exception:
        return 69
