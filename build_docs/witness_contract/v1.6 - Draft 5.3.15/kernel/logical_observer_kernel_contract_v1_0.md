...
## 17. Draft 5.2.1 Lean proof authority extension

Draft 5.2.1 adds `check_proof()` to the syscall table. The syscall compiles the active Lean/Lake package, scans executable Lean code for prohibited proof placeholders, extracts theorem status, and emits `LeanCompilerWitness` plus `ProofWitness`.

A theorem or proof-verified claim promotion now requires `TheoremPromotionGate` in addition to the existing proof witness, policy decision, and non-collapse transition. Advisory Lean checks are useful for development, but production theorem promotion requires a strict passed Lean compiler witness.
