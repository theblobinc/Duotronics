...
def _prepare_fixture(chroot_root: pathlib.Path) -> None:
    ...
    audit_record = {
        "key_id": "cache:audit:key:integration",
        "principal_id": "cache:audit:publisher:integration",
        "public_key_base64url": authority.public_key_raw_b64url(cache_audit.public_key()),
    ...
    audit_registry = authority.sign_record({
        "schema_version": "cache_audit_signing_registry/v1",
        "registry_id": "cache:audit:registry:integration",
        "governance_key_id": "governance:key:integration",
        "created_at": "2026-08-01T00:00:00Z", "keys": [audit_record],
    }, governance)
    write("cache-audit-registry.json", authority.canonical_bytes(audit_registry))
    ...
    config = {
        ...
        "cache_audit_signer_principal_id": audit_record["principal_id"],
        "cache_audit_signer_key_id": audit_record["key_id"],
...
def run_integration() -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="witness-loader-chroot-") as directory:
        ...
        socket_path = chroot_root / "run/witness-audit/publisher.sock"
        socket_path.parent.mkdir(parents=True, mode=0o755)
        socket_path.parent.chmod(0o755)
        publisher_socket = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
        publisher_socket.bind(str(socket_path))
        publisher_socket.listen(1)
        os.chown(socket_path, PUBLISHER_UID, PUBLISHER_GID)
            ...
            if child == 0:
                ...
                try:
                    ...
                    result = {
                        ...
                        "cache_audit_sink": "distinct_identity_unix_socket_signed_receipt_client",
                        "cache_audit_publisher_socket_owner": pathlib.Path("/run/witness-audit/publisher.sock").stat().st_uid,
