#!/usr/bin/env python3
"""Generate per-interpreter evidence and deterministically merge Draft 5.3.11."""
...
EVIDENCE_DIR = ROOT / "validation/python_matrix/draft5_3_11"
MATRIX_PATH = ROOT / "DRAFT5_3_11_PYTHON_MATRIX_VALIDATION.json"
COUNTS_PATH = ROOT / "DRAFT5_3_11_REGRESSION_COUNTS.json"
SUMMARY_PATH = ROOT / "DRAFT5_3_11_VALIDATION_SUMMARY.txt"
METADATA_PATH = ROOT / "PACKAGE_METADATA_v1_6_draft_5_3_11.json"
TARGETS = ("3.12", "3.13")
VALIDATOR = ROOT / "executable/validators/validate_draft5_3_11_corpus.py"

SPEC = importlib.util.spec_from_file_location("draft5311_validator_counts", VALIDATOR)
...
def record_current_interpreter() -> Path:
    ...
    if target not in TARGETS:
        raise SystemExit(f"Python {target} is outside the Draft 5.3.11 target matrix")
    record = {
...
def merge() -> dict[str, Any]:
    ...
    counts = {
        "schema_version": "draft5_3_11_regression_counts/v1",
        "package_version": "v1.6-draft-5.3.11",
        "generated_at": matrix["generated_at"],
        "draft5_3_10_baseline_tests_discovered": 230,
        "target_python_versions": list(TARGETS),
    ...
    summary = "\n".join([
        "Duotronic Witness Contract v1.6 Draft 5.3.11 — Generated Validation Summary",
        ...
        "Draft 5.3.10 baseline: 230 tests discovered",
