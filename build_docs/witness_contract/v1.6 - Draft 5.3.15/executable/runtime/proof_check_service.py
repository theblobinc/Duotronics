#!/usr/bin/env python3
"""Authenticated, cache-verifying proof-check boundary for Draft 5.3.15."""
...
def load_production_application(config_root: Path = Path("/etc/witness-authority")) -> ProofCheckApplication:
    ...
    audit_registry = load_trusted_canonical_json(
        config_root, config["cache_audit_signing_registry_file"], expected_uid=expected_uid,
    )
    audit_registry_sha256, audit_records, audit_verification_keys = validate_governed_signing_registry(
        audit_registry, governance_key, schema_validator=validator,
        surface="cache_audit_signing_registry", required_scope="cache_audit_record_signing",
        evaluated_at=now,
    )
    audit_record = audit_records.get(config["cache_audit_signer_key_id"])
    if audit_record is None:
        raise RuntimeError("configured cache audit key is absent from its governed registry")
    ...
    governed_audit_key = audit_verification_keys[config["cache_audit_signer_key_id"]]
    if audit_public != public_key_raw_b64url(governed_audit_key):
        raise RuntimeError("cache audit public key does not match its governed registry record")
    audit_validity = key_validity_evidence(
        registry_sha256=audit_registry_sha256, record=audit_record, evaluated_at=now,
    )
    validator.validate("cache_audit_key_validity_evidence", audit_validity)
    if audit_validity["decision"] != "active_time_valid_rotation_valid":
        raise RuntimeError("cache audit signing key is outside its governed active interval")
    ...
    audit_sink = UnixSocketAuditPublisherClient(
        ...
        verification_keys_by_id=audit_verification_keys,
        audit_signing_registry_sha256=audit_registry_sha256,
        schema_validator=validator, expected_socket_uid=publisher_uid,
    )
