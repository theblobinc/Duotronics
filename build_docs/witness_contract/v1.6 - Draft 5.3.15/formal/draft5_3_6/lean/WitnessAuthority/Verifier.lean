...
partial def levelFingerprint : Level → String
  ...
  | .param name => s!"p({name})"
  | .mvar ident => s!"m({ident})"

partial def expressionFingerprint : Expr → String
  | .bvar index => s!"b({index})"
  | .fvar ident => s!"f({ident})"
  | .mvar ident => s!"m({ident})"
  | .sort level => s!"sort({levelFingerprint level})"
  | .const name levels => s!"c({name};{String.intercalate "," (levels.toList.map levelFingerprint)})"
  | .app fn arg => s!"app({expressionFingerprint fn},{expressionFingerprint arg})"
  | .lam name type body info => s!"lam({name};{info};{expressionFingerprint type};{expressionFingerprint body})"
  | .forallE name type body info => s!"forall({name};{info};{expressionFingerprint type};{expressionFingerprint body})"
...
def inspectDeclaration
  ...
  | some declarationInfo =>
      ...
      let unsafe := declarationUnsafe declarationInfo || closure.any fun name =>
        match env.find? name with
      ...
      return {
        ...
        unsafeDependencyPresent := unsafe, opaqueDependencyPolicyPassed := opaquePolicy,
        expectedTypeFingerprint := expressionFingerprint expectedType,
