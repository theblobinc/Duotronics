# Duotronic Source Architecture Overview v1.6

**Status:** Internal source architecture draft  
**Version:** 1.6-source-overview  
...
The v1.6 overview keeps the v1.3 architecture map, fixes companion-document status entries, and adds a concise reference to the new DW-SSM research profile.
...
| Research Profiles | existing - research | EDO, acoustics, QCD analogy, selective witness state-space, GCD-jump recurrences, pronic-centered polygon families, and future physical profiles |
...
## 17. Pronic-centered polygon profile in the corpus

> **Status tag:** research
> **Related tags:** reference

The v1.3 Draft 4 corpus adds **Pronic-Centered Polygon Families as a Duotronic Visual Axiom Profile v1.0** as a bounded research profile.

The profile belongs beside the geometry and witness-family material. Its role is to formalize the user's visual axiom as a deterministic geometry-family generator:

```text
center dot -> even outer vertex counts -> pronic shell boundaries -> polygon chain selection -> canonical witness
```

The profile uses quaternions only as an optional orientation reducer:

```text
polygon witness -> optional quaternion pose normalization -> canonical geometry witness
```

The corpus-level boundary is:

```text
Quaternions rotate or normalize geometry.
Pronic shells generate the polygon family.
DPFC bridges and serializes canonical objects.
The Witness Contract decides trust.
```

The profile is intentionally referenced only in the owning documents: Source Architecture, DPFC boundary language, Witness Contract, Corpus Index, and the standalone pronic-centered polygon profile.
