# Duotronic Polygon Family Calculus (DPFC v5.11)

**Status:** Research specification draft  
**Version:** 5.11-standalone-revision  
...
## Revision 5.11 geometry-profile boundary note

> **Status tag:** reference
> **Related tags:** research

The v5.11 revision keeps DPFC focused on arithmetic, family-native values, bridges, canonical serialization, successor, conversion, and export policy. It adds concise boundary language for three optional runtime/research profiles:

1. DW-SSM, which may consume canonical DPFC features for selective recurrent state updates;
2. GCD-jump recurrences, which may use ordinary GCD arithmetic as an adapter-side sparse witness gate;
3. pronic-centered polygon families, which may generate center-aware polygon witnesses through pronic shells, even outer vertex counts, and center-presence flags.

None of these profiles redefine DPFC arithmetic. The governing boundary is:

```text
external/runtime/geometry pattern -> adapter or family declaration -> validation -> canonicalization -> optional runtime or witness use
```

A profile may consume canonical DPFC features or define a new declared family bridge. It may not mutate successor, addition, multiplication, inter-family conversion, canonical identity, or export correction.

## Prior v5.8 expansion summary

> **Status tag:** reference

The earlier v5.8 expansion kept all v5.4 material and expanded the document toward an internal source specification. The main additions are source architecture scaffolding, an absence-zero-origin rationale, a full ontology map, related-concept comparisons, a formal theorem registry, a normalizer profile language, a deeper geometry/group-action chapter, expanded conformance-harness guidance, migration/versioning rules, and a corpus-map appendix.

The v5.10 and v5.11 additions do not mutate the DPFC arithmetic core. They make the core easier to review, implement, falsify, and extend. In particular, the new material reinforces four boundaries:
...
DPFC v5.11 defines the Duotronic Polygon Family Calculus as a layered mathematical and implementation specification. It preserves the presence-first Duotronic stance:
...
The v5.11 revision additionally polishes the source-spec layer: single-primary status tags replace hybrid tags, revision summaries separate core refinements from revision-specific additions, and corpus-map entries now declare whether they are existing, draft-needed, or future work.

DPFC v5.11 should be read as a family-parametric representational layer over a shared positive core. It can be implemented, tested, serialized, converted, transported, and replayed. It is not a completed physical theory, and it does not claim that polygon diagrams, QCD symmetry diagrams, or virtual-particle language prove Duotronic arithmetic.
...
DPFC v5.11 does not claim that:
...
A future DPFC-Geometric-Successor profile may allow geometric walks to define successor directly, but that is not part of the v5.9 core.
...
```text
family:hex6 schema_version:dpfc-family@v5.9 digits:1 4 6
```
...
This section embeds the minimum adapter vocabulary needed to read DPFC v5.11 without relying on external source documents.
...
A DPFC v5.11 implementation conforms at the core layer if it implements:
...
{
  "family_id": "hex6",
  "schema_version": "dpfc-family@v5.9",
  "modulus": 6,
...
DPFC v5.11 is a disciplined, one-based, family-native calculus for realized magnitudes, scalar roles, witness geometry, canonical identity, and inter-family conversion.
...
The following assumptions are active for C2 theorem candidates in the DPFC v5.11 core.
...
```yaml
fixture_pack: dpfc-v5.9-core-fixtures
...
fixtures:
  ...
  - fixture_id: DPFC-FIXTURE-4-REFL3-ENCODE-10
    ...
    expected:
      word: [r3, r1]
      positional_value: 10
      canonical_storage: "family:refl3 schema_version:dpfc-family@v5.9 digits:3 1"
...
if __name__ == "__main__":
    ...
    print("DPFC v5.11 reference self-test passed")
```
...
```yaml
fixture_pack: dpfc-v5.9-edo31-fixtures
...
DPFC v5.11 treats these separations as architectural invariants:
...
output_schema: dpfc-family@v5.9
...
## 40. DPFC boundary for selective witness state-space models

> **Status tag:** reference
> **Related tags:** research

The optional DW-SSM profile belongs to the Witness/Runtime side of the corpus. DPFC may provide canonical features to such a runtime, but DPFC does not define neural state-space dynamics.

A selective witness state-space runtime MAY consume a canonical DPFC object through a feature map such as:

\[
\operatorname{Feat}_{\mathrm{DPFC}}(M)=
(\mathrm{family\_id},\mathrm{schema\_version},\mathrm{digit\_ordinals},\Phi_F(M),\mathrm{normalizer\_version},\mathrm{export\_policy}).
\]

The runtime MAY use these features for routing, gating, lookup selection, retention diagnostics, or state-update modulation after canonicalization succeeds.

It MUST NOT reinterpret or mutate:

1. family successor;
2. family-to-core bridge;
3. realized-step addition;
4. multiplication;
5. inter-family conversion;
6. canonical serialization;
7. export correction rules;
8. absence/numeric-zero separation.
...
The nonnegative export correction remains:

\[
E_{\mathbb U}(\mu_m\oplus_u\mu_n)=E_{\mathbb U}(\mu_m)+E_{\mathbb U}(\mu_n)+1.
\]

So the DPFC rule is simple: **canonical DPFC objects may inform runtime state selection; runtime state selection may not redefine DPFC objects.**

## 41. DPFC boundary for GCD-jump recurrence profiles

> **Status tag:** reference
> **Related tags:** research

The GCD-jump recurrence profile uses ordinary external arithmetic:

\[
R(n)=R(n-1)+\gcd(n,R(n-1)).
\]

This recurrence may be useful as a sparse witness-gate pattern because most increments are trivial and some increments are nontrivial jumps. That does not make it a DPFC successor law.

If a GCD-jump state is imported into a Duotronic runtime, it must enter through an adapter boundary:

```text
gcd recurrence state -> adapter profile -> validation -> canonicalization -> witness/runtime use
```

The adapter MAY emit a canonical witness record such as:

```yaml
GCDJumpWitness:
  recurrence_profile: rowland_seed_7
  step_n: integer
  previous_state: integer
  gcd_increment: integer
  event_class: trivial_continuation | nontrivial_jump
  adapter_version: string
```

The adapter MUST NOT claim that a GCD jump is a native DPFC digit, native DPFC zero, native DPFC absence, or DPFC arithmetic proof.

The intended DPFC relationship is limited:

1. DPFC may serialize or canonicalize the adapter output after declaration;
2. DPFC may represent the resulting positive values through declared families;
3. DPFC may test whether the adapter preserves declared invariants;
4. DPFC does not become a GCD recurrence system.


## 42. DPFC boundary for pronic-centered polygon families

> **Status tag:** reference
> **Related tags:** research

The pronic-centered polygon profile is a geometry/witness family profile. It defines a center-aware polygon generator using:

1. one center dot as least realized presence;
2. even outer vertex counts;
3. pronic shell boundaries;
4. center-present and center-absent structural states;
5. deterministic polygon chain selection;
6. optional quaternion orientation reduction.

It does not redefine DPFC successor, addition, multiplication, conversion, serialization, or export correction.

### 42.1 Pronic shell boundary
...
Shell \(n\) contains the even outer vertex counts:
...
### 42.2 Center flag boundary

The center flag is structural state:

\[
c\in\{\texttt{center\_absent},\texttt{center\_present}\}.
\]

The occupied site count under exported profile arithmetic is:

\[
A(v,c)=v+\epsilon(c).
\]

The single-dot configuration:
...
It is not absence and not exported numeric zero.

### 42.3 Reference ring bridge
...
This reference bridge is valid for `ring_identity_only` mode. If shape metadata is canonical identity, the profile must declare an additional deterministic enumeration over side count, stride, offset, and reducer fields.

### 42.4 Shape selection boundary

For a ring of \(v\) outer vertices, an \(s\)-sided regular polygon may be selected only when:
...
one canonical chain form is:

\[
C_{s,a}(v)=\{a+(\ell-1)d \pmod v\mid 1\leq \ell\leq s\}.
\]

This is geometry/witness selection. It is not the DPFC successor law.

### 42.5 Quaternion reducer boundary

Quaternion rotation may be used as an optional orientation reducer:

\[
v'=qv\bar q.
\]

The reducer may help normalize pose, orientation, or interpolation. It is not the generator of the pronic-centered family and is not DPFC arithmetic.

The DPFC rule is:

> Pronic shells generate a candidate family geometry; DPFC validates, bridges, serializes, and tests the canonical object.
