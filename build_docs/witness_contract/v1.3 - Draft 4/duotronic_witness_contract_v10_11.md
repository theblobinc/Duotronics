# Duotronic Witness Contract v10.11

**Status:** Research specification draft  
**Version:** 10.11-standalone-revision  
...
## Revision 10.11 runtime and geometry-profile addition note

> **Status tag:** reference
> **Related tags:** research

The v10.11 revision keeps the Witness Contract focused on runtime trust, normal-form construction, transport-before-semantics, recurrence, lookup memory, replay, policy, and safe promotion. It recognizes three optional profiles:

1. DW-SSM, for selective recurrent state updates controlled by canonical witness context;
2. GCD-jump sparse witness gates, for cheap recurrence/event gates that can trigger candidate witness updates;
3. pronic-centered polygon families, for center-aware polygon geometry witnesses and optional molecule/atom-like research fixtures.

These profiles are not new hard rules. They are bounded ways to make L2/L2M recurrence and geometry witness construction more concrete. The governing trust path remains:

```text
raw evidence -> validation -> canonicalization -> policy gate -> optional recurrent or geometry witness use
```

A generated polygon, detected jump, or orientation reducer result is a candidate witness, not authority. It must still pass validation, canonicalization, and policy gating before it may affect authoritative state.

## Prior v10.8 expansion summary

> **Status tag:** reference

The earlier v10.8 expansion kept all v10.4 material and expanded the Witness Contract into a fuller internal runtime source specification. The main additions are a source-architecture view from the witness side, a threat model, a witness lifecycle chapter, a failure-code registry, a replay identity profile, a telemetry catalog, policy-shield decision tables, an expanded conformance harness, migration/versioning rules, and a full end-to-end worked trace.

The v10.10 and v10.11 additions do not weaken any hard rule. They make the hard rules more operational:
...
The Duotronic Witness Contract v10.11 defines the runtime discipline for systems that operate on witness-bearing state.
...
The v10.11 revision keeps the strong v9/v10 idea that stable witness facts must be in canonical normal form before they are trusted, stored, retrieved, compared, replayed, or promoted.
...
A conforming implementation MAY realize L3 together with L4, L5, replay, and meta-object normalization as a standalone meta runtime package that consumes canonical family objects and witness metadata from lower layers.
...
```json
{
  ...
  "source_profile": "witness-key@v10.9",
  "target_profile": "witness-key@v10.9",
...
A standalone meta runtime is also conforming if it exposes an equivalent boundary such as:

```text
harness/runtime/runtime_ref/api
harness/runtime/runtime_ref/state
harness/runtime/runtime_ref/meta/witness
harness/runtime/runtime_ref/meta/diagnostics
harness/runtime/runtime_ref/meta/replay
harness/runtime/runtime_ref/meta/acceptance
harness/runtime/runtime_ref/policy/constraints
harness/runtime/runtime_ref/architecture/manager
harness/runtime/runtime_ref/architecture/state_migration
harness/runtime/runtime_ref/architecture/review
harness/runtime/runtime_ref/meta_objects/normalization
```

That boundary may consume canonical witness objects and DPFC family objects, but it MUST NOT bypass lower-layer validation or silently redefine canonical identity.
...
```json
{
  "schema_version": "witness-key@v10.9",
  "family_id": "hex6",
  "family_schema_version": "dpfc-family@v5.9",
  "normalizer_version": "hex6-normalizer@v1",
  "geometry_registry_version": null,
  "canonical_storage": "family:hex6 schema_version:dpfc-family@v5.9 digits:1 4 6",
...
### 30.5 MetaRecurrentWitness

```json
{
  "schema_version": "meta-recurrent-witness@v1",
  "log_decay_offset": {"temporal_local": 0.01},
  "callback_logit": 0.0,
  "meta_momentum": {"temporal_local": 0.02},
  "controller_confidence": 0.64,
  "accepted_updates": 3,
  "rejected_updates": 1,
  "epoch_step_count": 12
}
...
### 30.6 MetaDiagnostics

```json
{
  "schema_version": "meta-diagnostics@v1",
  "diagnostic_id": "l3diag-001",
  "objective_bundle_hash": "sha256:bundle-1",
  "replay_spec_hash": "sha256:replay-1",
  "replay_identity": "sha256:replay-1:sha256:bundle-1",
  "controller_confidence": 0.64,
...
### 30.7 ShadowReplaySpec

```json
{
  "schema_version": "shadow-replay-spec@v1",
  "version": 1,
  "seed": 7,
  "sequence_ids": ["seq-1", "seq-2"],
  "slice_boundaries": [[0, 1024]],
...
### 30.8 ArchitecturalWitness

```json
{
  "schema_version": "architectural-witness@v1",
  "incumbent_generation": 4,
  "promotion_budget": 1,
  "candidate_queue": [],
  "family_value_ema": {"cross_source": 0.62},
  "family_cost_ema": {"cross_source": 0.21}
}
```

### 30.9 PolicyShieldSnapshot

```json
{
  "schema_version": "policy-shield-snapshot@v1",
  "objective_version": "meta_runtime_stability@v1",
  "objective_bundle_hash": "sha256:bundle-1",
  "hard_constraints": {"max_families": 20, "max_memory_mb": 512, "max_step_time_ms": 50},
  "approval_policy": {"l3_auto_apply": true},
  "trust_region_policy": {"max_l3_log_decay_step": 0.03, "max_l3_callback_logit_step": 0.15}
}
```

### 30.10 MetaObjectAssertionBundle

```json
{
  "schema_version": "meta-object-assertion-bundle@v1",
  "bundle_id": "meta-bundle-1",
  ...
  "assertions": [
    {
      ...
      "category_path": "semantic/motif",
      "canonical_digest": "sha256:assertion-1"
    }
  ],
  "relations": [],
  "recurrence_edges": []
...
The Duotronic Witness Contract v10.11 is a trust contract.
...
```yaml
policy_id: witness-policy@v10.9-minsafe
mode_defaults:
...
```json
{
  ...
  "target_schema": "witness-key@v10.9",
  "affected_components": [
    "key_schema",
    "family_registry",
    "normalizer",
...
# Appendix F - release checklist for v10.9 implementation

> **Status tag:** reference

Before a v10.9 implementation is called conforming, verify:
...
```yaml
fixture_pack: witness-v10.9-runtime-fixtures
...
fixtures:
  - fixture_id: WITNESS-FIXTURE-1-HEX6-NORMAL-FORM
    ...
    expected:
      canonical_storage: "family:hex6 schema_version:dpfc-family@v5.9 digits:1 4"
...
HEX6 = Family("hex6", "dpfc-family@v5.9", ("h1", "h2", "h3", "h4", "h5", "h6"))
REFL3 = Family("refl3", "dpfc-family@v5.9", ("r1", "r2", "r3"))
...
def run_reference_self_test() -> None:
    assert canonicalize_witness_key_bundle({"family_id": "hex6", "family_schema_version": "dpfc-family@v5.9", "digits": ["h1", "h4"]})["core_magnitude"] == "mu_10"
...
if __name__ == "__main__":
    ...
    print("Witness Contract v10.9 reference self-test passed")
```
...
```yaml
fixture_pack: witness-v10.9-spectral-edo-fixtures
...
14. audit retention.
15. stale replay rejection in L3;
16. stale objective-bundle rejection in L3;
17. deterministic meta-object normalization;
18. L4 migration and rollback discipline;
19. L5 veto on hard-constraint violation.
...
A v10.9 implementation should not pass CI unless:
...
## S.4 Meta-runtime conformance

> **Status tag:** normative

If an implementation ships a standalone meta runtime, that layer MUST prove at least the following locally, without relying on production-side services:

1. stale replay identity rejects an L3 update;
2. stale objective bundle rejects an L3 update;
3. L5 hard-constraint violation yields veto or rollback action rather than silent acceptance;
4. any L4 proposal with semantic or state-shape impact includes migration and rollback discipline;
5. meta-object normalization is deterministic and replay-stable.
...
```yaml
trace_id: witness-trace-v10.9-example-1
transport:
  kind: dbp_frame
  shape_valid: true
...
# Appendix Y - Lab-Derived Witness Evidence Chains

> **Status tag:** reference

The lab archive supplies concrete evidence chains of the form `source artifact -> parser/canonicalizer -> run spec -> benchmark result -> gate report -> comparison artifact -> manifest/evidence bundle`.

```yaml
LabWitnessEvidence:
  source_lab: polygon-lab-v4 | engine-lab-v5
  ...
  trust_status: audit | research_valid | rejected
```

## Y.1 Witness import boundary

> **Status tag:** normative

A lab artifact may be used as witness evidence only after parser validation, artifact hashing, schema resolution, gate evaluation, and replay identity binding. External engine logs, CSV files, JSON files, screenshots, and plots MUST NOT become trusted witness facts merely because they appear in the archive.
...
## 36. Selective Witness State-Space Dynamics

> **Status tag:** research
> **Related tags:** reference

This section defines the bounded Duotronic interpretation of selective state-space dynamics. It is a runtime/research profile for L2, L2M, L3, and L5 behavior. It does not redefine DPFC arithmetic, family conversion, transport validation, canonicalization, migration, or policy authority.

The profile name is:

```text
DW-SSM: Duotronic Witness Selective State-Space Model
```

A standard state-space recurrence has the abstract form:
...
In a Duotronic witness system, the state-selection parameters are conditioned only by trusted or policy-bounded information:
...
2. \(u_t\) is the event/input embedding admitted after validation;
3. \(k_t^{\mathrm{nf}}\) is the canonical normal-form witness key;
4. \(d_t^{\mathrm{norm}}\) is canonicalization and normalization diagnostics;
5. \(p_t\) is the L5 policy state;
6. \(r_t\) is retention diagnostic context under declared metric specs.

### 36.1 Canonicalization-before-selection invariant

> **Status tag:** normative

A conforming selective witness state-space implementation MUST NOT compute authoritative \(A_t\), \(B_t\), \(C_t\), \(D_t\), state-write strength, lookup-injection strength, or promotion evidence from raw untrusted witness evidence when a canonical normal-form path exists.

If canonicalization fails, the implementation MUST use one of the policy states already defined by this contract: `degraded`, `family_bypass`, `transport_bypass`, `lookup_bypass`, or `full_bypass`.

### 36.2 Witness authority gate

> **Status tag:** reference

The witness authority gate is a bounded multiplier on state update authority:

\[
\alpha_t = \sigma(g_\alpha(k_t^{\mathrm{nf}}, d_t^{\mathrm{norm}}, p_t, r_t)).
\]

The gated update is:

\[
s_t = s_{t-1} + \alpha_t \odot (\widehat{s}_t - s_{t-1}),
\]

where:

\[
\widehat{s}_t = A_t s_{t-1} + B_t u_t.
\]

When transport validation fails, \(\alpha_t\) MUST be forced to zero for semantic payload updates. When the L5 mode is `full_bypass`, \(\alpha_t\) MUST be zero for authoritative state writes.

### 36.3 Input admission, retention, and lookup gates

> **Status tag:** reference

Selective witness state-space implementations SHOULD separate at least three gates:
...
\[
\lambda_t = \sigma(g_\lambda(k_t^{\mathrm{nf}}, r_t^{\mathrm{lookup}}, d_t^{\mathrm{norm}}, p_t)).
\]

Where:

1. \(\beta_t\) controls new event/input admission;
2. \(\rho_t\) controls previous-state retention or decay;
3. \(\lambda_t\) controls L2M lookup injection.

A common update form is:

\[
s_t = \rho_t \odot s_{t-1} + \beta_t \odot B_t u_t + \lambda_t \odot m_t,
\]

where \(m_t\) is a gated lookup-memory mixture from L2M.

Low normalization confidence MUST NOT increase \(\beta_t\), \(\lambda_t\), or promotion authority. A practical monotonic safety condition is:

\[
\frac{\partial \beta_t}{\partial c_t^{\mathrm{norm}}} \geq 0,
\qquad
\frac{\partial \lambda_t}{\partial c_t^{\mathrm{norm}}} \geq 0,
\]

for confidence coordinate \(c_t^{\mathrm{norm}}\), with policy clamps applied before use.

### 36.4 Continuous-to-discrete optional form

> **Status tag:** research

If a continuous-time state-space profile is used, let:

\[
\dot{s}(t)=A(\xi_t)s(t)+B(\xi_t)u(t),
\]

where \(\xi_t=(k_t^{\mathrm{nf}},d_t^{\mathrm{norm}},p_t,r_t)\). A discretized update over step \(\Delta_t\) may use:
...
\[
s_t=\overline{A}_t s_{t-1}+\overline{B}_t u_t.
\]

The discretization profile, floating-point policy, and deterministic seed MUST be replay-pinned if the result affects canonical replay or promotion evidence.

### 36.5 Policy-mode state-transition table

> **Status tag:** normative

| L5 mode | State retention | Input admission | Lookup injection | Promotion evidence |
|---|---|---|---|---|
| `normal` | allowed | allowed | allowed | allowed by policy |
| `degraded` | conservative | reduced | reduced | blocked or limited |
| `family_bypass` | generic only | family-sensitive update blocked | generic only | blocked |
| `transport_bypass` | prior state may persist | failed-frame semantic update blocked | blocked for failed frame | blocked |
| `lookup_bypass` | allowed | allowed | blocked | blocked if lookup-dependent |
| `full_bypass` | hold or minimal safe decay | blocked | blocked | blocked |

### 36.6 Reference fixture pack
...
### 36.7 Implementation note

> **Status tag:** reference

A minimal implementation may keep the neural base model unchanged and train only a witness encoder plus authority gate:

```python
class DuotronicWitnessAuthorityGate:
    def __call__(self, h_prev, canonical_witness_vec, diagnostics_vec, policy_vec):
        # Reference pseudocode only. Conforming code must apply policy clamps.
        alpha = sigmoid(linear(concat(h_prev, canonical_witness_vec, diagnostics_vec, policy_vec)))
        return clamp_by_policy(alpha, policy_vec)
```

This section is intentionally compatible with GRU/LSTM/RWKV/Mamba-style implementations. The Duotronic requirement is not a specific neural architecture; it is that trusted state evolution is selected by canonical witness context and policy, not by raw untrusted evidence.


---

## 37. GCD-Jump Sparse Witness Gates

> **Status tag:** research
> **Related tags:** reference

This section binds the standalone GCD-jump recurrence profile to the Witness Contract. It is optional and experimental. It does not redefine L1 through L5, DPFC arithmetic, transport validation, normalizer behavior, replay identity, or policy authority.

### 37.1 Reference recurrence
...
The witness event class is:

\[
\operatorname{event}(n)=
\begin{cases}
\texttt{trivial\_continuation}, & J(n)=1,\\
\texttt{nontrivial\_jump}, & J(n)>1.
\end{cases}
\]

### 37.2 Witness-contract interpretation

A GCD jump is a cheap recurrence-derived candidate event. It is useful because it separates long low-information stretches from sparse high-information jumps.

A conforming witness runtime MUST treat the jump as a candidate witness until the relevant input path satisfies:

1. validation;
2. adapter declaration;
3. canonicalization;
4. policy gating;
5. replay compatibility when stored or promoted.

The hard rule is:

```text
candidate jump event != trusted memory write
```

### 37.3 L2 and L2M usage

A recurrence-aware L2 runtime may use:

\[
\alpha_n = \operatorname{clamp}_{L5}\left(c_n^{norm}\cdot \mathbf{1}[J(n)>1]\cdot h(J(n))\right),
\]

where \(c_n^{norm}\) is normalization confidence and \(h\) is a bounded scoring or embedding function.

Then a recurrent update may include:

\[
\widetilde{W}_n = \widetilde{W}_{n-1} + \alpha_n \odot \Delta^J_n.
\]

For DW-SSM-compatible runtimes, the jump event may be used as an auxiliary event embedding:

\[
s_n=\rho_n\odot s_{n-1}+\beta_n\odot B_nu_n+\lambda_n\odot m_n+\eta_n\odot e^J_n.
\]

Where \(\eta_n\) is forced to zero when validation, canonicalization, or L5 policy blocks authority.

### 37.4 Performance boundary

GCD-jump recurrences SHOULD NOT be advertised as a faster general prime-generation method. Their Duotronic value is as a sparse update pattern:

1. cheap event detection;
2. sparse witness scheduling;
3. recurrence compression;
4. low-cost conformance fixtures;
5. selective state-update examples.

If performance claims are made, they require ordinary benchmark support against relevant baselines such as sieves, naive update scheduling, and ungated recurrent updates.

### 37.5 Fixtures

```yaml
fixture_pack: witness-gcd-jump-gates-v1-fixtures
fixtures:
  - fixture_id: WITNESS-GCD-JUMP-FIXTURE-1-CANDIDATE-JUMP-NOT-AUTHORITY
    given:
      gcd_jump: 5
  ...
  - fixture_id: WITNESS-GCD-JUMP-FIXTURE-2-VALID-JUMP-MAY-INCREASE-L2-AUTHORITY
    given:
      gcd_jump: 5
      canonicalization_result: canonical_success
      normalizer_confidence: 1.0
      policy_mode: normal
    operation: recurrence_event_to_l2_update
    expected:
      candidate_jump_detected: true
      update_authority_may_increase: true
      policy_clamp_required: true

  - fixture_id: WITNESS-GCD-JUMP-FIXTURE-3-TRANSPORT-BYPASS-ZEROES-JUMP-AUTHORITY
    given:
      gcd_jump: 5
      transport_validation: failed_integrity
      policy_mode: transport_bypass
...
## 38. Pronic-Centered Polygon Witnesses

> **Status tag:** research
> **Related tags:** reference

This section binds the standalone pronic-centered polygon profile to the Witness Contract. The profile is optional and experimental. It does not redefine L1 through L5, DPFC arithmetic, transport validation, normalizer behavior, replay identity, policy authority, or molecule/atom physics.

### 38.1 Witness role

A pronic-centered polygon witness describes a center-aware geometry object with:

1. an outer vertex count;
2. a center-present or center-absent structural flag;
3. a pronic shell address;
4. optional side-count, stride, offset, and chain-selection fields;
5. optional quaternion or angle-sort orientation reducer metadata;
6. a declared DPFC bridge or adapter profile.

The witness is useful for geometry canonicalization, polygon-family exploration, molecule-like research fixtures, and atom-like shell visualization.

### 38.2 Trust boundary

A generated polygon is not trusted merely because the generator produced it. A conforming witness runtime MUST apply:

1. schema validation;
2. center-flag validation;
3. shell-boundary validation;
4. chain-selection validation;
5. canonicalization;
6. replay identity pinning when stored;
7. L5 policy gating when used for state or lookup.

The hard rule is:

```text
beautiful geometry != trusted witness
```

### 38.3 Center-dot semantics

The center dot is structural state. The center-only object is the minimum realized geometry witness:

\[
(v=0,c=\texttt{center\_present})\mapsto \mu_1.
\]

It is not absence and not numeric zero. If a runtime sees:
...
then the runtime MUST treat it as absence, invalidity, or profile-specific rejection, not as the single-dot realized object.

### 38.4 Canonical witness shape
...
### 38.5 Optional quaternion orientation reducer

Quaternion reducers may be used to canonicalize orientation:

\[
v'=qv\bar q.
\]

The reducer is optional and external. It may influence canonical pose or witness metadata only under a declared reducer profile. It MUST NOT be treated as the generator of the pronic-centered family.

### 38.6 Molecule and atom research boundary

A pronic-centered polygon witness may be used for molecule-like or atom-like visualization. Such use remains research unless externally validated. The Witness Contract may track these objects as geometry witnesses, not as chemistry or physics truth.

### 38.7 Fixtures

```yaml
fixture_pack: witness-pronic-centered-polygon-v1-fixtures
fixtures:
  - fixture_id: WITNESS-PCP-FIXTURE-1-CENTER-ONLY-IS-MINIMUM-REALIZED
    given:
      outer_vertices: 0
      center: present
    operation: validate_pronic_centered_polygon_witness
    expected:
      valid_realized_witness: true
      core_image: mu_1
      absence: false

  - fixture_id: WITNESS-PCP-FIXTURE-2-EMPTY-CENTER-ABSENT-IS-NOT-MU1
    given:
      outer_vertices: 0
      center: absent
    operation: validate_pronic_centered_polygon_witness
    expected:
      valid_realized_witness: false
      must_not_decode_as_mu_1: true

  - fixture_id: WITNESS-PCP-FIXTURE-3-TRIANGLE-CHAIN-VALIDATES
    given:
      outer_vertices: 6
      side_count: 3
      start_offset: 1
      vertex_chain: [1, 3, 5]
    operation: validate_shape_chain
    expected:
      valid_chain: true
      shape: triangle
