# Duotronic Lookup Memory and Replay Profile v1.2

**Status:** Source-spec baseline candidate  
**Version:** lookup-memory-and-replay-profile@v1.2  
**Supersedes:** lookup-memory-and-replay-profile@v1.1  
...
## 13. Purge-aware lookup memory

> **Status tag:** normative

Lookup memory must support purge cascades.

If an evidence purge affects a lookup record, the lookup layer must:

1. remove the record;
2. tombstone the key;
3. expire derived cache entries;
4. invalidate vector/graph/time-series indexes;
5. mark affected high-speed loops stale;
6. block use in restricted or normal runtime;
7. emit or reference a purge event.

```yaml
LookupPurgeImpact:
  lookup_purge_impact_id: string
  purge_event_id: string
  affected_lookup_record_ids: []
  affected_index_refs: []
  affected_loop_profile_ids: []
  action: remove | tombstone | demote_to_audit | rebuild | block
  completed_at: string | null
```

### 13.1 Cross-version replay

A replay query may reference old lookup records only if:

1. record version is pinned;
2. policy snapshot is pinned;
3. staleness/purge state is recorded;
4. replay mode is audit-only unless policy exception permits more.

Purge-impacted lookup records must not be used for new runtime decisions.

---

## 14. Completion-pass external index invalidation API

> **Status tag:** normative

Lookup memory implementations that use external indexes must support the external index invalidation records defined by the Evidence Purge and Privacy Deletion Contract.

Minimum required integration:

1. register every external index as `ExternalIndexTarget`;
2. map lookup records to external index keys, vector IDs, graph nodes, graph edges, document IDs, cache keys, or time ranges;
3. accept `ExternalIndexInvalidationRequest`;
4. produce `ExternalIndexInvalidationResult`;
5. mark residual risk if the external index cannot prove deletion;
6. route incomplete invalidation to policy.

### 14.1 Index mapping record

```yaml
LookupExternalIndexMapping:
  lookup_external_index_mapping_id: string
  lookup_record_id: string
...
A lookup record with no external index mapping is assumed to have no purge-aware external index dependency. If that assumption is wrong, the implementation is nonconforming.
