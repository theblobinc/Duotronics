#!/usr/bin/env python3
"""Authenticated, cache-verifying proof-check boundary for Draft 5.3.13."""
...
class SignedAppendOnlyAuditSink:
    """Durable canonical JSONL audit chain signed by a dedicated Ed25519 key."""

    def __init__(
        self, path: Path, signing_key: Ed25519PrivateKey, verification_key: Ed25519PublicKey, *,
        signer_principal_id: str, signer_key_id: str, schema_validator: CanonicalSchemaValidator,
        maximum_record_bytes: int, maximum_log_bytes: int, maximum_records: int,
        rotation_policy: str = "manual_governance_sealed_segment",
        ...
        if self.rotation_policy != "manual_governance_sealed_segment":
        ...
        descriptor = self._open_locked_log(create=True)
        try:
            self._sequence, self._previous_record_sha256 = self._verify_descriptor(descriptor)
        finally:
            fcntl.flock(descriptor, fcntl.LOCK_UN)
            os.close(descriptor)

    def _open_locked_log(self, *, create: bool) -> int:
        flags = os.O_APPEND | os.O_RDWR | O_NOFOLLOW
        if create:
            flags |= os.O_CREAT
    ...
    def _verify_descriptor(self, descriptor: int) -> tuple[int, str | None]:
        ...
        for raw_line in data.splitlines():
            ...
            count += 1
            if record.get("sequence") != count or record.get("previous_audit_record_sha256") != previous:
                raise RuntimeError("cache audit sequence or hash chain is invalid")
            ...
            identity = {
                "sequence": count, "previous_audit_record_sha256": previous,
                "event_sha256": record["event_sha256"], "audit_signer_key_id": record["audit_signer_key_id"],
            }
    ...
    def __call__(self, evidence: dict[str, Any]) -> None:
        event_bytes = canonical_bytes(evidence)
        event_json = event_bytes.decode("utf-8")
        event_sha256 = sha256_bytes(event_bytes)
        with self._lock:
            descriptor = self._open_locked_log(create=False)
            try:
                # Refresh from the locked on-disk tail so independently initialized
                # worker processes cannot reuse a sequence number or predecessor hash.
                sequence_before, previous_before = self._verify_descriptor(descriptor)
                ...
                identity = {
                    "sequence": sequence, "previous_audit_record_sha256": previous_before,
                    "event_sha256": event_sha256, "audit_signer_key_id": self.signer_key_id,
                }
                unsigned = {
                    "schema_version": "cache_audit_record/v1",
                    "audit_record_id": "audit:" + sha256_bytes(canonical_bytes(identity)),
                    "sequence": sequence,
                    "previous_audit_record_sha256": previous_before,
                    "event_schema_version": str(evidence.get("schema_version", "")),
                ...
                self._previous_record_sha256 = sha256_bytes(encoded[:-1])
            finally:
                fcntl.flock(descriptor, fcntl.LOCK_UN)
                os.close(descriptor)
            try:
                directory_fd = os.open(self.path.parent, os.O_RDONLY | O_DIRECTORY | O_NOFOLLOW)
                try:
                    os.fsync(directory_fd)
                finally:
                    os.close(directory_fd)
            except OSError:
                # Some filesystems do not support directory fsync; the record file
                # itself is still synchronously persisted and startup-verified.
                pass
...
class ProofCheckApplication:
    ...
    def _publish_cache_evidence(self, evidence: dict[str, Any]) -> None:
        try:
            ...
            raise AuthorityFailure(
                ...
                "cache verification evidence could not be durably published",
            ) from error
    ...
    def _cached_result(
        ...
        if stored_registry_sha256 != self.cache_registry_sha256:
            ...
            claim_hash = sha256_bytes(canonical_bytes(request["canonical_claim"]))
            stale_evidence = {
                "schema_version": "cache_stale_row_evidence/v3",
            ...
            self._publish_cache_evidence(stale_evidence)
            raise AuthorityFailure(
...
def load_production_application(config_root: Path = Path("/etc/witness-authority")) -> ProofCheckApplication:
    ...
    audit_sink = SignedAppendOnlyAuditSink(
        config_root.joinpath(*audit_relative.parts), audit_signing_key, audit_verification_key,
