#!/usr/bin/env python3
"""Generate per-interpreter evidence and deterministically merge Draft 5.3.12."""
...
EVIDENCE_DIR = ROOT / "validation/python_matrix/draft5_3_12"
MATRIX_PATH = ROOT / "DRAFT5_3_12_PYTHON_MATRIX_VALIDATION.json"
COUNTS_PATH = ROOT / "DRAFT5_3_12_REGRESSION_COUNTS.json"
SUMMARY_PATH = ROOT / "DRAFT5_3_12_VALIDATION_SUMMARY.txt"
METADATA_PATH = ROOT / "PACKAGE_METADATA_v1_6_draft_5_3_12.json"
TARGETS = ("3.12", "3.13")
VALIDATOR = ROOT / "executable/validators/validate_draft5_3_12_corpus.py"

SPEC = importlib.util.spec_from_file_location("draft5312_validator_counts", VALIDATOR)
...
def record_current_interpreter() -> Path:
    ...
    if target not in TARGETS:
        raise SystemExit(f"Python {target} is outside the Draft 5.3.12 target matrix")
    record = {
...
def merge() -> dict[str, Any]:
    ...
    for target in TARGETS:
        if target in runs_by_target:
            ...
            entry = {**record, "evidence_file": relative, "evidence_sha256": digest(path)}
            runs.append(entry)
        else:
        ...
        evidence_set.append({"path": relative, "sha256": digest(path), "target_python_version": target})
    ...
    matrix = {
        ...
        "evidence_set_sha256": hashlib.sha256(canonical_bytes(evidence_set)).hexdigest(),
    ...
    counts = {
        "schema_version": "draft5_3_12_regression_counts/v1",
        "package_version": "v1.6-draft-5.3.12",
        "generated_at": matrix["generated_at"],
        "draft5_3_11_baseline_tests_discovered": 248,
        "target_python_versions": list(TARGETS),
        ...
        "python_matrix_sha256": digest(MATRIX_PATH),
    }
    ...
    summary = "\n".join([
        "Duotronic Witness Contract v1.6 Draft 5.3.12 — Generated Validation Summary",
        ...
        "Draft 5.3.11 baseline: 248 tests discovered",
