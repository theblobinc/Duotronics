...
## Draft 2 Addendum: Direct MCP Mutation Tools

Draft 2 recognizes `write_file_system` and `execute_system_command` as elevated mutation tools. They must follow `duotronic_direct_mutation_tools_security_addendum_v1_0.md`.
...
unless a break-glass policy snapshot enables them.

Any enablement must produce:

1. policy decision;
2. audit log with redacted sensitive arguments;
3. backup snapshot;
4. git sync record if repository state changes;
5. rollback note.
