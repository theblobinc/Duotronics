# Duotronic Profile Synthesis Registry v1.2

**Status:** Source-spec baseline candidate  
**Version:** profile-synthesis-registry@v1.2  
**Supersedes:** profile-synthesis-registry@v1.1  
...
## 11. Draft 5 exclusive promotion path

> **Status tag:** normative

The Profile Synthesis Registry is the only path that may create a `ProfilePromotionRequest`.

Planner-triggered learning runs, model witnesses, source records, human annotations, or action outcomes may recommend that promotion be considered. They may not promote a profile directly.

A promotion request must be created by a Profile Synthesis Registry workflow and must include:

1. candidate profile ID;
2. fixture pack;
3. replay trace set;
4. retention metrics where applicable;
5. policy gate;
6. purge lineage check;
7. human review decision where required;
8. Family Registry handoff if target status is `reference_profile` or `normative_profile`.

---

## 12. Purge impact on profile candidates

> **Status tag:** normative

If evidence used by a profile candidate is purged, the registry must re-evaluate that profile.

```yaml
ProfilePurgeImpactRecord:
  profile_purge_impact_record_id: string
  purge_event_id: string
  affected_candidate_profile_ids: []
  affected_fixture_pack_ids: []
  affected_replay_trace_ids: []
  required_action: rebuild_lineage | remove_fixture | demote | reject | purge_candidate | human_review
  completed_at: string | null
```

A profile candidate must not hide purged evidence in lineage, fixtures, embeddings, summaries, or lab imports.
