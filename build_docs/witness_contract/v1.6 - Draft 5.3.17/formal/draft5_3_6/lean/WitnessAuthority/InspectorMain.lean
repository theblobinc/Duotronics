...
def moduleNameFromPath (value : String) : Except String Name := do
  ...
  let withoutSuffix := (value.dropEnd 5).toString
...
unsafe def runInspection (requestPath handoffRoot : String) : IO UInt32 := do
  ...
  | .ok json =>
      ...
      | .error error =>
          ...
          return 65
      | .ok request =>
          match moduleNameFromPath request.generated_binding_module_path with
          | .error error =>
              IO.eprintln error
              return 65
          | .ok moduleName =>
              ...
              try
                ...
                let coreContext : Core.Context := {
                  fileName := "<witness-authority-inspector>"
                  fileMap := FileMap.ofString ""
                }
                let inspected ← Meta.MetaM.toIO (inspectionJson env request) coreContext { env := env } {} {}
...
unsafe def main (args : List String) : IO UInt32 :=
  WitnessAuthority.mainImpl args
