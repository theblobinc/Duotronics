...
CREATE TABLE wc_governance_authorization_witnesses_v2 (
  ...
  signed_payload_shake256_512 TEXT NOT NULL CHECK (length(signed_payload_shake256_512) = 128 AND signed_payload_shake256_512 NOT GLOB '*[^0-9a-f]*'),
...
BEGIN
  ...
  SELECT CASE WHEN NOT EXISTS (
    ...
    WHERE g.governance_key_id = NEW.signer_key_id
      ...
      AND wc_ml_dsa_87_verify(g.public_key_base64url, NEW.signed_payload_canonical_json, NEW.signature) = 1
...
CREATE TABLE wc_compiler_profiles_v2 (
  ...
  registry_shake256_512 TEXT NOT NULL CHECK (length(registry_shake256_512) = 128),
  oci_image_digest TEXT NOT NULL CHECK (oci_image_digest GLOB 'shake256-512:*'),
  oci_runtime_shake256_512 TEXT NOT NULL CHECK (length(oci_runtime_shake256_512) = 128),
  oci_runtime_version TEXT NOT NULL,
  verifier_executable_shake256_512 TEXT NOT NULL CHECK (length(verifier_executable_shake256_512) = 128),
  lean_executable_shake256_512 TEXT NOT NULL CHECK (length(lean_executable_shake256_512) = 128),
  lake_executable_shake256_512 TEXT NOT NULL CHECK (length(lake_executable_shake256_512) = 128),
  lean_stdlib_tree_shake256_512 TEXT NOT NULL CHECK (length(lean_stdlib_tree_shake256_512) = 128),
  dependency_closure_shake256_512 TEXT NOT NULL CHECK (length(dependency_closure_shake256_512) = 128),
  sandbox_policy_shake256_512 TEXT NOT NULL CHECK (length(sandbox_policy_shake256_512) = 128),
  ...
  verifier_result_public_key_base64url TEXT NOT NULL CHECK (length(verifier_result_public_key_base64url) = 3456),
  valid_from TEXT NOT NULL CHECK (julianday(valid_from) IS NOT NULL),
  valid_until TEXT CHECK (valid_until IS NULL OR julianday(valid_until) IS NOT NULL),
...
BEGIN
  ...
  SELECT CASE WHEN NOT EXISTS (
    ...
    WHERE g.governance_key_id = NEW.governance_key_id
      ...
      AND json_extract(NEW.signed_payload_canonical_json, '$.registry_shake256_512') = NEW.registry_shake256_512
      AND json_extract(NEW.signed_payload_canonical_json, '$.oci_image_digest') = NEW.oci_image_digest
      AND json_extract(NEW.signed_payload_canonical_json, '$.oci_runtime_shake256_512') = NEW.oci_runtime_shake256_512
      AND json_extract(NEW.signed_payload_canonical_json, '$.oci_runtime_version') = NEW.oci_runtime_version
      AND json_extract(NEW.signed_payload_canonical_json, '$.verifier_executable_shake256_512') = NEW.verifier_executable_shake256_512
      AND json_extract(NEW.signed_payload_canonical_json, '$.lean_executable_shake256_512') = NEW.lean_executable_shake256_512
      AND json_extract(NEW.signed_payload_canonical_json, '$.lake_executable_shake256_512') = NEW.lake_executable_shake256_512
      AND json_extract(NEW.signed_payload_canonical_json, '$.lean_stdlib_tree_shake256_512') = NEW.lean_stdlib_tree_shake256_512
      AND json_extract(NEW.signed_payload_canonical_json, '$.dependency_closure_shake256_512') = NEW.dependency_closure_shake256_512
      AND json_extract(NEW.signed_payload_canonical_json, '$.sandbox_policy_shake256_512') = NEW.sandbox_policy_shake256_512
...
CREATE TABLE wc_lean_compiler_witnesses_v3 (
  ...
  claim_content_shake256_512 TEXT NOT NULL CHECK (length(claim_content_shake256_512) = 128),
  theorem_statement_shake256_512 TEXT NOT NULL CHECK (length(theorem_statement_shake256_512) = 128),
  immutable_snapshot_id TEXT NOT NULL CHECK (immutable_snapshot_id GLOB 'shake256-512:*'),
  immutable_snapshot_tree_shake256_512 TEXT NOT NULL CHECK (length(immutable_snapshot_tree_shake256_512) = 128),
  ...
  proof_artifact_shake256_512 TEXT NOT NULL CHECK (length(proof_artifact_shake256_512) = 128),
  generated_binding_module_shake256_512 TEXT NOT NULL CHECK (length(generated_binding_module_shake256_512) = 128),
  generated_binding_request_shake256_512 TEXT NOT NULL CHECK (length(generated_binding_request_shake256_512) = 128),
  compiler_profile_id TEXT NOT NULL,
  verifier_result_payload_shake256_512 TEXT NOT NULL CHECK (length(verifier_result_payload_shake256_512) = 128),
  verifier_result_signer_key_id TEXT NOT NULL,
  ...
  expected_type_expression_hash TEXT NOT NULL CHECK (length(expected_type_expression_hash) = 128),
  actual_type_expression_hash TEXT NOT NULL CHECK (length(actual_type_expression_hash) = 128),
  axiom_set_shake256_512 TEXT NOT NULL CHECK (length(axiom_set_shake256_512) = 128),
...
BEGIN
  ...
  SELECT CASE WHEN NOT EXISTS (
    ...
    WHERE k.key_id = NEW.key_id
      AND wc_ml_dsa_87_verify(k.public_key_base64url, NEW.signed_payload_canonical_json, NEW.signature) = 1
  ...
  SELECT CASE WHEN json_extract(NEW.signed_payload_canonical_json, '$.schema_version') <> NEW.schema_version
    ...
    OR json_extract(NEW.signed_payload_canonical_json, '$.claim_content_shake256_512') <> NEW.claim_content_shake256_512
    OR json_extract(NEW.signed_payload_canonical_json, '$.theorem_statement_shake256_512') <> NEW.theorem_statement_shake256_512
    OR json_extract(NEW.signed_payload_canonical_json, '$.immutable_snapshot_id') <> NEW.immutable_snapshot_id
    OR json_extract(NEW.signed_payload_canonical_json, '$.immutable_snapshot_tree_shake256_512') <> NEW.immutable_snapshot_tree_shake256_512
    OR json_extract(NEW.signed_payload_canonical_json, '$.proof_artifact_relative_path') <> NEW.proof_artifact_relative_path
    OR json_extract(NEW.signed_payload_canonical_json, '$.proof_artifact_shake256_512') <> NEW.proof_artifact_shake256_512
    OR json_extract(NEW.signed_payload_canonical_json, '$.generated_binding_module_shake256_512') <> NEW.generated_binding_module_shake256_512
    OR json_extract(NEW.signed_payload_canonical_json, '$.generated_binding_request_shake256_512') <> NEW.generated_binding_request_shake256_512
    OR json_extract(NEW.signed_payload_canonical_json, '$.compiler_profile_id') <> NEW.compiler_profile_id
    OR json_extract(NEW.signed_payload_canonical_json, '$.verifier_result_payload_shake256_512') <> NEW.verifier_result_payload_shake256_512
    ...
    OR json_extract(NEW.signed_payload_canonical_json, '$.axiom_set_shake256_512') <> NEW.axiom_set_shake256_512
  ...
  SELECT CASE WHEN NEW.result = 'passed' AND (wc_is_canonical_json(NEW.verifier_result_signed_payload_canonical_json) <> 1
    OR wc_shake256_512(NEW.verifier_result_signed_payload_canonical_json) <> NEW.verifier_result_payload_shake256_512
    OR NOT EXISTS (
      ...
      WHERE p.compiler_profile_id = NEW.compiler_profile_id
        ...
        AND wc_ml_dsa_87_verify(p.verifier_result_public_key_base64url, NEW.verifier_result_signed_payload_canonical_json, NEW.verifier_result_signature) = 1
...
CREATE TABLE wc_authority_events_v1 (
  ...
  signed_payload_canonical_json TEXT NOT NULL CHECK (json_valid(signed_payload_canonical_json) = 1),
  canonical_payload_shake256_512 TEXT NOT NULL CHECK (length(canonical_payload_shake256_512) = 128),
  signature TEXT NOT NULL,
...
BEGIN
  ...
  SELECT CASE WHEN wc_is_canonical_json(NEW.signed_payload_canonical_json) <> 1
    OR wc_shake256_512(NEW.signed_payload_canonical_json) <> NEW.canonical_payload_shake256_512
...
CREATE TABLE wc_authority_snapshots_v2 (
  ...
  event_set_root_shake256_512 TEXT NOT NULL CHECK (length(event_set_root_shake256_512) = 128),
...
BEGIN
  ...
  SELECT CASE WHEN NEW.event_set_root_shake256_512 <> wc_authority_event_root(COALESCE((
    SELECT '[' || group_concat('[' || authority_event_sequence || ',"' || canonical_payload_shake256_512 || '"]', ',') || ']'
    FROM (SELECT authority_event_sequence, canonical_payload_shake256_512 FROM wc_authority_events_v1
  ...
  SELECT CASE WHEN NOT EXISTS (
    ...
    WHERE a.authorization_witness_id = NEW.authorization_witness_id
      ...
      AND wc_ml_dsa_87_verify(g.public_key_base64url, NEW.signed_payload_canonical_json, NEW.snapshot_signature) = 1
  ...
  SELECT CASE WHEN json_extract(NEW.signed_payload_canonical_json, '$.schema_version') <> NEW.schema_version
    ...
    OR json_extract(NEW.signed_payload_canonical_json, '$.event_set_root_shake256_512') <> NEW.event_set_root_shake256_512
