#!/usr/bin/env python3
"""Descriptor-reconciled fail-closed validator for Draft 5.3.8."""
...
DESCRIPTOR_PATH = ROOT / "CANONICAL_CORPUS_v1_6_draft_5_3_8.json"
REPORT_PATH = ROOT / "DRAFT5_3_8_VALIDATION_REPORT.json"
REGRESSION_COUNTS_PATH = ROOT / "DRAFT5_3_8_REGRESSION_COUNTS.json"
VALIDATION_SUMMARY_PATH = ROOT / "DRAFT5_3_8_VALIDATION_SUMMARY.txt"
LINEAGE_PATH = ROOT / "history/SOURCE_PACKAGE_LINEAGE_v1_6_draft_5_3_8.json"
...
def _run_discovered_suite(*, development_mode: bool) -> dict[str, Any]:
    ...
    assert_true(major_minor in {(3, 12), (3, 13)}, "unsupported Python version; Draft 5.3.8 supports Python 3.12 and 3.13")
    return {
        "command": command,
        **structured,
...
def phase_descriptor() -> dict[str, Any]:
    ...
    assert_true(descriptor["active_version"] == "v1.6-draft-5.3.8", "active version mismatch")
    ...
    for token in (DESCRIPTOR_PATH.name, "PACKAGE_INVENTORY_v1_6_draft_5_3_8.json", "CHECKSUMS_v1_6_draft_5_3_8.shake256_512"):
        assert_true(token in resolver, f"resolver does not boot canonical token {token}")
    accounting = _run_python_tests("-W", ["error", "executable/tests/test_validator_phase_reconciliation_v538.py"])
...
def phase_dependencies() -> dict[str, Any]:
    ...
    try:
        ...
        connection.close()
    scope = load_json("refs/validation_scope_v1_6_draft_5_3_8.json")
...
def phase_hash_closure() -> dict[str, Any]:
    inventory = load_json("PACKAGE_INVENTORY_v1_6_draft_5_3_8.json")
    ...
    checksum_lines = (ROOT / "refs/manifest/CHECKSUMS_v1_6_draft_5_3_8.shake256_512").read_text(encoding="utf-8").splitlines()
...
def _schema_phase(name: str) -> dict[str, Any]:
    command = ["node", "executable/validators/validate_draft5_3_8_schemas.mjs", "--phase", name]
...
def phase_python_version_portability() -> dict[str, Any]:
    generated = load_json(REGRESSION_COUNTS_PATH.name)
    matrix = load_json("DRAFT5_3_8_PYTHON_MATRIX_VALIDATION.json")
    ...
    assert_true(matrix.get("overall_status") == "passed", "Python matrix did not pass")
    runs = matrix.get("runs", [])
    assert_true({".".join(item["python_version"].split(".")[:2]) for item in runs} == {"3.12", "3.13"}, "Python 3.12/3.13 execution evidence is incomplete")
    for item in runs:
...
def phase_openapi_conformance() -> dict[str, Any]:
    ...
    spec = yaml.safe_load((ROOT / "executable/openapi/draft5_3_8_evidence_language_openapi.yaml").read_text(encoding="utf-8"))
    assert_true(spec["info"]["version"] == "v1.6-draft-5.3.8", "OpenAPI version mismatch")
...
def phase_schema_registry_authority() -> dict[str, Any]:
    registry = load_json("refs/schema_registry_v1_6_draft_5_3_8.json")
...
def phase_version_consistency() -> dict[str, Any]:
    expected = "v1.6-draft-5.3.8"
    descriptor = load_json(DESCRIPTOR_PATH.name)
    metadata = load_json("PACKAGE_METADATA_v1_6_draft_5_3_8.json")
    registry = load_json("refs/schema_registry_v1_6_draft_5_3_8.json")
    package = load_json("package.json")
    lock = load_json("package-lock.json")
    tla = load_json("refs/formal_toolchain/tla_toolchain_manifest_v1_0.json")
    gate_status = load_json("DRAFT5_3_8_RELEASE_GATE_STATUS.json")
    assert_true(descriptor["active_version"] == metadata["version"] == expected, "descriptor/package metadata version mismatch")
    assert_true(package["version"] == "1.6.5-3-8" and lock["version"] == package["version"], "npm package/lock version mismatch")
    assert_true(package["name"].endswith("draft-5-3-8-validation") and lock["name"] == package["name"], "npm package/lock identity mismatch")
    assert_true(registry.get("registry_version") == expected, "schema registry version mismatch")
    assert_true("Draft 5.3.8" in tla["package"] and any(item["module"] == "ProofAuthorityV8" for item in tla["tla_modules"]), "formal manifest version/active model mismatch")
    assert_true(gate_status["gates"]["I_package_provenance"] == "unpublished_workspace", "release provenance status is not fail-closed")
    assert_true('v!"5.3.8"' in (ROOT / "lakefile.lean").read_text(encoding="utf-8"), "Lake package version mismatch")
    active_documents = (
        "README.md", "START_HERE.md", "CANONICAL_CORPUS_v1_6_draft_5_3_8.json",
        "CORPUS_INDEX_v1_6_draft_5_3_8.md", "duotronic_witness_contract_v1_6_draft_5_3_8.md",
        "RELEASE_NOTES_v1_6_draft_5_3_8.md", "PACKAGE_METADATA_v1_6_draft_5_3_8.json",
    ...
    for relative in active_documents:
        ...
        assert_true("5.3.8" in text_value, f"active entrypoint lacks 5.3.8 identity: {relative}")
    ...
    for report_path in ROOT.glob("DRAFT5_3_8*.json"):
...
def phase_package_provenance() -> dict[str, Any]:
    provenance = load_json("PACKAGE_PROVENANCE_v1_6_draft_5_3_8.json")
    required = {
...
def phase_standalone_merged_corpus() -> dict[str, Any]:
    ...
    assert_true(releases[-1].get("version") == "v1.6-draft-5.3.7" and releases[-1].get("shake256_512") == "ba728768f5f1375654efb805856d1f02c187aeca0f60b8442b3a55170db61206", "immediate predecessor lineage mismatch")
...
def _build_attestation(subject_type: str) -> dict[str, Any]:
    status = load_json("DRAFT5_3_8_BUILD_ATTESTATION_STATUS.json")
...
def phase_external_signature() -> dict[str, Any]:
    status = load_json("refs/trust/RELEASE_SIGNATURE_STATUS_v1_6_draft_5_3_8.json")
...
def main() -> int:
    ...
    inventory = load_json("PACKAGE_INVENTORY_v1_6_draft_5_3_8.json") if (ROOT / "PACKAGE_INVENTORY_v1_6_draft_5_3_8.json").is_file() else {"files": []}
    regular_directories = [
    ...
    report = {
        "schema_version": "draft5_3_8_validation_report/v1",
        "package_version": "v1.6-draft-5.3.8",
