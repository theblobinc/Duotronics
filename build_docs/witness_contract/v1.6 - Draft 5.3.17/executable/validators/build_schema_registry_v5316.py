#!/usr/bin/env python3
"""Generate the complete lifecycle-classified Draft 5.3.16 schema registry."""
...
OUTPUT = ROOT / "refs/schema_registry_v1_6_draft_5_3_16.json"
...
CANONICAL = {
    ...
    "cache_stale_row_evidence_v4", "cache_verification_evidence_v1", "cache_audit_record_v3",
    "cache_audit_checkpoint_v2", "cache_audit_segment_seal_v2",
    "cache_audit_signing_registry_v1", "cache_audit_anchor_registry_v1",
    "cache_audit_key_validity_evidence_v1", "cache_audit_anchor_state_v2",
    "cache_audit_genesis_authorization_v1", "cache_audit_segment_transition_v1",
    "cache_audit_recovery_authorization_v2", "cache_audit_recovery_evidence_v2",
    "cache_audit_publication_receipt_v2", "cache_audit_service_request_v1", "domain_execution_evidence_v1",
...
LEGACY = {
    ...
    "cache_audit_record_v2", "cache_audit_checkpoint_v1", "cache_audit_segment_seal_v1",
    "cache_audit_anchor_state_v1", "cache_audit_recovery_authorization_v1",
    "cache_audit_recovery_evidence_v1", "cache_audit_publication_receipt_v1",
}

POST_5316: set[str] = set()
...
def main() -> int:
    ...
    schema_paths = sorted((path for path in (ROOT / "schemas").glob("*.schema.*") if schema_name(path) not in POST_5316), key=lambda path: path.name)
    for path in schema_paths:
        relative = path.relative_to(ROOT).as_posix(); name = schema_name(path); document = load(path)
        if name in CANONICAL:
            lifecycle = "canonical"; active = True; rationale = "Active Draft 5.3.16 API, persistence, witness, or validation surface."
        elif name in LEGACY:
        ...
        runtime_valid = {
            ...
            "cache_audit_record_v3": "runtime:test_signed_append_only_chain_is_durable_and_externally_anchored",
            "cache_audit_checkpoint_v2": "runtime:test_coordinated_local_log_and_checkpoint_rollback_is_rejected_by_external_anchor",
            "cache_audit_segment_seal_v2": "runtime:test_normal_capacity_always_reserves_terminal_seal_record",
            "cache_audit_signing_registry_v1": "runtime:test_registry_preserves_historical_keys_and_governs_active_selection",
            "cache_audit_anchor_registry_v1": "runtime:test_privileged_file_anchor_round_trip_over_unix_socket",
            "cache_audit_key_validity_evidence_v1": "runtime:test_registry_preserves_historical_keys_and_governs_active_selection",
            "cache_audit_anchor_state_v2": "runtime:test_same_sequence_anchor_rewrite_and_registry_change_are_rejected",
            "cache_audit_genesis_authorization_v1": "runtime:test_competing_genesis_requires_external_anchor_compare_and_swap",
            "cache_audit_segment_transition_v1": "runtime:test_successor_requires_real_sealed_predecessor_and_governed_transition",
            "cache_audit_recovery_authorization_v2": "runtime:test_expired_and_replayed_recovery_authorizations_fail",
            "cache_audit_recovery_evidence_v2": "runtime:test_expired_and_replayed_recovery_authorizations_fail",
            "cache_audit_publication_receipt_v2": "runtime:test_unix_publisher_client_hard_times_out_without_waiting_for_server",
            "cache_audit_service_request_v1": "runtime:test_unrelated_peer_is_rejected_before_dispatch",
    ...
    registry = {
        ...
        "registry_version": "v1.6-draft-5.3.16", "active_version_scope": "v1.6-draft-5.3.16",
        "entries": entries,
        "rules": {
