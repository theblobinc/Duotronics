...
def main() -> int:
    ...
    for _ in range(max(1, len(pending))):
        ...
        for source in list(pending):
            ...
            if process.returncode == 0 and not warning_diagnostic_present(diagnostic):
                ...
                compiled.append({"module": module, "role": "submitted_source", "source_shake256_512": digest(source), "olean_path": target.relative_to(HANDOFF).as_posix(), "olean_shake256_512": digest(target)})
                compilation_commands.append({"module": module, "role": "submitted_source", "argv_shake256_512": hashlib.shake_256(canonical(command)).hexdigest(64)})
            else:
                diagnostics.append({"module": relative.as_posix(), "shake256_512": hashlib.shake_256(diagnostic.encode()).hexdigest(64)})
    ...
    compiled.append({
        ...
        "source_shake256_512": digest(binding),
        "olean_path": binding_target.relative_to(HANDOFF).as_posix(),
        "olean_shake256_512": digest(binding_target),
    })
    compilation_commands.append({
        "module": binding_module,
        "role": "generated_binding",
        "argv_shake256_512": hashlib.shake_256(canonical(binding_command)).hexdigest(64),
    })
    ...
    manifest = {
        ...
        "lean_executable_shake256_512": digest(LEAN),
        "working_directory": str(WORK),
        "compilation_environment_shake256_512": hashlib.shake_256(canonical(environment)).hexdigest(64),
        "binding_module_shake256_512": digest(binding),
        "binding_olean_path": binding_target.relative_to(HANDOFF).as_posix(),
        "binding_olean_shake256_512": digest(binding_target),
        "artifacts": artifacts,
