...
class ExactSQLiteSchemaTests(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with closing(sqlite3.connect(path)) as connection:
                connection.execute(
                    ...
                    ("principal", "key", "a" * 128, "inflight", "owner", time.time() + 60, None, time.time(), None),
                )
                connection.commit()
            ...
            with closing(sqlite3.connect(path)) as connection:
                ...
                self.assertEqual(
                    connection.execute("SELECT canonical_schema_shake256_512 FROM idempotency_schema_metadata").fetchone()[0],
                    store.canonical_schema_shake256_512(),
                )
            store.close()
    ...
    def test_weakened_equivalent_name_schemas_are_rejected(self):
        weak_tables = {
            "missing_pk_constraints_and_checks": """CREATE TABLE proof_check_idempotency (
                principal_id TEXT, idempotency_key TEXT, request_shake256_512 TEXT, state TEXT,
            ...
            "wrong_affinities": """CREATE TABLE proof_check_idempotency (
                principal_id BLOB NOT NULL, idempotency_key TEXT NOT NULL, request_shake256_512 TEXT NOT NULL,
    ...
    def test_primary_key_uniqueness_and_state_check_are_behavioral(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with closing(sqlite3.connect(path)) as connection:
                row = ("principal", "key", "a" * 128, "inflight", "owner", time.time() + 60, None, time.time(), None)
                ...
                with self.assertRaises(sqlite3.IntegrityError):
                    connection.execute(
                        "INSERT INTO proof_check_idempotency VALUES (?,?,?,?,?,?,?,?,?)",
                        ("principal", "other", "b" * 128, "invented", None, None, None, time.time(), None),
                    )
            store.close()
    ...
    def test_schema_digest_tampering_is_rejected(self):
        with tempfile.TemporaryDirectory() as directory:
            path = pathlib.Path(directory) / "cache.sqlite"; store = DurableIdempotencyStore(path)
            with closing(sqlite3.connect(path)) as connection:
                connection.execute("UPDATE idempotency_schema_metadata SET canonical_schema_shake256_512=?", ("0" * 128,))
                connection.commit()
...
class CacheKeyLifecycleTests(unittest.TestCase):
    ...
    def test_rotation_predecessors_are_complete_ordered_and_acyclic(self):
        ...
        first = self.record(
            "cache:first", valid_from="2025-01-01T00:00:00Z",
    ...
    def test_malformed_valid_until_is_schema_invalid(self):
        ...
        registry = {
            ...
            "signed_payload_shake256_512": "a" * 128, "signature": "A" * 86,
        }
    ...
    def test_future_expired_retired_and_revoked_keys_fail_closed(self):
        authority = FakeAuthority(); cache_key = MLDSA87PrivateKey.generate()
        base = {
            ...
            with self.subTest(name=name), tempfile.TemporaryDirectory() as directory:
                with self.assertRaises(AuthorityFailure):
                    ProofCheckApplication(
                        ...
                        cache_key_record=record, cache_registry_shake256_512="b" * 128,
                        cache_clock=clock,
                    )
    ...
    def test_cache_replay_rechecks_current_key_and_records_registry_evidence(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(evidence[0]["cache_registry_shake256_512"], application.cache_registry_shake256_512)
...
class DeadlineAndCompileEvidenceTests(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with closing(sqlite3.connect(path, isolation_level=None)) as blocker:
                ...
                with self.assertRaises((sqlite3.OperationalError, TimeoutError)):
                    store.acquire("principal", "key", "a" * 128, deadline_monotonic=started + 0.12)
    ...
    def test_trusted_consumer_recomputes_ordered_complete_compile_commands(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            mutations = []
            altered = json.loads(json.dumps(original)); altered["compilation_commands"][0]["argv_shake256_512"] = "0" * 128; mutations.append(altered)
