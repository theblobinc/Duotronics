...
class AuthorityDatabase534:
    ...
    def add_profile(self, profile_id: str):
        unsigned = {
            ...
            "registry_shake256_512": "a" * 128, "oci_image_digest": "shake256-512:" + "b" * 128,
            "oci_runtime_shake256_512": "c" * 128, "oci_runtime_version": "podman version 5.4.0",
            "verifier_executable_shake256_512": "d" * 128, "lean_executable_shake256_512": "e" * 128,
            "lake_executable_shake256_512": "f" * 128, "lean_stdlib_tree_shake256_512": "0" * 128,
            "dependency_closure_shake256_512": "1" * 128, "sandbox_policy_shake256_512": "2" * 128,
        ...
        self.connection.execute(
            ...
            (profile_id, unsigned["schema_version"], unsigned["registry_shake256_512"], unsigned["oci_image_digest"],
             unsigned["oci_runtime_shake256_512"], unsigned["oci_runtime_version"], unsigned["verifier_executable_shake256_512"],
             unsigned["lean_executable_shake256_512"], unsigned["lake_executable_shake256_512"], unsigned["lean_stdlib_tree_shake256_512"],
             unsigned["dependency_closure_shake256_512"], unsigned["sandbox_policy_shake256_512"], unsigned["verifier_source_revision"],
             ...
             signed["signed_payload_shake256_512"], signed["signature"], unsigned["created_at"]),
        )
    ...
    def add_verifier(self, key_id="key:534", principal_id="verifier:534"):
        ...
        self.connection.execute(
            ...
            (principal_id, "verifier_principal/v2", key_id, "ML-DSA-87", fingerprint, "active",
             "2026-01-01T00:00:00Z", "2027-01-01T00:00:00Z", "2026-01-01T00:00:00Z"),
        )
        self.connection.execute(
            "INSERT INTO wc_verifier_keys_v3 VALUES (?,?,?,?,?,?,?,?,?)",
            (key_id, "verifier_key/v3", principal_id, "ML-DSA-87", authority.public_key_raw_b64url(public),
    ...
    def add_compiler_witness(self, snapshot_id: str, cutoff: int, *, witness_id="lean:534", result_tamper=False):
        result_unsigned = {
            ...
            "request_shake256_512": "0" * 128, "compiler_profile_id": "profile:534",
            "claim_content_shake256_512": "1" * 128, "theorem_statement_shake256_512": "2" * 128,
            "proof_artifact_shake256_512": "3" * 128, "immutable_snapshot_id": "shake256-512:" + "4" * 128,
            "immutable_snapshot_tree_shake256_512": "4" * 128, "generated_binding_module_shake256_512": "5" * 128,
            "effective_sandbox_invocation_shake256_512": "6" * 128,
        }
        ...
        witness_unsigned = {
            ...
            "semantic_witness_content_id": "lean-semantic:" + "7" * 128, "claim_id": "claim:534",
            "claim_content_shake256_512": "1" * 128, "theorem_statement_shake256_512": "2" * 128,
            "immutable_snapshot_id": "shake256-512:" + "4" * 128, "immutable_snapshot_tree_shake256_512": "4" * 128,
            "proof_artifact_relative_path": "Proof.lean", "proof_artifact_shake256_512": "3" * 128,
            "generated_binding_module_shake256_512": "5" * 128, "generated_binding_request_shake256_512": "8" * 128,
            "compiler_profile_id": "profile:534", "verifier_result_payload_shake256_512": result_signed["signed_payload_shake256_512"],
            "verifier_result_signer_key_id": "result-key:1", "verifier_result_signature": result_signed["signature"],
            "expected_type_expression_hash": "9" * 128, "actual_type_expression_hash": "9" * 128,
            "axiom_set_shake256_512": "a" * 128, "result": "passed", "theorem_status": "proved",
        ...
        columns = list(witness_unsigned) + ["verifier_result_signed_payload_canonical_json", "signed_payload_canonical_json", "signed_payload_shake256_512", "signature"]
        values = [witness_unsigned[name] for name in witness_unsigned] + [
            ...
            witness_signed["signed_payload_shake256_512"], witness_signed["signature"],
        ]
        self.connection.execute(
    ...
    def activate_release(self):
        ...
        self.connection.execute(
            ...
            (unsigned["package_version"], 1, 1, 1, 1, 1, 1, 1, 1, unsigned["signer_key_id"],
             unsigned["created_at"], authority.signed_payload_canonical_json(signed), signed["signed_payload_shake256_512"], signed["signature"]),
        )
    ...
    def authorization(self, scope: str, target_type: str, target_id: str, auth_id: str, *,
        ...
        self.connection.execute(
            ...
            (auth_id, unsigned["schema_version"], scope, target_type, target_id, unsigned["principal_id"],
             ...
             unsigned["signer_key_id"], canonical, signed["signed_payload_shake256_512"], signed["signature"], unsigned["created_at"]),
        )
        return auth_id
    ...
    def event(self, event_type: str, scope: str, target_type: str, target_id: str, event_id: str, *,
        ...
        self.connection.execute(
            ...
            (sequence, event_id, unsigned["schema_version"], event_type, scope, target_type, target_id,
             ...
             backdate_authorization_id, canonical, signed["signed_payload_shake256_512"], signed["signature"]),
        )
        return event_id

    def event_root(self, cutoff: int) -> str:
        events = self.connection.execute(
            "SELECT authority_event_sequence, canonical_payload_shake256_512 FROM wc_authority_events_v1 WHERE authority_event_sequence <= ? ORDER BY authority_event_sequence",
            (cutoff,),
        ).fetchall()
    ...
    def snapshot(self, snapshot_id: str, cutoff: int, *, as_of="2026-07-31T12:30:00Z", root_override=None):
        ...
        unsigned = {
            ...
            "event_set_root_shake256_512": root_override or self.event_root(cutoff),
        ...
        self.connection.execute(
            ...
            (snapshot_id, unsigned["schema_version"], as_of, cutoff, unsigned["event_set_root_shake256_512"],
             ...
             authority.signed_payload_canonical_json(signed), signed["signed_payload_shake256_512"], signed["signature"]),
        )


class SqlAuthorityLifecycle534Tests(unittest.TestCase):
    def database(self):
        database = AuthorityDatabase534()
    ...
    def test_monotonic_signed_event_is_accepted(self):
        db = self.database(); db.add_profile("profile:a")
    ...
    def test_snapshot_rejects_wrong_event_root_and_future_cutoff(self):
        db = self.database(); db.add_profile("profile:a")
        db.event("compiler_profile_activate", "compiler_profile_activate", "compiler_profile", "profile:a", "event:activate")
        rejected(self, lambda: db.snapshot("snapshot:bad-root", 1, root_override="0" * 128))
    ...
    def test_typed_supersession_happy_path_and_self_reference(self):
        db = self.database(); db.add_profile("profile:a"); db.add_profile("profile:b")
        ...
        db2 = self.database(); db2.add_profile("profile:a")
    ...
    def test_supersession_rejects_nonexistent_wrong_type_cycle_and_revoked_replacement(self):
        db = self.database(); db.add_profile("profile:a"); db.add_profile("profile:b"); db.add_profile("profile:c")
    ...
    def test_release_activation_requires_signed_complete_external_evidence_and_is_immutable(self):
        ...
        rejected(self, lambda: db.connection.execute(
            ...
            ("v1.6-draft-5.3.4", 1, 1, 1, 1, 1, 1, 1, 1, "governance:key:534",
             "2026-07-31T12:00:00Z", "{}", authority.shake256_512_bytes(b"{}"), "invalid"),
        ))
        unsigned = {
            "schema_version": "release_activation_evidence/v1", "package_version": "v1.6-draft-5.3.4",
            "result_channel_isolation": 1, "snapshot_identity": 1, "historical_snapshot_immutability": 1,
        ...
        db.connection.execute(
            ...
            (unsigned["package_version"], 1, 1, 1, 1, 1, 1, 1, 1, unsigned["signer_key_id"],
             ...
             signed["signed_payload_shake256_512"], signed["signature"]),
        )
