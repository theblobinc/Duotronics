...
from proof_authority import CanonicalSchemaValidator, OciSandboxRunner, canonical_bytes, shake256_512_bytes  # noqa: E402
...
class CompileHandoffIntegrationTests(unittest.TestCase):
    ...
    def test_true_producer_output_is_accepted_by_trusted_consumer(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            profile = {"lean_executable_shake256_512": compile_lean.digest(lean)}
            with patch.multiple(
                verify_lean, HANDOFF=handoff, HANDOFF_OLEAN=handoff / "olean", WORK=work, LEAN=lean,
            ):
                accepted, _ = verify_lean.validate_handoff(
                    CanonicalSchemaValidator(ROOT / "schemas"), invocation, profile,
                )
...
class LifecycleAndDatabaseTests(unittest.TestCase):
    ...
    def test_active_execution_renews_owner_fenced_lease(self):
        ...
        class ControlledAuthority(FakeAuthority):
            def verify(self, **arguments):
                started.set()
                if not release.wait(timeout=5):
                    raise RuntimeError("test authority was not released")
                return super().verify(**arguments)

        class ObservedStore(DurableIdempotencyStore):
            def __init__(self, *arguments, **keywords):
                self.initial_expiry: float | None = None
                self.renewed_expiry: float | None = None
                self.first_renewal = threading.Event()
                super().__init__(*arguments, **keywords)

            def _expiry(self) -> float:
                with closing(sqlite3.connect(self.path)) as connection:
                    return float(connection.execute(
                        "SELECT lease_expires_at FROM proof_check_idempotency WHERE state='inflight'"
                    ).fetchone()[0])

            def acquire(self, *arguments, **keywords):
                action = super().acquire(*arguments, **keywords)
                if action[0] == "execute":
                    self.initial_expiry = self._expiry()
                return action

            def renew(self, *arguments, **keywords):
                renewed = super().renew(*arguments, **keywords)
                if renewed:
                    self.renewed_expiry = self._expiry()
                    if self.initial_expiry is not None and self.renewed_expiry > self.initial_expiry:
                        self.first_renewal.set()
                return renewed

        with tempfile.TemporaryDirectory() as directory:
            root = pathlib.Path(directory)
            store = ObservedStore(root / "cache.sqlite", lease_seconds=0.3)
            authority = ControlledAuthority(); application = self._application(root, authority, store)
            results: list[dict] = []; errors: list[BaseException] = []

            def invoke() -> None:
                try:
                    results.append(application.handle(self.request(), authenticated_principal_id=self.PRINCIPAL))
                except BaseException as error:
                    errors.append(error)

            first = threading.Thread(target=invoke)
            first.start()
            self.assertTrue(started.wait(timeout=2), "authority execution did not start")
            self.assertTrue(store.first_renewal.wait(timeout=3), "active owner lease was not renewed")
            self.assertIsNotNone(store.initial_expiry)
            self.assertGreater(store.renewed_expiry, store.initial_expiry)
            replay_thread = threading.Thread(target=invoke)
            replay_thread.start()
            release.set()
            first.join(timeout=5); replay_thread.join(timeout=5)
            self.assertFalse(first.is_alive() or replay_thread.is_alive())
            self.assertEqual(errors, [])
            self.assertEqual(len(results), 2); self.assertEqual(results[0], results[1]); self.assertEqual(authority.calls, 1)
    ...
    def test_completion_bound_rejects_growth_before_publication(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            action, owner = store.acquire("principal", "key", "a" * 128); self.assertEqual(action, "execute")
            with self.assertRaisesRegex(RuntimeError, "envelope byte limit"):
                store.complete("principal", "key", str(owner), "a" * 128, {"payload": "x" * 100000})
    ...
    def test_cached_json_must_be_duplicate_free_and_canonical(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            _, owner = store.acquire("principal", "key", "a" * 128); store.complete("principal", "key", str(owner), "a" * 128, {"a": 1})
            ...
            with self.assertRaisesRegex(RuntimeError, "canonical duplicate-free"):
                store.acquire("principal", "key", "a" * 128)
