...
def compiler_profile(result_key: MLDSA87PrivateKey, runtime_hash: str) -> authority.CompilerProfile:
    return authority.CompilerProfile.from_record({
        ...
        "image_reference":"registry.example.invalid/witness/lean-verifier","oci_image_digest":"shake256-512:" + "1" * 128,
        "oci_runtime_shake256_512":runtime_hash,"oci_runtime_version":"podman version 5.4.0",
        "lake_executable_shake256_512":"3" * 128,"lean_executable_shake256_512":"4" * 128,"lean_stdlib_tree_shake256_512":"5" * 128,
        "dependency_closure_shake256_512":"6" * 128,"verifier_executable_shake256_512":"7" * 128,"verifier_source_revision":"git:535",
        "verifier_build_attestation_id":"attestation:535","sandbox_policy_shake256_512":"8" * 128,
...
class RunnerFixture:
    def __init__(self):
        ...
        self.key = MLDSA87PrivateKey.generate(); self.profile = compiler_profile(self.key, authority.shake256_512_file(self.runtime))
    ...
    def runner(self, **overrides):
        values = dict(
            ...
            seccomp_profile_path=self.seccomp, expected_seccomp_profile_shake256_512=authority.shake256_512_file(self.seccomp),
            supported_controls=authority.OciSandboxRunner.REQUIRED_CONTROLS,
        ); values.update(overrides)
        return authority.OciSandboxRunner(self.runtime, authority.shake256_512_file(self.runtime), self.profile.oci_runtime_version, **values)
...
class OciSandboxTests(unittest.TestCase):
    ...
    def test_every_effective_control_has_runtime_mapping(self):
        ...
        expected_fragments = ("--entrypoint=", "--workdir=/work", "--network=none", "--read-only", "--cap-drop=ALL", "no-new-privileges", "seccomp=", "apparmor=", "--userns=keep-id", "--pids-limit=", "--memory=", "--cpus=", "--user=65534:65534", "--ulimit=fsize=", "--ulimit=nofile=", "--volume=", "--tmpfs=/tmp:")
    ...
    def test_invocation_digest_changes_for_every_declared_control(self):
        _, invocation = self.invocation(); baseline = invocation.shake256_512
        mutations = {
        ...
        for field, value in mutations.items():
                with self.subTest(field=field): self.assertNotEqual(dataclasses.replace(invocation, **{field:value}).shake256_512, baseline)
...
class PolicyTests(unittest.TestCase):
    def setUp(self):
        self.key = MLDSA87PrivateKey.generate(); self.now = datetime(2026, 7, 31, tzinfo=timezone.utc)
    ...
    def registry(self, **changes):
        ...
        decision.update(changes); decision["canonical_record_shake256_512"] = authority.shake256_512_bytes(authority.canonical_bytes(decision))
    ...
    def test_policy_status_scope_and_time_are_enforced_before_execution(self):
        ...
        self.assertEqual(resolved.canonical_record_shake256_512, self.registry()["decisions"][0]["canonical_record_shake256_512"])
    ...
    def test_policy_decision_and_record_hash_are_signature_bound(self):
        resolver = authority.ProofPolicyResolver(self.registry(), self.key.public_key(), clock=lambda:self.now)
        resolved = resolver.resolve("policy:1", subject_id="principal:1", operation="proof_check", compiler_profile_id="profile:1", source_bundle_id="bundle:1")
        self.assertEqual(len(resolved.canonical_record_shake256_512), 128)
...
class FormalBuildTests(unittest.TestCase):
    ...
    def test_attested_inspector_hash_matches_image_binary(self):
        ...
        self.assertIn("output_binary_shake256_512", schema["required"])
...
class ApiConformanceTests(unittest.TestCase):
    class Resolver:
        def resolve(self, policy_decision_id, **_):
            return authority.ResolvedPolicyDecision(policy_decision_id, {}, "3" * 128, 600, 1000000)

    class Authority:
        def __init__(self, witness):
            self.witness = witness; self.calls = 0; self.signing_key = MLDSA87PrivateKey.generate()
            self.key_id = "witness-key:test"; self.verifier_principal_id = "verifier:test"
        def verify(self, **arguments):
            self.calls += 1; result = authority.signed_payload(json.loads(json.dumps(self.witness)))
            result.update({
                "service_request_id": arguments["service_request_id"],
                "authenticated_principal_id": arguments["authenticated_principal_id"],
                "source_bundle_id": arguments["source_bundle_id"], "claim_id": arguments["claim_id"],
                "claim_content_shake256_512": authority.shake256_512_bytes(authority.canonical_bytes(arguments["canonical_claim"])),
                "theorem_statement_shake256_512": authority.shake256_512_bytes(arguments["theorem_statement"].encode("utf-8")),
                ...
                "policy_decision_id": arguments["policy_decision_id"], "policy_decision_shake256_512": arguments["policy_decision_shake256_512"],
                "key_id": self.key_id, "verifier_principal_id": self.verifier_principal_id,
            })
            return authority.sign_record(result, self.signing_key)
    ...
    def test_openapi_response_matches_running_wsgi_adapter_and_idempotency(self):
        ...
        witness["schema_version"] = "lean_compiler_witness/v8"
        witness.update({
            "semantic_witness_content_id":"lean-semantic:"+"5"* 128,
            "execution_evidence_content_id":"lean-execution:"+"6"* 128,
            "request_deadline_evidence":{"schema_version":"request_deadline_evidence/v1","request_budget_seconds":600,"elapsed_at_witness_signing_ms":1,"remaining_at_witness_signing_ms":599999,"deadline_source":"caller_shared_monotonic","enforced_stages":["snapshot","untrusted_compilation","trusted_inspection","trusted_result_validation","compiler_witness_signing"]},
            "service_request_id":"request-001", "authenticated_principal_id":"principal-reviewer-001", "source_bundle_id":"bundle-001",
            "normalized_executed_argv_shake256_512":"1"* 128, "handoff_manifest_shake256_512":"2"* 128,
            ...
            "effective_resource_limits_shake256_512":"3"* 128, "trust_root_attestation_registry_shake256_512":"4"* 128,
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            request = {"request_id":"request-001","idempotency_key":"proof-check-001","compiler_profile_id":"lean-4.29.1-hermetic-3","claim_id":"claim-001","canonical_claim":{"statement":"True"},"theorem_statement":"True","theorem_name":"t","source_bundle_id":"bundle-001","proof_artifact_relative_path":"Proof.lean","policy_decision_id":"policy-proof-001"}
            ...
            environ["witness.authenticated_principal_id"] = "principal-reviewer-001"
        ...
        result_schema = json.loads((ROOT / "schemas/proof_check_result_v7.schema.json").read_text(encoding="utf-8"))
        self.assertTrue(set(result_schema["required"]).issubset(result))
        witness_schema = json.loads((ROOT / "schemas/lean_compiler_witness_v8.schema.json").read_text(encoding="utf-8"))
