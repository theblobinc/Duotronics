# Duotronic Witness Contract v1.6 — Draft 5.3.6
...
Draft 5.3.6 carries the complete v1.6 corpus forward and corrects the
...
Draft 5.3.5. Earlier evidence-language, replay, non-collapse,
positive-baseline, bijective-mathematics, governance, and historical material
remains present.

`CANONICAL_CORPUS_v1_6_draft_5_3_6.json` is the sole active root selector.
...
is `shake256_512:<tree digest>` and is reverified after execution.
...
`EffectiveSandboxInvocation/v3` is the single source from which the OCI adapter
...
The model separates `requested_controls`, `emitted_controls`,
`accepted_controls`, `measured_controls`, and host-derived isolation facts.
Requested, emitted, and accepted controls must agree for an eligible run, and
every required control must be observed as measured. Missing network, LSM,
cgroup, environment, mount, or identity evidence remains unverified. The exact
normalized OCI argv—including host mount paths, tmpfs quotas, resource flags,
entrypoint, image, and arguments—determines the sandbox invocation digest.
...
collects the recursively closed type-and-value dependency graph, queries the
compiled axiom closure at every node, and rejects `sorryAx`, unauthorized
axioms, unsafe or forbidden opaque declarations, unresolved
...
## 14. Compiler witness v5

`lean_compiler_witness/v5` binds claim, theorem type, artifact, sealed snapshot,
...
template and exact executed invocation, requested/emitted/accepted/measured
control sets, effective resource-limit hash, exact compile-handoff manifest,
stream hashes and output-limit status, policy-decision ID and hash, signed
verifier result, structural expression fingerprints, recursive dependency and
axiom results, trust-artifact registry, authority snapshot and ledger cutoff,
deterministic semantic ID, server time, and final signature.
...
These rules directly cover configuration, policy and compiler registries,
keys, OCI runtime, and seccomp policy. Migrations, inspector executable, image
metadata, and dependency manifests must be either securely loaded under those
same rules or bound by a governance-signed trusted-artifact attestation
registry. Documentation may not claim broader direct coverage than the loader
implements.
...
`refs/schema_registry_v1_6_draft_5_3_6.json` classifies every schema as
...
versions or registry paths, and requires positive and negative Draft 5.3.6
...
The Draft 5.3.6 migration adds v5 compiler witnesses and v5 theorem gates over
the retained immutable signed proof-policy registries and decisions. A gate requires
...
views therefore return no new Draft 5.3.6 theorem authority unless all signed
...
ProofAuthority V7 models recursive dependency safety, exact executed argv,
policy-derived resource limits, trust-boundary schema validation,
requested/emitted/accepted/measured controls, compile-handoff verification,
durable idempotency, trusted-artifact attestations, and external activation.
The strict TLA manifest retains V2–V6 and adds V7 and its configuration; static
name coverage is not TLC execution.
...
sandbox control mapping, recursive inspection, handoff verification,
policy-derived split resource limits, durable idempotency, synchronous API conformance,
...
Draft 5.3.6 is a completed standalone corrective corpus when every required
