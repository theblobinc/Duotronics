#!/usr/bin/env python3
"""Authenticated, cache-verifying proof-check boundary for Draft 5.3.14."""
...
class SignedAppendOnlyAuditSink:
    """Signed segmented JSONL audit chain anchored by a protected tail checkpoint."""
    ...
    EVENT_SCHEMA_SURFACES = {
        ...
        "cache_audit_segment_seal/v1": "cache_audit_segment_seal",
    }

    def __init__(
        self, path: Path, checkpoint_path: Path,
        signing_key: Ed25519PrivateKey, verification_key: Ed25519PublicKey, *,
        segment_id: str, signer_principal_id: str, signer_key_id: str,
        schema_validator: CanonicalSchemaValidator,
        maximum_record_bytes: int, maximum_log_bytes: int, maximum_records: int,
        previous_sealed_segment_tail_sha256: str | None = None,
        rotation_policy: str = "signed_sealed_segment_with_external_checkpoint",
        clock: Callable[[], datetime] | None = None,
        monotonic: Callable[[], float] | None = None,
        sleep: Callable[[float], None] | None = None,
        fsync: Callable[[int], None] | None = None,
        provision: bool = False,
        require_separate_checkpoint_directory: bool = False,
    ):
        self.path = Path(path)
        ...
        self.previous_sealed_segment_tail_sha256 = previous_sealed_segment_tail_sha256
        ...
        if not IDENTIFIER.fullmatch(self.segment_id):
            raise ValueError("cache audit segment identifier is not canonical")
        if self.rotation_policy != "signed_sealed_segment_with_external_checkpoint":
            raise ValueError("cache audit rotation policy is not governed")
        if self.maximum_record_bytes < 1024 or self.maximum_log_bytes < self.maximum_record_bytes or self.maximum_records < 1:
            raise ValueError("cache audit bounds are invalid")
        if previous_sealed_segment_tail_sha256 is not None and not re.fullmatch(r"[0-9a-f]{64}", previous_sealed_segment_tail_sha256):
            raise ValueError("previous sealed audit segment tail is invalid")
        if public_key_raw_b64url(self.signing_key.public_key()) != public_key_raw_b64url(self.verification_key):
            raise ValueError("cache audit private and public keys do not form one Ed25519 keypair")
        if require_separate_checkpoint_directory and self.path.parent.resolve() == self.checkpoint_path.parent.resolve():
            raise ValueError("production audit checkpoint must use an independently protected directory")
        if provision:
            self._provision_segment()
        descriptor = self._open_locked_log(deadline_monotonic=None)
        try:
            sequence, tail = self._verify_descriptor(descriptor, deadline_monotonic=None)
            checkpoint = self._load_checkpoint()
            self._verify_checkpoint(checkpoint, sequence=sequence, tail=tail)
    ...
    def _provision_segment(self) -> None:
        ...
        if self.path.exists() or self.checkpoint_path.exists():
            raise RuntimeError("cache audit segment provisioning refuses an existing log or checkpoint")
        ...
        self._write_checkpoint(sequence=0, tail=None, segment_status="open", deadline_monotonic=None)
    ...
    def _open_locked_log(self, *, deadline_monotonic: float | None) -> int:
        ...
        try:
            ...
            if not stat.S_ISREG(info.st_mode) or info.st_nlink != 1 or stat.S_IMODE(info.st_mode) != 0o600:
                raise RuntimeError("cache audit log is not one private regular file")
            if info.st_uid != os.getuid():
                raise RuntimeError("cache audit log is not owned by the service identity")
            return descriptor
        except Exception:
            try:
                fcntl.flock(descriptor, fcntl.LOCK_UN)
    ...
    def _verify_descriptor(self, descriptor: int, *, deadline_monotonic: float | None) -> tuple[int, str | None]:
        data = self._read_descriptor(descriptor)
        self._deadline_check(deadline_monotonic, "after chain read")
        if len(data) > self.maximum_log_bytes:
        ...
        for raw_line in data.splitlines():
            ...
            if not verify_record(record, self.verification_key):
                raise RuntimeError("cache audit record signature is invalid")
            count += 1
            if (
                ...
                raise RuntimeError("cache audit segment, sequence, or hash chain is invalid")
            ...
            self._validate_event(event, str(record.get("event_schema_version", "")))
            identity = {
    ...
    def _load_checkpoint(self) -> dict[str, Any]:
        ...
        try:
            ...
            raise AuthorityFailure("cache_audit_integrity_invalid", "cache audit tail checkpoint is missing") from error
        try:
            info = os.fstat(descriptor)
        ...
        try:
            ...
            raise RuntimeError("cache audit checkpoint is not canonical JSON") from error
        if canonical_bytes(checkpoint) + b"\n" != raw:
            raise RuntimeError("cache audit checkpoint is not canonical JSON with one terminal newline")
        self.schema_validator.validate("cache_audit_checkpoint", checkpoint)
        if not verify_record(checkpoint, self.verification_key):
            raise RuntimeError("cache audit checkpoint signature is invalid")
        return checkpoint

    def _verify_checkpoint(self, checkpoint: dict[str, Any], *, sequence: int, tail: str | None) -> None:
        if (
            ...
            raise AuthorityFailure("cache_audit_integrity_invalid", "cache audit log tail differs from its protected checkpoint")
        if checkpoint.get("segment_status") == "sealed":
            if sequence < 1:
                raise AuthorityFailure("cache_audit_integrity_invalid", "empty audit segment cannot be sealed")

    def _checkpoint_record(self, *, sequence: int, tail: str | None, segment_status: str) -> dict[str, Any]:
        unsigned = {
            "schema_version": "cache_audit_checkpoint/v1",
            "segment_id": self.segment_id,
            "sequence": sequence,
            ...
            "checkpoint_created_at": self.clock().astimezone(timezone.utc).isoformat(),
        }
    ...
    def _write_checkpoint(
        ...
        self._secure_parent(self.checkpoint_path, create=True)
        encoded = canonical_bytes(self._checkpoint_record(sequence=sequence, tail=tail, segment_status=segment_status)) + b"\n"
    ...
    def _append_locked(
        ...
        sequence_before, previous_before = self._verify_descriptor(descriptor, deadline_monotonic=deadline_monotonic)
        checkpoint = self._load_checkpoint()
        self._verify_checkpoint(checkpoint, sequence=sequence_before, tail=previous_before)
        ...
        if sequence_before >= self.maximum_records:
            raise RuntimeError("cache audit segment is full and requires governed rotation")
        sequence = sequence_before + 1
        identity = {
            "segment_id": self.segment_id,
            "sequence": sequence,
            "previous_audit_record_sha256": previous_before,
            "previous_sealed_segment_tail_sha256": self.previous_sealed_segment_tail_sha256,
            "event_sha256": event_sha256,
            "audit_signer_key_id": self.signer_key_id,
        }
        unsigned = {
            "schema_version": "cache_audit_record/v2",
            ...
            "event_canonical_json": event_json,
            "event_sha256": event_sha256,
        ...
        append_overran = deadline_monotonic is not None and self.monotonic() >= deadline_monotonic
        ...
        self._fsync_directory(self.path.parent, deadline_monotonic=None if append_overran else deadline_monotonic)
        self._sequence = sequence
        self._previous_record_sha256 = tail
        self._segment_status = segment_status_after
        if append_overran:
            raise TimeoutError("cache audit persistence exceeded the request deadline")
        self._deadline_check(deadline_monotonic, "after audit persistence")

    def __call__(self, evidence: dict[str, Any], *, deadline_monotonic: float | None = None) -> None:
        with self._lock:
    ...
    def seal_segment(self, *, deadline_monotonic: float | None = None) -> dict[str, Any]:
        with self._lock:
            descriptor = self._open_locked_log(deadline_monotonic=deadline_monotonic)
            try:
                sequence, tail = self._verify_descriptor(descriptor, deadline_monotonic=deadline_monotonic)
                checkpoint = self._load_checkpoint()
                self._verify_checkpoint(checkpoint, sequence=sequence, tail=tail)
                if checkpoint["segment_status"] != "open":
                    raise AuthorityFailure("cache_audit_integrity_invalid", "cache audit segment is already sealed")
                event = {
                    "schema_version": "cache_audit_segment_seal/v1",
                    "segment_id": self.segment_id,
                    "sealed_through_sequence": sequence + 1,
                    "sealed_predecessor_tail_sha256": tail,
                    "previous_sealed_segment_tail_sha256": self.previous_sealed_segment_tail_sha256,
                    "decision": "terminal_segment_seal",
                    "sealed_at": self.clock().astimezone(timezone.utc).isoformat(),
                }
                self._append_locked(
                    descriptor, event, deadline_monotonic=deadline_monotonic,
                    segment_status_after="sealed",
                )
...
def load_governed_cache_registry_snapshots(
    ...
    snapshots: dict[str, GovernedCacheRegistrySnapshot] = {
        current_digest: build_governed_cache_registry_snapshot(
            ...
            lineage_signed_payload_sha256=lineage_payload_sha256, lineage_created_at=lineage["created_at"], evaluated_at=evaluated_at,
        ),
    }
    ...
    for entry in lineage["historical_registries"]:
        ...
        snapshots[registry_digest] = build_governed_cache_registry_snapshot(
            ...
            lineage_signed_payload_sha256=lineage_payload_sha256, lineage_created_at=lineage["created_at"],
        )
...
class ProofCheckApplication:
    ...
    def _validate_cache_envelope_bindings(
        ...
        request_sha256: str, resolved_policy, stored_record: dict[str, Any],
    ) -> str:
        expected = {
            "request_sha256": request_sha256, "request_id": request["request_id"],
            "idempotency_key": request["idempotency_key"], "authenticated_principal_id": principal_id,
        ...
        return result["compiler_witness"]["signed_payload_sha256"]
    ...
    def _cached_result(
        self, envelope: dict[str, Any], request: dict[str, Any], principal_id: str,
        request_sha256: str, resolved_policy, deadline_monotonic: float,
    ) -> dict[str, Any]:
        ...
        compiler_witness_sha256 = self._validate_cache_envelope_bindings(
            envelope, request, principal_id, request_sha256, resolved_policy, stored_record,
        )
        ...
        if stored_registry_sha256 != self.cache_registry_sha256:
            ...
            stale_evidence = {
                ...
                "cache_envelope_signed_payload_sha256": envelope["signed_payload_sha256"],
                "cache_envelope_canonical_sha256": sha256_bytes(canonical_bytes(envelope)),
                ...
                "source_bundle_id": request["source_bundle_id"],
                "proof_artifact_relative_path": request["proof_artifact_relative_path"],
                "compiler_witness_signed_payload_sha256": compiler_witness_sha256,
    ...
    def handle(self, request: dict[str, Any], *, authenticated_principal_id: str) -> dict[str, Any]:
        ...
        while owner is None:
            ...
            if action == "completed":
                ...
                cached = self._cached_result(value, request, principal_id, request_sha256, resolved_policy, deadline)
...
def load_production_application(config_root: Path = Path("/etc/witness-authority")) -> ProofCheckApplication:
    ...
    audit_signing_key = _private_key_from_bytes(
        secure_read_bytes(config_root, config["cache_audit_private_key_file"], expected_uid=expected_uid)
    )
    audit_verification_key = _public_key_from_bytes(
        secure_read_bytes(config_root, config["cache_audit_public_key_file"], expected_uid=expected_uid)
    )
    audit_public = public_key_raw_b64url(audit_verification_key)
    if public_key_raw_b64url(audit_signing_key.public_key()) != audit_public:
        raise RuntimeError("cache audit private and public keys do not form one Ed25519 keypair")
    if audit_public in {public_key_raw_b64url(cache_verification_key), public_key_raw_b64url(authority.signing_key.public_key())}:
        raise RuntimeError("cache audit signing authority must be distinct from cache and compiler-witness authorities")
    audit_relative = _safe_relative_path(config["cache_audit_log_file"])
    checkpoint_relative = _safe_relative_path(config["cache_audit_checkpoint_file"])
    previous_sealed_tail = config["cache_audit_previous_sealed_segment_tail_sha256"]
    audit_sink = SignedAppendOnlyAuditSink(
        config_root.joinpath(*audit_relative.parts),
        config_root.joinpath(*checkpoint_relative.parts),
        audit_signing_key, audit_verification_key,
        segment_id=config["cache_audit_segment_id"],
        previous_sealed_segment_tail_sha256=previous_sealed_tail,
        signer_principal_id=config["cache_audit_signer_principal_id"],
        signer_key_id=config["cache_audit_signer_key_id"], schema_validator=validator,
        maximum_record_bytes=int(config["cache_audit_maximum_record_bytes"]),
        maximum_log_bytes=int(config["cache_audit_maximum_log_bytes"]),
        maximum_records=int(config["cache_audit_maximum_records"]),
        rotation_policy=config["cache_audit_rotation_policy"],
        require_separate_checkpoint_directory=True,
    )
