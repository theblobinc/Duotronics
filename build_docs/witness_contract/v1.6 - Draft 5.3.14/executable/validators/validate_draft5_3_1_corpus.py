...
EXPECTED_HISTORY_SHA256 = "437395d28c452f0a20937eaf562020afae2214edf01a8df2fe6dadd062201c22"
...
def phase_descriptor() -> dict[str, Any]:
    ...
    for token in ("CANONICAL_CORPUS_v1_6_draft_5_3_1.json", "PACKAGE_INVENTORY_v1_6_draft_5_3_1.json", "CHECKSUMS_v1_6_draft_5_3_1.sha256"):
...
def phase_hash_closure() -> dict[str, Any]:
    ...
    checksum_lines = (ROOT / "refs/manifest/CHECKSUMS_v1_6_draft_5_3_1.sha256").read_text(encoding="utf-8").splitlines()
    checksums = {}
...
def _insert_happy_authority_chain(con: sqlite3.Connection) -> dict[str, str]:
    ...
    con.execute("INSERT INTO wc_verifier_principals_v2 VALUES (?,?,?,?,?,?,?,?,?)", ("verifier:1", "verifier_principal/v2", "key:1", "Ed25519", f, "active", now, None, now))
    con.execute("""INSERT INTO wc_lean_compiler_witnesses_v2 (
      ...
      "promotion_sound", "proved", "verifier:1", "key:1", "Ed25519", one, sig_a, now))
    con.execute("""INSERT INTO wc_proof_witnesses_v2 (
      ...
      verifier_principal_id,signed_payload_sha256,signature,created_at
    ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)""", (
...
def phase_sql_authority() -> dict[str, Any]:
    ...
    _expect_integrity_error(lambda: con.execute("""INSERT INTO wc_theorem_promotion_gates_v2 VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)""", (
        "gate:badhash", "theorem_promotion_gate/v2", "claim:proof", "9"*64, values["theorem_hash"], "status:valid",
    ...
    con.execute("INSERT INTO wc_replay_manifests_v2 VALUES (?,?,?,?,?,?,?,?)", (
      ...
      '[{"assumption_id":"a1","required_for_pass":true,"status":"satisfied"}]', '["a1"]', "block", "3"*64, values["now"]))
    con.execute("INSERT INTO wc_verification_grammars_v2 VALUES (?,?,?,?,?)", (
      "grammar:ok", "verification_grammar/v2", '[{"instruction_id":"i1"},{"instruction_id":"i2"}]', "4"*64, values["now"]))
    con.execute("INSERT INTO wc_verification_results_v2 VALUES (?,?,?,?,?,?,?,?,?,?,?,?)", (
      "result:ok", "verification_result/v2", "grammar:ok", "4"*64, "manifest:ok", "3"*64, "artifact:1", "pass",
    ...
    _expect_integrity_error(lambda: con.execute("INSERT INTO wc_verification_results_v2 VALUES (?,?,?,?,?,?,?,?,?,?,?,?)", (
      "result:failed-step", "verification_result/v2", "grammar:ok", "4"*64, "manifest:ok", "3"*64, "artifact:2", "pass",
    ...
    con.execute("INSERT INTO wc_replay_manifests_v2 VALUES (?,?,?,?,?,?,?,?)", (
      ...
      '[{"assumption_id":"a2","required_for_pass":true,"status":"unknown"}]', '["a2"]', "block", "5"*64, values["now"]))
    _expect_integrity_error(lambda: con.execute("INSERT INTO wc_verification_results_v2 VALUES (?,?,?,?,?,?,?,?,?,?,?,?)", (
      "result:unsatisfied", "verification_result/v2", "grammar:ok", "4"*64, "manifest:unsatisfied", "5"*64, "artifact:3", "pass",
...
def phase_proof_signature_runtime() -> dict[str, Any]:
    ...
    return {"tests": 3, "stdout": process.stdout.strip(), "stderr": process.stderr.strip(), "algorithm": "Ed25519"}


def phase_historical_package() -> dict[str, Any]:
    archive = ROOT / "history/source_packages/v1.6 - Draft 5.2.2.zip"
    assert_true(sha256(archive) == EXPECTED_HISTORY_SHA256, "retained Draft 5.2.2 archive hash mismatch")
    ...
    return {"sha256": EXPECTED_HISTORY_SHA256, "file_count": 492, "zip_integrity": "passed"}
