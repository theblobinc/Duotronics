...
def _prepare_fixture(chroot_root: pathlib.Path) -> None:
    ...
    audit_log = trust_root / config["cache_audit_log_file"]
    audit_checkpoint = trust_root / config["cache_audit_checkpoint_file"]
    audit_log.parent.mkdir(parents=True, mode=0o700)
    audit_checkpoint.parent.mkdir(parents=True, mode=0o700)
    service.SignedAppendOnlyAuditSink(
        audit_log, audit_checkpoint, cache_audit, cache_audit.public_key(),
        segment_id=config["cache_audit_segment_id"],
        previous_sealed_segment_tail_sha256=config["cache_audit_previous_sealed_segment_tail_sha256"],
        signer_principal_id=config["cache_audit_signer_principal_id"],
        signer_key_id=config["cache_audit_signer_key_id"],
        schema_validator=authority.CanonicalSchemaValidator(ROOT / "schemas"),
        maximum_record_bytes=config["cache_audit_maximum_record_bytes"],
        maximum_log_bytes=config["cache_audit_maximum_log_bytes"],
        maximum_records=config["cache_audit_maximum_records"],
        rotation_policy=config["cache_audit_rotation_policy"],
        provision=True,
    )
    for item in (audit_log.parent, audit_checkpoint.parent, audit_log, audit_checkpoint):
            ...
            if child == 0:
                ...
                try:
                    ...
                    result = {
                        ...
                        "cache_audit_log_owner": (config_root / "cache-audit-segments/cache-audit-0001.jsonl").stat().st_uid,
                        "cache_audit_log_mode": oct((config_root / "cache-audit-segments/cache-audit-0001.jsonl").stat().st_mode & 0o777),
                    }
