#!/usr/bin/env python3
"""Descriptor-reconciled fail-closed validator for Draft 5.3.12."""
...
DESCRIPTOR_PATH = ROOT / "CANONICAL_CORPUS_v1_6_draft_5_3_12.json"
REPORT_PATH = ROOT / "DRAFT5_3_12_VALIDATION_REPORT.json"
REGRESSION_COUNTS_PATH = ROOT / "DRAFT5_3_12_REGRESSION_COUNTS.json"
VALIDATION_SUMMARY_PATH = ROOT / "DRAFT5_3_12_VALIDATION_SUMMARY.txt"
LINEAGE_PATH = ROOT / "history/SOURCE_PACKAGE_LINEAGE_v1_6_draft_5_3_12.json"
...
def _run_discovered_suite(*, development_mode: bool) -> dict[str, Any]:
    ...
    assert_true(major_minor in {(3, 12), (3, 13)}, "Python version is outside the Draft 5.3.12 target matrix")
    return {
        "command": command,
        **structured,
...
def phase_descriptor() -> dict[str, Any]:
    ...
    assert_true(descriptor["active_version"] == "v1.6-draft-5.3.12", "active version mismatch")
    ...
    for token in (DESCRIPTOR_PATH.name, "PACKAGE_INVENTORY_v1_6_draft_5_3_12.json", "CHECKSUMS_v1_6_draft_5_3_12.sha256"):
        assert_true(token in resolver, f"resolver does not boot canonical token {token}")
    accounting = _run_python_tests("-W", ["error", "executable/tests/test_validator_phase_reconciliation_v5312.py"])
...
def phase_dependencies() -> dict[str, Any]:
    ...
    try:
        ...
        connection.close()
    scope = load_json("refs/validation_scope_v1_6_draft_5_3_12.json")
...
def phase_hash_closure() -> dict[str, Any]:
    inventory = load_json("PACKAGE_INVENTORY_v1_6_draft_5_3_12.json")
    ...
    checksum_lines = (ROOT / "refs/manifest/CHECKSUMS_v1_6_draft_5_3_12.sha256").read_text(encoding="utf-8").splitlines()
...
def _schema_phase(name: str) -> dict[str, Any]:
    command = ["node", "executable/validators/validate_draft5_3_12_schemas.mjs", "--phase", name]
...
def phase_python_version_portability() -> dict[str, Any]:
    generated = load_json(REGRESSION_COUNTS_PATH.name)
    matrix = load_json("DRAFT5_3_12_PYTHON_MATRIX_VALIDATION.json")
    metadata = load_json("PACKAGE_METADATA_v1_6_draft_5_3_12.json")
    ...
    for item in [*runs, *unavailable_runs]:
        ...
        assert_true(isinstance(relative, str) and relative.startswith("validation/python_matrix/draft5_3_12/"), "Python run evidence path is invalid")
        path = ROOT / relative
    ...
    assert_true(generated.get("python_matrix_sha256") == sha256(ROOT / "DRAFT5_3_12_PYTHON_MATRIX_VALIDATION.json"), "regression record does not bind the Python matrix")
    for item in runs:
...
def phase_openapi_conformance() -> dict[str, Any]:
    ...
    spec = yaml.safe_load((ROOT / "executable/openapi/draft5_3_12_evidence_language_openapi.yaml").read_text(encoding="utf-8"))
    assert_true(spec["info"]["version"] == "v1.6-draft-5.3.12", "OpenAPI version mismatch")
...
def phase_schema_registry_authority() -> dict[str, Any]:
    registry = load_json("refs/schema_registry_v1_6_draft_5_3_12.json")
...
def phase_version_consistency() -> dict[str, Any]:
    ...
    gate_status = load_json("DRAFT5_3_12_RELEASE_GATE_STATUS.json")
    assert_true(descriptor["active_version"] == metadata["version"] == expected, "descriptor/package metadata version mismatch")
    assert_true(package["version"] == "1.6.5-3-12" and lock["version"] == package["version"], "npm package/lock version mismatch")
    assert_true(package["name"].endswith("draft-5-3-12-validation") and lock["name"] == package["name"], "npm package/lock identity mismatch")
    assert_true(registry.get("registry_version") == expected, "schema registry version mismatch")
    assert_true("Draft 5.3.12" in tla["package"] and any(item["module"] == "ProofAuthorityV8" for item in tla["tla_modules"]), "formal manifest version/active model mismatch")
    assert_true(gate_status["gates"]["I_package_provenance"] == "unpublished_workspace", "release provenance status is not fail-closed")
    assert_true('v!"5.3.12"' in (ROOT / "lakefile.lean").read_text(encoding="utf-8"), "Lake package version mismatch")
    active_documents = (
        "README.md", "START_HERE.md", "CANONICAL_CORPUS_v1_6_draft_5_3_12.json",
        "CORPUS_INDEX_v1_6_draft_5_3_12.md", "duotronic_witness_contract_v1_6_draft_5_3_12.md",
        "RELEASE_NOTES_v1_6_draft_5_3_12.md", "PACKAGE_METADATA_v1_6_draft_5_3_12.json",
    ...
    for relative in active_documents:
        ...
        assert_true("5.3.12" in text_value, f"active entrypoint lacks 5.3.12 identity: {relative}")
    ...
    for report_path in ROOT.glob("DRAFT5_3_12*.json"):
...
def phase_package_provenance() -> dict[str, Any]:
    provenance = load_json("PACKAGE_PROVENANCE_v1_6_draft_5_3_12.json")
    required = {
...
def phase_standalone_merged_corpus() -> dict[str, Any]:
    ...
    assert_true(
        releases[-1].get("version") == "v1.6-draft-5.3.11"
        and releases[-1].get("sha256") == "f1643c2f1a9cb4902d3685b87056548ddd8677278d7db4169e03fe82d03c62bc",
...
def _build_attestation(subject_type: str) -> dict[str, Any]:
    status = load_json("DRAFT5_3_12_BUILD_ATTESTATION_STATUS.json")
...
def phase_external_signature() -> dict[str, Any]:
    status = load_json("refs/trust/RELEASE_SIGNATURE_STATUS_v1_6_draft_5_3_12.json")
...
def phase_bounded_validator_reaping() -> dict[str, Any]:
    ...
    evidence_records = [
        load_json("validation/validator_reliability/draft5_3_12/python_3_12.json"),
        load_json("validation/validator_reliability/draft5_3_12/python_3_13.json"),
    ]
    assert_true(
        {record.get("python_version") for record in evidence_records} == {"3.12.13", "3.13.5"},
        "validator reliability evidence does not cover both exact interpreter lines",
    )
    for record in evidence_records:
        runs = record.get("runs", [])
        assert_true(
            record.get("status") == "passed"
            and record.get("requested_repetitions") == record.get("completed_repetitions") == len(runs) == 10,
            "validator reliability campaign is incomplete",
        )
        assert_true(all(
            run.get("required_phases_passed") == 85
    ...
    return {
        **tests, "repetitions_per_interpreter": 10,
        "validated_python_versions": ["3.12.13", "3.13.5"],
...
def phase_production_nonroot_loader_contract() -> dict[str, Any]:
    ...
    evidence = load_json("validation/production_loader/draft5_3_12_nonroot_loader_evidence.json")
...
def main() -> int:
    ...
    inventory = load_json("PACKAGE_INVENTORY_v1_6_draft_5_3_12.json") if (ROOT / "PACKAGE_INVENTORY_v1_6_draft_5_3_12.json").is_file() else {"files": []}
    regular_directories = [
    ...
    report = {
        "schema_version": "draft5_3_12_validation_report/v1",
        "package_version": "v1.6-draft-5.3.12",
