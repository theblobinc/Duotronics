Draft 5.2.2 note: this package includes TLA+ state-machine checks, Lean/Lake proof authority, hardened SQL persistence, and exact theorem-gate witness-ID checks.
...
11. Validate the corpus and fixtures with `executable/validators/validate_draft5_2_2_corpus.py`.
...
For the TLA+ state-machine formal layer, start with `DRAFT5_2_TLA_PLUS_IMPLEMENTATION_REPORT_v1_0.md`, then run:

```bash
python executable/formal/run_tla_model_check.py --mode advisory
```

Use strict mode once TLC is installed or `TLA2TOOLS_JAR` points to `tla2tools.jar`. Lean/Lake proof-authority is added by Draft 5.2.1 and hardened in Draft 5.2.2; strict theorem promotion still requires an actual `lake build` pass in an environment with Lean/Lake installed.


## Draft 5.2.2 note

This package contains both TLA+ state-machine checks and Lean/Lake proof-authority checks. SQL persistence now fail-closes theorem/proof_verified status changes unless proof, Lean compiler, non-collapse, policy, and theorem-promotion-gate evidence are present.
