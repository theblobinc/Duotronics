...
The v1.6 Draft 1 implementation-readiness corpus now includes an MCP endpoint query contract. During this packaging pass, the generation environment did not expose a callable MCP endpoint tool or endpoint URL, so no live SRNN MCP response is recorded here.
