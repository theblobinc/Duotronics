...
## v1.5 distributed delegation action kind

> **Status tag:** normative

v1.5 adds a distributed action kind:

```yaml
action_kind: delegate_task
```

A `delegate_task` action must reference a `TaskDelegationActionPayload` from `duotronic_distributed_task_delegation_and_resource_witness_contract_v1_0.md`.

A planner may propose delegation, but it cannot create scheduling authority.

The policy shield must check:

1. target node admission;
2. fresh `ResourceAvailabilityWitness`;
3. fresh node self-model snapshot;
4. task queue pressure;
5. privacy permission;
6. DBP S2 transport profile;
7. lane layout and replay identity;
8. cluster learning mode if the task starts profile learning;
9. resource lease availability;
10. conflict resolution.

If a delegated task is deferred, stale resource or policy state requires revalidation before execution.
