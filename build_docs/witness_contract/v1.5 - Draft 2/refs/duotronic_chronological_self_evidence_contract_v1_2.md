# Duotronic Chronological Self-Evidence Contract v1.2

**Status:** Source-spec baseline candidate  
**Version:** chronological-self-evidence-contract@v1.2  
**Supersedes:** chronological-self-evidence-contract@v1.1  
...
## 16. Evidence purge and self-evidence streams

> **Status tag:** normative

Actor-scoped chronological streams must support purge propagation.

If a purge affects a stream, personal-history record, evidence bundle, or actor scope, the system must:

1. remove or tombstone affected personal-history records;
2. update or purge chronological evidence streams;
3. invalidate timeline witnesses;
4. invalidate self-model snapshots;
5. remove or demote preference, habit, and likely-action witnesses;
6. invalidate lookup memory records;
7. re-evaluate decision contexts and lazy actions;
8. block replay unless tombstone-only or policy-exception replay is allowed.

`EvidencePurgeEvent` is owned by `duotronic_evidence_purge_and_privacy_deletion_contract_v1_0.md`.

### 16.1 Self-model replay after purge

A self-model snapshot that depends on purged evidence may be replayed only if:

1. replay identity records the original snapshot version;
2. purged evidence is represented by tombstones or omitted according to policy;
3. replay mode is audit-only unless policy grants exception;
4. no new runtime action is justified from the purged data.
