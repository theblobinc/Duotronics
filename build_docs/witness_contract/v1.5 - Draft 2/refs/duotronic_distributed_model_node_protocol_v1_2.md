# Duotronic Distributed Model Node Protocol v1.2

**Status:** Source-spec baseline candidate  
**Version:** distributed-model-node-protocol@v1.2  
**Supersedes:** distributed-model-node-protocol@v1.1  
...
## 14. Chronological stream node role

> **Status tag:** normative

Draft 3 adds a `timeline_node` role for ingestion, ordering, replay, and high-speed loop preparation.

A timeline node may:

1. order evidence bundles by declared time policy;
2. create timeline witnesses;
3. maintain parent/self-reference edges;
4. generate derived timeline indexes;
5. support high-speed loop profiles;
6. emit invalidation events when sources change.

A timeline node may not directly create self-model authority. It emits candidate or canonical timeline witnesses according to policy.
...
TimelineNodeEvent:
  ...
  input_evidence_ids: []
  output_timeline_witness_id: string
  derived_index_refs: []
  deterministic: true | false
  replay_ref: string | null
  local_policy_mode: audit_only | sandbox | restricted | normal
```

---

## 15. Planner node role

> **Status tag:** normative

Draft 3 adds a `planner_node` role.

A planner node may propose action candidates. It may not execute actions without policy.
...
PlannerNodeEvent:
  ...
  proposed_action_candidate_ids: []
  model_witness_ids: []
  policy_precheck_id: string | null
  deterministic: true | false
  replay_ref: string | null
```

Planner nodes must follow the non-deterministic node policy. Non-deterministic planner output is audit-only or sandbox unless independently replayed or policy-approved for a lower-risk scope.

---

## 16. Model diversity reporting by node

> **Status tag:** normative

A node emitting model witnesses must report enough metadata for model diversity and oracle-risk governance.

Required additions for model-related node events:

1. model family;
2. model provider or local source class where policy permits;
3. model version;
4. training-overlap class if known;
5. architecture family if known;
6. inference settings hash;
7. calibration profile if available;
8. known failure modes.

A node that cannot provide these fields must mark model independence as `unknown`.
