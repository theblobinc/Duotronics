# Duotronic Lab Evidence Registry v1.7

**Status:** Source-spec baseline candidate  
**Version:** lab-evidence-registry@v1.7  
**Supersedes:** lab-evidence-registry@v1.6  
**Supersedes:** lab-evidence-registry@v1.5  
...
## 10. v1.5 distributed cluster fixtures

> **Status tag:** reference

v1.5 adds lab fixture classes for:

1. valid Xeon worker resource witness;
2. valid GPU worker resource witness;
3. delegated task round trip;
4. resource staleness invalidation;
5. overcommit conflict;
6. Open/S1 frame rejection for authority-bearing command;
7. NodeHello accepted;
8. NodeHello rejected for unsupported security;
9. replay identity includes lane direction;
10. cluster-wide learning mode blocks profile-learning task;
11. least-loaded node selection;
12. GPU task avoids CPU-only node;
13. DW-SSM authority zero on transport failure;
14. DW-SSM authority positive under S2 and L5 clamp;
15. node disconnect reassigns tasks;
16. privacy conflict blocks delegation;
17. valid DBP lane layout;
18. malformed lane direction;
19. raw metrics have no authority;
20. Docker auto-federation round trip.

Fixture owner:

```text
refs/duotronic_v1_5_cluster_conformance_fixtures_v1_0.md
```


---

## 11. v1.5 Draft 2 WG-RNN fixtures

> **Status tag:** reference

v1.5 Draft 2 adds fixture classes for:

1. high-confidence canonical evidence write;
2. blocked write under invalidation;
3. quarantine under high novelty and low confidence;
4. promotion after repeated stable recurrence;
5. contradiction split;
6. purge cascade on memory slots;
7. transport failure zeroing authority;
8. human-review-required candidate-only write;
9. chronological music preference slot construction;
10. unchecked novelty quarantine.
