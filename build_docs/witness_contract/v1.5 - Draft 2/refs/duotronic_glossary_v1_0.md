...
## 15. v1.5 distributed cluster terms

### 15.1 Resource availability witness

A resource availability witness is a canonical witness fact describing a node's current CPU, RAM, GPU, disk, network, and container capacity for scheduling.

### 15.2 Task delegation

Task delegation is the policy-gated assignment of work from a coordinator or planner to a target node.

### 15.3 Node federation

Node federation is the process by which a node joins a Duotronic cluster through authenticated DBP transport, identity/capability checks, registry synchronization, lane layout assignment, and heartbeat publication.

### 15.4 NodeHello

A NodeHello is the federation message sent by a candidate node containing identity, DBP capabilities, registry hashes, resource self-model references, and initial node witness events.

### 15.5 NodeAccept

A NodeAccept is the coordinator's policy-gated admission response assigning runtime mode, task permissions, semantic descriptor, registry hashes, lane layout, and heartbeat rules.

### 15.6 DBP cluster full-duplex profile

The DBP cluster full-duplex profile is the inter-node DBP v2 profile that defines coordinator-to-worker downlink lanes, worker-to-coordinator uplink lanes, S2 requirements, lane layout, and replay binding.

### 15.7 Task queue witness

A task queue witness is a canonical witness describing pending, running, blocked, and resource-reserved work for a node.

### 15.8 Cluster decision context

A cluster decision context is a decision context that includes available node self-models, resource witnesses, task queue witnesses, cluster policy, and task requests.

### 15.9 Resource lease

A resource lease is a policy-scoped reservation of CPU, RAM, GPU, container slots, or bandwidth for a delegated task.

### 15.10 Cluster learning mode

Cluster learning mode constrains distributed profile-learning tasks across all nodes, independent of any local planner request.

---

## 16. v1.5 Draft 2 WG-RNN terms

### 16.1 Witness-Gated Recurrent Cell

A Witness-Gated Recurrent Cell is a Duotronic runtime component that uses canonical witness features and policy clamps to control persistent memory writes, decay, quarantine, and promotion.

### 16.2 Witness feature vector

A witness feature vector is a structured Duotronic object derived from canonical witness facts or policy-approved audit candidates. It includes confidence, contradiction, novelty, recurrence, source integrity, replayability, policy booleans, staleness, invalidation, human-review, and risk features.

### 16.3 Persistent witness memory

Persistent witness memory is a bank of structured memory slots whose updates are replayable, policy-scoped, and witness-derived.

### 16.4 Memory slot

A memory slot is a persistent WG-RNN memory object with content vector, stability, contradiction, provenance, replay identity, trust status, and purge state.

### 16.5 Memory update record

A memory update record is the replayable record of a WG-RNN persistent memory update, including input refs, gate values before and after clamps, authority, affected slots, and policy decision.

### 16.6 Slot promotion

Slot promotion is the policy-approved movement of a candidate memory slot to stable memory after replay, retention, contradiction, purge, and policy checks.
