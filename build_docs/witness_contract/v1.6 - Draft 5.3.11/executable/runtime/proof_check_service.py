#!/usr/bin/env python3
"""Authenticated, cache-verifying proof-check boundary for Draft 5.3.11."""
...
class ProofCheckApplication:
    ...
    def _cached_result(
        ...
        stored_verification_key = self.cache_verification_keys_by_id.get(stored_key_id)
        if stored_verification_key is not None and not verify_record(envelope, stored_verification_key):
        ...
        if envelope.get("cache_registry_sha256") != self.cache_registry_sha256:
            self.cache_verification_evidence_sink({
                "schema_version": "cache_stale_row_evidence/v1",
                "cache_envelope_id": envelope.get("cache_envelope_id"),
                "stored_cache_registry_sha256": envelope.get("cache_registry_sha256"),
                "current_cache_registry_sha256": self.cache_registry_sha256,
                "decision": "reject_preserve_row_require_new_idempotency_key",
                "reason": "cache_signing_registry_replaced_or_rotated",
                "evaluated_at": self.cache_clock().astimezone(timezone.utc).isoformat(),
            })
            raise AuthorityFailure(
                "cache_key_rotation_requires_new_idempotency_key",
                "cached result uses a superseded cache-signing registry; submit a new idempotency key",
            )
        if stored_verification_key is None:
            raise AuthorityFailure("cache_integrity_invalid", "idempotency-cache signer is absent from the current verification registry")
        ...
        if not isinstance(signing_evidence, dict) or any(
            signing_evidence.get(field) != value for field, value in {
                ...
                "principal_id": self.cache_signer_principal_id,
                "status": "active",
                "valid_from": self.cache_key_record["valid_from"],
                "valid_until": self.cache_key_record.get("valid_until"),
                "status_changed_at": self.cache_key_record["status_changed_at"],
                "rotation_predecessor_key_id": self.cache_key_record.get("rotation_predecessor_key_id"),
                "use": "signing",
                "decision": "active_time_valid_rotation_valid",
            }.items()
        ):
