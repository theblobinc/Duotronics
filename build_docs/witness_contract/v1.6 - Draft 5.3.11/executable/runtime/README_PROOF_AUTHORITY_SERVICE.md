...
contain an Ed25519-signed `idempotency_cache_envelope/v3`. Every hit:
...
as untrusted evidence and replay returns the stable
`cache_key_rotation_requires_new_idempotency_key` conflict. The caller must
use a new idempotency key. The service does not delete or transparently
re-execute the stale row.
...
merged without overwriting another interpreter's record. This package
validates Python 3.12.13 and Python 3.13.5, including development
warnings-as-errors execution.
