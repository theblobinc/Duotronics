...
VALUES ('v1.6-draft-5.3.6', '2026-07-31T00:00:00-07:00', 'v1.6-draft-5.3.5', NULL);

-- Draft 5.3.6 records the complete sealed execution and recursive-inspection closure.
CREATE TABLE wc_lean_compiler_witnesses_v5 (
  lean_compiler_witness_id TEXT PRIMARY KEY,
  schema_version TEXT NOT NULL CHECK (schema_version = 'lean_compiler_witness/v5'),
  ...
  exact_executed_argv_sha256 TEXT NOT NULL CHECK (length(exact_executed_argv_sha256) = 64),
  effective_resource_limits_sha256 TEXT NOT NULL CHECK (length(effective_resource_limits_sha256) = 64),
  compile_handoff_manifest_sha256 TEXT NOT NULL CHECK (length(compile_handoff_manifest_sha256) = 64),
  trust_root_attestation_registry_sha256 TEXT NOT NULL CHECK (length(trust_root_attestation_registry_sha256) = 64),
...
CREATE TRIGGER wc_compiler_witness_v5_insert_guard BEFORE INSERT ON wc_lean_compiler_witnesses_v5 BEGIN
  SELECT CASE WHEN wc_is_canonical_json(NEW.signed_payload_canonical_json) <> 1
    ...
    OR EXISTS (SELECT value FROM json_each(NEW.accepted_controls_json) EXCEPT SELECT value FROM json_each(NEW.measured_controls_json))
    ...
    OR json_extract(NEW.signed_payload_canonical_json, '$.exact_executed_argv_sha256') <> NEW.exact_executed_argv_sha256
    OR json_extract(NEW.signed_payload_canonical_json, '$.effective_resource_limits_sha256') <> NEW.effective_resource_limits_sha256
    OR json_extract(NEW.signed_payload_canonical_json, '$.compile_handoff_manifest_sha256') <> NEW.compile_handoff_manifest_sha256
    OR json_extract(NEW.signed_payload_canonical_json, '$.trust_root_attestation_registry_sha256') <> NEW.trust_root_attestation_registry_sha256
    THEN RAISE(ABORT, 'Draft 5.3.6 compiler witness lacks execution, resource, handoff, or recursive-inspection closure') END;
END;

CREATE TABLE wc_theorem_promotion_gates_v5 (
  ...
  policy_decision_sha256 TEXT NOT NULL CHECK (length(policy_decision_sha256) = 64),
  created_at TEXT NOT NULL CHECK (julianday(created_at) IS NOT NULL),
  FOREIGN KEY (compiler_witness_id) REFERENCES wc_lean_compiler_witnesses_v5(lean_compiler_witness_id),
  FOREIGN KEY (authority_snapshot_id) REFERENCES wc_authority_snapshots_v2(snapshot_id),
  FOREIGN KEY (approval_event_id) REFERENCES wc_authority_events_v1(event_id),
  FOREIGN KEY (policy_decision_id) REFERENCES wc_proof_policy_decisions_v1(policy_decision_id)
);

CREATE TRIGGER wc_theorem_gate_v5_insert_guard BEFORE INSERT ON wc_theorem_promotion_gates_v5 BEGIN
  SELECT CASE WHEN NOT EXISTS (
    SELECT 1 FROM wc_lean_compiler_witnesses_v5 c
    ...
    WHERE c.lean_compiler_witness_id = NEW.compiler_witness_id AND c.result = 'passed'
      ...
      AND c.policy_decision_sha256 = NEW.policy_decision_sha256
  ...
  ) THEN RAISE(ABORT, 'theorem gate lacks the exact Draft 5.3.6 authority closure') END;
  SELECT CASE WHEN NOT EXISTS (
    SELECT 1 FROM wc_release_activation_evidence_v1
    WHERE package_version = 'v1.6-draft-5.3.6'
  ) THEN RAISE(ABORT, 'theorem authority remains disabled until every 5.3.6 activation gate passes') END;
END;

CREATE VIEW wc_authoritative_theorems_v5 AS
SELECT g.* FROM wc_theorem_promotion_gates_v5 g
JOIN wc_release_activation_evidence_v1 a ON a.package_version = 'v1.6-draft-5.3.6';

CREATE TRIGGER wc_compiler_witness_v5_no_update BEFORE UPDATE ON wc_lean_compiler_witnesses_v5 BEGIN SELECT RAISE(ABORT, 'append-only'); END;
CREATE TRIGGER wc_compiler_witness_v5_no_delete BEFORE DELETE ON wc_lean_compiler_witnesses_v5 BEGIN SELECT RAISE(ABORT, 'append-only'); END;
CREATE TRIGGER wc_theorem_gate_v5_no_update BEFORE UPDATE ON wc_theorem_promotion_gates_v5 BEGIN SELECT RAISE(ABORT, 'append-only'); END;
CREATE TRIGGER wc_theorem_gate_v5_no_delete BEFORE DELETE ON wc_theorem_promotion_gates_v5 BEGIN SELECT RAISE(ABORT, 'append-only'); END;
