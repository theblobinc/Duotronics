...
def main() -> int:
    ...
    for _ in range(max(1, len(pending))):
        ...
        for source in list(pending):
            ...
            if process.returncode == 0:
                ...
                compiled.append({"module": relative.with_suffix("").as_posix().replace("/", "."), "source_sha256": digest(source), "olean_path": target.relative_to(HANDOFF).as_posix(), "olean_sha256": digest(target)})
            else:
    ...
    if total_handoff_bytes > args.handoff_limit:
        return 75
    manifest = {
        "schema_version": "lean_compile_handoff/v2",
