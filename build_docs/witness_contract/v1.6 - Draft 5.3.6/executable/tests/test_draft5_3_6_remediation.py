...
class ApiConformanceTests(unittest.TestCase):
    ...
    def test_openapi_response_matches_running_wsgi_adapter_and_idempotency(self):
        ...
        witness["schema_version"] = "lean_compiler_witness/v5"
        witness.update({
            ...
            "accepted_controls":["lsm_profile"], "measured_controls":["lsm_profile"],
            "derived_controls":["result_channel_isolated"], "control_evidence":[],
        })
        witness.pop("applied_controls"); witness.pop("verified_controls")
        ...
        result_schema = json.loads((ROOT / "schemas/proof_check_result_v4.schema.json").read_text(encoding="utf-8"))
        self.assertTrue(set(result_schema["required"]).issubset(result))
        witness_schema = json.loads((ROOT / "schemas/lean_compiler_witness_v5.schema.json").read_text(encoding="utf-8"))
