#!/usr/bin/env python3
"""Fail-closed Lean proof authority for Witness Contract Draft 5.3.6.
...
ACTIVE_SCHEMA_FILES = {
    ...
    "sandbox_invocation": "effective_sandbox_invocation_v3.schema.json",
    "verifier_result": "lean_verifier_result_v4.schema.json",
    "compiler_witness": "lean_compiler_witness_v5.schema.json",
    "proof_check_request": "proof_check_request_v1.schema.json",
    "proof_check_result": "proof_check_result_v4.schema.json",
...
class EffectiveSandboxInvocation:
    ...
    def executed_argv(self) -> tuple[str, ...]:
        fsize = self.compiler_artifact_file_size_limit if self.domain == "untrusted_compilation" else self.inspection_output_bytes_limit
        command = [
...
class OciSandboxRunner:
    ...
    def _invocation(
        ...
        emitted = tuple(sorted(self.REQUIRED_CONTROLS))
        ...
        return EffectiveSandboxInvocation(
            ...
            requested_controls=emitted, emitted_controls=emitted,
            accepted_controls=emitted, applied_controls=emitted,
    ...
    def run(self, *, profile: CompilerProfile, snapshot_root: Path, generated_module: Path, request: dict[str, Any], timeout_seconds: int) -> SandboxExecution:
        ...
        with tempfile.TemporaryDirectory(prefix="wc-handoff-") as handoff_name, tempfile.TemporaryDirectory(prefix="wc-control-") as control_name, tempfile.TemporaryDirectory(prefix="wc-inspection-parent-") as inspection_parent_name, tempfile.TemporaryDirectory(prefix="wc-result-parent-") as result_parent_name:
            ...
            verifier_manifest = (
                {"source": f"snapshot:{request['immutable_snapshot_tree_sha256']}", "host_path": str(snapshot_root), "destination": "/input/source", "mode": "ro", "domain": "trusted_verifier", "purpose": "sealed_source", "lifecycle": "request"},
                {"source": f"generated:{request['generated_binding_module_sha256']}", "host_path": str(generated_root), "destination": "/input/generated", "mode": "ro", "domain": "trusted_verifier", "purpose": "generated_type_binding", "lifecycle": "request"},
                {"source": "ephemeral:compile-handoff", "host_path": str(handoff), "destination": "/handoff", "mode": "ro", "domain": "trusted_verifier", "purpose": "compiled_artifact_input", "lifecycle": "request"},
                {"source": f"control:{request['request_id']}", "host_path": str(control), "destination": "/input/control", "mode": "ro", "domain": "trusted_verifier", "purpose": "trusted_verifier_request", "lifecycle": "request"},
                {"source": "ephemeral:trusted-inspection", "host_path": str(inspection_directory), "destination": "/trusted-inspection", "mode": "rw", "domain": "trusted_verifier", "purpose": "private_inspection_output", "lifecycle": "request"},
            )
            verifier_invocation = self._invocation(domain="trusted_verifier", profile=profile, entrypoint="/opt/witness-authority/bin/verify-lean", arguments=("--request", "/input/control/request.json", "--result-dir", "/trusted-inspection"), mounts=verifier_manifest, limits=limits, uid=self.authority_uid, gid=self.authority_gid)
            ...
            if inspection is not None:
                candidate = sign_record(inspection, self.verifier_result_signing_key)
            ...
            derived_controls = ("result_channel_isolated", "trusted_inspection_only") if structured else ()
            ...
            return SandboxExecution(
                ...
                requested_controls=verifier_invocation.requested_controls,
                applied_controls=verifier_invocation.applied_controls,
                verified_controls=verified_controls, output_limit_exceeded=False,
...
class ProofAuthorityService:
    ...
    def verify(
        ...
        emitted = set(execution.emitted_controls or execution.applied_controls)
        accepted = set(execution.accepted_controls or execution.applied_controls)
        measured = set(execution.measured_controls or execution.verified_controls)
        exact_controls = (
        ...
        semantic_result = {
            ...
            for key in (
                ...
                "requested_controls", "emitted_controls", "accepted_controls", "measured_controls", "derived_controls",
        ...
        unsigned = {
            "schema_version": "lean_compiler_witness/v5", "lean_compiler_witness_id": "lean:" + sha256_bytes(canonical_bytes(semantic))[:32],
            ...
            "emitted_controls": sorted(execution.emitted_controls or execution.applied_controls),
            "accepted_controls": sorted(execution.accepted_controls or execution.applied_controls),
            "measured_controls": sorted(execution.measured_controls or execution.verified_controls),
    ...
    def _structured_result_valid(self, result: dict[str, Any] | None, request: dict[str, Any], request_hash: str, profile: CompilerProfile, execution: SandboxExecution) -> tuple[bool, str | None]:
        if not isinstance(result, dict) or result.get("schema_version") != "wc_lean_verifier_result/v4":
        ...
        for field in ("expected_type_expression_hash", "actual_type_expression_hash", "transitive_dependencies_root", "axiom_set_sha256", "normalized_executed_argv_sha256", "handoff_manifest_sha256"):
            if not SHA256_HEX.fullmatch(str(result.get(field, ""))):
                return False, "verifier_result_invalid"
        for field in ("direct_dependencies", "dependency_closure", "axiom_set", "forbidden_axiom_set", "requested_controls", "emitted_controls", "accepted_controls", "measured_controls", "derived_controls"):
        ...
        if result["axiom_set_sha256"] != sha256_bytes(canonical_bytes(result["axiom_set"])):
            return False, "verifier_result_invalid"
        if result["requested_controls"] != sorted(execution.requested_controls) or result["emitted_controls"] != sorted(execution.emitted_controls or execution.applied_controls):
            return False, "verifier_result_invalid"
        if result["accepted_controls"] != sorted(execution.accepted_controls or execution.applied_controls):
            return False, "verifier_result_invalid"
        if result["measured_controls"] != sorted(execution.measured_controls or execution.verified_controls):
            return False, "verifier_result_invalid"
