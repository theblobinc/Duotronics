Draft 5.2.1 additive update: Lean proof authority integration is included. The root now contains a Lake package, proof witness schemas, a check_proof syscall, SQL/OpenAPI persistence, and a theorem promotion gate.
...
16. `executable/validators/validate_draft5_2_corpus.py`
...
Draft 5.2 now includes a TLA+-only formal execution surface. The integration is intentionally scoped to TLA+ and does not add a Lean compiler or Lake package. Use `python executable/formal/run_tla_model_check.py --mode advisory` for portable checks, or `--mode strict` when `TLA2TOOLS_JAR` or `tools/tla2tools.jar` is available. See `DRAFT5_2_TLA_PLUS_IMPLEMENTATION_REPORT_v1_0.md`.
