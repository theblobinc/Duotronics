...
def check_sql(errors):
    try:
        ...
        # Exact JSON membership guard: arrays may not merely be non-empty; they must contain the gate's exact proof/Lean IDs.
        try:
            con.execute('BEGIN')
            con.execute("INSERT INTO srnn_lean_compiler_witnesses VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", ('lean:valid','lean_compiler_witness/v1','leanprover/lean4:v4.29.1','a'*64,'b'*64,'["lake","build"]','passed',0,0,0,'["Duotronic"]','[{"theorem_name":"t","status":"proved"}]',None,None,'2026-05-11T00:00:00Z'))
            con.execute("INSERT INTO srnn_proof_witnesses VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)", ('proof:valid','proof_witness/v1','claim:mismatch','theorem_promotion_requires_valid_lean_proof','Duotronic/ProofAuthority.lean','lean:valid','passed','proved',1,1,1,1,'policy:allow','2026-05-11T00:00:00Z'))
            con.execute("INSERT INTO srnn_non_collapse_transitions VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", ('nc:mismatch','non_collapse_transition/v1','state:a','state:b','conjectural','theorem','epistemic_state','epistemic_state','proof_upgrade','transition_witnessed','allowed','external:proof','proof:valid','policy:allow',1,'proof upgrade witnessed','2026-05-11T00:00:00Z'))
            con.execute("""INSERT INTO srnn_claim_status_transitions (
                claim_status_transition_id, schema_version, claim_id, source_status, target_status,
                transition_kind, allowed, policy_decision_id, required_witness_refs_json,
                proof_witness_refs_json, lean_compiler_witness_refs_json, theorem_promotion_gate_id,
                ...
                'transition:mismatch','claim_status_transition/v1','claim:mismatch','conjecture','theorem',
                'prove',1,'policy:allow','["proof:other","lean:other","gate:mismatch"]','["proof:other"]','["lean:other"]','gate:mismatch',None,'nc:mismatch','mismatched JSON arrays must fail at gate','2026-05-11T00:00:00Z'
            ))
            con.execute("""INSERT INTO srnn_evidence_claims (
                ...
                'claim:mismatch','evidence_claim/v1','proof_claim','subject:t','is_theorem','{}','theorem','theorem','assert','mathematical_claim','production','["evidence:base"]','policy:allow','pragmatic:1',None,'ncstate:1','["proof:other"]','["lean:other"]','gate:mismatch','transition:mismatch',None,'principal:formal',1.0,'{}','2026-05-11T00:00:00Z'
            ))
            con.execute("INSERT INTO srnn_theorem_promotion_gates VALUES (?,?,?,?,?,?,?,?,?,?,?,?)", ('gate:mismatch','theorem_promotion_gate/v1','claim:mismatch','transition:mismatch','proof:valid','lean:valid','nc:mismatch','policy:allow',1,1,None,'2026-05-11T00:00:00Z'))
            con.commit()
            errors.append('sql guard: mismatched claim/transition proof/Lean JSON arrays committed against theorem gate')
...
def main():
    ...
    if errors:
        print('\n'.join(errors))
        return 1
    print('Draft 5.2.2 corpus validation checks passed.')
    return 0
