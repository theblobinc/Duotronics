#!/usr/bin/env python3
"""Authenticated, cache-verifying proof-check boundary for Draft 5.3.8."""
...
class DurableIdempotencyStore:
    ...
    def _prepare_secure_path(self) -> None:
        ...
        if not stat.S_ISDIR(info.st_mode) or stat.S_ISLNK(info.st_mode):
            raise RuntimeError("idempotency parent must be a real directory")
    ...
    def _secure_database_files(self) -> None:
        for path in (self.path, Path(str(self.path) + "-wal"), Path(str(self.path) + "-shm")):
            ...
            if stat.S_ISLNK(info.st_mode) or not stat.S_ISREG(info.st_mode) or info.st_nlink != 1:
                raise RuntimeError(f"unsafe SQLite file identity: {path.name}")
            if info.st_uid != self.expected_uid:
                raise RuntimeError(f"unsafe SQLite file owner: {path.name}")
...
class ProofCheckApplication:
    def __init__(
        ...
        self.cache_signing_key = cache_signing_key or getattr(authority, "signing_key", None)
        if self.cache_signing_key is None:
            raise ValueError("a protected idempotency-cache signing key is required")
    ...
    def _cache_envelope(self, result: dict[str, Any], request: dict[str, Any], principal_id: str, request_sha256: str) -> dict[str, Any]:
        ...
        unsigned = {
            ...
            "source_bundle_id": request["source_bundle_id"],
            "cache_signer_principal_id": self.authority_principal_id,
            "cache_signer_key_id": self.authority_key_id,
            "result": result,
        }
