# Duotronic Witness Contract v10.10

**Status:** Research specification draft  
**Version:** 10.10-standalone-revision  
...
## Revision 10.10 runtime-profile addition note

> **Status tag:** reference
> **Related tags:** research

The v10.10 revision keeps the Witness Contract focused on runtime trust, normal-form construction, transport-before-semantics, recurrence, lookup memory, replay, policy, and safe promotion. It adds two optional runtime/research profiles:
...
These profiles are not new hard rules. They are bounded ways to make L2/L2M recurrence more concrete. The governing path remains:

```text
raw evidence -> validation -> canonicalization -> policy gate -> optional recurrent state update
```

A detected recurrence jump is a candidate event, not authority. It must still pass validation, canonicalization, and policy gating before it may affect authoritative state.

## Prior v10.8 expansion summary

> **Status tag:** reference

The v10.10 revision keeps all v10.4 material and expands the Witness Contract into a fuller internal runtime source specification. The main additions are a source-architecture view from the witness side, a threat model, a witness lifecycle chapter, a failure-code registry, a replay identity profile, a telemetry catalog, policy-shield decision tables, an expanded conformance harness, migration/versioning rules, and a full end-to-end worked trace.

The v10.10 additions do not weaken any hard rule. They make the hard rules more operational:
...
The Duotronic Witness Contract v10.10 defines the runtime discipline for systems that operate on witness-bearing state.
...
The v10.10 revision keeps the strong v9/v10 idea that stable witness facts must be in canonical normal form before they are trusted, stored, retrieved, compared, replayed, or promoted.
...
The Duotronic Witness Contract v10.10 is a trust contract.
