# Duotronic Polygon Family Calculus (DPFC v5.10)

**Status:** Research specification draft  
**Version:** 5.10-standalone-revision  
...
## Revision 5.10 runtime-profile boundary note

> **Status tag:** reference
> **Related tags:** research

The v5.10 revision keeps DPFC focused on arithmetic, family-native values, bridges, canonical serialization, successor, conversion, and export policy. It adds only concise boundary language for two optional runtime/research profiles:
...
Neither profile redefines DPFC arithmetic. The governing boundary is:

```text
external/runtime pattern -> adapter declaration -> validation -> canonicalization -> optional runtime use
```

A runtime profile may consume canonical DPFC features. It may not mutate successor, addition, multiplication, inter-family conversion, canonical identity, or export correction.

## Prior v5.8 expansion summary

> **Status tag:** reference

The v5.10 revision keeps all v5.4 material and expands the document toward an internal source specification. The main additions are source architecture scaffolding, an absence-zero-origin rationale, a full ontology map, related-concept comparisons, a formal theorem registry, a normalizer profile language, a deeper geometry/group-action chapter, expanded conformance-harness guidance, migration/versioning rules, and a corpus-map appendix.

The v5.10 additions do not mutate the DPFC arithmetic core. They make the core easier to review, implement, falsify, and extend. In particular, the new material reinforces four boundaries:
...
DPFC v5.10 defines the Duotronic Polygon Family Calculus as a layered mathematical and implementation specification. It preserves the presence-first Duotronic stance:
...
The v5.10 revision additionally polishes the source-spec layer: single-primary status tags replace hybrid tags, revision summaries separate core refinements from revision-specific additions, and corpus-map entries now declare whether they are existing, draft-needed, or future work.

DPFC v5.10 should be read as a family-parametric representational layer over a shared positive core. It can be implemented, tested, serialized, converted, transported, and replayed. It is not a completed physical theory, and it does not claim that polygon diagrams, QCD symmetry diagrams, or virtual-particle language prove Duotronic arithmetic.
...
DPFC v5.10 does not claim that:
...
This section embeds the minimum adapter vocabulary needed to read DPFC v5.10 without relying on external source documents.
...
A DPFC v5.10 implementation conforms at the core layer if it implements:
...
DPFC v5.10 is a disciplined, one-based, family-native calculus for realized magnitudes, scalar roles, witness geometry, canonical identity, and inter-family conversion.
...
The following assumptions are active for C2 theorem candidates in the DPFC v5.10 core.
...
if __name__ == "__main__":
    ...
    print("DPFC v5.10 reference self-test passed")
```
...
DPFC v5.10 treats these separations as architectural invariants:
