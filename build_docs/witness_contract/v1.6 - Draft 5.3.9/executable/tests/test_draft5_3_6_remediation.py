...
class ApiConformanceTests(unittest.TestCase):
    ...
    def test_openapi_response_matches_running_wsgi_adapter_and_idempotency(self):
        ...
        witness["schema_version"] = "lean_compiler_witness/v7"
        witness.update({
        ...
        result_schema = json.loads((ROOT / "schemas/proof_check_result_v6.schema.json").read_text(encoding="utf-8"))
        self.assertTrue(set(result_schema["required"]).issubset(result))
        witness_schema = json.loads((ROOT / "schemas/lean_compiler_witness_v7.schema.json").read_text(encoding="utf-8"))
