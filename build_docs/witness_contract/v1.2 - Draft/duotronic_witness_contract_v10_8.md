# Duotronic Witness Contract v10.8

**Status:** Research specification draft  
**Version:** 10.8-standalone-revision  
...
The v10.8 revision keeps all v10.4 material and expands the Witness Contract into a fuller internal runtime source specification. The main additions are a source-architecture view from the witness side, a threat model, a witness lifecycle chapter, a failure-code registry, a replay identity profile, a telemetry catalog, policy-shield decision tables, an expanded conformance harness, migration/versioning rules, and a full end-to-end worked trace.

The v10.8 additions do not weaken any hard rule. They make the hard rules more operational:
...
The Duotronic Witness Contract v10.8 defines the runtime discipline for systems that operate on witness-bearing state.
...
The v10.8 revision keeps the strong v9/v10 idea that stable witness facts must be in canonical normal form before they are trusted, stored, retrieved, compared, replayed, or promoted.
...
```json
{
  ...
  "source_profile": "witness-key@v10.8",
  "target_profile": "witness-key@v10.8",
...
```json
{
  "schema_version": "witness-key@v10.8",
  "family_id": "hex6",
...
The Duotronic Witness Contract v10.8 is a trust contract.
...
```yaml
policy_id: witness-policy@v10.8-minsafe
mode_defaults:
...
```json
{
  ...
  "target_schema": "witness-key@v10.8",
  "affected_components": [
    "key_schema",
    "family_registry",
    "normalizer",
...
# Appendix F - release checklist for v10.8 implementation

> **Status tag:** reference

Before a v10.8 implementation is called conforming, verify:
...
```yaml
fixture_pack: witness-v10.8-runtime-fixtures
...
def run_reference_self_test() -> None:
    assert canonicalize_witness_key_bundle({"family_id": "hex6", "family_schema_version": "dpfc-family@v5.8", "digits": ["h1", "h4"]})["core_magnitude"] == "mu_10"
...
if __name__ == "__main__":
    ...
    print("Witness Contract v10.8 reference self-test passed")
```
...
```yaml
fixture_pack: witness-v10.8-spectral-edo-fixtures
...
A v10.8 implementation should not pass CI unless:
...
```yaml
trace_id: witness-trace-v10.8-example-1
transport:
  kind: dbp_frame
  shape_valid: true
