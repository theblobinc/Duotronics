...
from .crypto_primitives import shake256_duoid

from .evidence import CorpusRef, EvidenceKernel, canonical_json, shake256_ref
...
def file_digest(path: Path) -> str:
    ...
    with path.open("rb") as f:
        for chunk in iter(lambda: f.read(1024 * 1024), b""):
            h.update(chunk)
    encoded = base64.urlsafe_b64encode(h.digest(64)).decode("ascii").rstrip("=")
...
class CorpusManager:
    ...
    def __init__(self, corpus_dir: Path, observer_id: str = "srnn-corpus-manager", store: Any | None = None) -> None:
        ...
        # absolute path -> (size, mtime_ns, SHAKE256-512 digest)
    ...
    def _manifest_path(self) -> Path | None:
        for name in CORPUS_MANIFEST_NAMES:
            ...
            if p.exists():
                return p
        canonical = sorted(
            self.corpus_dir.glob("MANIFEST_v1_6_draft_*.json"),
    ...
    def inspect(self, *, force: bool = False) -> dict[str, Any]:
        ...
        for path in sorted(self.corpus_dir.rglob("*")):
            ...
            if stat.st_size <= 5_000_000:
                ...
                digest = "duoid:shake256-512:skipped-large-file"
        ...
        digest = str(manifest.get("corpus_root_id") or shake256_duoid({"manifest": manifest, "files": files}))
        version = str(
