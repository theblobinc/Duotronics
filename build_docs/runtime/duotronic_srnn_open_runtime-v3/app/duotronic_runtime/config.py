...
def active_corpus_dir() -> Path:
    """Resolve an MCP-selected published corpus inside the read-only history mount."""
    fallback = Path(os.environ.get("CORPUS_DIR", "/runtime/corpus"))
    state_path = Path(os.environ.get("ACTIVE_WITNESS_CONTRACT_STATE", "/runtime/config/active_witness_contract.json"))
    history = Path(os.environ.get("CORPUS_HISTORY_DIR", "/runtime/corpus-history"))
    if not state_path.is_file():
        return fallback
    try:
        state = json.loads(state_path.read_text(encoding="utf-8"))
        directory = str(state.get("directory_name") or "")
        if not re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9._ -]{0,119}", directory) or ".." in directory:
            return fallback
        history_root = history.resolve(strict=True)
        candidate = (history / directory).resolve(strict=True)
        if candidate.parent != history_root or not candidate.is_dir():
            return fallback
        return candidate
    except Exception:
        return fallback
...
class Settings:
    ...
    corpus_dir: Path = active_corpus_dir()
    corpus_history_dir: Path = Path(os.environ.get("CORPUS_HISTORY_DIR", "/runtime/corpus-history"))
    active_witness_contract_state: Path = Path(os.environ.get("ACTIVE_WITNESS_CONTRACT_STATE", "/runtime/config/active_witness_contract.json"))
