...
LEGACY_HASH_NAME = "sha" + "256"
LEGACY_HASH_LABEL = "SHA" + "-" + "256"

ACTIVE_PATTERNS = [
    ("legacy-hash-call", re.compile(r"\bhashlib\." + LEGACY_HASH_NAME + r"\s*\(", re.I)),
    ("legacy-hash-constructor", re.compile(r"\bhashlib\.new\s*\(\s*[\'\"]" + LEGACY_HASH_NAME + r"[\'\"]", re.I)),
    ("legacy-hmac", re.compile(r"\bhmac\.(?:new|digest)\s*\([^\n]*" + LEGACY_HASH_NAME, re.I)),
    ("ed25519", re.compile(r"\bed25519\b", re.I)),
]
LEGACY_IDENTITY_PATTERNS = [
    ("legacy-hash-identifier", re.compile(r"[\'\"]" + LEGACY_HASH_NAME + r":", re.I)),
    ("legacy-hash-field", re.compile(r"[\'\"][A-Za-z0-9_]*" + LEGACY_HASH_NAME + r"[A-Za-z0-9_]*[\'\"]\s*:", re.I)),
]
ALLOWED_INTEROP = [
...
def scan(roots: list[Path]) -> dict[str, Any]:
    ...
    return {
        ...
        "rules": {
            ...
            "legacy-SHA2-256": "forbidden-active-and-identity",
            "external-legacy-SHA2-256": "untrusted-interoperability-only",
        },
    }
