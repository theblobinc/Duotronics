...
def v3_restart_runtime_only() -> dict[str, Any]:
    script = Path("/home/tbi/xavi-coordinated-restart-runtime.sh")
    return run_cmd(["/bin/bash", str(script), "Native Ops v3 runtime restart requested"], cwd=_v3_runtime_dir(), timeout=300)
...
ALLOWED_COMMANDS: dict[str, dict[str, Any]] = {
    ...
    "v3_restart_runtime_only": {
        "cmd": ["/bin/bash", "/home/tbi/xavi-coordinated-restart-runtime.sh", "Native Ops allowed-command runtime restart requested"],
        "cwd": Path("/var/www/xavi/Duotronics/build_docs/runtime/duotronic_srnn_open_runtime-v3"),
        "timeout": 300,
    },
    "v3_git_status": {
