# Duotronic Polygon Family Calculus (DPFC v5.6)

**Status:** Research specification draft  
**Version:** 5.6-standalone-revision  
...
The v5.6 revision keeps all v5.4 material and expands the document toward an internal source specification. The main additions are source architecture scaffolding, an absence-zero-origin rationale, a full ontology map, related-concept comparisons, a formal theorem registry, a normalizer profile language, a deeper geometry/group-action chapter, expanded conformance-harness guidance, migration/versioning rules, and a corpus-map appendix.

The v5.6 additions do not mutate the DPFC arithmetic core. They make the core easier to review, implement, falsify, and extend. In particular, the new material reinforces four boundaries:
...
DPFC v5.6 defines the Duotronic Polygon Family Calculus as a layered mathematical and implementation specification. It preserves the presence-first Duotronic stance:
...
The v5.6 revision additionally polishes the source-spec layer: single-primary status tags replace hybrid tags, revision summaries separate core refinements from revision-specific additions, and corpus-map entries now declare whether they are existing, draft-needed, or future work.

DPFC v5.6 should be read as a family-parametric representational layer over a shared positive core. It can be implemented, tested, serialized, converted, transported, and replayed. It is not a completed physical theory, and it does not claim that polygon diagrams, QCD symmetry diagrams, or virtual-particle language prove Duotronic arithmetic.
...
A future DPFC-Geometric-Successor profile may allow geometric walks to define successor directly, but that is not part of the v5.6 core.
...
```text
family:hex6 schema_version:dpfc-family@v5.6 digits:1 4 6
```
...
This section embeds the minimum adapter vocabulary needed to read DPFC v5.6 without relying on external source documents.
...
A DPFC v5.6 implementation conforms at the core layer if it implements:
...
DPFC v5.6 is a disciplined, one-based, family-native calculus for realized magnitudes, scalar roles, witness geometry, canonical identity, and inter-family conversion.
...
The following assumptions are active for C2 theorem candidates in the DPFC v5.6 core.
...
```yaml
fixture_pack: dpfc-v5.6-core-fixtures
...
fixtures:
  ...
  - fixture_id: DPFC-FIXTURE-4-REFL3-ENCODE-10
    ...
    expected:
      word: [r3, r1]
      positional_value: 10
      canonical_storage: "family:refl3 schema_version:dpfc-family@v5.6 digits:3 1"
...
```yaml
fixture_pack: dpfc-v5.6-edo31-fixtures
...
DPFC v5.6 treats these separations as architectural invariants:
...
output_schema: dpfc-family@v5.6
