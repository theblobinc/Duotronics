...
Production audit state is no longer inferred from the mutable JSONL segment alone. Each provisioned segment has a separately stored, canonical, signed checkpoint binding the segment identifier, expected sequence, expected tail-record SHAKE256-512, predecessor sealed-segment tail, signer key, creation time, and segment status.
...
`cache_stale_row_evidence/v4` binds the exact verified envelope through its signed-payload SHAKE256-512, canonical full-envelope SHAKE256-512, and original envelope signature in addition to all request, principal, slot, policy, source, witness, and chronology bindings.
