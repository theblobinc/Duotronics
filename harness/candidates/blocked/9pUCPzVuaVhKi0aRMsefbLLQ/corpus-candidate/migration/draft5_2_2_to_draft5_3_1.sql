...
CREATE TABLE IF NOT EXISTS wc_claims_v2 (
  ...
  claim_content_shake256_512 TEXT NOT NULL CHECK (length(claim_content_shake256_512) = 128 AND claim_content_shake256_512 NOT GLOB '*[^0-9a-f]*'),
  theorem_statement TEXT,
  theorem_statement_shake256_512 TEXT,
  issuer_principal_id TEXT NOT NULL,
  created_at TEXT NOT NULL,
  CHECK (
    (claim_kind = 'proof_claim' AND theorem_statement IS NOT NULL AND length(theorem_statement) > 0 AND length(theorem_statement_shake256_512) = 128 AND theorem_statement_shake256_512 NOT GLOB '*[^0-9a-f]*')
    OR
    (claim_kind <> 'proof_claim' AND theorem_statement IS NULL AND theorem_statement_shake256_512 IS NULL)
  )
);
...
CREATE TABLE IF NOT EXISTS wc_verifier_principals_v2 (
  ...
  signature_algorithm TEXT NOT NULL CHECK (signature_algorithm IN ('ML-DSA-87','ECDSA-P256-SHAKE256_512','RSA-PSS-SHAKE256_512')),
...
CREATE TABLE IF NOT EXISTS wc_lean_compiler_witnesses_v2 (
  ...
  claim_id TEXT NOT NULL,
  claim_content_shake256_512 TEXT NOT NULL CHECK (length(claim_content_shake256_512) = 128 AND claim_content_shake256_512 NOT GLOB '*[^0-9a-f]*'),
  theorem_statement_shake256_512 TEXT NOT NULL CHECK (length(theorem_statement_shake256_512) = 128 AND theorem_statement_shake256_512 NOT GLOB '*[^0-9a-f]*'),
  proof_artifact_shake256_512 TEXT NOT NULL CHECK (length(proof_artifact_shake256_512) = 128 AND proof_artifact_shake256_512 NOT GLOB '*[^0-9a-f]*'),
  source_tree_shake256_512 TEXT NOT NULL CHECK (length(source_tree_shake256_512) = 128 AND source_tree_shake256_512 NOT GLOB '*[^0-9a-f]*'),
  lakefile_shake256_512 TEXT NOT NULL CHECK (length(lakefile_shake256_512) = 128 AND lakefile_shake256_512 NOT GLOB '*[^0-9a-f]*'),
  build_output_shake256_512 TEXT NOT NULL CHECK (length(build_output_shake256_512) = 128 AND build_output_shake256_512 NOT GLOB '*[^0-9a-f]*'),
...
CREATE TABLE IF NOT EXISTS wc_proof_witnesses_v2 (
  ...
  claim_id TEXT NOT NULL,
  claim_content_shake256_512 TEXT NOT NULL,
  theorem_statement_shake256_512 TEXT NOT NULL,
  proof_artifact_shake256_512 TEXT NOT NULL,
...
BEGIN
  SELECT CASE WHEN NOT EXISTS (
    ...
    WHERE l.lean_compiler_witness_id = NEW.lean_compiler_witness_id
      ...
      AND l.claim_content_shake256_512 = NEW.claim_content_shake256_512
      AND l.theorem_statement_shake256_512 = NEW.theorem_statement_shake256_512
      AND l.proof_artifact_shake256_512 = NEW.proof_artifact_shake256_512
...
SELECT g.theorem_promotion_gate_id, g.claim_id, g.claim_content_shake256_512,
       g.theorem_statement_shake256_512, g.proof_witness_id,
...
CREATE TABLE IF NOT EXISTS wc_replay_manifests_v2 (
  ...
  manifest_shake256_512 TEXT NOT NULL,
  created_at TEXT NOT NULL
);
...
CREATE TABLE IF NOT EXISTS wc_verification_grammars_v2 (
  ...
  grammar_shake256_512 TEXT NOT NULL,
  created_at TEXT NOT NULL
);
...
BEGIN
  SELECT CASE WHEN NOT EXISTS (
    ...
    WHERE g.grammar_id = NEW.grammar_id AND g.grammar_shake256_512 = NEW.grammar_shake256_512
  ) THEN RAISE(ABORT, 'verification result grammar hash mismatch') END;
  SELECT CASE WHEN NOT EXISTS (
    SELECT 1 FROM wc_replay_manifests_v2 m
    WHERE m.manifest_id = NEW.manifest_id AND m.manifest_shake256_512 = NEW.manifest_shake256_512
