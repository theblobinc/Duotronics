...
ALTER TABLE wc_lean_compiler_witnesses_v2 ADD COLUMN original_source_tree_shake256_512 TEXT;
ALTER TABLE wc_lean_compiler_witnesses_v2 ADD COLUMN immutable_snapshot_shake256_512 TEXT;
ALTER TABLE wc_lean_compiler_witnesses_v2 ADD COLUMN compiler_profile_id TEXT;
ALTER TABLE wc_lean_compiler_witnesses_v2 ADD COLUMN compiler_registry_shake256_512 TEXT;
ALTER TABLE wc_lean_compiler_witnesses_v2 ADD COLUMN lake_executable_shake256_512 TEXT;
ALTER TABLE wc_lean_compiler_witnesses_v2 ADD COLUMN lean_executable_shake256_512 TEXT;
ALTER TABLE wc_lean_compiler_witnesses_v2 ADD COLUMN lean_stdlib_tree_shake256_512 TEXT;
ALTER TABLE wc_lean_compiler_witnesses_v2 ADD COLUMN dependency_closure_shake256_512 TEXT;
ALTER TABLE wc_lean_compiler_witnesses_v2 ADD COLUMN execution_image_digest TEXT;
ALTER TABLE wc_lean_compiler_witnesses_v2 ADD COLUMN sandbox_policy_shake256_512 TEXT;
ALTER TABLE wc_lean_compiler_witnesses_v2 ADD COLUMN verifier_binary_shake256_512 TEXT;
ALTER TABLE wc_lean_compiler_witnesses_v2 ADD COLUMN structured_result_shake256_512 TEXT;
...
CREATE TABLE wc_governance_authorities_v1 (
  ...
  public_key_base64url TEXT NOT NULL CHECK (length(public_key_base64url) = 3456 AND public_key_base64url NOT GLOB '*[^A-Za-z0-9_-]*'),
  public_key_fingerprint_shake256_512 TEXT NOT NULL CHECK (length(public_key_fingerprint_shake256_512) = 128 AND public_key_fingerprint_shake256_512 NOT GLOB '*[^0-9a-f]*'),
...
BEGIN
  SELECT CASE WHEN wc_public_key_fingerprint(NEW.public_key_base64url) IS NULL
    OR wc_public_key_fingerprint(NEW.public_key_base64url) <> NEW.public_key_fingerprint_shake256_512
...
CREATE TABLE wc_governance_authorization_witnesses_v1 (
  ...
  signature TEXT NOT NULL CHECK (length(signature) = 6170 AND signature NOT GLOB '*[^A-Za-z0-9_-]*'),
  created_at TEXT NOT NULL CHECK (julianday(created_at) IS NOT NULL),
  CHECK (valid_until IS NULL OR julianday(valid_until) >= julianday(valid_from)),
...
CREATE TABLE wc_verifier_key_status_events_v2 (
  ...
  signed_payload_canonical_json TEXT NOT NULL CHECK (json_valid(signed_payload_canonical_json) = 1 AND json_type(signed_payload_canonical_json) = 'object'),
  signed_payload_shake256_512 TEXT NOT NULL,
...
BEGIN
  SELECT CASE WHEN wc_is_canonical_json(NEW.signed_payload_canonical_json) <> 1 OR wc_shake256_512(NEW.signed_payload_canonical_json) <> NEW.signed_payload_shake256_512
...
SELECT s.authority_snapshot_id, s.evaluated_at, k.key_id, k.verifier_principal_id,
       k.signature_algorithm, k.public_key_base64url, k.public_key_fingerprint_shake256_512,
...
SELECT k.key_id, k.verifier_principal_id, k.signature_algorithm, k.public_key_base64url,
       k.public_key_fingerprint_shake256_512, k.valid_from, k.valid_until
...
BEGIN
  ...
  SELECT CASE WHEN NOT EXISTS (
    ...
    WHERE l.lean_compiler_witness_id = NEW.record_id AND l.key_id = NEW.key_id
      AND l.verifier_principal_id = k.verifier_principal_id AND l.signed_payload_shake256_512 = NEW.signed_payload_shake256_512
      ...
      AND json_extract(NEW.signed_payload_canonical_json, '$.claim_content_shake256_512') = l.claim_content_shake256_512
      AND json_extract(NEW.signed_payload_canonical_json, '$.theorem_statement_shake256_512') = l.theorem_statement_shake256_512
      AND json_extract(NEW.signed_payload_canonical_json, '$.proof_artifact_shake256_512') = l.proof_artifact_shake256_512
      AND json_extract(NEW.signed_payload_canonical_json, '$.original_source_tree_shake256_512') = l.original_source_tree_shake256_512
      AND json_extract(NEW.signed_payload_canonical_json, '$.immutable_snapshot_shake256_512') = l.immutable_snapshot_shake256_512
      AND json_extract(NEW.signed_payload_canonical_json, '$.compiler_profile_id') = l.compiler_profile_id
      AND json_extract(NEW.signed_payload_canonical_json, '$.compiler_registry_shake256_512') = l.compiler_registry_shake256_512
      AND json_extract(NEW.signed_payload_canonical_json, '$.lake_executable_shake256_512') = l.lake_executable_shake256_512
      AND json_extract(NEW.signed_payload_canonical_json, '$.lean_executable_shake256_512') = l.lean_executable_shake256_512
      AND json_extract(NEW.signed_payload_canonical_json, '$.lean_stdlib_tree_shake256_512') = l.lean_stdlib_tree_shake256_512
      AND json_extract(NEW.signed_payload_canonical_json, '$.dependency_closure_shake256_512') = l.dependency_closure_shake256_512
      AND json_extract(NEW.signed_payload_canonical_json, '$.execution_image_digest') = l.execution_image_digest
      AND json_extract(NEW.signed_payload_canonical_json, '$.sandbox_policy_shake256_512') = l.sandbox_policy_shake256_512
      AND json_extract(NEW.signed_payload_canonical_json, '$.verifier_binary_shake256_512') = l.verifier_binary_shake256_512
      AND json_extract(NEW.signed_payload_canonical_json, '$.structured_result_shake256_512') = l.structured_result_shake256_512
      ...
      AND wc_ml_dsa_87_verify(k.public_key_base64url, NEW.signed_payload_canonical_json, l.signature) = 1
...
BEGIN
  SELECT CASE WHEN NOT EXISTS (
    ...
    WHERE c.claim_id = NEW.claim_id AND c.claim_kind = 'proof_claim'
      AND c.claim_content_shake256_512 = NEW.claim_content_shake256_512 AND c.theorem_statement_shake256_512 = NEW.theorem_statement_shake256_512
      AND p.claim_id = NEW.claim_id AND p.decision = 'allow' AND p.scope = 'theorem_promotion'
      AND lc.claim_id = NEW.claim_id AND lc.claim_content_shake256_512 = NEW.claim_content_shake256_512 AND lc.theorem_statement_shake256_512 = NEW.theorem_statement_shake256_512
      ...
      AND lc.original_source_tree_shake256_512 = lc.immutable_snapshot_shake256_512 AND lc.source_tree_shake256_512 = lc.immutable_snapshot_shake256_512
      ...
      AND lc.compiler_profile_id IS NOT NULL AND lc.compiler_registry_shake256_512 IS NOT NULL
      AND lc.lake_executable_shake256_512 IS NOT NULL AND lc.lean_executable_shake256_512 IS NOT NULL
      AND lc.lean_stdlib_tree_shake256_512 IS NOT NULL AND lc.dependency_closure_shake256_512 IS NOT NULL
      AND lc.execution_image_digest GLOB 'shake256-512:*' AND lc.sandbox_policy_shake256_512 IS NOT NULL AND lc.verifier_binary_shake256_512 IS NOT NULL
      AND lc.compiler_executable_shake256_512 = lc.lean_executable_shake256_512
      AND lc.verifier_principal_id = NEW.verifier_principal_id AND lc.key_id = NEW.key_id
      AND pw.claim_id = NEW.claim_id AND pw.claim_content_shake256_512 = NEW.claim_content_shake256_512 AND pw.theorem_statement_shake256_512 = NEW.theorem_statement_shake256_512
...
SELECT g.theorem_promotion_gate_id, g.claim_id, g.claim_content_shake256_512, g.theorem_statement_shake256_512,
...
SELECT s.authority_snapshot_id, s.evaluated_at, g.theorem_promotion_gate_id, g.claim_id,
       g.claim_content_shake256_512, g.theorem_statement_shake256_512, g.proof_witness_id,
