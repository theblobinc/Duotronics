...
from proof_authority import (  # noqa: E402
    ...
    canonical_json_loads, shake256_512_bytes, shake256_512_file,
)
...
def _evidence(control: str, status: str, source: str, observation: str) -> dict:
    return {
        ...
        "observation_shake256_512": shake256_512_bytes(observation.encode("utf-8")),
    }
...
def _expected_compilation_environment() -> dict[str, str]:
    return {
        "HOME": "/nonexistent", "LANG": "C.UTF-8", "LC_ALL": "C.UTF-8",
        "TZ": "UTC", "SOURCE_DATE_EPOCH": "0", "LEAN_ABORT_ON_PANIC": "1",
        "LEAN_PATH": str(HANDOFF_OLEAN),
    }


def _reconstructed_compile_command(module: dict) -> list[str]:
    module_path = module["module"].replace(".", "/")
    expected_olean = f"olean/{module_path}.olean"
    if module["olean_path"] != expected_olean:
        raise ValueError("compiled module path is not the canonical module-derived output target")
    return [
        str(LEAN), WARNING_AS_ERROR_OPTION, "-o", str(HANDOFF / expected_olean),
        str(WORK / f"{module_path}.lean"),
    ]


def validate_handoff(schema_validator: CanonicalSchemaValidator, invocation: dict, profile: dict) -> tuple[dict, str]:
    ...
    if binding_path not in artifact_by_path or artifact_by_path[binding_path]["shake256_512"] != manifest["binding_olean_shake256_512"]:
    ...
    binding_modules = [item for item in manifest["compiled_modules"] if item["role"] == "generated_binding"]
    if len(binding_modules) != 1:
        raise ValueError("compiled module set must contain exactly one generated binding")
    binding_module = binding_modules[0]
    if binding_module["olean_path"] != binding_path or binding_module["olean_shake256_512"] != manifest["binding_olean_shake256_512"] or binding_module["source_shake256_512"] != manifest["binding_module_shake256_512"]:
        raise ValueError("generated binding record differs from the dedicated binding identity")
    command_identity = {(item["module"], item["role"]) for item in manifest["compilation_commands"]}
    module_identity = {(item["module"], item["role"]) for item in manifest["compiled_modules"]}
    if len(command_identity) != len(manifest["compilation_commands"]) or command_identity != module_identity:
        raise ValueError("compiled modules and governed compile-command digests differ")
    if manifest["warnings_as_errors"] is not True or manifest["warning_as_error_cli_option"] != "-DwarningAsError=true":
        raise ValueError("handoff does not prove the governed warning-as-error policy")
    if manifest["command_reconstruction_policy"] != COMMAND_RECONSTRUCTION_POLICY:
        raise ValueError("handoff compile-command reconstruction policy is unknown")
    if manifest["lean_executable_path"] != str(LEAN) or manifest["working_directory"] != str(WORK):
        raise ValueError("handoff Lean executable or working directory is not the governed identity")
    if not LEAN.is_file() or shake256_512_file(LEAN) != manifest["lean_executable_shake256_512"] or manifest["lean_executable_shake256_512"] != profile["lean_executable_shake256_512"]:
        raise ValueError("handoff Lean executable digest differs from the trusted pinned executable")
    expected_environment_shake256_512 = shake256_512_bytes(canonical_bytes(_expected_compilation_environment()))
    if manifest["compilation_environment_shake256_512"] != expected_environment_shake256_512:
        raise ValueError("handoff compilation environment identity is not independently reproducible")
    reconstructed_commands = []
    for module in manifest["compiled_modules"]:
        reconstructed_commands.append({
            "module": module["module"], "role": module["role"],
            "argv_shake256_512": shake256_512_bytes(canonical_bytes(_reconstructed_compile_command(module))),
        })
    reconstructed_commands.sort(key=lambda item: item["module"])
    if manifest["compilation_commands"] != reconstructed_commands:
        raise ValueError("handoff compile-command digests are missing, reordered, extra, or altered")
    for module in manifest["compiled_modules"]:
        artifact = artifact_by_path.get(module["olean_path"])
        if artifact is None or artifact["shake256_512"] != module["olean_shake256_512"]:
    ...
    for item in manifest["artifacts"]:
        ...
        if format(info.st_mode & 0o777, "04o") != item["mode"] or info.st_size != item["size_bytes"] or shake256_512_file(path) != item["shake256_512"]:
    ...
    return manifest, shake256_512_file(manifest_path)
...
def main() -> int:
    ...
    if request.get("schema_version") != "wc_lean_verifier_request/v5":
        return 64
    try:
        handoff_manifest, handoff_manifest_shake256_512 = validate_handoff(schema_validator, invocation, profile)
    except Exception:
        return 69
    for executable, field in ((INSPECTOR, "verifier_executable_shake256_512"), (Path("/opt/lean/bin/lean"), "lean_executable_shake256_512"), (Path("/opt/lean/bin/lake"), "lake_executable_shake256_512")):
        if not executable.is_file() or shake256_512_file(executable) != profile[field]:
            return 65
    ...
    structured = {
        ...
        "expected_type_expression_hash": shake256_512_bytes(inspection["expected_type_expression_fingerprint"].encode("utf-8")),
        "actual_type_expression_hash": shake256_512_bytes(inspection["actual_type_expression_fingerprint"].encode("utf-8")),
        "direct_dependencies": inspection["direct_dependencies"],
        "dependency_closure": inspection["dependency_closure"],
        "transitive_dependencies_root": shake256_512_bytes(canonical_bytes(inspection["dependency_closure"])),
        "axiom_set": inspection["axiom_set"],
        "axiom_set_shake256_512": shake256_512_bytes(canonical_bytes(inspection["axiom_set"])),
        ...
        "handoff_manifest_shake256_512": handoff_manifest_shake256_512,
        "handoff_search_path": HANDOFF_OLEAN.as_posix(),
        "ambient_lean_path_ignored": True,
    }
    ...
    unsigned = {
        ...
        "request_id": request["request_id"], "request_shake256_512": shake256_512_bytes(canonical_bytes(request)),
        "service_request_id": request["service_request_id"],
        "authenticated_principal_id": request["authenticated_principal_id"],
        "source_bundle_id": request["source_bundle_id"],
        "compiler_profile_id": request["compiler_profile_id"], "claim_content_shake256_512": request["claim_content_shake256_512"],
        ...
        "oci_runtime_version_output_shake256_512": invocation["oci_runtime_version_output_shake256_512"],
        "sandbox_policy_shake256_512": profile["sandbox_policy_shake256_512"], "effective_sandbox_invocation_shake256_512": shake256_512_bytes(canonical_bytes(invocation)),
        "normalized_executed_argv_shake256_512": invocation["normalized_executed_argv_shake256_512"],
        ...
        "prebuilt_artifacts_used": False, "warnings_as_errors": handoff_manifest["warnings_as_errors"],
        "warning_as_error_cli_option": handoff_manifest["warning_as_error_cli_option"], "exit_status": process.returncode,
        ...
        "stdout_shake256_512": process.stdout_shake256_512, "stderr_shake256_512": process.stderr_shake256_512,
        "output_limit_exceeded": process.output_limit_exceeded,
    }
    preview = dict(unsigned)
    preview["signed_payload_shake256_512"] = shake256_512_bytes(canonical_bytes(unsigned))
