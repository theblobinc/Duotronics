#!/usr/bin/env python3
"""Authenticated, cache-verifying proof-check boundary for Draft 5.3.16."""
...
from proof_authority import (
    ...
    load_production_authority_service, load_trusted_canonical_json, secure_read_bytes, shake256_512_bytes, sign_record,
...
from cache_audit import (
    InMemoryMonotonicAuditAnchor, SignedAppendOnlyAuditSink, SupervisedAuditPublisher,
    UnixSocketAuditAnchorClient, UnixSocketAuditPublisherClient, key_validity_evidence, validate_governed_signing_registry,
)
...
class DurableIdempotencyStore:
    ...
    TABLE_SQL = """CREATE TABLE proof_check_idempotency (
        ...
        request_shake256_512 TEXT NOT NULL,
    ...
    METADATA_SQL = """CREATE TABLE idempotency_schema_metadata (
        ...
        canonical_schema_shake256_512 TEXT NOT NULL CHECK (
            length(canonical_schema_shake256_512) = 128
            AND canonical_schema_shake256_512 NOT GLOB '*[^0-9a-f]*'
        )
    )"""
    INDEX_SQL = {
    ...
    TABLE_XINFO = (
        ...
        (2, "request_shake256_512", "TEXT", 1, None, 0, 0),
    ...
    METADATA_XINFO = (
        ...
        (1, "canonical_schema_shake256_512", "TEXT", 1, None, 0, 0),
    )
    INDEX_LAYOUT = {
        "proof_check_idempotency_completed": {
    ...
    def __init__(
        ...
        connection_mode: str = "private_parent_absolute_path",
    ):
        self.path = Path(path)
        self.expected_uid = os.getuid() if expected_uid is None else expected_uid
        if connection_mode not in {"private_parent_absolute_path", "descriptor_via_procfs"}:
            raise ValueError("idempotency SQLite connection mode is unknown")
    ...
    def _connect(self, deadline_monotonic: float | None = None) -> sqlite3.Connection:
        self._secure_database_files()
        target = (
            f"/proc/self/fd/{self._directory_fd}/{self.path.name}"
            if self.connection_mode == "descriptor_via_procfs"
            else str(self.path)
        )
        if self.connection_mode == "descriptor_via_procfs" and not Path("/proc/self/fd").is_dir():
            raise RuntimeError("descriptor-via-procfs SQLite mode requires a procfs fd projection")
    ...
    @classmethod
    def canonical_schema_shake256_512(cls) -> str:
        return shake256_512_bytes(canonical_bytes(cls._schema_descriptor(include_metadata=True)))
    ...
    def _create_sqlite_schema(cls, connection: sqlite3.Connection) -> None:
        ...
        try:
            ...
            connection.execute(
                f"INSERT INTO {cls.METADATA_TABLE}(schema_version,canonical_schema_shake256_512) VALUES (?,?)",
                (cls.SCHEMA_VERSION, cls.canonical_schema_shake256_512()),
            )
    ...
    def _verify_sqlite_schema(self, connection: sqlite3.Connection) -> None:
        ...
        metadata = connection.execute(
            f"SELECT schema_version,canonical_schema_shake256_512 FROM {self.METADATA_TABLE}"
        ).fetchall()
        if metadata != [(self.SCHEMA_VERSION, self.canonical_schema_shake256_512())]:
    ...
    def acquire(self, principal_id: str, key: str, request_shake256_512: str, *, deadline_monotonic: float | None = None) -> tuple[str, str | dict[str, Any] | None]:
        now = time.time()
        ...
        with closing(self._connect(deadline_monotonic)) as connection:
            try:
                ...
                row = connection.execute(
                    "SELECT request_shake256_512,state,lease_expires_at,result_canonical_json FROM proof_check_idempotency WHERE principal_id=? AND idempotency_key=?",
                    (principal_id, key),
                ).fetchone()
                if row is None:
                    ...
                    connection.execute(
                        ...
                        (principal_id, key, request_shake256_512, "inflight", owner, now + self.lease_seconds, None, now, None),
                    )
                    connection.execute("COMMIT")
                    return "execute", owner
                prior_hash, state, _lease_expires_at, result_json = row
                if prior_hash != request_shake256_512:
    ...
    def complete(self, principal_id: str, key: str, owner: str, request_shake256_512: str, envelope: dict[str, Any], *, deadline_monotonic: float | None = None) -> None:
        ...
        with closing(self._connect(deadline_monotonic)) as connection:
            try:
                ...
                changed = connection.execute(
                    "UPDATE proof_check_idempotency SET state='completed',lease_owner=NULL,lease_expires_at=NULL,result_canonical_json=?,completed_at=? WHERE principal_id=? AND idempotency_key=? AND request_shake256_512=? AND state='inflight' AND lease_owner=? AND lease_expires_at>?",
                    (canonical_result, now, principal_id, key, request_shake256_512, owner, now),
                ).rowcount
                if changed != 1:
                ...
                try:
                    ...
                    connection.execute("BEGIN IMMEDIATE")
                    connection.execute(
                        "DELETE FROM proof_check_idempotency WHERE principal_id=? AND idempotency_key=? AND state='completed' AND request_shake256_512=?",
                        (principal_id, key, request_shake256_512),
                    )
                    connection.execute("COMMIT")
                    connection.execute("PRAGMA wal_checkpoint(TRUNCATE)")
                    raise
    ...
    def renew(self, principal_id: str, key: str, owner: str, request_shake256_512: str, *, deadline_monotonic: float | None = None) -> bool:
        ...
        with closing(self._connect(deadline_monotonic)) as connection:
            try:
                connection.execute("BEGIN IMMEDIATE")
                changed = connection.execute(
                    "UPDATE proof_check_idempotency SET lease_expires_at=? WHERE principal_id=? AND idempotency_key=? AND request_shake256_512=? AND state='inflight' AND lease_owner=? AND lease_expires_at>?",
                    (now + self.lease_seconds, principal_id, key, request_shake256_512, owner, now),
                ).rowcount
                connection.execute("COMMIT")
                return changed == 1
            except Exception:
...
class GovernedCacheRegistrySnapshot:
    ...
    registry_shake256_512: str
    registry_id: str
    registry_created_at: str
    registry_signed_payload_shake256_512: str
    records_by_id: dict[str, dict[str, Any]]
    verification_keys_by_id: dict[str, MLDSA87PublicKey]
    successor_registry_shake256_512: str | None
    stale_replay_policy: str
    revoked_key_ids: frozenset[str]
    lineage_signed_payload_shake256_512: str | None
    lineage_created_at: str | None
...
def build_governed_cache_registry_snapshot(
    ...
    registry_shake256_512 = shake256_512_bytes(canonical_bytes(registry))
    if successor_registry_shake256_512 is not None and not re.fullmatch(r"[0-9a-f]{128}", successor_registry_shake256_512):
    ...
    for key_id, record in records.items():
        ...
        if len(raw) != 2592:
            raise RuntimeError("cache-signing registry contains a non-ML-DSA-87 public key")
        keys[key_id] = MLDSA87PublicKey.from_public_bytes(raw)
    return GovernedCacheRegistrySnapshot(
        ...
        registry_signed_payload_shake256_512=registry["signed_payload_shake256_512"],
        records_by_id=records,
        verification_keys_by_id=keys,
        successor_registry_shake256_512=successor_registry_shake256_512,
        stale_replay_policy=stale_replay_policy,
        revoked_key_ids=frozenset(revoked_key_ids),
        lineage_signed_payload_shake256_512=lineage_signed_payload_shake256_512,
        lineage_created_at=lineage_created_at,
    )


def validate_cache_registry_snapshot_lineage(
    snapshots: dict[str, GovernedCacheRegistrySnapshot], *, current_registry_shake256_512: str,
    production_mode: bool,
) -> dict[str, tuple[str, ...]]:
    """Require every governed historical snapshot to terminate at the current registry."""
    if current_registry_shake256_512 not in snapshots:
        raise ValueError("cache registry snapshots omit the current registry")
    current = snapshots[current_registry_shake256_512]
    if current.registry_shake256_512 != current_registry_shake256_512:
        raise ValueError("cache registry snapshot map key differs from its canonical digest")
    if current.successor_registry_shake256_512 is not None or current.stale_replay_policy != "current_registry":
    ...
    for digest_value, snapshot in snapshots.items():
        if digest_value != snapshot.registry_shake256_512:
            raise ValueError("cache registry snapshot map key differs from its canonical digest")
        if production_mode and snapshot.lineage_signed_payload_shake256_512 is None:
            raise ValueError("production cache registry snapshot lacks signed lineage evidence")
        if snapshot.lineage_signed_payload_shake256_512 is not None and not re.fullmatch(r"[0-9a-f]{128}", snapshot.lineage_signed_payload_shake256_512):
            raise ValueError("cache registry snapshot lineage digest is invalid")
        if production_mode and snapshot.lineage_created_at is None:
            raise ValueError("production cache registry snapshot lacks lineage creation time")
        if snapshot.lineage_created_at is not None:
            lineage_time = _parse_registry_datetime(snapshot.lineage_created_at, "lineage.created_at")
            registry_time = _parse_registry_datetime(snapshot.registry_created_at, "registry.created_at")
            if lineage_time < registry_time:
                raise ValueError("cache registry lineage predates a referenced registry")
        if digest_value != current_registry_shake256_512:
            if snapshot.stale_replay_policy != "authenticate_then_conflict" or snapshot.successor_registry_shake256_512 is None:
                raise ValueError("historical cache registry lacks a fail-closed successor policy")
        visited: list[str] = []
        cursor: str | None = digest_value
        while cursor != current_registry_shake256_512:
            ...
            successor = snapshots[cursor].successor_registry_shake256_512
            ...
            cursor = successor
        paths[digest_value] = tuple([*visited, current_registry_shake256_512])
    return paths


def load_governed_cache_registry_snapshots(
    config_root: Path, lineage_relative_path: str, *, expected_uid: int,
    schema_validator: CanonicalSchemaValidator, governance_key: MLDSA87PublicKey,
    ...
    current_digest = shake256_512_bytes(canonical_bytes(current_registry))
    if lineage.get("current_registry_shake256_512") != current_digest:
        raise RuntimeError("cache registry lineage does not bind the selected current registry")
    lineage_payload_shake256_512 = lineage["signed_payload_shake256_512"]
    snapshots: dict[str, GovernedCacheRegistrySnapshot] = {
        current_digest: build_governed_cache_registry_snapshot(
            ...
            successor_registry_shake256_512=None, stale_replay_policy="current_registry",
            lineage_signed_payload_shake256_512=lineage_payload_shake256_512, lineage_created_at=lineage["created_at"], evaluated_at=evaluated_at,
        ),
    }
    seen_files: set[str] = set()
    for entry in lineage["historical_registries"]:
        registry_digest = entry["registry_shake256_512"]
        ...
        if shake256_512_bytes(canonical_bytes(historical_registry)) != registry_digest:
        ...
        snapshots[registry_digest] = build_governed_cache_registry_snapshot(
            ...
            successor_registry_shake256_512=entry["successor_registry_shake256_512"],
            stale_replay_policy=entry["stale_replay_policy"],
            revoked_key_ids=frozenset(entry["revoked_key_ids"]),
            lineage_signed_payload_shake256_512=lineage_payload_shake256_512, lineage_created_at=lineage["created_at"],
        )
    validate_cache_registry_snapshot_lineage(snapshots, current_registry_shake256_512=current_digest, production_mode=True)
    return snapshots
...
class ProofCheckApplication:
    def __init__(
        ...
        cache_signing_key: MLDSA87PrivateKey | None = None,
        cache_verification_key: MLDSA87PublicKey | None = None,
        cache_verification_keys_by_id: dict[str, MLDSA87PublicKey] | None = None,
        compiler_witness_verification_key: MLDSA87PublicKey | None = None,
        ...
        cache_registry_shake256_512: str | None = None,
        cache_registry_snapshots_by_shake256_512: dict[str, GovernedCacheRegistrySnapshot] | None = None,
        ...
        if cache_signing_key is None:
            ...
            cache_signing_key = MLDSA87PrivateKey.generate()
        ...
        self.cache_registry_shake256_512 = cache_registry_shake256_512 or shake256_512_bytes(canonical_bytes({
            "development_cache_key": self.cache_key_record,
        }))
        if not re.fullmatch(r"[0-9a-f]{128}", self.cache_registry_shake256_512):
            raise ValueError("cache-signing registry snapshot digest is invalid")
        if cache_registry_snapshots_by_shake256_512 is None:
            cache_registry_snapshots_by_shake256_512 = {
                self.cache_registry_shake256_512: GovernedCacheRegistrySnapshot(
                    registry_shake256_512=self.cache_registry_shake256_512,
                    registry_id="development:current",
                    registry_created_at=self.cache_key_record["status_changed_at"],
                    registry_signed_payload_shake256_512=self.cache_registry_shake256_512,
                    ...
                    successor_registry_shake256_512=None,
                    stale_replay_policy="current_registry",
                    revoked_key_ids=frozenset(),
                    lineage_signed_payload_shake256_512=None,
                    lineage_created_at=None,
                ),
            }
        self.cache_registry_snapshots_by_shake256_512 = dict(cache_registry_snapshots_by_shake256_512)
        self.cache_registry_lineage_paths = validate_cache_registry_snapshot_lineage(
            self.cache_registry_snapshots_by_shake256_512,
            current_registry_shake256_512=self.cache_registry_shake256_512,
            production_mode=production_mode,
        )
        current_snapshot = self.cache_registry_snapshots_by_shake256_512[self.cache_registry_shake256_512]
        ...
        if production_mode and cache_verification_evidence_sink is None:
            raise ValueError("production requires a durable cache-verification evidence sink")
    ...
    def _validate_result_bindings(
        ...
        request_shake256_512: str, resolved_policy,
    ) -> None:
        ...
        claim_hash = shake256_512_bytes(canonical_bytes(request["canonical_claim"]))
        theorem_hash = shake256_512_bytes(request["theorem_statement"].encode("utf-8"))
        exact_result = {
            "request_id": request["request_id"], "request_shake256_512": request_shake256_512,
            "idempotency_key": request["idempotency_key"], "authenticated_principal_id": principal_id,
            "claim_id": request["claim_id"], "claim_content_shake256_512": claim_hash,
            "theorem_statement_shake256_512": theorem_hash, "compiler_profile_id": request["compiler_profile_id"],
        ...
        exact_witness = {
            ...
            "claim_content_shake256_512": claim_hash, "theorem_statement_shake256_512": theorem_hash,
    ...
    def _cache_envelope(
        self, result: dict[str, Any], request: dict[str, Any], principal_id: str,
        request_shake256_512: str, deadline_monotonic: float,
    ) -> dict[str, Any]:
        ...
        identity = {
            "request_shake256_512": request_shake256_512, "authenticated_principal_id": principal_id,
            "compiler_witness_signed_payload_shake256_512": result["compiler_witness"]["signed_payload_shake256_512"],
        }
        unsigned = {
            "schema_version": "idempotency_cache_envelope/v3",
            "cache_envelope_id": "cache:" + shake256_512_bytes(canonical_bytes(identity)),
            "request_shake256_512": request_shake256_512, "request_id": request["request_id"],
            "idempotency_key": request["idempotency_key"], "authenticated_principal_id": principal_id,
            "claim_id": request["claim_id"], "compiler_profile_id": request["compiler_profile_id"],
            "policy_decision_id": result["policy_decision_id"],
            "policy_decision_shake256_512": result["policy_decision_shake256_512"],
            "source_bundle_id": request["source_bundle_id"],
            "cache_signer_principal_id": self.cache_signer_principal_id,
            "cache_signer_key_id": self.cache_signer_key_id,
            "cache_registry_shake256_512": self.cache_registry_shake256_512,
    ...
    def _validate_snapshot_signing_evidence(
        ...
        registry_created_at = _parse_registry_datetime(snapshot.registry_created_at, "historical.registry_created_at")
        if registry_created_at > signed_at:
            raise AuthorityFailure("cache_integrity_invalid", "historical cache registry did not exist at the cache signing time")
        ...
        expected = {
            "schema_version": "cache_key_validity_evidence/v1",
            "registry_shake256_512": snapshot.registry_shake256_512,
    ...
    def _validate_cache_envelope_bindings(
        self, envelope: dict[str, Any], request: dict[str, Any], principal_id: str,
        request_shake256_512: str, resolved_policy, stored_record: dict[str, Any],
    ) -> str:
        expected = {
            "request_shake256_512": request_shake256_512, "request_id": request["request_id"],
            "idempotency_key": request["idempotency_key"], "authenticated_principal_id": principal_id,
            ...
            "policy_decision_shake256_512": resolved_policy.canonical_record_shake256_512,
            "source_bundle_id": request["source_bundle_id"],
            "cache_signer_principal_id": stored_record.get("principal_id"),
            "cache_signer_key_id": stored_record.get("key_id"),
        }
        ...
        if not isinstance(result, dict):
            raise AuthorityFailure("cache_integrity_invalid", "idempotency-cache result is absent")
        self._validate_result_bindings(result, request, principal_id, request_shake256_512, resolved_policy)
        return result["compiler_witness"]["signed_payload_shake256_512"]

    def _publish_cache_evidence(self, evidence: dict[str, Any], deadline_monotonic: float) -> None:
        try:
            self._check_deadline(deadline_monotonic, "before cache-audit publication")
            try:
                parameters = inspect.signature(self.cache_verification_evidence_sink).parameters
                deadline_aware = "deadline_monotonic" in parameters or any(
                    item.kind is inspect.Parameter.VAR_KEYWORD for item in parameters.values()
                )
            except (TypeError, ValueError):
                deadline_aware = False
            if deadline_aware:
                self.cache_verification_evidence_sink(evidence, deadline_monotonic=deadline_monotonic)
            else:
                self.cache_verification_evidence_sink(evidence)
            self._check_deadline(deadline_monotonic, "after cache-audit publication")
        except AuthorityFailure:
            raise
        except (TimeoutError, OSError, RuntimeError) as error:
            raise AuthorityFailure(
                "cache_audit_publication_failed",
                "cache verification evidence could not be durably published within the request deadline",
            ) from error

    def _cached_result(
        self, envelope: dict[str, Any], request: dict[str, Any], principal_id: str,
        request_shake256_512: str, resolved_policy, deadline_monotonic: float,
    ) -> dict[str, Any]:
        # Replay order: canonical parse; schema; signer lookup; signature;
        # signed snapshot lineage; signing chronology; every current request,
        # slot, policy, source and compiler-witness binding; only then stale
        # rotation evidence and the stable conflict.
        self.schema_validator.validate("idempotency_cache_envelope", envelope)
        stored_key_id = envelope.get("cache_signer_key_id")
        stored_registry_shake256_512 = envelope.get("cache_registry_shake256_512")
        snapshot = self.cache_registry_snapshots_by_shake256_512.get(stored_registry_shake256_512)
        if snapshot is None:
        ...
        lineage_paths = validate_cache_registry_snapshot_lineage(
            self.cache_registry_snapshots_by_shake256_512,
            current_registry_shake256_512=self.cache_registry_shake256_512,
            production_mode=any(item.lineage_signed_payload_shake256_512 is not None for item in self.cache_registry_snapshots_by_shake256_512.values()),
        )
        ...
        compiler_witness_shake256_512 = self._validate_cache_envelope_bindings(
            envelope, request, principal_id, request_shake256_512, resolved_policy, stored_record,
        )
        result = envelope["result"]
        if stored_registry_shake256_512 != self.cache_registry_shake256_512:
            ...
            if snapshot.lineage_created_at is None:
                raise AuthorityFailure("cache_integrity_invalid", "historical cache registry lacks lineage creation chronology")
            claim_hash = shake256_512_bytes(canonical_bytes(request["canonical_claim"]))
            stale_evidence = {
                "schema_version": "cache_stale_row_evidence/v4",
                "cache_envelope_id": envelope["cache_envelope_id"],
                "cache_envelope_signed_payload_shake256_512": envelope["signed_payload_shake256_512"],
                "cache_envelope_canonical_shake256_512": shake256_512_bytes(canonical_bytes(envelope)),
                "cache_envelope_signature": envelope["signature"],
                "stored_cache_registry_shake256_512": stored_registry_shake256_512,
                "current_cache_registry_shake256_512": self.cache_registry_shake256_512,
                "historical_registry_id": snapshot.registry_id,
                "historical_registry_signed_payload_shake256_512": snapshot.registry_signed_payload_shake256_512,
                "registry_lineage_signed_payload_shake256_512": snapshot.lineage_signed_payload_shake256_512,
                ...
                "request_shake256_512": request_shake256_512,
                "request_id": request["request_id"],
                "claim_id": request["claim_id"],
                "claim_content_shake256_512": claim_hash,
                "compiler_profile_id": request["compiler_profile_id"],
                "policy_decision_id": resolved_policy.policy_decision_id,
                "policy_decision_shake256_512": resolved_policy.canonical_record_shake256_512,
                "source_bundle_id": request["source_bundle_id"],
                "proof_artifact_relative_path": request["proof_artifact_relative_path"],
                "compiler_witness_signed_payload_shake256_512": compiler_witness_shake256_512,
                "cache_signed_at": envelope["cache_signed_at"],
                "historical_registry_created_at": snapshot.registry_created_at,
                "registry_lineage_created_at": snapshot.lineage_created_at,
                "binding_verification_result": "all_request_slot_policy_source_and_compiler_witness_bindings_verified",
                "signature_verified": True, "registry_lineage_verified": True,
                "registry_lineage_path": list(lineage_paths[stored_registry_shake256_512]),
            ...
            self._publish_cache_evidence(stale_evidence, deadline_monotonic)
            raise AuthorityFailure(
        ...
        self.schema_validator.validate("cache_verification_evidence", replay_event)
        self._publish_cache_evidence(replay_event, deadline_monotonic)
    ...
    def handle(self, request: dict[str, Any], *, authenticated_principal_id: str) -> dict[str, Any]:
        ...
        request_shake256_512 = shake256_512_bytes(canonical_bytes(request))
        idempotency_key = request["idempotency_key"]
        ...
        while owner is None:
            action, value = self.idempotency_store.acquire(
                principal_id, idempotency_key, request_shake256_512, deadline_monotonic=deadline,
            )
            if action == "completed":
                assert isinstance(value, dict)
                self._check_deadline(deadline, "before cached-result verification")
                cached = self._cached_result(value, request, principal_id, request_shake256_512, resolved_policy, deadline)
            ...
            while not stop_heartbeat.wait(interval):
                try:
                    if not self.idempotency_store.renew(
                        principal_id, idempotency_key, owner, request_shake256_512,
        ...
        try:
            ...
            result = {
                ...
                "claim_content_shake256_512": shake256_512_bytes(canonical_bytes(request["canonical_claim"])),
                "theorem_statement_shake256_512": shake256_512_bytes(request["theorem_statement"].encode("utf-8")),
                "compiler_profile_id": request["compiler_profile_id"],
                "policy_decision_id": resolved_policy.policy_decision_id,
                "policy_decision_shake256_512": resolved_policy.canonical_record_shake256_512,
            ...
            envelope = self._cache_envelope(result, request, principal_id, request_shake256_512, deadline)
            stop_heartbeat.set()
            ...
            self.idempotency_store.complete(
                principal_id, idempotency_key, owner, request_shake256_512, envelope,
                deadline_monotonic=deadline,
            )
...
def load_production_application(config_root: Path = Path("/etc/witness-authority")) -> ProofCheckApplication:
    ...
    store = DurableIdempotencyStore(
        ...
        connection_mode=config["idempotency_sqlite_connection_mode"],
    )
    ...
    if public_key_raw_b64url(cache_signing_key.public_key()) != public_key_raw_b64url(cache_verification_key):
        raise RuntimeError("cache private and public keys do not form one ML-DSA-87 keypair")
    ...
    cache_verification_keys_by_id = {
        key_id: MLDSA87PublicKey.from_public_bytes(_b64url_decode(record["public_key_base64url"]))
    ...
    # The proof process trusts only the governed receipt-verification registry.  It
    # never receives a record-signing private key or direct audit-storage access.
    receipt_registry = load_trusted_canonical_json(
        config_root, config["cache_audit_signing_registry_file"], expected_uid=expected_uid,
    )
    receipt_registry_shake256_512, receipt_records, receipt_verification_keys = validate_governed_signing_registry(
        receipt_registry, governance_key, schema_validator=validator,
        surface="cache_audit_signing_registry", required_scope="cache_audit_receipt_signing",
        evaluated_at=now,
    )
    receipt_record = receipt_records.get(config["cache_audit_signer_key_id"])
    if receipt_record is None or receipt_record.get("principal_id") != config["cache_audit_signer_principal_id"]:
        raise RuntimeError("configured cache audit receipt key is absent from its governed registry")
    receipt_verification_key = _public_key_from_bytes(
        secure_read_bytes(config_root, config["cache_audit_public_key_file"], expected_uid=expected_uid)
    )
    receipt_public = public_key_raw_b64url(receipt_verification_key)
    governed_receipt_key = receipt_verification_keys[config["cache_audit_signer_key_id"]]
    if receipt_public != public_key_raw_b64url(governed_receipt_key):
        raise RuntimeError("cache audit receipt public key does not match its governed registry record")
    receipt_validity = key_validity_evidence(
        registry_shake256_512=receipt_registry_shake256_512, record=receipt_record, evaluated_at=now,
    )
    validator.validate("cache_audit_key_validity_evidence", receipt_validity)
    if receipt_validity["decision"] != "active_time_valid_rotation_valid":
        raise RuntimeError("cache audit receipt key is outside its governed active interval")
    if receipt_public in {public_key_raw_b64url(cache_verification_key), public_key_raw_b64url(authority.signing_key.public_key())}:
        raise RuntimeError("cache audit receipt authority must be distinct from cache and compiler-witness authorities")
    audit_request_signing_key = _private_key_from_bytes(
        secure_read_bytes(config_root, config["cache_audit_request_private_key_file"], expected_uid=expected_uid)
    )
    request_public = public_key_raw_b64url(audit_request_signing_key.public_key())
    if request_public in {
        receipt_public, public_key_raw_b64url(cache_verification_key),
        public_key_raw_b64url(authority.signing_key.public_key()),
    }:
        raise RuntimeError("cache audit request authority must use a distinct key")
    publisher_uid = int(config["cache_audit_publisher_uid"])
    publisher_gid = int(config["cache_audit_publisher_gid"])
    if publisher_uid in {0, expected_uid}:
        raise RuntimeError("cache audit publisher must use a distinct non-root service identity")
    audit_sink = UnixSocketAuditPublisherClient(
        Path(config["cache_audit_publisher_socket"]),
        verification_keys_by_id=receipt_verification_keys,
        receipt_signing_registry_shake256_512=receipt_registry_shake256_512,
        receipt_registry_records=receipt_records,
        schema_validator=validator,
        request_signing_key=audit_request_signing_key,
        request_signer_key_id=config["cache_audit_request_signer_key_id"],
        peer_principal_id=config["cache_audit_request_principal_id"],
        expected_socket_uid=publisher_uid,
        expected_socket_gid=publisher_gid,
        expected_socket_mode=int(config["cache_audit_publisher_socket_mode"]),
    )
    return ProofCheckApplication(
        ...
        cache_registry_shake256_512=shake256_512_bytes(canonical_bytes(cache_registry)),
        cache_registry_snapshots_by_shake256_512=cache_registry_snapshots,
        cache_verification_evidence_sink=audit_sink,
        production_mode=True,
    )


__all__ = [
    "DurableIdempotencyStore", "SignedAppendOnlyAuditSink", "ProofCheckApplication", "load_production_application",
