...
POST_5314 = {
    ...
    "cache_audit_publication_receipt_v1", "cache_audit_record_v3",
...
def fixture_map() -> tuple[dict[str, list[str]], dict[str, list[str]]]:
    ...
    for disposition, target in (("valid", valid), ("invalid", invalid)):
        ...
        for path in sorted((ROOT / "executable/tests/fixtures").glob(f"draft5_3_*/{disposition}/*.json")):
            generation = tuple(int(part) for part in path.parents[1].name.removeprefix("draft").split("_"))
            if generation > (5, 3, 14):
                continue
...
def main() -> int:
    ...
    schema_paths = sorted((path for path in (ROOT / "schemas").glob("*.schema.*") if schema_name(path) not in POST_5314), key=lambda path: path.name)
