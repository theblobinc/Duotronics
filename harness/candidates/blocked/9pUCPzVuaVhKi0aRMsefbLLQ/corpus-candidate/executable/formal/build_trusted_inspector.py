...
from proof_authority import canonical_bytes, run_bounded_process, shake256_512_file  # noqa: E402
...
def tree_hash(root: Path) -> str:
    ...
    for path in sorted(root.rglob("*.lean")):
        records.append([path.relative_to(root).as_posix(), shake256_512_file(path)])
    return hashlib.shake_256(canonical_bytes(records)).hexdigest(64)
...
def build_once(lake: str) -> tuple[str | None, dict]:
    with tempfile.TemporaryDirectory(prefix="wc-inspector-build-") as temporary:
        ...
        output = copy / OUTPUT
        return (shake256_512_file(output) if result.returncode == 0 and output.is_file() else None), {
            "returncode": result.returncode, "timed_out": result.timed_out,
            "output_limit_exceeded": result.output_limit_exceeded,
            "stdout_shake256_512": result.stdout_shake256_512, "stderr_shake256_512": result.stderr_shake256_512,
        }
...
def main() -> int:
    ...
    result = {
        ...
        "source_tree_shake256_512": tree_hash(ROOT / SOURCE_ROOT),
        "build_command": ["lake", "build", TARGET], "lake_available": bool(lake),
        "builds": [], "output_binary_shake256_512": None, "status": "not_run",
    }
    code = 2
    if lake:
        ...
        if first_hash and first_hash == second_hash:
            result["output_binary_shake256_512"] = first_hash; result["status"] = "passed"; code = 0
        else:
