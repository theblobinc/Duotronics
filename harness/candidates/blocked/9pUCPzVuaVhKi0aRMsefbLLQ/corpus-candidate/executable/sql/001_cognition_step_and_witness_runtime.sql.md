# SQL Migration 001 — Cognition Step and Witness Runtime

Status: executable SQL in Markdown wrapper.

To use: extract the SQL block into `001_cognition_step_and_witness_runtime.sql`.
...
```sql
BEGIN;
DROP TABLE IF EXISTS srnn.decay_intent_witnesses;
DROP TABLE IF EXISTS srnn.absence_witnesses;
DROP TABLE IF EXISTS srnn.memory_update_records;
DROP TABLE IF EXISTS srnn.temporal_witnesses;
-- Do not drop srnn_cognition_snapshots.step by default; legacy tools may now depend on it.
COMMIT;
```
