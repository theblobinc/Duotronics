...
def profile_record(result_key: MLDSA87PrivateKey) -> dict:
    return {
        ...
        "oci_runtime_shake256_512": "2" * 128,
        "oci_runtime_version": "podman version 5.4.0",
        "lake_executable_shake256_512": "3" * 128,
        "lean_executable_shake256_512": "4" * 128,
        "lean_stdlib_tree_shake256_512": "5" * 128,
        "dependency_closure_shake256_512": "6" * 128,
        "verifier_executable_shake256_512": "7" * 128,
        "verifier_source_revision": "git:0123456789abcdef",
        "verifier_build_attestation_id": "attestation:verifier:test",
        "sandbox_policy_shake256_512": "8" * 128,
...
class FakeTwoDomainRunner:
    ...
    def __init__(self, result_key: MLDSA87PrivateKey, mode: str = "pass", mutate_original: pathlib.Path | None = None):
    ...
    def run(self, *, profile, snapshot_root, generated_module, request, timeout_seconds):
        ...
        type_hash = authority.shake256_512_bytes(b"elaborated:True")
        axioms: list[str] = []
        forbidden: list[str] = []
        compile_evidence = self._domain_evidence("untrusted_compilation", request, profile, "compile")
        inspection_evidence = self._domain_evidence("trusted_inspection", request, profile, "inspect")
        result = {
            "schema_version": "wc_lean_verifier_result/v6",
            "status": "passed",
            "request_id": request["request_id"],
            "request_shake256_512": authority.shake256_512_bytes(authority.canonical_bytes(request)),
            ...
            "claim_content_shake256_512": request["claim_content_shake256_512"],
            "policy_decision_id": request["policy_decision_id"],
            "policy_decision_shake256_512": request["policy_decision_shake256_512"],
            "theorem_statement_shake256_512": request["theorem_statement_shake256_512"],
            "proof_artifact_shake256_512": request["proof_artifact_shake256_512"],
            "immutable_snapshot_id": request["immutable_snapshot_id"],
            "immutable_snapshot_tree_shake256_512": request["immutable_snapshot_tree_shake256_512"],
            "generated_binding_module_shake256_512": request["generated_binding_module_shake256_512"],
            "lake_executable_shake256_512": profile.lake_executable_shake256_512,
            "lean_executable_shake256_512": profile.lean_executable_shake256_512,
            "lean_stdlib_tree_shake256_512": profile.lean_stdlib_tree_shake256_512,
            "dependency_closure_shake256_512": profile.dependency_closure_shake256_512,
            "oci_image_digest": profile.oci_image_digest,
            "oci_runtime_shake256_512": profile.oci_runtime_shake256_512,
            "oci_runtime_version": profile.oci_runtime_version,
            "oci_runtime_version_output_shake256_512": authority.shake256_512_bytes(profile.oci_runtime_version.encode("utf-8")),
            "verifier_executable_shake256_512": profile.verifier_executable_shake256_512,
            "sandbox_policy_shake256_512": profile.sandbox_policy_shake256_512,
            "effective_sandbox_invocation_shake256_512": self.INVOCATION_HASH,
            "normalized_executed_argv_shake256_512": self.ARGV_HASH,
            "handoff_manifest_shake256_512": "7" * 128,
            "handoff_search_path": "/handoff/olean", "ambient_lean_path_ignored": True,
            ...
            "transitive_dependencies_root": authority.shake256_512_bytes(b"deps"),
            "axiom_set": axioms,
            "axiom_set_shake256_512": authority.shake256_512_bytes(authority.canonical_bytes(axioms)),
            ...
            "control_evidence": [
                *[
                    {"control": control, "status": "measured_pass", "source": "fake_governed_measurement", "observation_shake256_512": authority.shake256_512_bytes(control.encode("utf-8"))}
                    for control in self.CONTROLS
                ],
                *[
                    {"control": control, "status": "derived_pass", "source": "fake_sealed_topology", "observation_shake256_512": authority.shake256_512_bytes(control.encode("utf-8"))}
                    for control in self._security()
                ],
            ],
            "stdout_shake256_512": authority.shake256_512_bytes(b""), "stderr_shake256_512": authority.shake256_512_bytes(b""),
        ...
        if self.mode == "statement_mismatch":
            result["declaration_type_matches"] = False; result["actual_type_expression_hash"] = "a" * 128; result["status"] = "failed"
        elif self.mode == "comment_only":
            result["declaration_found"] = False; result["status"] = "failed"
        elif self.mode == "sorry":
            result["axiom_set"] = ["sorryAx"]; result["axiom_set_shake256_512"] = authority.shake256_512_bytes(authority.canonical_bytes(["sorryAx"])); result["sorry_ax_present"] = True; result["status"] = "failed"
        elif self.mode == "forbidden_axiom":
            result["axiom_set"] = ["Unsafe.magic"]; result["axiom_set_shake256_512"] = authority.shake256_512_bytes(authority.canonical_bytes(["Unsafe.magic"])); result["forbidden_axiom_set"] = ["Unsafe.magic"]; result["status"] = "failed"
        elif self.mode == "unsafe":
            result["unsafe_dependency_present"] = True; result["status"] = "failed"
        elif self.mode == "wrong_request":
            result["request_shake256_512"] = "0" * 128
        elif self.mode == "wrong_snapshot":
            result["immutable_snapshot_tree_shake256_512"] = "0" * 128
        elif self.mode == "wrong_invocation":
            result["effective_sandbox_invocation_shake256_512"] = "0" * 128
        elif self.mode == "duplicate_axiom":
            result["axiom_set"] = ["propext", "propext"]
        elif self.mode == "unsigned":
            signed = result
        elif self.mode == "wrong_signer":
            signed = authority.sign_record(result, MLDSA87PrivateKey.generate())
            ...
            return authority.SandboxExecution(
                ...
                self.CONTROLS, normalized_executed_argv_shake256_512=self.ARGV_HASH,
                domain_file_size_limit=request["effective_resource_limits"]["inspection_output_bytes"],
            )
        ...
        return authority.SandboxExecution(
            ...
            False, authority.shake256_512_bytes(b"diagnostic only"), authority.shake256_512_bytes(b""),
            ...
            normalized_executed_argv=("verify",), normalized_executed_argv_shake256_512=self.ARGV_HASH,
            domain_file_size_limit=request["effective_resource_limits"]["inspection_output_bytes"],
            compile_execution_evidence=compile_evidence,
            inspection_execution_evidence=inspection_evidence,
        )
    ...
    def _domain_evidence(self, domain, request, profile, stage):
        command = [stage, "--governed"]
        mounts = [{
            "source": f"fake:{stage}", "host_path": f"/tmp/fake-{stage}",
            "destination": "/handoff" if domain == "untrusted_compilation" else "/trusted-inspection",
            "mode": "rw", "domain": domain, "purpose": f"fake_{stage}", "lifecycle": "request",
        }]
        control_evidence = [{
            "control": control, "status": "measured_pass", "source": "fake_trusted_runtime_inspect",
            "observation_shake256_512": authority.shake256_512_bytes(f"{domain}:{control}".encode("utf-8")),
        } for control in self.CONTROLS]
        return {
            "schema_version": "domain_execution_evidence/v1", "domain": domain,
            "evidence_source": "trusted_host_launcher_and_runtime_inspection/v1",
            "invocation_shake256_512": self.INVOCATION_HASH,
            "normalized_executed_argv": command,
            "normalized_executed_argv_shake256_512": authority.shake256_512_bytes(authority.canonical_bytes(command)),
            ...
            "requested_controls": list(self.CONTROLS), "emitted_controls": list(self.CONTROLS),
            "accepted_controls": list(self.CONTROLS), "applied_controls": list(self.CONTROLS),
            "measured_controls": list(self.CONTROLS), "control_evidence": control_evidence,
            "container_uid": 65534, "container_gid": 65534,
            "host_directory_uid": 65534, "host_directory_gid": 65534,
            "user_namespace_mode": "keep-id",
            "domain_file_size_limit": request["effective_resource_limits"]["compiler_artifact_file_bytes" if domain == "untrusted_compilation" else "inspection_output_bytes"],
            "stdout_bytes_limit": request["effective_resource_limits"]["stdout_bytes"],
            "stderr_bytes_limit": request["effective_resource_limits"]["stderr_bytes"],
            "combined_output_bytes_limit": request["effective_resource_limits"]["combined_output_bytes"],
            "request_budget_seconds": request["effective_resource_limits"]["timeout_seconds"],
            "remaining_at_start_ms": 600000, "elapsed_ms": 1, "remaining_at_finish_ms": 599999,
...
class ProofAuthorityTests(unittest.TestCase):
    def setUp(self):
        self.governance_private = MLDSA87PrivateKey.generate()
        self.witness_private = MLDSA87PrivateKey.generate()
        self.result_private = MLDSA87PrivateKey.generate()
        registry_unsigned = {
    ...
    def verify(self, service, root, **overrides):
        values = {
            ...
            "policy_decision_id": "policy:proof:test", "policy_decision_shake256_512": "a" * 128,
        }
    ...
    def test_valid_signed_result_passes_and_binds_complete_closure(self):
        ...
        self.assertEqual(
            authority.shake256_512_bytes(record["verifier_result_signed_payload_canonical_json"].encode("utf-8")),
            record["verifier_result_payload_shake256_512"],
        )
    ...
    def test_artifact_and_metadata_hashes_come_from_snapshot(self):
        ...
        with temp:
            ...
            self.assertEqual(record["proof_artifact_shake256_512"], authority.shake256_512_bytes(runner.snapshot_artifact_bytes))
            self.assertEqual(record["lakefile_shake256_512"], authority.shake256_512_file(root / "lakefile.lean"))
            self.assertEqual(record["lake_manifest_shake256_512"], authority.shake256_512_file(root / "lake-manifest.json"))
            self.assertEqual(record["lean_toolchain_shake256_512"], authority.shake256_512_file(root / "lean-toolchain"))

    def test_mutation_before_snapshot_is_captured_not_prehashed(self):
        temp, root = self.project()
        with temp:
            original = authority.shake256_512_file(root / "Proof.lean")
            (root / "Proof.lean").write_text("theorem t : True := True.intro\n", encoding="utf-8")
            service, runner = self.service(); record = self.verify(service, root)
        self.assertNotEqual(original, record["proof_artifact_shake256_512"])
        self.assertEqual(record["proof_artifact_shake256_512"], authority.shake256_512_bytes(runner.snapshot_artifact_bytes))
    ...
    def test_semantic_identity_excludes_ephemeral_host_path_execution_hashes(self):
        class VaryingExecutionRunner(FakeTwoDomainRunner):
            def run(self, **arguments):
                ...
                invocation_hash = authority.shake256_512_bytes(f"/tmp/random-invocation-{self.run_count}".encode("utf-8"))
                argv_hash = authority.shake256_512_bytes(f"--volume=/tmp/random-{self.run_count}:/input/source:ro".encode("utf-8"))
                unsigned = authority.signed_payload(execution.structured_result)
                unsigned["effective_sandbox_invocation_shake256_512"] = invocation_hash
                unsigned["normalized_executed_argv_shake256_512"] = argv_hash
                signed = authority.sign_record(unsigned, self.result_key)
                return replace(
                    execution, structured_result=signed,
                    effective_sandbox_invocation_shake256_512=invocation_hash,
                    normalized_executed_argv_shake256_512=argv_hash,
                )
        ...
        self.assertNotEqual(first["effective_sandbox_invocation_shake256_512"], second["effective_sandbox_invocation_shake256_512"])
    ...
    def test_request_api_cannot_supply_paths_hashes_environment_or_timestamp(self):
        ...
        forbidden = {"lake_executable", "lean_executable", "expected_lake_shake256_512", "created_at", "environment", "oci_runtime"}
    ...
    def test_bad_registry_unknown_profile_and_toolchain_mismatch_are_rejected(self):
        tampered = json.loads(json.dumps(self.registry)); tampered["profiles"][0]["lean_executable_shake256_512"] = "f" * 128
        with self.assertRaises(ValueError):
    ...
    def test_trusted_root_loader_rejects_group_write_symlink_hardlink_and_traversal(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "final private trust root"):
    ...
    def test_event_set_root_is_ordered_and_duplicate_safe(self):
        ...
        with self.assertRaises(ValueError): authority.authority_event_set_root([[1, "a" * 128], [1, "b" * 128]])
    ...
    def test_real_oci_command_binds_private_host_paths_and_complete_argv(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            seccomp_hash = authority.shake256_512_file(seccomp)
            runtime_hash = authority.shake256_512_file(runtime)
            record = profile_record(self.result_private)
            record["oci_runtime_shake256_512"] = runtime_hash
            ...
            runner = authority.OciSandboxRunner(
                ...
                seccomp_profile_path=seccomp, expected_seccomp_profile_shake256_512=seccomp_hash,
            ...
            logical = ({"source": "snapshot:" + "a" * 128, "host_path": "/tmp/random-a", "destination": "/input/source", "mode": "ro", "domain": "untrusted_compilation", "purpose": "sealed_source", "lifecycle": "request"},)
        ...
        self.assertEqual(invocation.shake256_512, repeated.shake256_512)
        self.assertNotEqual(invocation.shake256_512, changed.shake256_512)
