...
A governed rotation appends a signed terminal `cache_audit_segment_seal/v1` record. The successor segment binds the predecessor terminal-record SHAKE256-512 and cannot start an unrelated chain. Production configuration requires the checkpoint to reside outside the mutable log directory. External WORM or transparency-log anchoring remains recommended for activation deployments.
