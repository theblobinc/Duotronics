...
class FormalObserverFleet:
    ...
    def lean_check(self, file_path: str, timeout_s: int = 30) -> dict[str, Any]:
        ...
        payload["file_digest"] = shake256_ref(path.read_text(errors="ignore"))
    ...
    def tla_check(self, spec_path: str, config_path: str | None = None, timeout_s: int = 60) -> dict[str, Any]:
        ...
        if not spec.exists():
            ...
            return {"ok": False, "witness": witness}
        payload["spec_digest"] = shake256_ref(spec.read_text(errors="ignore"))
        if config_path and Path(config_path).exists():
            payload["config_digest"] = shake256_ref(Path(config_path).read_text(errors="ignore"))
