...
def test_operation_plan_is_read_only_and_routes_code_goal():
    ...
    assert plan["plan_digest"].startswith("duoid:shake256-512:")
...
def test_operation_plan_http_and_mcp_surface_names_are_wired():
    ...
    assert '"required": ["goal"]' in mcp
    assert '"max_candidates": {"type": "integer", "minimum": 1, "maximum": 32, "default": 6}' in mcp
