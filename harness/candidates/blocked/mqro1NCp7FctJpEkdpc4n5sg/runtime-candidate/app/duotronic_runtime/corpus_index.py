...
class PostgresCorpusBackend:
    ...
    Content is SHA-256-addressed and shared across releases. Corpus releases are
    ...
    def import_release(self, root: Path, *, release_name: str | None=None, activate: bool=False) -> ImportStats:
        ...
        for path in sorted(root.rglob('*')):
            ...
            file_count += 1
            byte_count += size
            release_hasher.update(rel.encode('utf-8',errors='ignore')+b'\0'+str(size).encode()+b'\0'+digest.encode()+b'\n')
            ...
            if indexable and text.strip():
                for idx,(start,end,content) in enumerate(_chunk_text(text)):
                    new_chunks.append((digest,idx,_sha256_bytes(content.encode('utf-8')),start,end,content))
                    chunk_count += 1

        manifest_ref=canonical_manifest_ref or manifest_ref
        release_digest='sha256:'+release_hasher.hexdigest()
        metadata={
