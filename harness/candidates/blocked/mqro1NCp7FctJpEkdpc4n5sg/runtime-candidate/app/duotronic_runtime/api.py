...
def _compact_wgrnn_chat_messages(messages: list[ChatMessage]) -> list[ChatMessage]:
    """Remove client-framework prompt bloat before WG-RNN adds its own context.

    Morphic/LibreChat may send multi-kilobyte system prompts and tool-framework
    instructions. WG-RNN already constructs its own governed system/evidence
    prompt, so replaying those large wrappers into the provider duplicates
    context and can serialize the interactive Ollama lane for tens of seconds.
    Keep recent semantic conversation while bounding caller system guidance.
    """
    ...
    for msg in reversed(messages):
        ...
        if role == "system":
            if kept_system:
                continue
            kept_system = True
            # Large framework prompts are not user conversation. Preserve only
            # compact caller guidance; drop oversized agent boilerplate entirely.
        ...
        text = text[:budget]
        budget -= len(text)
        selected.append(ChatMessage(role=role, content=text))
    selected.reverse()
    return selected
...
def _wgrnn_chat_model_candidates(kernel: RuntimeKernel, *, needs_vision: bool = False) -> list[dict[str, Any]]:
...
def _select_wgrnn_chat_model(kernel: RuntimeKernel, *, needs_vision: bool = False) -> dict[str, Any]:
    return _wgrnn_chat_model_candidates(kernel, needs_vision=needs_vision)[0]
    ...
    async def _stream_chat_completions(req: ChatCompletionRequest, prompt: str):
        ...
        if req.model and req.model.startswith("wg-rnn:"):
            ...
            if mode == "chat":
                ...
                model_candidates = _wgrnn_chat_model_candidates(kernel, needs_vision=needs_vision)
                ...
                chat_messages = prepared.get("system_messages", []) + [
                    {"role": "user", "content": prepared.get("response_prompt") or raw_prompt}
                ]
                ollama_options = {
    ...
    async def chat_completions(
        ...
        if req.model and req.model.startswith("wg-rnn:"):
            ...
            if mode == "chat":
                ...
                chat_messages = prepared.get("system_messages", []) + [
                    {"role": "user", "content": prepared.get("response_prompt") or compact_prompt}
                ]
                ollama_options = {
                    "temperature": req.temperature if req.temperature is not None else 0.15,
                    "top_p": req.top_p if req.top_p is not None else 0.85,
