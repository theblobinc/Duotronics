...
def conn():
    ...
    CREATE TABLE IF NOT EXISTS files(relpath TEXT PRIMARY KEY,size INTEGER,mtime_ns INTEGER,category TEXT,priority INTEGER,stage TEXT DEFAULT 'new',sha256 TEXT,artifact_id TEXT,last_error TEXT,attempts INTEGER DEFAULT 0,discovered_ms INTEGER,processed_ms INTEGER);
...
def process(c,row):
    ...
    art=(result.get('artifact') or {}); aid=art.get('artifact_id'); sd=art.get('source_digest') or sha(p)
    ...
    c.execute("UPDATE files SET sha256=?,artifact_id=?,stage='done',processed_ms=?,last_error=NULL,training_stage='pending',training_next_chunk=0,training_chunk_count=NULL,training_attempts=0,training_error=NULL,trained_ms=NULL WHERE relpath=?",(sd,aid,int(time.time()*1000),rel))
    for typ,val in refs: c.execute('INSERT OR IGNORE INTO refs(ref_type,ref_value,relpath) VALUES(?,?,?)',(typ,val,rel))
    c.commit()
    dups=c.execute("SELECT relpath FROM files WHERE sha256=? AND stage='done' ORDER BY relpath",(sd,)).fetchall()
