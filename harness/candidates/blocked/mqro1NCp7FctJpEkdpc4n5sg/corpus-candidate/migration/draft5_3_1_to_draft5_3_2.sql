...
ALTER TABLE wc_lean_compiler_witnesses_v2 ADD COLUMN generated_witness_module_shake256_512 TEXT;
...
ALTER TABLE wc_lean_compiler_witnesses_v2 ADD COLUMN compiler_executable_shake256_512 TEXT;
...
CREATE TABLE wc_verifier_keys_v3 (
  ...
  signature_algorithm TEXT NOT NULL CHECK (signature_algorithm = 'ML-DSA-87'),
...
BEGIN
  ...
  SELECT CASE WHEN NOT EXISTS (
    ...
    WHERE p.verifier_principal_id = NEW.verifier_principal_id
      ...
      AND p.public_key_fingerprint_shake256_512 = NEW.public_key_fingerprint_shake256_512
      AND p.status = 'active'
...
SELECT
  ...
  k.public_key_fingerprint_shake256_512,
  k.valid_from,
  k.valid_until
...
BEGIN
  SELECT CASE WHEN wc_shake256_512(NEW.signed_payload_canonical_json) <> NEW.signed_payload_shake256_512
  ...
  SELECT CASE WHEN NOT EXISTS (
    ...
    WHERE l.lean_compiler_witness_id = NEW.record_id
      ...
      AND l.signed_payload_shake256_512 = NEW.signed_payload_shake256_512
      ...
      AND json_extract(NEW.signed_payload_canonical_json, '$.source_tree_shake256_512') = l.source_tree_shake256_512
      AND json_extract(NEW.signed_payload_canonical_json, '$.lakefile_shake256_512') = l.lakefile_shake256_512
      AND json_extract(NEW.signed_payload_canonical_json, '$.generated_witness_module_shake256_512') = l.generated_witness_module_shake256_512
      ...
      AND json_extract(NEW.signed_payload_canonical_json, '$.compiler_executable_shake256_512') = l.compiler_executable_shake256_512
      AND json_extract(NEW.signed_payload_canonical_json, '$.build_output_shake256_512') = l.build_output_shake256_512
...
BEGIN
  ...
  SELECT CASE WHEN NOT EXISTS (
    ...
    WHERE p.proof_witness_id = NEW.record_id
      ...
      AND p.signed_payload_shake256_512 = NEW.signed_payload_shake256_512
      ...
      AND json_extract(NEW.signed_payload_canonical_json, '$.claim_content_shake256_512') = p.claim_content_shake256_512
      AND json_extract(NEW.signed_payload_canonical_json, '$.theorem_statement_shake256_512') = p.theorem_statement_shake256_512
      AND json_extract(NEW.signed_payload_canonical_json, '$.proof_artifact_shake256_512') = p.proof_artifact_shake256_512
      ...
      AND wc_ml_dsa_87_verify(k.public_key_base64url, NEW.signed_payload_canonical_json, p.signature) = 1
...
BEGIN
  SELECT CASE WHEN NOT EXISTS (
    ...
    WHERE c.claim_id = NEW.claim_id
      ...
      AND c.claim_content_shake256_512 = NEW.claim_content_shake256_512
      AND c.theorem_statement_shake256_512 = NEW.theorem_statement_shake256_512
      AND p.claim_id = NEW.claim_id AND p.decision = 'allow' AND p.scope = 'theorem_promotion'
      AND lc.claim_id = NEW.claim_id
      AND lc.claim_content_shake256_512 = NEW.claim_content_shake256_512
      AND lc.theorem_statement_shake256_512 = NEW.theorem_statement_shake256_512
      ...
      AND lc.generated_witness_module_shake256_512 IS NOT NULL
      AND lc.generated_witness_module_path = lc.exact_build_target
      AND lc.compiler_executable_shake256_512 IS NOT NULL
      AND lc.verifier_principal_id = NEW.verifier_principal_id
      AND lc.key_id = NEW.key_id
      AND pw.claim_id = NEW.claim_id
      AND pw.claim_content_shake256_512 = NEW.claim_content_shake256_512
      AND pw.theorem_statement_shake256_512 = NEW.theorem_statement_shake256_512
...
SELECT
  ...
  g.claim_id,
  g.claim_content_shake256_512,
  g.theorem_statement_shake256_512,
  g.proof_witness_id,
