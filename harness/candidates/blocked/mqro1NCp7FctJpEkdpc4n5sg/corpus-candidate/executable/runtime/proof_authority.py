#!/usr/bin/env python3
"""Fail-closed Lean proof authority for Witness Contract Draft 5.3.17.
...
SHAKE256_512_HEX = re.compile(r"^[0-9a-f]{128}$")
IMAGE_DIGEST = re.compile(r"^shake256-512:[0-9a-f]{128}$")
...
def shake256_512_bytes(value: bytes) -> str:
...
def load_private_key(path: Path) -> MLDSA87PrivateKey:
    return MLDSA87PrivateKey.from_private_bytes(path.read_bytes())


def load_public_key(path: Path) -> MLDSA87PublicKey:
    return MLDSA87PublicKey.from_public_bytes(path.read_bytes())


def _private_key_from_bytes(data: bytes) -> MLDSA87PrivateKey:
    return MLDSA87PrivateKey.from_private_bytes(data)


def _public_key_from_bytes(data: bytes) -> MLDSA87PublicKey:
    return MLDSA87PublicKey.from_public_bytes(data)


def public_key_raw_b64url(key: MLDSA87PublicKey) -> str:
    raw = key.public_bytes(serialization.Encoding.Raw, serialization.PublicFormat.Raw)
    return _b64url(raw)


def public_key_fingerprint(key: MLDSA87PublicKey) -> str:
    raw = key.public_bytes(serialization.Encoding.Raw, serialization.PublicFormat.Raw)
    return shake256_512_bytes(raw)


def signed_payload(record: dict[str, Any]) -> dict[str, Any]:
    return {key: value for key, value in record.items() if key not in {"signed_payload_shake256_512", "signature"}}
...
def sign_record(record: dict[str, Any], private_key: MLDSA87PrivateKey) -> dict[str, Any]:
    unsigned = signed_payload(record)
    payload = canonical_bytes(unsigned)
    result = dict(unsigned)
    result["signed_payload_shake256_512"] = shake256_512_bytes(payload)
    result["signature"] = _b64url(private_key.sign(payload))
    return result


def verify_record(record: dict[str, Any], public_key: MLDSA87PublicKey) -> bool:
    try:
        payload = canonical_bytes(signed_payload(record))
        if record.get("signed_payload_shake256_512") != shake256_512_bytes(payload):
            return False
...
def authority_event_set_root(events: list[list[Any]] | list[tuple[Any, ...]]) -> str:
    ...
    if any(not SHAKE256_512_HEX.fullmatch(item[1]) for item in normalized):
        raise ValueError("invalid authority event payload digest")
    return shake256_512_bytes(canonical_bytes(normalized))


def register_sqlite_crypto_functions(connection: sqlite3.Connection) -> None:
    """Register fail-closed canonicalization, event-root, and ML-DSA-87 functions."""

    def sql_shake256_512(value: Any) -> str | None:
        return shake256_512_bytes(value.encode("utf-8")) if isinstance(value, str) else None

    def sql_key_fingerprint(public_key_b64url: Any) -> str | None:
        try:
            return shake256_512_bytes(_b64url_decode(str(public_key_b64url)))
        except (TypeError, ValueError):
            return None

    def sql_ml_dsa_87_verify(public_key_b64url: Any, payload: Any, signature: Any) -> int:
        try:
            key = MLDSA87PublicKey.from_public_bytes(_b64url_decode(str(public_key_b64url)))
    ...
    connection.create_function("wc_shake256_512", 1, sql_shake256_512, deterministic=True)
    connection.create_function("wc_public_key_fingerprint", 1, sql_key_fingerprint, deterministic=True)
    connection.create_function("wc_ml_dsa_87_verify", 3, sql_ml_dsa_87_verify, deterministic=True)
...
class CompilerProfile:
    ...
    oci_runtime_shake256_512: str
    oci_runtime_version: str
    lake_executable_shake256_512: str
    lean_executable_shake256_512: str
    lean_stdlib_tree_shake256_512: str
    dependency_closure_shake256_512: str
    verifier_executable_shake256_512: str
    verifier_source_revision: str
    verifier_build_attestation_id: str
    sandbox_policy_shake256_512: str
    ...
    def from_record(cls, record: dict[str, Any]) -> "CompilerProfile":
        digest_fields = (
            "oci_runtime_shake256_512", "lake_executable_shake256_512", "lean_executable_shake256_512",
            "lean_stdlib_tree_shake256_512", "dependency_closure_shake256_512",
            "verifier_executable_shake256_512", "sandbox_policy_shake256_512",
        )
        if any(not SHAKE256_512_HEX.fullmatch(str(record.get(name, ""))) for name in digest_fields):
            raise ValueError("compiler profile contains an invalid SHAKE256-512 digest")
        ...
        if len(public_raw) != 2592:
            raise ValueError("compiler profile verifier-result key is not ML-DSA-87")
        return cls(
            ...
            oci_runtime_shake256_512=record["oci_runtime_shake256_512"], oci_runtime_version=record["oci_runtime_version"],
            lake_executable_shake256_512=record["lake_executable_shake256_512"], lean_executable_shake256_512=record["lean_executable_shake256_512"],
            lean_stdlib_tree_shake256_512=record["lean_stdlib_tree_shake256_512"], dependency_closure_shake256_512=record["dependency_closure_shake256_512"],
            verifier_executable_shake256_512=record["verifier_executable_shake256_512"], verifier_source_revision=record["verifier_source_revision"],
            verifier_build_attestation_id=record["verifier_build_attestation_id"], sandbox_policy_shake256_512=record["sandbox_policy_shake256_512"],
    ...
    @property
    def verifier_result_public_key(self) -> MLDSA87PublicKey:
        return MLDSA87PublicKey.from_public_bytes(_b64url_decode(self.verifier_result_public_key_base64url))
    ...
    def as_verifier_record(self) -> dict[str, Any]:
        ...
        return {
            ...
            "oci_runtime_shake256_512": self.oci_runtime_shake256_512,
            "oci_runtime_version": self.oci_runtime_version,
            "lake_executable_shake256_512": self.lake_executable_shake256_512,
            "lean_executable_shake256_512": self.lean_executable_shake256_512,
            "lean_stdlib_tree_shake256_512": self.lean_stdlib_tree_shake256_512,
            "dependency_closure_shake256_512": self.dependency_closure_shake256_512,
            "verifier_executable_shake256_512": self.verifier_executable_shake256_512,
            "verifier_source_revision": self.verifier_source_revision,
            "verifier_build_attestation_id": self.verifier_build_attestation_id,
            "sandbox_policy_shake256_512": self.sandbox_policy_shake256_512,
...
def verify_governed_registry(record: dict[str, Any], governance_key: MLDSA87PublicKey) -> dict[str, CompilerProfile]:
...
def verify_trusted_artifact_registry(
    record: dict[str, Any], governance_key: MLDSA87PublicKey, *,
    ...
    for artifact in artifacts:
        if not isinstance(artifact, dict) or set(artifact) != {"artifact_id", "role", "verification_mode", "relative_path", "shake256_512", "attestation_id"}:
        ...
        if not SHAKE256_512_HEX.fullmatch(str(artifact["shake256_512"])):
        ...
        if artifact["verification_mode"] == "direct_trusted_root_file":
            ...
            if shake256_512_bytes(data) != artifact["shake256_512"]:
    ...
    return shake256_512_bytes(canonical_bytes(record))
...
class ResolvedPolicyDecision:
    ...
    canonical_record_shake256_512: str
...
class ProofPolicyResolver:
    ...
    def __init__(
        self, registry: dict[str, Any], governance_key: MLDSA87PublicKey, *,
        ...
        for decision in decisions:
            ...
            canonical_hash = shake256_512_bytes(canonical_bytes(decision))
            if decision.get("canonical_record_shake256_512") != canonical_hash:
                # The hash field itself is intentionally excluded to avoid recursion.
                without_hash = {key: value for key, value in decision.items() if key != "canonical_record_shake256_512"}
                canonical_hash = shake256_512_bytes(canonical_bytes(without_hash))
                if decision.get("canonical_record_shake256_512") != canonical_hash:
    ...
    def resolve(
        ...
        without_hash = {key: value for key, value in decision.items() if key != "canonical_record_shake256_512"}
        observed_hash = shake256_512_bytes(canonical_bytes(without_hash))
        if observed_hash != decision["canonical_record_shake256_512"]:
...
class EffectiveResourceLimits:
    ...
    @property
    def shake256_512(self) -> str:
        return shake256_512_bytes(canonical_bytes(self.as_dict()))
...
def _tree_records(root: Path) -> list[dict[str, Any]]:
    ...
    for path in sorted((item for item in root.rglob("*") if item.is_file()), key=lambda item: item.relative_to(root).as_posix()):
        ...
        records.append({
            ...
            "shake256_512": shake256_512_bytes(data),
            "executable": bool(info.st_mode & (stat.S_IXUSR | stat.S_IXGRP | stat.S_IXOTH)),
        })
    return records


def content_tree_shake256_512(source_root: Path) -> str:
    ...
    for relative in _source_relatives(source_root):
        ...
        records.append({"path": relative.as_posix(), "size": len(data), "shake256_512": shake256_512_bytes(data), "executable": False})
    return shake256_512_bytes(canonical_bytes(records))
...
def create_immutable_snapshot(
    ...
    *,
    ...
    try:
        for relative, expected_identity in enumerated:
            ...
            try:
                ...
                digest = hashlib.shake_256(); copied = 0
                try:
                    while True:
                        _check_snapshot_deadline(started, selected)
                ...
                source_hashes[relative.as_posix()] = digest.hexdigest(64)
            finally:
                os.close(source_fd)
            if after_file_copy:
        ...
        for relative, expected_identity in enumerated:
            ...
            try:
                ...
                os.close(file_fd)
            if _source_identity(before) != _source_identity(after) or digest.hexdigest(64) != source_hashes[relative.as_posix()]:
    ...
    tree_hash = shake256_512_bytes(canonical_bytes(records))
    verify_sealed_snapshot(destination, tree_hash)
    return ImmutableSnapshot(destination, f"shake256-512:{tree_hash}", tree_hash, tuple(records))


def verify_sealed_snapshot(root: Path, expected_tree_shake256_512: str) -> None:
    ...
    observed = shake256_512_bytes(canonical_bytes(_tree_records(root)))
    if observed != expected_tree_shake256_512:
...
class EffectiveSandboxInvocation:
    ...
    oci_runtime_shake256_512: str
    oci_runtime_version: str
    oci_runtime_version_output_shake256_512: str
    image_reference: str
    image_digest: str
    entrypoint: str
    arguments: tuple[str, ...]
    ...
    seccomp_profile_shake256_512: str
    ...
    def as_dict(self) -> dict[str, Any]:
        ...
        record["normalized_executed_argv_shake256_512"] = shake256_512_bytes(canonical_bytes(argv))
        return record
...
class SandboxExecution:
    ...
    effective_sandbox_invocation_shake256_512: str = ""
    ...
    normalized_executed_argv_shake256_512: str = ""
...
class AtomicResultPublisher:
    ...
    def publish(self, record: dict[str, Any], signing_key: MLDSA87PrivateKey) -> Path:
        self._validate_directory()
    ...
    def publish_unsigned(self, record: dict[str, Any]) -> Path:
        ...
        nonce = shake256_512_bytes(data)[:24]
...
class OciSandboxRunner:
    ...
    def __init__(
        self, runtime_executable: Path, expected_runtime_shake256_512: str, expected_runtime_version: str, *,
        verifier_result_signing_key: MLDSA87PrivateKey, authority_uid: int | None = None,
        authority_gid: int | None = None, seccomp_profile_path: Path | None = None,
        expected_seccomp_profile_shake256_512: str | None = None,
        ...
        if not runtime.is_file() or not os.access(runtime, os.X_OK):
            raise ValueError("configured OCI runtime must be an absolute non-symlink executable")
        if not SHAKE256_512_HEX.fullmatch(expected_runtime_shake256_512) or shake256_512_file(runtime) != expected_runtime_shake256_512:
        ...
        if seccomp_profile_path is None or expected_seccomp_profile_shake256_512 is None:
        ...
        if seccomp.stat().st_mode & (stat.S_IWGRP | stat.S_IWOTH) or shake256_512_file(seccomp) != expected_seccomp_profile_shake256_512:
        ...
        self.runtime_shake256_512 = expected_runtime_shake256_512
        self.runtime_version = normalized_version
        self.runtime_version_output_shake256_512 = shake256_512_bytes(normalized_version.encode("utf-8"))
        ...
        self.seccomp_profile_shake256_512 = expected_seccomp_profile_shake256_512
    ...
    def _invocation(
        ...
        invocation = EffectiveSandboxInvocation(
            ...
            oci_runtime_shake256_512=self.runtime_shake256_512, oci_runtime_version=self.runtime_version,
            oci_runtime_version_output_shake256_512=self.runtime_version_output_shake256_512,
            ...
            security_options=("no-new-privileges",), seccomp_profile_shake256_512=self.seccomp_profile_shake256_512,
            ...
            immutable_policy_files=({"path": str(self.seccomp_profile_path), "shake256_512": self.seccomp_profile_shake256_512},),
        )
    ...
    def _runtime_control_evidence(self, invocation: EffectiveSandboxInvocation, inspected: dict[str, Any]) -> tuple[tuple[str, ...], tuple[dict[str, Any], ...]]:
        ...
        evidence = tuple({
            ...
            "observation_shake256_512": shake256_512_bytes(canonical_bytes({"control": control, "inspection": inspected})),
    ...
    def _domain_execution_evidence(
        ...
        *, budget_seconds: int, remaining_at_start_ms: int, elapsed_ms: int,
        ...
        record = {
            ...
            "invocation_shake256_512": invocation.shake256_512,
            "normalized_executed_argv": list(command),
            "normalized_executed_argv_shake256_512": shake256_512_bytes(canonical_bytes(list(command))),
    ...
    def run(self, *, profile: CompilerProfile, snapshot_root: Path, generated_module: Path, request: dict[str, Any], timeout_seconds: int) -> SandboxExecution:
        if profile.oci_runtime_shake256_512 != self.runtime_shake256_512 or profile.oci_runtime_version != self.runtime_version:
        ...
        verify_sealed_snapshot(snapshot_root, request["immutable_snapshot_tree_shake256_512"])
        ...
        with tempfile.TemporaryDirectory(prefix="wc-handoff-") as handoff_name, tempfile.TemporaryDirectory(prefix="wc-control-") as control_name, tempfile.TemporaryDirectory(prefix="wc-inspection-parent-") as inspection_parent_name, tempfile.TemporaryDirectory(prefix="wc-result-parent-") as result_parent_name:
            ...
            compile_manifest = (
                {"source": f"snapshot:{request['immutable_snapshot_tree_shake256_512']}", "host_path": str(snapshot_root), "destination": "/input/source", "mode": "ro", "domain": "untrusted_compilation", "purpose": "sealed_source", "lifecycle": "request"},
                {"source": f"generated:{request['generated_binding_module_shake256_512']}", "host_path": str(generated_root), "destination": "/input/generated", "mode": "ro", "domain": "untrusted_compilation", "purpose": "generated_type_binding", "lifecycle": "request"},
            ...
            if compiled.timed_out or compiled.output_limit_exceeded or compiled.returncode != 0:
                return SandboxExecution(
                    ...
                    effective_sandbox_invocation_shake256_512=compile_invocation.shake256_512,
                    ...
                    stdout_shake256_512=compiled.stdout_shake256_512, stderr_shake256_512=compiled.stderr_shake256_512,
                    emitted_controls=compile_invocation.emitted_controls,
                    normalized_executed_argv=untrusted_command,
                    normalized_executed_argv_shake256_512=shake256_512_bytes(canonical_bytes(list(untrusted_command))),
            ...
            verifier_manifest = (
                {"source": f"snapshot:{request['immutable_snapshot_tree_shake256_512']}", "host_path": str(snapshot_root), "destination": "/input/source", "mode": "ro", "domain": "trusted_inspection", "purpose": "sealed_source", "lifecycle": "request"},
                {"source": f"generated:{request['generated_binding_module_shake256_512']}", "host_path": str(generated_root), "destination": "/input/generated", "mode": "ro", "domain": "trusted_inspection", "purpose": "generated_type_binding", "lifecycle": "request"},
            ...
            if verified.timed_out or verified.output_limit_exceeded:
                return SandboxExecution(
                    ...
                    verified.timed_out, untrusted_command, trusted_command, verifier_invocation.shake256_512,
                    ...
                    stdout_shake256_512=verified.stdout_shake256_512, stderr_shake256_512=verified.stderr_shake256_512,
                    emitted_controls=verifier_invocation.emitted_controls,
                    normalized_executed_argv=trusted_command,
                    normalized_executed_argv_shake256_512=shake256_512_bytes(canonical_bytes(list(trusted_command))),
            ...
            if inspection is not None:
                topology_root = shake256_512_bytes(canonical_bytes({
                ...
                derived_evidence = [
                    {"control": "result_channel_isolated", "status": "derived_pass", "source": "sealed_execution_topology", "observation_shake256_512": topology_root},
                    {"control": "trusted_inspection_only", "status": "derived_pass", "source": "sealed_execution_topology", "observation_shake256_512": topology_root},
                ]
            ...
            return SandboxExecution(
                ...
                effective_sandbox_invocation_shake256_512=verifier_invocation.shake256_512,
                security_properties=security_properties,
                requested_controls=verifier_invocation.requested_controls,
                applied_controls=applied_controls,
                verified_controls=measured_controls, output_limit_exceeded=False,
                stdout_shake256_512=shake256_512_bytes((compiled.stdout + verified.stdout).encode("utf-8")),
                stderr_shake256_512=shake256_512_bytes((compiled.stderr + verified.stderr).encode("utf-8")),
...
class ProofAuthorityService:
    def __init__(
        self, *, governed_registry: dict[str, Any], governance_public_key: MLDSA87PublicKey,
        verifier_principal_id: str, key_id: str, signing_key: MLDSA87PrivateKey,
        ...
        schema_validator: CanonicalSchemaValidator | None = None,
        trust_root_attestation_registry_shake256_512: str | None = None,
    ):
        self.profiles = verify_governed_registry(governed_registry, governance_public_key)
        self.registry_shake256_512 = shake256_512_bytes(canonical_bytes(governed_registry))
        ...
        self.schema_validator = schema_validator or CanonicalSchemaValidator(Path(__file__).resolve().parents[2] / "schemas")
        self.trust_root_attestation_registry_shake256_512 = trust_root_attestation_registry_shake256_512 or shake256_512_bytes(canonical_bytes({"status": "portable_unattested"}))
    ...
    def verify(
        ...
        policy_decision_id: str, policy_decision_shake256_512: str,
        ...
        if not isinstance(policy_decision_id, str) or not policy_decision_id or not SHAKE256_512_HEX.fullmatch(policy_decision_shake256_512):
            raise AuthorityFailure("policy_decision_invalid", "resolved policy identity and canonical hash are required")
        claim_hash = shake256_512_bytes(canonical_bytes(canonical_claim)); theorem_hash = shake256_512_bytes(theorem_statement.encode("utf-8"))
        ...
        with tempfile.TemporaryDirectory(prefix="wc-snapshot-parent-") as snapshot_parent_name, tempfile.TemporaryDirectory(prefix="wc-generated-parent-") as generated_parent_name:
            ...
            artifact_hash = shake256_512_file(snapshot_artifact)
            ...
            metadata_hashes = {name: shake256_512_file(path) for name, path in required_metadata.items()}

            descriptor_seed = shake256_512_bytes(canonical_bytes({
                "snapshot": snapshot.tree_shake256_512, "artifact": artifact_hash, "claim": claim_hash,
                "theorem": theorem_hash, "profile": compiler_profile_id,
                "authenticated_principal_id": authenticated_principal_id,
                "source_bundle_id": source_bundle_id,
                "policy_decision_id": policy_decision_id, "policy_decision_shake256_512": policy_decision_shake256_512,
            }))
            ...
            generated_hash = shake256_512_file(generated_path)
            ...
            sandbox_template = {
                ...
                "trusted_inspection_mount": True, "runtime_shake256_512": profile.oci_runtime_shake256_512,
            }
            request = {
                ...
                "authenticated_principal_id": authenticated_principal_id,
                "source_bundle_id": source_bundle_id,
                "compiler_profile_id": compiler_profile_id, "claim_content_shake256_512": claim_hash,
                "policy_decision_id": policy_decision_id, "policy_decision_shake256_512": policy_decision_shake256_512,
                "theorem_statement_shake256_512": theorem_hash, "proof_artifact_relative_path": submitted_relative.as_posix(),
                "proof_artifact_shake256_512": artifact_hash, "proof_module": proof_module, "theorem_name": theorem_name,
                "immutable_snapshot_id": snapshot.snapshot_id, "immutable_snapshot_tree_shake256_512": snapshot.tree_shake256_512,
                "lakefile_shake256_512": metadata_hashes["lakefile.lean"], "lake_manifest_shake256_512": metadata_hashes["lake-manifest.json"],
                "lean_toolchain_shake256_512": metadata_hashes["lean-toolchain"],
                "generated_binding_module_shake256_512": generated_hash, "generated_binding_module_path": generated_relative,
                ...
                "sandbox_policy_shake256_512": profile.sandbox_policy_shake256_512, "sandbox_template_shake256_512": shake256_512_bytes(canonical_bytes(sandbox_template)),
                "effective_resource_limits": limits.as_dict(), "effective_resource_limits_shake256_512": limits.shake256_512,
            }
            self.schema_validator.validate("verifier_request", request)
            request_hash = shake256_512_bytes(canonical_bytes(request))
            verify_sealed_snapshot(snapshot.root, snapshot.tree_shake256_512)
            remaining = deadline - time.monotonic()
            if remaining <= 0:
        ...
        structured_payload_hash = structured.get("signed_payload_shake256_512", shake256_512_bytes(canonical_bytes({}))) if structured else shake256_512_bytes(canonical_bytes({}))
        diagnostic_hash = shake256_512_bytes(canonical_bytes({"returncode": execution.returncode, "timed_out": execution.timed_out, "stdout": execution.stdout, "stderr": execution.stderr}))
        semantic_result = {
            key: structured.get(key) if structured else None
            for key in (
                "status", "compiler_profile_id", "claim_content_shake256_512", "theorem_statement_shake256_512",
                "policy_decision_id", "policy_decision_shake256_512",
                "proof_artifact_shake256_512", "immutable_snapshot_id", "immutable_snapshot_tree_shake256_512",
                "generated_binding_module_shake256_512", "theorem_declaration", "declaration_found",
                ...
                "axiom_set_shake256_512", "forbidden_axiom_set", "sorry_ax_present", "unsafe_dependency_present",
        ...
        semantic = {
            "claim": claim_hash, "theorem": theorem_hash, "artifact": artifact_hash, "snapshot": snapshot.tree_shake256_512,
            "generated": generated_hash, "profile": compiler_profile_id,
            "source_bundle_id": source_bundle_id,
            "authenticated_principal_id": authenticated_principal_id,
            "policy_decision_id": policy_decision_id, "policy_decision_shake256_512": policy_decision_shake256_512,
            "registry": self.registry_shake256_512,
            "semantic_result_shake256_512": shake256_512_bytes(canonical_bytes(semantic_result)),
        }
        semantic_id = "lean-semantic:" + shake256_512_bytes(canonical_bytes(semantic))
        execution_evidence = {
            ...
            "service_request_id": service_request_id,
            "generated_binding_request_shake256_512": request_hash,
            "verifier_result_payload_shake256_512": structured_payload_hash,
            "effective_sandbox_invocation_shake256_512": execution.effective_sandbox_invocation_shake256_512,
            "normalized_executed_argv_shake256_512": execution.normalized_executed_argv_shake256_512,
            "handoff_manifest_shake256_512": structured.get("handoff_manifest_shake256_512") if structured else None,
            "oci_runtime_version_output_shake256_512": structured.get("oci_runtime_version_output_shake256_512") if structured else None,
            "effective_resource_limits_shake256_512": limits.shake256_512,
            ...
            "diagnostic_shake256_512": diagnostic_hash,
        }
        execution_evidence_id = "lean-execution:" + shake256_512_bytes(canonical_bytes(execution_evidence))
        ...
        execution_evidence_id = "lean-execution:" + shake256_512_bytes(canonical_bytes(execution_evidence))
        unsigned = {
            "schema_version": "lean_compiler_witness/v8", "lean_compiler_witness_id": "lean:" + shake256_512_bytes(canonical_bytes(semantic))[:32],
            ...
            "authenticated_principal_id": authenticated_principal_id,
            "source_bundle_id": source_bundle_id,
            "claim_id": claim_id, "claim_content_shake256_512": claim_hash,
            "theorem_statement_shake256_512": theorem_hash, "submitted_source_locator": "caller-bundle-redacted",
            "policy_decision_id": policy_decision_id, "policy_decision_shake256_512": policy_decision_shake256_512,
            "immutable_snapshot_id": snapshot.snapshot_id, "immutable_snapshot_tree_shake256_512": snapshot.tree_shake256_512,
            "proof_artifact_relative_path": submitted_relative.as_posix(), "proof_artifact_shake256_512": artifact_hash,
            "lakefile_shake256_512": metadata_hashes["lakefile.lean"], "lake_manifest_shake256_512": metadata_hashes["lake-manifest.json"],
            "lean_toolchain_shake256_512": metadata_hashes["lean-toolchain"], "generated_binding_module_shake256_512": generated_hash,
            "generated_binding_request_shake256_512": request_hash, "generated_binding_module_path": generated_relative,
            "compiler_profile_id": compiler_profile_id, "compiler_registry_shake256_512": self.registry_shake256_512,
            "verifier_executable_shake256_512": profile.verifier_executable_shake256_512, "lean_executable_shake256_512": profile.lean_executable_shake256_512,
            "lake_executable_shake256_512": profile.lake_executable_shake256_512, "lean_stdlib_tree_shake256_512": profile.lean_stdlib_tree_shake256_512,
            "dependency_closure_shake256_512": profile.dependency_closure_shake256_512, "oci_image_digest": profile.oci_image_digest,
            "oci_runtime_shake256_512": profile.oci_runtime_shake256_512, "oci_runtime_version": profile.oci_runtime_version,
            "oci_runtime_version_output_shake256_512": structured.get("oci_runtime_version_output_shake256_512", shake256_512_bytes(b"")) if structured else shake256_512_bytes(b""),
            "sandbox_policy_shake256_512": profile.sandbox_policy_shake256_512,
            "sandbox_template_shake256_512": request["sandbox_template_shake256_512"],
            "effective_sandbox_invocation_shake256_512": execution.effective_sandbox_invocation_shake256_512,
            "normalized_executed_argv_shake256_512": structured.get("normalized_executed_argv_shake256_512", shake256_512_bytes(canonical_bytes([]))) if structured else shake256_512_bytes(canonical_bytes([])),
            "handoff_manifest_shake256_512": structured.get("handoff_manifest_shake256_512", shake256_512_bytes(canonical_bytes({}))) if structured else shake256_512_bytes(canonical_bytes({})),
            "effective_resource_limits": limits.as_dict(), "effective_resource_limits_shake256_512": limits.shake256_512,
            "domain_file_size_limit": execution.domain_file_size_limit,
            "trust_root_attestation_registry_shake256_512": self.trust_root_attestation_registry_shake256_512,
            "verifier_result_payload_shake256_512": structured_payload_hash,
            ...
            "expected_type_expression_hash": structured.get("expected_type_expression_hash", shake256_512_bytes(b"")) if structured else shake256_512_bytes(b""),
            "actual_type_expression_hash": structured.get("actual_type_expression_hash", shake256_512_bytes(b"")) if structured else shake256_512_bytes(b""),
            ...
            "axiom_set_shake256_512": shake256_512_bytes(canonical_bytes(axioms)), "direct_dependencies": structured.get("direct_dependencies", []) if structured_valid and structured else [],
            "dependency_closure": structured.get("dependency_closure", []) if structured_valid and structured else [],
            "transitive_dependencies_root": structured.get("transitive_dependencies_root", shake256_512_bytes(b"")) if structured_valid and structured else shake256_512_bytes(b""),
            ...
            "build_output_shake256_512": diagnostic_hash, "stdout_shake256_512": execution.stdout_shake256_512 or shake256_512_bytes(execution.stdout.encode("utf-8")),
            "stderr_shake256_512": execution.stderr_shake256_512 or shake256_512_bytes(execution.stderr.encode("utf-8")),
            ...
            "signature_algorithm": "ML-DSA-87", "trusted_timestamp_source": self.timestamp_source, "created_at": now.isoformat(),
        }
    ...
    def _structured_result_valid(self, result: dict[str, Any] | None, request: dict[str, Any], request_hash: str, profile: CompilerProfile, execution: SandboxExecution) -> tuple[bool, str | None]:
        ...
        if result.get("warnings_as_errors") is not True or result.get("warning_as_error_cli_option") != "-DwarningAsError=true":
            return False, "verifier_result_invalid"
        exact = {
            "request_id": request["request_id"], "request_shake256_512": request_hash,
            "service_request_id": request["service_request_id"],
            "authenticated_principal_id": request["authenticated_principal_id"],
            "source_bundle_id": request["source_bundle_id"],
            "compiler_profile_id": profile.compiler_profile_id, "claim_content_shake256_512": request["claim_content_shake256_512"],
            "policy_decision_id": request["policy_decision_id"], "policy_decision_shake256_512": request["policy_decision_shake256_512"],
            "theorem_statement_shake256_512": request["theorem_statement_shake256_512"], "proof_artifact_shake256_512": request["proof_artifact_shake256_512"],
            "immutable_snapshot_id": request["immutable_snapshot_id"], "immutable_snapshot_tree_shake256_512": request["immutable_snapshot_tree_shake256_512"],
            "generated_binding_module_shake256_512": request["generated_binding_module_shake256_512"],
            "lake_executable_shake256_512": profile.lake_executable_shake256_512, "lean_executable_shake256_512": profile.lean_executable_shake256_512,
            "lean_stdlib_tree_shake256_512": profile.lean_stdlib_tree_shake256_512, "dependency_closure_shake256_512": profile.dependency_closure_shake256_512,
            "oci_image_digest": profile.oci_image_digest, "oci_runtime_shake256_512": profile.oci_runtime_shake256_512,
            "oci_runtime_version": profile.oci_runtime_version, "verifier_executable_shake256_512": profile.verifier_executable_shake256_512,
            "sandbox_policy_shake256_512": profile.sandbox_policy_shake256_512,
            "effective_sandbox_invocation_shake256_512": execution.effective_sandbox_invocation_shake256_512,
            "normalized_executed_argv_shake256_512": execution.normalized_executed_argv_shake256_512,
            "effective_resource_limits_shake256_512": request["effective_resource_limits_shake256_512"],
            "domain_file_size_limit": execution.domain_file_size_limit,
        }
        if any(result.get(key) != value for key, value in exact.items()):
            return False, "verifier_result_invalid"
        for field in ("expected_type_expression_hash", "actual_type_expression_hash", "transitive_dependencies_root", "axiom_set_shake256_512", "normalized_executed_argv_shake256_512", "handoff_manifest_shake256_512"):
            if not SHAKE256_512_HEX.fullmatch(str(result.get(field, ""))):
                return False, "verifier_result_invalid"
        for field in ("direct_dependencies", "dependency_closure", "axiom_set", "forbidden_axiom_set", "requested_controls", "emitted_controls", "accepted_controls", "applied_controls", "measured_controls", "derived_controls"):
            ...
            if not isinstance(values, list) or values != sorted(set(values)) or not all(isinstance(item, str) and item for item in values):
                return False, "verifier_result_invalid"
        if result["axiom_set_shake256_512"] != shake256_512_bytes(canonical_bytes(result["axiom_set"])):
            return False, "verifier_result_invalid"
...
def load_production_authority_service(config_root: Path = Path("/etc/witness-authority")) -> ProofAuthorityService:
    ...
    allowed = {
        ...
        "oci_runtime_shake256_512", "oci_runtime_version", "verifier_principal_id", "key_id", "authority_snapshot_id",
        "authority_ledger_high_water_sequence", "artifact_store_root", "policy_registry_file",
        "seccomp_profile_file", "seccomp_profile_shake256_512", "authority_uid", "authority_gid",
    ...
    trust_registry_shake256_512 = verify_trusted_artifact_registry(
    ...
    runner = OciSandboxRunner(
        runtime_path, config["oci_runtime_shake256_512"], config["oci_runtime_version"],
        ...
        expected_seccomp_profile_shake256_512=config["seccomp_profile_shake256_512"],
    ...
    return ProofAuthorityService(
        ...
        schema_validator=schema_validator,
        trust_root_attestation_registry_shake256_512=trust_registry_shake256_512,
    )
...
__all__ = [
    ...
    "content_tree_shake256_512", "create_immutable_snapshot", "generated_witness_module",
    "load_production_authority_service", "load_trusted_canonical_json", "public_key_fingerprint", "public_key_raw_b64url",
    "read_private_result", "register_sqlite_crypto_functions", "secure_read_bytes", "shake256_512_bytes",
