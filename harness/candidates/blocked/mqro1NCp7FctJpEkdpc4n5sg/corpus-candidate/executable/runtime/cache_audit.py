#!/usr/bin/env python3
"""Governed, authenticated cache-audit persistence for Draft 5.3.16.
...
from proof_authority import (
    ...
    public_key_raw_b64url, shake256_512_bytes, sign_record, verify_record,
)

IDENTIFIER = re.compile(r"^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$")
SHAKE256_512 = re.compile(r"^[0-9a-f]{128}$")
...
def socket_identity_shake256_512(path: Path) -> str:
    info = Path(path).lstat()
    identity = {
        "path": str(Path(path)), "device": info.st_dev, "inode": info.st_ino,
        "uid": info.st_uid, "gid": info.st_gid, "mode": stat.S_IMODE(info.st_mode),
        "type": "socket" if stat.S_ISSOCK(info.st_mode) else "other",
    }
    return shake256_512_bytes(canonical_bytes(identity))


def build_authenticated_service_request(
    *, operation: str, payload: dict[str, Any], peer_principal_id: str,
    request_signer_key_id: str, request_signing_key: MLDSA87PrivateKey,
    target_service: str, socket_path: Path, deadline_monotonic: float,
) -> dict[str, Any]:
    remaining = deadline_monotonic - time.monotonic()
    if remaining <= 0:
        raise TimeoutError("service request deadline expired before signing")
    unsigned = {
        "schema_version": "cache_audit_service_request/v1",
        "operation": operation,
        "request_id": "request:" + uuid.uuid4().hex,
        "peer_principal_id": peer_principal_id,
        "payload_shake256_512": shake256_512_bytes(canonical_bytes(payload)),
        "deadline_unix_ns": time.time_ns() + int(remaining * 1_000_000_000),
        "nonce": uuid.uuid4().hex,
        "target_service": target_service,
        "socket_identity_shake256_512": socket_identity_shake256_512(socket_path),
        "payload": payload,
        "request_signer_key_id": request_signer_key_id,
    }
    return sign_record(unsigned, request_signing_key)


def validate_governed_signing_registry(
    registry: dict[str, Any], governance_key: MLDSA87PublicKey, *,
    schema_validator: CanonicalSchemaValidator, surface: str, required_scope: str,
    evaluated_at: datetime,
) -> tuple[str, dict[str, dict[str, Any]], dict[str, MLDSA87PublicKey]]:
    """Validate a governance-signed lifecycle registry and all rotation edges."""
    ...
    registry_created = _parse_time(registry["created_at"], f"{surface}.created_at")
    digest = shake256_512_bytes(canonical_bytes(registry))
    records: dict[str, dict[str, Any]] = {}
    keys: dict[str, MLDSA87PublicKey] = {}
    import base64
    for record in registry["keys"]:
        ...
        if valid_until is not None and valid_until <= valid_from:
            raise RuntimeError(f"{surface} contains an empty key-validity interval")
        ...
        if registry_created < valid_from:
            raise RuntimeError(f"{surface} retroactively authorizes a key")
        ...
        try:
            key = MLDSA87PublicKey.from_public_bytes(base64.urlsafe_b64decode(padded.encode("ascii")))
        except Exception as error:
            raise RuntimeError(f"{surface} contains an invalid ML-DSA-87 public key") from error
    ...
    # Complete cycle detection, including cycles longer than two keys.
    for origin in records:
        seen: set[str] = set()
        current: str | None = origin
        while current is not None:
            if current in seen:
                raise RuntimeError(f"{surface} contains a rotation cycle")
            seen.add(current)
            current = records[current]["rotation_predecessor_key_id"]
    for key_id, record in records.items():
        predecessor = record["rotation_predecessor_key_id"]
        if predecessor is None:
            continue
        previous = records[predecessor]
        if previous["principal_id"] != record["principal_id"] or previous["authorization_scope"] != record["authorization_scope"]:
            raise RuntimeError(f"{surface} rotation changes principal or scope")
        if _parse_time(previous["valid_from"], "predecessor.valid_from") >= _parse_time(record["valid_from"], "successor.valid_from"):
            raise RuntimeError(f"{surface} rotation chronology is not increasing")
    active = []
    now = evaluated_at.astimezone(timezone.utc)
    for record in records.values():
        evidence = key_validity_evidence(registry_shake256_512=digest, record=record, evaluated_at=now)
        if evidence["decision"] == "active_time_valid_rotation_valid":
            active.append(record["key_id"])
    if len(active) != 1:
        raise RuntimeError(f"{surface} must contain exactly one active key for its operational role")
    return digest, records, keys


def key_validity_evidence(
    *, registry_shake256_512: str, record: dict[str, Any], evaluated_at: datetime,
    ...
    return {
        ...
        "registry_shake256_512": registry_shake256_512,
        "key_id": record["key_id"],
...
def verify_artifact_key_lifecycle(
    *, registry_shake256_512: str, registry_records: Mapping[str, dict[str, Any]],
    key_id: str, principal_id: str, required_scope: str, artifact_timestamp: str,
    embedded_evidence: dict[str, Any] | None, schema_validator: CanonicalSchemaValidator,
    allow_retired_historical: bool = False,
) -> dict[str, Any]:
    record = registry_records.get(key_id)
    if record is None:
        raise RuntimeError("signed artifact key is absent from its governed registry")
    if record["principal_id"] != principal_id or record["authorization_scope"] != required_scope:
        raise RuntimeError("signed artifact key principal or scope is invalid")
    evaluated_at = _parse_time(artifact_timestamp, "artifact timestamp")
    if record["status"] == "revoked":
        raise RuntimeError("revoked audit key cannot authorize historical artifacts")
    historical_record = copy.deepcopy(record)
    if allow_retired_historical and record["status"] == "retired" and evaluated_at < _parse_time(record["status_changed_at"], "status_changed_at"):
        historical_record["status"] = "active"
        historical_record["status_changed_at"] = historical_record["valid_from"]
    evidence = key_validity_evidence(
        registry_shake256_512=registry_shake256_512, record=historical_record, evaluated_at=evaluated_at,
        required_status="active",
    )
    # Evidence retains the governed current lifecycle fields while the decision is
    # reconstructed at the artifact timestamp.
    evidence["status"] = record["status"]
    evidence["status_changed_at"] = record["status_changed_at"]
    if evidence["decision"] != "active_time_valid_rotation_valid":
        raise RuntimeError("signed artifact key was expired, revoked, or unauthorized at its timestamp")
    if embedded_evidence is not None:
        schema_validator.validate("cache_audit_key_validity_evidence", embedded_evidence)
        if canonical_bytes(embedded_evidence) != canonical_bytes(evidence):
            raise RuntimeError("signed artifact key-validity evidence was not reconstructed from governance")
    return evidence
...
class AuditAnchorClient:
    ...
    def compare_and_swap(self, expected_state_shake256_512: str | None, unsigned_state: dict[str, Any]) -> dict[str, Any]:  # pragma: no cover - interface
        raise NotImplementedError
...
class InMemoryMonotonicAuditAnchor(AuditAnchorClient):
    ...
    def __init__(
        self, signing_key: MLDSA87PrivateKey, *, key_id: str,
        ...
        self.anchor_registry_records = {key: copy.deepcopy(value) for key, value in anchor_registry_records.items()}
    ...
    @staticmethod
    def _unsigned_view(state: dict[str, Any]) -> dict[str, Any]:
        return {key: value for key, value in state.items() if key not in {"anchor_key_id", "signed_payload_shake256_512", "signature"}}

    @staticmethod
    def _validate_transition(current: dict[str, Any] | None, new: dict[str, Any]) -> None:
        if current is None:
            if (
                new["anchor_epoch"] != 0 or new["sequence"] != 0
                or new["segment_status"] != "open"
                or new.get("previous_anchor_state_shake256_512") is not None
            ):
                raise RuntimeError("audit anchor genesis is not canonical")
            return
        current_digest = InMemoryMonotonicAuditAnchor.state_shake256_512(current)
        if new.get("previous_anchor_state_shake256_512") != current_digest:
            raise RuntimeError("audit anchor state does not hash-chain to its predecessor")
        ...
        if new["segment_id"] == current["segment_id"]:
            immutable = (
                "anchor_namespace", "anchor_epoch", "segment_id",
                "previous_sealed_segment_tail_shake256_512", "transition_authorization_shake256_512",
                "audit_signing_registry_shake256_512", "anchor_registry_shake256_512",
            )
            if any(new.get(field) != current.get(field) for field in immutable):
                raise RuntimeError("audit anchor immutable segment governance changed")
            if current["segment_status"] == "sealed":
                raise RuntimeError("sealed audit anchor state is immutable")
            if new["sequence"] != current["sequence"] + 1:
                raise RuntimeError("audit anchor sequence must advance by exactly one")
            if new["segment_status"] not in {"open", "sealed"}:
                raise RuntimeError("audit anchor segment status is invalid")
            if new["tail_record_shake256_512"] == current["tail_record_shake256_512"]:
                raise RuntimeError("audit anchor advancement must bind a new record tail")
        else:
            if current["segment_status"] != "sealed":
            ...
            if new["previous_sealed_segment_tail_shake256_512"] != current["tail_record_shake256_512"]:
                raise RuntimeError("audit anchor successor does not bind the predecessor tail")
            if new["tail_record_shake256_512"] is not None or new["segment_status"] != "open":
                raise RuntimeError("audit anchor successor must begin as an empty open segment")
            if new["anchor_namespace"] != current["anchor_namespace"]:
                raise RuntimeError("audit anchor successor namespace changed")

    def compare_and_swap(self, expected_state_shake256_512: str | None, unsigned_state: dict[str, Any]) -> dict[str, Any]:
        with self._lock:
            if self.state_shake256_512(self._state) != expected_state_shake256_512:
            ...
            updated_at = unsigned_state.get("updated_at")
            validity = verify_artifact_key_lifecycle(
                ...
                artifact_timestamp=updated_at, embedded_evidence=None,
...
class UnixSocketAuditAnchorClient(AuditAnchorClient):
    """Authenticated client for the separately privileged anchor service."""
    def __init__(
        self, socket_path: Path, *, anchor_namespace: str,
        verification_keys_by_id: Mapping[str, MLDSA87PublicKey],
        anchor_registry_shake256_512: str, anchor_registry_records: Mapping[str, dict[str, Any]],
        schema_validator: CanonicalSchemaValidator,
        request_signing_key: MLDSA87PrivateKey, request_signer_key_id: str,
        peer_principal_id: str,
        expected_socket_uid: int = 0, expected_socket_gid: int | None = None,
        expected_socket_mode: int = 0o660, timeout_seconds: float = 5.0,
    ):
        ...
        self.request_signer_key_id = request_signer_key_id
        self.peer_principal_id = peer_principal_id
        self.expected_socket_uid = int(expected_socket_uid)
        self.expected_socket_gid = os.getgid() if expected_socket_gid is None else int(expected_socket_gid)
        self.expected_socket_mode = int(expected_socket_mode)
    ...
    def _verify_socket(self) -> None:
        ...
        if (
            not stat.S_ISSOCK(info.st_mode) or info.st_uid != self.expected_socket_uid
            or info.st_gid != self.expected_socket_gid or stat.S_IMODE(info.st_mode) != self.expected_socket_mode
            or self.expected_socket_mode & 0o002
        ):
            raise RuntimeError("audit anchor endpoint identity or mode is not governed")
        ...
        while True:
            ...
            if item.st_mode & stat.S_IWOTH:
                ...
                if not sticky_root:
                    raise RuntimeError("audit anchor socket ancestry is world writable")
    ...
    def _request(self, payload: dict[str, Any]) -> dict[str, Any] | None:
        ...
        request = build_authenticated_service_request(
            operation=payload["operation"], payload=payload,
            peer_principal_id=self.peer_principal_id,
            request_signer_key_id=self.request_signer_key_id,
            request_signing_key=self.request_signing_key,
            target_service="cache_audit_anchor", socket_path=self.socket_path,
        ...
        if state.get("anchor_namespace") != self.anchor_namespace or state.get("anchor_registry_shake256_512") != self.anchor_registry_shake256_512:
    ...
    def compare_and_swap(self, expected_state_shake256_512: str | None, unsigned_state: dict[str, Any]) -> dict[str, Any]:
        state = self._request({
            ...
            "expected_state_shake256_512": expected_state_shake256_512, "new_state": unsigned_state,
        })
        if state is None:
...
class SignedAppendOnlyAuditSink:
    ...
    EVENT_SCHEMA_SURFACES = {
        ...
        "cache_audit_recovery_evidence/v2": "cache_audit_recovery_evidence",
    }

    def __init__(
        self, path: Path, checkpoint_path: Path,
        signing_key: MLDSA87PrivateKey, verification_keys_by_id: Mapping[str, MLDSA87PublicKey], *,
        segment_id: str, signer_principal_id: str, signer_key_id: str,
        audit_signing_registry_shake256_512: str, audit_registry_records: Mapping[str, dict[str, Any]],
        schema_validator: CanonicalSchemaValidator,
        maximum_record_bytes: int, maximum_log_bytes: int, maximum_event_records: int,
        terminal_seal_reserved_bytes: int,
        anchor_client: AuditAnchorClient, anchor_namespace: str, anchor_registry_shake256_512: str,
        ...
        governance_key: MLDSA87PublicKey | None = None,
        ...
        recovery_consumed_ledger_path: Path | None = None,
        maximum_recovery_authorization_lifetime_seconds: int = 300,
    ):
        self.path = Path(path)
        ...
        self.audit_signing_registry_shake256_512 = audit_signing_registry_shake256_512
        self.audit_registry_records = {key: copy.deepcopy(value) for key, value in audit_registry_records.items()}
        self.schema_validator = schema_validator
        ...
        self.recovery_consumed_ledger_path = Path(recovery_consumed_ledger_path) if recovery_consumed_ledger_path is not None else self.checkpoint_path.parent / "recovery-authorizations.consumed.jsonl"
        self.maximum_recovery_authorization_lifetime_seconds = int(maximum_recovery_authorization_lifetime_seconds)
        if self.maximum_recovery_authorization_lifetime_seconds <= 0 or self.maximum_recovery_authorization_lifetime_seconds > 3600:
            raise ValueError("recovery authorization lifetime bound is invalid")
        self._lock = threading.Lock()
        ...
        self.audit_key_validity_evidence = verify_artifact_key_lifecycle(
            registry_shake256_512=self.audit_signing_registry_shake256_512,
            registry_records=self.audit_registry_records,
            key_id=self.signer_key_id, principal_id=self.signer_principal_id,
            required_scope="cache_audit_record_signing", artifact_timestamp=_utc(self.clock),
        ...
        self.previous_sealed_segment_tail_shake256_512 = anchor["previous_sealed_segment_tail_shake256_512"]
        ...
        try:
            ...
            self._previous_record_shake256_512 = tail
    ...
    def _verify_governance_record(self, surface: str, record: dict[str, Any] | None, governance_key: MLDSA87PublicKey | None) -> dict[str, Any]:
    ...
    def _provision_segment(
        self, *, governance_key: MLDSA87PublicKey | None,
        ...
        if current_anchor is None:
            ...
            expected = {
                ...
                "audit_signing_registry_shake256_512": self.audit_signing_registry_shake256_512,
                "anchor_registry_shake256_512": self.anchor_registry_shake256_512,
                "decision": "authorize_audit_genesis",
            }
            if any(authorization.get(key) != value for key, value in expected.items()):
                raise RuntimeError("audit genesis authorization does not bind this segment")
            previous_tail = None
            transition_digest = authorization["signed_payload_shake256_512"]
            epoch = 0
        else:
            ...
            predecessor_checkpoint_sha = shake256_512_bytes(canonical_bytes(predecessor_checkpoint))
            predecessor_terminal_sha = shake256_512_bytes(canonical_bytes(predecessor_terminal_record))
            expected = {
                "predecessor_segment_id": current_anchor["segment_id"],
                "predecessor_sealed_checkpoint_shake256_512": predecessor_checkpoint_sha,
                "predecessor_terminal_record_shake256_512": predecessor_terminal_sha,
                "predecessor_tail_record_shake256_512": current_anchor["tail_record_shake256_512"],
                "predecessor_anchor_state_shake256_512": self._state_digest(current_anchor),
            ...
            if predecessor_checkpoint.get("segment_status") != "sealed" or predecessor_checkpoint.get("tail_record_shake256_512") != current_anchor["tail_record_shake256_512"]:
                raise RuntimeError("predecessor checkpoint is not the anchored terminal checkpoint")
            if predecessor_terminal_sha != predecessor_checkpoint.get("tail_record_shake256_512") or predecessor_terminal_sha != current_anchor.get("tail_record_shake256_512"):
                raise RuntimeError("predecessor terminal record digest is not the anchored tail")
            if (
                predecessor_terminal_record.get("sequence") != predecessor_checkpoint.get("sequence")
                or predecessor_terminal_record.get("segment_id") != current_anchor.get("segment_id")
                or terminal_event.get("segment_id") != current_anchor.get("segment_id")
                or terminal_event.get("terminal_record_sequence") != predecessor_terminal_record.get("sequence")
                or terminal_event.get("sealed_predecessor_tail_shake256_512") != predecessor_terminal_record.get("previous_audit_record_shake256_512")
            ):
                raise RuntimeError("predecessor terminal record is not the actual anchored segment seal")
            previous_tail = current_anchor["tail_record_shake256_512"]
            transition_digest = transition["signed_payload_shake256_512"]
        ...
        self._anchor_state = current_anchor
        ...
        try:
            ...
            self.previous_sealed_segment_tail_shake256_512 = previous_tail
    ...
    def _anchor_unsigned(
        ...
        return {
            ...
            "sequence": sequence,
            "tail_record_shake256_512": tail,
            "segment_status": segment_status,
            "previous_sealed_segment_tail_shake256_512": previous_tail,
            "transition_authorization_shake256_512": transition_digest,
            "audit_signing_registry_shake256_512": self.audit_signing_registry_shake256_512,
            "anchor_registry_shake256_512": self.anchor_registry_shake256_512,
            "previous_anchor_state_shake256_512": self._state_digest(getattr(self, "_anchor_state", None)),
    ...
    def _verify_anchor_state(self, anchor: dict[str, Any], *, require_current_segment: bool = True) -> None:
        ...
        if (
            ...
            or anchor.get("audit_signing_registry_shake256_512") != self.audit_signing_registry_shake256_512
            or anchor.get("anchor_registry_shake256_512") != self.anchor_registry_shake256_512
        ):
    ...
    def _verify_descriptor(
        ...
        for raw_line in data.splitlines():
            ...
            if (
                ...
                or record.get("previous_audit_record_shake256_512") != previous
                or record.get("previous_sealed_segment_tail_shake256_512") != self.previous_sealed_segment_tail_shake256_512
                or record.get("audit_signing_registry_shake256_512") != self.audit_signing_registry_shake256_512
                or record.get("anchor_namespace") != self.anchor_namespace
                or record.get("anchor_registry_shake256_512") != self.anchor_registry_shake256_512
            ):
                raise RuntimeError("cache audit segment, sequence, governance, or hash chain is invalid")
            verify_artifact_key_lifecycle(
                registry_shake256_512=self.audit_signing_registry_shake256_512,
                registry_records=self.audit_registry_records,
                key_id=record.get("audit_signer_key_id"),
                principal_id=record.get("audit_signer_principal_id"),
                required_scope="cache_audit_record_signing",
                artifact_timestamp=record.get("created_at"),
                embedded_evidence=record.get("audit_key_validity_evidence"),
                schema_validator=self.schema_validator,
                allow_retired_historical=True,
            )
            ...
            if canonical_bytes(event).decode("utf-8") != event_json or shake256_512_bytes(event_json.encode("utf-8")) != record.get("event_shake256_512"):
            ...
            event_id = "event:" + shake256_512_bytes(canonical_bytes(event))
            ...
            identity = {
                ...
                "previous_audit_record_shake256_512": previous,
                "previous_sealed_segment_tail_shake256_512": self.previous_sealed_segment_tail_shake256_512,
                "event_shake256_512": record["event_shake256_512"], "event_idempotency_key": event_id,
                "audit_signer_key_id": record["audit_signer_key_id"],
                "audit_signing_registry_shake256_512": self.audit_signing_registry_shake256_512,
                "anchor_namespace": self.anchor_namespace,
            }
            if record.get("audit_record_id") != "audit:" + shake256_512_bytes(canonical_bytes(identity)):
            ...
            previous = shake256_512_bytes(raw_line)
    ...
    def _load_checkpoint(self) -> dict[str, Any]:
        ...
        verify_artifact_key_lifecycle(
            registry_shake256_512=self.audit_signing_registry_shake256_512,
            registry_records=self.audit_registry_records,
            key_id=checkpoint.get("audit_signer_key_id"),
            principal_id=self.audit_registry_records.get(checkpoint.get("audit_signer_key_id"), {}).get("principal_id", ""),
            required_scope="cache_audit_record_signing",
            artifact_timestamp=checkpoint.get("checkpoint_created_at"),
            embedded_evidence=checkpoint.get("audit_key_validity_evidence"),
            schema_validator=self.schema_validator,
            allow_retired_historical=True,
        )
        return checkpoint
    ...
    def _verify_checkpoint(self, checkpoint: dict[str, Any], *, sequence: int, tail: str | None, anchor: dict[str, Any]) -> None:
        if (
            ...
            or checkpoint.get("tail_record_shake256_512") != tail
            or checkpoint.get("previous_sealed_segment_tail_shake256_512") != self.previous_sealed_segment_tail_shake256_512
            or checkpoint.get("audit_signer_key_id") != self.signer_key_id
            or checkpoint.get("audit_signing_registry_shake256_512") != self.audit_signing_registry_shake256_512
            or checkpoint.get("anchor_namespace") != self.anchor_namespace
            or checkpoint.get("anchor_state_shake256_512") != self._state_digest(anchor)
            ...
            or anchor.get("tail_record_shake256_512") != tail
        ):
    ...
    def _checkpoint_record(self, *, sequence: int, tail: str | None, segment_status: str, anchor: dict[str, Any]) -> dict[str, Any]:
        ...
        validity = verify_artifact_key_lifecycle(
            ...
            registry_records=self.audit_registry_records,
            key_id=self.signer_key_id, principal_id=self.signer_principal_id,
            required_scope="cache_audit_record_signing", artifact_timestamp=created_at,
            embedded_evidence=None, schema_validator=self.schema_validator,
        )
        unsigned = {
            ...
            "audit_signing_registry_shake256_512": self.audit_signing_registry_shake256_512,
            "audit_key_validity_evidence": validity,
            "anchor_namespace": self.anchor_namespace,
            "anchor_registry_shake256_512": self.anchor_registry_shake256_512,
            "anchor_epoch": anchor["anchor_epoch"],
            "anchor_state_shake256_512": self._state_digest(anchor),
            "segment_status": segment_status,
    ...
    def _build_record(self, evidence: dict[str, Any], *, sequence: int, previous: str | None) -> tuple[dict[str, Any], bytes, str]:
        ...
        event_shake256_512 = shake256_512_bytes(event_bytes)
        event_id = "event:" + event_shake256_512
        identity = {
            "segment_id": self.segment_id, "sequence": sequence,
            "previous_audit_record_shake256_512": previous,
            "previous_sealed_segment_tail_shake256_512": self.previous_sealed_segment_tail_shake256_512,
            "event_shake256_512": event_shake256_512, "event_idempotency_key": event_id,
            "audit_signer_key_id": self.signer_key_id,
            "audit_signing_registry_shake256_512": self.audit_signing_registry_shake256_512,
            "anchor_namespace": self.anchor_namespace,
        }
        ...
        unsigned = {
            ...
            "audit_record_id": "audit:" + shake256_512_bytes(canonical_bytes(identity)),
            "event_idempotency_key": event_id,
            "segment_id": self.segment_id, "sequence": sequence,
            "previous_audit_record_shake256_512": previous,
            "previous_sealed_segment_tail_shake256_512": self.previous_sealed_segment_tail_shake256_512,
            "event_schema_version": version,
            "event_canonical_json": event_bytes.decode("utf-8"), "event_shake256_512": event_shake256_512,
    ...
    def _append_locked(
        ...
        event_id = "event:" + shake256_512_bytes(canonical_bytes(evidence))
        matches = [record for record in records if record.get("event_idempotency_key") == event_id]
        if matches:
            if len(matches) != 1 or matches[0]["event_shake256_512"] != event_id.split(":", 1)[1]:
        ...
        tail = shake256_512_bytes(encoded[:-1])
        self._anchor_state = anchor_before
        unsigned_anchor = self._anchor_unsigned(
            epoch=anchor_before["anchor_epoch"], sequence=sequence, tail=tail,
            segment_status=segment_status_after,
            previous_tail=self.previous_sealed_segment_tail_shake256_512,
            transition_digest=anchor_before["transition_authorization_shake256_512"],
        )
        ...
        with self._lock:
            ...
            try:
                ...
                validity = verify_artifact_key_lifecycle(
                    ...
                    registry_records=self.audit_registry_records,
                    key_id=self.signer_key_id, principal_id=self.signer_principal_id,
                    required_scope="cache_audit_record_signing", artifact_timestamp=sealed_at,
                    embedded_evidence=None, schema_validator=self.schema_validator,
                )
                event = {
                    ...
                    "sealed_predecessor_tail_shake256_512": tail,
                    "previous_sealed_segment_tail_shake256_512": self.previous_sealed_segment_tail_shake256_512,
                    "audit_signing_registry_shake256_512": self.audit_signing_registry_shake256_512,
                    "audit_key_validity_evidence": validity,
                    "anchor_namespace": self.anchor_namespace,
    ...
    def _signed_recovery_evidence(self, *, action: str, before: dict[str, Any], after: dict[str, Any], authorization: dict[str, Any]) -> dict[str, Any]:
        ...
        validity = verify_artifact_key_lifecycle(
            ...
            registry_records=self.audit_registry_records,
            key_id=self.signer_key_id, principal_id=self.signer_principal_id,
            required_scope="cache_audit_record_signing", artifact_timestamp=recovered_at,
            embedded_evidence=None, schema_validator=self.schema_validator,
        )
        unsigned = {
            "schema_version": "cache_audit_recovery_evidence/v2",
            "recovery_id": "recovery:" + shake256_512_bytes(canonical_bytes({"segment_id": self.segment_id, "action": action, "authorization": authorization["signed_payload_shake256_512"]})),
            "segment_id": self.segment_id, "action": action,
            "before_state": before, "after_state": after,
            "recovery_authorization_shake256_512": authorization["signed_payload_shake256_512"],
            "audit_signing_registry_shake256_512": self.audit_signing_registry_shake256_512,
            "audit_signer_key_id": self.signer_key_id,
            "audit_signer_principal_id": self.signer_principal_id,
            "audit_key_validity_evidence": validity,
    ...
    def _file_identity(path: Path) -> dict[str, Any]:
        info = path.lstat()
        return {
            "name": path.name, "device": info.st_dev, "inode": info.st_ino,
            "size": info.st_size, "mode": stat.S_IMODE(info.st_mode), "uid": info.st_uid,
        }

    def _consume_recovery_authorization(
        self, authorization: dict[str, Any], governance_key: MLDSA87PublicKey, *,
        action: str, target_file_identities: list[dict[str, Any]],
    ) -> None:
        self._verify_governance_record("cache_audit_recovery_authorization", authorization, governance_key)
        now = self.clock().astimezone(timezone.utc)
        created = _parse_time(authorization["created_at"], "recovery.created_at")
        expires = _parse_time(authorization["expires_at"], "recovery.expires_at")
        if not (created <= now < expires):
            raise RuntimeError("recovery authorization is not currently valid")
        if (expires - created).total_seconds() > self.maximum_recovery_authorization_lifetime_seconds:
            raise RuntimeError("recovery authorization lifetime exceeds its governed maximum")
        if authorization.get("segment_id") != self.segment_id or authorization.get("action") != action:
            raise RuntimeError("recovery authorization does not bind this segment and action")
        if authorization.get("target_file_identities") != target_file_identities:
            raise RuntimeError("recovery authorization does not bind the exact target file identities")
        ledger = self.recovery_consumed_ledger_path
        ledger.parent.mkdir(mode=0o700, parents=True, exist_ok=True)
        os.chmod(ledger.parent, 0o700)
        descriptor = os.open(ledger, os.O_CREAT | os.O_APPEND | os.O_RDWR | O_NOFOLLOW, 0o600)
        try:
            ...
            size = os.fstat(descriptor).st_size
            if size > 4 * 1024 * 1024:
                raise RuntimeError("consumed recovery authorization ledger exceeds its bound")
            while len(data) < size:
                chunk = os.read(descriptor, min(65536, size - len(data)))
                if not chunk:
                    raise RuntimeError("consumed recovery authorization ledger changed during read")
                data += chunk
            authorization_id = authorization["authorization_id"]
            for line in data.splitlines():
                item = canonical_json_loads(line.decode("utf-8"))
                if item.get("authorization_id") == authorization_id:
                    raise RuntimeError("recovery authorization has already been consumed")
            consumption = {
                "authorization_id": authorization_id,
                "authorization_shake256_512": authorization["signed_payload_shake256_512"],
                "action": action, "segment_id": self.segment_id,
                "consumed_at": now.isoformat(),
            }
            encoded = canonical_bytes(consumption) + b"\n"
            written = 0
            while written < len(encoded):
                amount = os.write(descriptor, encoded[written:])
                if amount <= 0:
                    raise OSError("recovery consumption ledger write made no progress")
                written += amount
            self.fsync(descriptor)
        finally:
            try: fcntl.flock(descriptor, fcntl.LOCK_UN)
            finally: os.close(descriptor)
        self._fsync_directory(ledger.parent, deadline_monotonic=None)
    ...
    def recover_dangling_tail(
        ...
        self._consume_recovery_authorization(
            authorization, governance_key, action="reconcile_signed_dangling_tail",
        ...
        try:
            ...
            expected = {
                ...
                "checkpoint_tail_before": checkpoint["tail_record_shake256_512"],
            ...
            before = {"checkpoint_sequence": checkpoint["sequence"], "checkpoint_tail": checkpoint["tail_record_shake256_512"], "anchor_state_shake256_512": self._state_digest(anchor)}
            ...
            if anchor["sequence"] == checkpoint["sequence"]:
                ...
                unsigned_anchor = self._anchor_unsigned(
                    ...
                    previous_tail=self.previous_sealed_segment_tail_shake256_512,
                    transition_digest=anchor["transition_authorization_shake256_512"],
                )
                anchor = self.anchor_client.compare_and_swap(self._state_digest(anchor), unsigned_anchor)
            elif anchor["sequence"] != sequence or anchor["tail_record_shake256_512"] != tail:
            ...
            after = {"checkpoint_sequence": sequence, "checkpoint_tail": tail, "anchor_state_shake256_512": self._state_digest(anchor)}
    ...
    def cleanup_checkpoint_temporaries(
        self, authorization: dict[str, Any], governance_key: MLDSA87PublicKey, *, evidence_path: Path,
    ) -> dict[str, Any]:
        pattern = f".{self.checkpoint_path.name}.*.tmp"
        candidates = [path for path in sorted(self.checkpoint_path.parent.glob(pattern)) if path.exists()]
        identities = [self._file_identity(path) for path in candidates]
        self._consume_recovery_authorization(
            authorization, governance_key, action="remove_orphan_checkpoint_temporaries",
            target_file_identities=identities,
        )
        removed: list[str] = []
        for path, identity in zip(candidates, identities):
            if self._file_identity(path) != identity:
                raise RuntimeError("recovery target identity changed before deletion")
            info = path.lstat()
            if stat.S_ISREG(info.st_mode) and info.st_uid == self.expected_uid:
                path.unlink()
                removed.append(path.name)
        before = {"temporary_count": len(removed), "temporary_names_shake256_512": shake256_512_bytes(canonical_bytes(removed))}
        after = {"temporary_count": 0, "temporary_names_shake256_512": shake256_512_bytes(canonical_bytes([]))}
...
class UnixSocketAuditPublisherClient:
    """Hard-deadline, peer-bound client for the audit publisher service."""
    def __init__(
        self, socket_path: Path, *, verification_keys_by_id: Mapping[str, MLDSA87PublicKey],
        receipt_signing_registry_shake256_512: str, receipt_registry_records: Mapping[str, dict[str, Any]],
        schema_validator: CanonicalSchemaValidator,
        request_signing_key: MLDSA87PrivateKey, request_signer_key_id: str,
        peer_principal_id: str,
        expected_socket_uid: int, expected_socket_gid: int | None = None,
    ...
    def _verify_endpoint(self) -> None:
        ...
        if (
            ...
            raise RuntimeError("audit publisher endpoint identity or mode is not governed")
        parent = self.socket_path.parent
        while True:
            item = parent.lstat()
            if stat.S_ISLNK(item.st_mode) or not stat.S_ISDIR(item.st_mode):
    ...
    def __call__(self, evidence: dict[str, Any], *, deadline_monotonic: float) -> dict[str, Any]:
        ...
        payload = {
            "operation": "publish",
            "event_idempotency_key": "event:" + event_shake256_512,
            "event_shake256_512": event_shake256_512,
            "event": evidence,
        }
        request = build_authenticated_service_request(
            operation="publish", payload=payload,
            peer_principal_id=self.peer_principal_id,
            request_signer_key_id=self.request_signer_key_id,
            request_signing_key=self.request_signing_key,
            target_service="cache_audit_publisher", socket_path=self.socket_path,
        ...
        key = self.verification_keys_by_id.get(receipt.get("receipt_signer_key_id"))
        ...
        verify_artifact_key_lifecycle(
            ...
            key_id=receipt.get("receipt_signer_key_id"),
            principal_id=receipt.get("receipt_signer_principal_id"),
            required_scope="cache_audit_receipt_signing",
            artifact_timestamp=receipt.get("completed_at"),
            embedded_evidence=receipt.get("receipt_key_validity_evidence"),
            schema_validator=self.schema_validator,
            allow_retired_historical=True,
        )
        if (
            receipt.get("event_idempotency_key") != payload["event_idempotency_key"]
            or receipt.get("event_shake256_512") != event_shake256_512
            or receipt.get("receipt_signing_registry_shake256_512") != self.receipt_signing_registry_shake256_512
        ):
...
__all__ = [
    ...
    "verify_artifact_key_lifecycle", "build_authenticated_service_request",
    "socket_identity_shake256_512",
]
