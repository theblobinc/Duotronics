#!/usr/bin/env python3
"""Build final-byte provenance, inventory, checksum, and human manifest for Draft 5.3.6."""
...
INVENTORY = "PACKAGE_INVENTORY_v1_6_draft_5_3_6.json"
CHECKSUMS = "refs/manifest/CHECKSUMS_v1_6_draft_5_3_6.shake256_512"
MANIFEST = "refs/manifest/MANIFEST_v1_6_draft_5_3_6_complete.md"
REPORT = "DRAFT5_3_6_VALIDATION_REPORT.json"
PROVENANCE = "PACKAGE_PROVENANCE_v1_6_draft_5_3_6.json"
REVIEW = "refs/review/DRAFT5_3_5_INDEPENDENT_REMEDIATION_CHECKLIST.md"

# These four files recursively describe the complete file set. Their own size
# and hash fields are explicitly null; every other final file is fully covered.
...
def write_provenance() -> None:
    ...
    record = {
        ...
        "source_subtree_path": ".",
        "source_subtree_shake256_512": tree_digest(current, SOURCE_DIGEST_EXCLUDED),
        "source_digest_exclusions": sorted(SOURCE_DIGEST_EXCLUDED),
        "generator_name": generator,
        "generator_version": "draft5_3_6_manifest_builder/v1",
        "generator_shake256_512": digest(ROOT / generator),
        "source_generation": "v1.6-draft-5.3.5",
        "base_revision": "v1.6-draft-5.3.5",
        "transformation_change_set": REVIEW,
        "transformation_change_set_shake256_512": digest(ROOT / REVIEW),
...
def main() -> int:
    ...
    records = []
    checksum_lines = [
        "# SHAKE256-512 closure for Duotronic Witness Contract v1.6 Draft 5.3.6",
    ...
    for path in paths:
        ...
        records.append({
            ...
            "excluded_fields": ["shake256_512", "size_bytes"] if excluded else [],
    ...
    if excluded_count != len(RECURSIVE_EXCLUDED):
        raise SystemExit(f"hash exclusion count mismatch: {excluded_count}")

    inventory = {
        "schema_version": "package_inventory/v3",
        "package": "duotronic-witness-contract-v1.6-draft-5.3.6",
        "status": "completed_corrective_development_draft_permanently_not_frozen",
        "historical_source_packages": [
            {"path": f"history/source_packages/{name}", "shake256_512": value, "file_count": count}
            for name, value, count in HISTORY
        ],
        ...
        "excluded_field_policy": "recursive entries set shake256_512 and size_bytes to null and declare excluded_fields plus reason",
        "checksum_file": CHECKSUMS,
        "canonical_descriptor": "CANONICAL_CORPUS_v1_6_draft_5_3_6.json",
    ...
    manifest_lines = [
        "# Complete Manifest — Duotronic Witness Contract v1.6 Draft 5.3.6",
        "",
        ...
        "- Exact historical Draft 5.2.2 through 5.3.5 archives: retained and verified",
    ...
    manifest_lines.extend([
        ...
        "",
        "The active inventory, checksum file, this human manifest, and validation report describe package closure and therefore set only their own recursive `shake256_512` and `size_bytes` inventory fields to `null`. Each record names the excluded fields and reason. Every other final regular file is size- and SHAKE256-512-covered.",
        "",
        "## Documentation scope",
        "",
        "Vendored dependency documentation is excluded from first-party link-quality checks while the dependency bytes remain hash-covered.",
        "",
    ])
    (ROOT / MANIFEST).write_text("\n".join(manifest_lines), encoding="utf-8")

    # Verify final stable bytes after every write.
    final = files()
    actual = {path.relative_to(ROOT).as_posix(): path for path in final}
    for record in records:
        relative = record["path"]
        if relative in RECURSIVE_EXCLUDED:
            if record["shake256_512"] is not None or record["size_bytes"] is not None:
                raise SystemExit(f"recursive field is not null: {relative}")
            continue
        path = actual[relative]
        if record["shake256_512"] != digest(path) or record["size_bytes"] != path.stat().st_size:
