#!/usr/bin/env python3
"""Generate the complete lifecycle-classified Draft 5.3.15 schema registry."""
...
OUTPUT = ROOT / "refs/schema_registry_v1_6_draft_5_3_15.json"
...
CANONICAL = {
    ...
    "cache_audit_key_validity_evidence_v1", "cache_audit_anchor_state_v1",
    "cache_audit_genesis_authorization_v1", "cache_audit_segment_transition_v1",
    "cache_audit_recovery_authorization_v1", "cache_audit_recovery_evidence_v1",
    "cache_audit_publication_receipt_v1", "domain_execution_evidence_v1",
...
def main() -> int:
    ...
    schema_paths = sorted((path for path in (ROOT / "schemas").glob("*.schema.*") if schema_name(path) not in POST_5315), key=lambda path: path.name)
    for path in schema_paths:
        relative = path.relative_to(ROOT).as_posix(); name = schema_name(path); document = load(path)
        if name in CANONICAL:
            lifecycle = "canonical"; active = True; rationale = "Active Draft 5.3.15 API, persistence, witness, or validation surface."
        elif name in LEGACY:
        ...
        runtime_valid = {
            ...
            "cache_audit_anchor_state_v1": "runtime:test_privileged_file_anchor_round_trip_over_unix_socket",
            ...
            "cache_audit_recovery_authorization_v1": "runtime:test_checkpoint_failure_removes_temporary_and_governed_recovery_reconciles",
            "cache_audit_recovery_evidence_v1": "runtime:test_checkpoint_failure_removes_temporary_and_governed_recovery_reconciles",
            "cache_audit_publication_receipt_v1": "runtime:test_unix_publisher_client_hard_times_out_without_waiting_for_server",
    ...
    registry = {
        ...
        "registry_version": "v1.6-draft-5.3.15", "active_version_scope": "v1.6-draft-5.3.15",
        "entries": entries,
        "rules": {
