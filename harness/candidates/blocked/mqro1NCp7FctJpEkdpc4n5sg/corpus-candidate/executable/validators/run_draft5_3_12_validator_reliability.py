#!/usr/bin/env python3
"""Repeat the complete Draft 5.3.12 validator with bounded outer orchestration."""
...
VALIDATOR_PATH = ROOT / "executable/validators/validate_draft5_3_12_corpus.py"
REPORT_PATH = ROOT / "DRAFT5_3_12_VALIDATION_REPORT.json"

SPECIFICATION = importlib.util.spec_from_file_location("draft5312_reliability_validator", VALIDATOR_PATH)
...
def main() -> int:
    ...
    if output.is_file():
        ...
        if (
            prior.get("package_version") != "v1.6-draft-5.3.12"
    ...
    def write_checkpoint(status: str) -> None:
        evidence = {
            ...
            "complete_validator": "executable/validators/validate_draft5_3_12_corpus.py",
            "required_phase_count": 85,
    ...
    for sequence in range(len(completed_runs) + 1, final_sequence + 1):
        ...
        if (
            ...
            or statuses != {"passed": 85, "failed": 0, "skipped": 8}
        ):
