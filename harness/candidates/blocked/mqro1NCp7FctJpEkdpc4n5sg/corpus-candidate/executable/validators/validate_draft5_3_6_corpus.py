#!/usr/bin/env python3
"""Descriptor-reconciled fail-closed validator for Draft 5.3.6."""
...
DESCRIPTOR_PATH = ROOT / "CANONICAL_CORPUS_v1_6_draft_5_3_6.json"
REPORT_PATH = ROOT / "DRAFT5_3_6_VALIDATION_REPORT.json"
HISTORY_ARCHIVES = {
...
def phase_descriptor() -> dict[str, Any]:
    ...
    assert_true(descriptor["active_version"] == "v1.6-draft-5.3.6", "active version mismatch")
    ...
    for token in (DESCRIPTOR_PATH.name, "PACKAGE_INVENTORY_v1_6_draft_5_3_6.json", "CHECKSUMS_v1_6_draft_5_3_6.shake256_512"):
        assert_true(token in resolver, f"resolver does not boot canonical token {token}")
    accounting = _run_python_tests("-W", ["error", "executable/tests/test_validator_phase_reconciliation_v536.py"])
...
def phase_dependencies() -> dict[str, Any]:
    ...
    try:
        ...
        connection.close()
    scope = load_json("refs/validation_scope_v1_6_draft_5_3_6.json")
...
def phase_hash_closure() -> dict[str, Any]:
    inventory = load_json("PACKAGE_INVENTORY_v1_6_draft_5_3_6.json")
    ...
    checksum_lines = (ROOT / "refs/manifest/CHECKSUMS_v1_6_draft_5_3_6.shake256_512").read_text(encoding="utf-8").splitlines()
...
def _schema_phase(name: str) -> dict[str, Any]:
    command = ["node", "executable/validators/validate_draft5_3_6_schemas.mjs", "--phase", name]
...
def phase_sql_migration() -> dict[str, Any]:
    ...
    assert_true("v1.6-draft-5.3.6" in generations, "5.3.6 schema generation missing")
    columns = {row[1] for row in connection.execute("PRAGMA table_info(wc_lean_compiler_witnesses_v5)")}
    hardening = {
    ...
    required_objects = (
        ...
        "wc_lean_compiler_witnesses_v5", "wc_theorem_promotion_gates_v5",
        "wc_authoritative_theorems_v5",
    )
    ...
    connection.close()
    assert_true(objects == len(required_objects), "5.3.6 policy/witness objects missing")
    return {"generations": generations, "compiler_witness_v5_binding_columns": len(hardening), "authority_objects": objects}
...
def phase_sql_v536_authority_closure() -> dict[str, Any]:
    connection = _new_database()
    sql = (ROOT / "migration/draft5_3_5_to_draft5_3_6.sql").read_text(encoding="utf-8")
    for token in (
        "recursive_dependency_policy_result = 'passed'", "exact_executed_argv_shake256_512",
        "effective_resource_limits_shake256_512", "compile_handoff_manifest_shake256_512",
        "trust_root_attestation_registry_shake256_512", "wc_compiler_witness_v5_no_update",
        "package_version = 'v1.6-draft-5.3.6'",
    ):
        assert_true(token in sql, f"Draft 5.3.6 SQL closure omits {token}")
    count = connection.execute("SELECT count(*) FROM wc_authoritative_theorems_v5").fetchone()[0]
    connection.close()
    assert_true(count == 0, "portable database unexpectedly activates theorem authority")
    return {"v5_authority_view_empty": True, "closure_tokens": 7}
...
def phase_openapi_conformance() -> dict[str, Any]:
    ...
    spec = yaml.safe_load((ROOT / "executable/openapi/draft5_3_6_evidence_language_openapi.yaml").read_text(encoding="utf-8"))
    assert_true(spec["info"]["version"] == "v1.6-draft-5.3.6", "OpenAPI version mismatch")
...
def phase_effective_sandbox_invocation() -> dict[str, Any]:
    profile = load_json("authority/hermetic_lean_sandbox_profile_v3.json")
    ...
    assert_true(profile["effective_invocation_schema"] == "schemas/effective_sandbox_invocation_v3.schema.json", "sandbox profile does not select v3 effective invocation")
...
def phase_formal_manifest_coverage() -> dict[str, Any]:
    ...
    required_models = {"ProofAuthorityV2", "ProofAuthorityV3", "ProofAuthorityV4", "ProofAuthorityV5", "ProofAuthorityV6", "ProofAuthorityV7"}
...
def phase_schema_registry_authority() -> dict[str, Any]:
    registry = load_json("refs/schema_registry_v1_6_draft_5_3_6.json")
...
def phase_version_consistency() -> dict[str, Any]:
    expected = "v1.6-draft-5.3.6"
    descriptor = load_json(DESCRIPTOR_PATH.name)
    metadata = load_json("PACKAGE_METADATA_v1_6_draft_5_3_6.json")
    registry = load_json("refs/schema_registry_v1_6_draft_5_3_6.json")
    package = load_json("package.json")
    lock = load_json("package-lock.json")
    tla = load_json("refs/formal_toolchain/tla_toolchain_manifest_v1_0.json")
    gate_status = load_json("DRAFT5_3_6_RELEASE_GATE_STATUS.json")
    assert_true(descriptor["active_version"] == metadata["version"] == expected, "descriptor/package metadata version mismatch")
    assert_true(package["version"] == "1.6.5-3-6" and lock["version"] == package["version"], "npm package/lock version mismatch")
    assert_true(package["name"].endswith("draft-5-3-6-validation") and lock["name"] == package["name"], "npm package/lock identity mismatch")
    assert_true(registry.get("registry_version") == expected, "schema registry version mismatch")
    assert_true("Draft 5.3.6" in tla["package"] and any(item["module"] == "ProofAuthorityV7" for item in tla["tla_modules"]), "formal manifest version/active model mismatch")
    assert_true(gate_status["gates"]["I_package_provenance"] == "unpublished_workspace", "release provenance status is not fail-closed")
    assert_true('v!"5.3.6"' in (ROOT / "lakefile.lean").read_text(encoding="utf-8"), "Lake package version mismatch")
    active_documents = (
        "README.md", "START_HERE.md", "CANONICAL_CORPUS_v1_6_draft_5_3_6.json",
        "CORPUS_INDEX_v1_6_draft_5_3_6.md", "duotronic_witness_contract_v1_6_draft_5_3_6.md",
        "RELEASE_NOTES_v1_6_draft_5_3_6.md", "PACKAGE_METADATA_v1_6_draft_5_3_6.json",
    ...
    for relative in active_documents:
        ...
        assert_true("5.3.6" in text_value, f"active entrypoint lacks 5.3.6 identity: {relative}")
    ...
    for report_path in ROOT.glob("DRAFT5_3_6*.json"):
        report = json.loads(report_path.read_text(encoding="utf-8"))
        assert_true(report.get("package_version") == expected, f"active report version mismatch: {report_path.name}")
    return {"active_version": expected, "npm_version": package["version"], "formal_model": "ProofAuthorityV7", "entrypoints_checked": len(active_documents)}


def phase_package_provenance() -> dict[str, Any]:
    provenance = load_json("PACKAGE_PROVENANCE_v1_6_draft_5_3_6.json")
    required = {
...
def _build_attestation(subject_type: str) -> dict[str, Any]:
    status = load_json("DRAFT5_3_6_BUILD_ATTESTATION_STATUS.json")
...
def phase_external_signature() -> dict[str, Any]:
    status = load_json("refs/trust/RELEASE_SIGNATURE_STATUS_v1_6_draft_5_3_6.json")
...
REQUIRED_PHASE_FUNCTIONS: dict[str, Callable[[], dict[str, Any]]] = {
    ...
    "sql_v536_authority_closure": phase_sql_v536_authority_closure,
...
def main() -> int:
    ...
    report = {
        "schema_version": "draft5_3_6_validation_report/v1",
        "package_version": "v1.6-draft-5.3.6",
