...
class TrustedRootTierTests(unittest.TestCase):
    ...
    def _make_loader_root(self, parent: pathlib.Path, uid: int, gid: int) -> pathlib.Path:
        ...
        verifier = MLDSA87PrivateKey.generate()
        ...
        config = {
            ...
            "oci_runtime_shake256_512": "0" * 128,
            ...
            "seccomp_profile_shake256_512": "0" * 128,
            "authority_uid": uid,
            "authority_gid": gid,
            ...
            "cache_audit_request_signer_key_id": "key:cache-audit-request",
            "cache_audit_request_principal_id": "principal:proof-service",
            "cache_audit_publisher_socket": "/run/witness-cache-audit/publisher.sock",
            "cache_audit_publisher_uid": 65533,
            "cache_audit_publisher_gid": 65534,
            "cache_audit_publisher_socket_mode": 432,
        }
...
class CacheChronologyAndRecoveryTests(unittest.TestCase):
    ...
    def test_registry_rotation_preserves_stale_row_and_requires_new_key(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            new_key = MLDSA87PrivateKey.generate()
            ...
            old_digest = application.cache_registry_shake256_512
            current_digest = "f" * 128
            lineage_digest = "d" * 128
            snapshots = {
                old_digest: service.GovernedCacheRegistrySnapshot(
                    registry_shake256_512=old_digest, registry_id="development:old",
                    registry_created_at="1970-01-01T00:00:00+00:00",
                    registry_signed_payload_shake256_512=old_digest,
                    ...
                    successor_registry_shake256_512=current_digest,
                    stale_replay_policy="authenticate_then_conflict", revoked_key_ids=frozenset(),
                    lineage_signed_payload_shake256_512=lineage_digest, lineage_created_at="1970-01-03T00:00:00+00:00",
                ),
                current_digest: service.GovernedCacheRegistrySnapshot(
                    registry_shake256_512=current_digest, registry_id="development:current",
                    registry_created_at="1970-01-02T00:00:00+00:00",
                    registry_signed_payload_shake256_512=current_digest,
            ...
            rotated = service.ProofCheckApplication(
                ...
                cache_registry_shake256_512=current_digest,
                cache_registry_snapshots_by_shake256_512=snapshots,
                cache_verification_evidence_sink=events.append,
            )
    ...
    def test_wsgi_returns_stable_conflict_for_rotated_cache_row(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            application.cache_registry_shake256_512 = current_digest
            application.cache_registry_snapshots_by_shake256_512 = {
            ...
            application.cache_registry_lineage_paths = service.validate_cache_registry_snapshot_lineage(
                application.cache_registry_snapshots_by_shake256_512,
                current_registry_shake256_512=current_digest, production_mode=True,
            )
...
class PythonMatrixAssemblyTests(unittest.TestCase):
    ...
    def run_record(target: str, exact: str, generated_at: str) -> dict:
        return {
            ...
            "normal_stdout_shake256_512": "0" * 128, "normal_stderr_shake256_512": "1" * 128,
            "development_stdout_shake256_512": "2" * 128, "development_stderr_shake256_512": "3" * 128,
        }
